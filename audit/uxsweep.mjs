 ...
 *              และ **ไม่นับของที่เป็น "เส้น" (ด้านใดด้านหนึ่ง ≤ 3px)** — เส้นบางๆ ไม่ใช่พื้นที่ตกแต่ง
 *              แต่เป็น **ตัวชี้ตำแหน่ง**: playhead เวลาปัจจุบัน (`.now-line` 2px) · ลำแสงใต้แท็บที่เลือก ·
 *              เส้นคั่น · แถบสีบนขอบการ์ด — ไล่เฉดบนเส้นพวกนี้ทำให้ปลายจางลง = บอกทิศทาง/จุดยึด
 *              (เคยนับแล้วได้ `.now-line` โผล่ทุกหน้าที่มีไทม์ไลน์ = ตัวเลขบวกคงที่ที่แก้ไม่ได้)
 ...
 *  4. `flat`   แถวการ์ดที่มี **"ยอดรวม" ปนกับ "ส่วนประกอบของยอดนั้น"** แต่ทุกใบขนาดเท่ากันหมด
 *              ⇒ จอไม่ได้บอกว่าใบไหนคือตัวหาร ใบไหนคือชิ้นส่วน · ตาไม่รู้จะเริ่มอ่านตรงไหน
 *              🔑 **ตรวจ 2 ทาง อย่างใดอย่างหนึ่งก็พอ** (ทางเดียวไม่พอ — เหตุผลอยู่ท้ายข้อ):
 *                 (ก) ใบที่มากที่สุด ≈ ผลบวกของใบที่เหลือ (±2%) = มีผลรวมอยู่จริง
 *                 (ข) มีใบไหนที่ **ป้ายชื่อบอกว่าเป็นยอดรวม** (ทั้งหมด/รวม/total/ยอดรวม)
 *                 ⇒ ผลรวมต้องเป็นพระเอก (ใหญ่กว่า/กว้างกว่า) และส่วนประกอบต้องเขียนสัดส่วนกำกับ
 *              ⚠️ ต้องมี (ข) เพราะ **ข้อมูลใน harness ส่วนใหญ่เป็น 0** ⇒ "14 / 0 / 0 / 0" สอบ (ก) ไม่ผ่าน
 *                 ทั้งที่เป็นแถว "ทั้งหมด + แยกตามสถานะ" ชัดๆ (เคสจริงที่เจอ: /event-log)
 *              · ชุดที่ **เป็นพี่น้องกันจริง** (OEE/OOE/TEEP ที่ต่างกันแค่ฐานเวลา · ตัวนับคนละเรื่อง)
 *                บวกกันแล้วไม่ได้อะไร ⇒ **ไม่นับ** เพราะมันควรเท่ากันทั้งแถวอยู่แล้ว
 *              — รอบแรกเช็คแค่ "ขนาดเท่ากันไหม" แล้วได้ 6 หน้าที่ไล่ดูแล้วไม่มีอะไรให้แก้
...
for (const name of PAGES) {
  ...
  try {
    ...
    const r = await p.evaluate(() => {
      ...
      /* หน่วยนับที่ระบบนี้ใช้จริง — เพิ่ม ไลน์/บริษัท/โซน/พาร์ท/เครื่อง (24/09) เพราะมันเป็น "หน่วย"
         เท่ากับ ชิ้น/คน ที่มีอยู่แล้ว · ⚠️ ห้ามขยายรายการนี้เพื่อให้หน้าใดหน้าหนึ่งผ่านด่าน
         เติมได้เฉพาะคำที่เป็นหน่วยนับจริงๆ ของโรงงาน */
      const CTX = /เป้า|target|เทียบ|\bvs\b|\/|%|ชิ้น|คน|นาที|น\.|ชม\.|บาท|รายการ|จุด|ใบ|ครั้ง|กะ|วัน|เดือน|ไลน์|บริษัท|โซน|กลุ่ม|พาร์ท|เครื่อง|kWh|ppm|PPM/;
      ...
      for (const el of document.querySelectorAll('*')) {
        ...
        if (bi.includes('gradient') && !bi.includes('repeating-')) {
          ...
          const hairline = r.width <= 3 || r.height <= 3;   // เส้น ไม่ใช่พื้นที่ (ดูหัวไฟล์)
          if (optedOut(el)) { out.skipped++; } else if (!flatTint && !hairline) {
      ...
      // 4) แถวที่มี "ยอดรวม" ปนกับ "ส่วนประกอบ" แต่ทุกใบขนาดเท่ากัน (ดูหัวไฟล์)
      const numOf = (t) => { const m = String(t).replace(/,/g, '').match(/-?\d+(?:\.\d+)?/); return m ? Number(m[0]) : null; };
      ...
      for (const n of bigNums) {
        ...
        if (!(fsMax / fsMin < 1.15 && wMax / wMin < 1.12)) continue;   // มีพระเอกอยู่แล้ว

        /* มี "ยอดรวม" อยู่ในแถวจริงไหม — 2 ทาง (ดูหัวไฟล์) */
        const TOTAL_WORD = /ทั้งหมด|ยอดรวม|รวมทั้ง|\bรวม\b|\btotal\b/i;
        /* 🔴 ดู **บรรทัดแรกของการ์ด (= ป้ายชื่อ) เท่านั้น** ห้ามสแกนทั้งใบ —
           คำว่า "รวม/ทั้งหมด" โผล่ในประโยคอธิบายได้ตลอด (เจอจริง: การ์ด OOE เขียนว่า
           "ฐาน = เวลากะทั้งหมด (รวมพัก + หยุดตามแผน)" แล้วถูกจับว่าเป็นใบยอดรวม) */
        const byLabel = sibs.some(s => {
          const c2 = cardOf(s.el);
          return c2 && TOTAL_WORD.test(((c2.innerText || '').split('\n')[0] || ''));
        });
        const vals = sibs.map(s => numOf(s.t));
        let bySum = false, total = null;
        if (vals.every(v => v != null && Number.isFinite(v))) {
          total = Math.max(...vals);
          const rest = vals.reduce((a, v) => a + v, 0) - total;
          bySum = total > 0 && Math.abs(rest - total) / total <= 0.02;
        }
        if (!byLabel && !bySum) continue;                              // พี่น้องกันจริง = ไม่นับ
        out.flat.push(`${sibs.length} ใบ @${Math.round(fsMax)}px — `
          + (bySum ? `ใบ "${total}" คือผลรวมของอีก ${sibs.length - 1} ใบ` : 'มีใบที่ป้ายบอกว่าเป็น "ยอดรวม"'));
      }
