...
export function moveTargets(mat, ctx = {}) {
  ...
  /* 🔴 ตะกร้าสุดท้าย — ไลน์ที่เหลือทั้งโรงงาน (2026-09-09)
     ที่มา (feedback หน้างาน): *"มันขึ้นไลน์ไม่ครบนะ LINE C (200 Ton), (110&300 Ton) ก็ไม่ขึ้น"*
     ⚠️ ต้นเหตุ: กลุ่มข้ามแผนกกลุ่มเดียวที่มีคือ `forming_out` ซึ่งกรองด้วย `isFormingLine(type)`
        ⇒ ไลน์ที่ **ยังไม่ได้ตั้ง `line_type`** (LINE C/LINE D ของ PD1) ตกหมดทุกกลุ่ม = เลือกไม่ได้เลย
        ซึ่งขัดกฎของไฟล์นี้เอง ("เสนอลำดับ ไม่ตัดตัวเลือกทิ้ง") — master ไม่ครบไม่ควรทำให้เป็นทางตัน
     ⇒ ปิดท้ายด้วยไลน์ที่เหลือทุกตัวเสมอ · อยู่ล่างสุด (ลำดับความน่าจะใช่ต่ำสุด) และ
        ตอนกดย้ายมีด่านยืนยัน "ข้ามแผนก" + ด่าน "ไม่ใช่ไลน์ขึ้นรูป" คุมอยู่แล้ว */
  push('any', '🏢 ไลน์อื่นทั้งโรงงาน', 'ปลายทางสำรอง — ตั้ง line_type ให้ครบแล้วจะถูกจัดขึ้นกลุ่มบน',
    allLeaf.map(norm).filter(n => n && n !== norm(at)));
