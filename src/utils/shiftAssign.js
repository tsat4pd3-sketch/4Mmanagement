...
/**
 * ทีมของ "พนักงาน" ที่หัวหน้าคนนี้ต้องเห็นในใบเช็คชื่อ — คืน `null` = ไม่จำกัดทีม
 *
 * 🔴 ที่มา (feedback หน้างาน 2026-09-21): *"พนักงานบางคนมีปัญหาด้านสุขภาพ เค้าลงทีม C
 *    เพราะไม่ได้สลับกะ แต่หัวหน้าจะไม่เห็นเค้าในการเช็คชื่อ"*
 *
 * 21/08 แก้ไปแล้วครึ่งเดียว — แก้ฝั่ง "หัวหน้าเป็นทีม C" (seesAllTeams) แต่ **ฝั่งตรงข้ามยังพัง**:
 * หัวหน้าทีม A/B กรอง `.eq('team', ตัวเอง)` ⇒ **พนักงานทีม C หายจากใบเช็คชื่อทั้งของหัวหน้าทีม A
 * และทีม B** ทั้งที่ทีม C = กะเช้าตลอด จึงยืนอยู่หน้างานกับทีมที่เข้าเช้าเสมอ
 * วัดจริง 21/09: พนักงานทีม C 23 คน (operator 7) — **2 คนไม่เคยถูกเช็คชื่อเลยสักครั้ง**
 * (ปิยพงษ์ · มาเนตร — LINE D 110&300 Ton) และอีก 2 คนหยุดถูกเช็คพร้อมกันตั้งแต่ 27/08
 *
 * ⇒ หัวหน้าที่หมุนกะต้องเห็น **ทีมตัวเอง + ทีมที่ไม่หมุนกะ** เสมอ
 *   ไม่ต้องกลัวว่าจะรกตอนกะดึก: `resolveAssignedShift()` ให้ทีม C = `day` เสมอ และจอเช็คชื่อ
 *   กรอง "เฉพาะกะนี้" อยู่แล้ว ⇒ กะดึกยังไม่เห็นเขา · กะเช้าเห็น = ถูกต้องตามของจริง
 *
 * ⚠️ ขอบเขต "ไลน์" ยังคุมเหมือนเดิม — ปลดเฉพาะแกนทีม
 */
export function teamsVisibleToLeader(team) {
  if (seesAllTeams(team)) return null;                       // หัวหน้าที่ไม่หมุนกะ = เห็นทุกทีมอยู่แล้ว
  const own = String(team).trim().toUpperCase();
  return [...new Set([own, ...NON_ROTATING_TEAMS])];
}
