...
test('🔴 parsePullFile — แถวที่อ่านไม่ออกต้องถูกนับรายงาน ห้ามข้ามเงียบ', () => {
  ...
  // นับเฉพาะคำเตือน "ข้ามแถว" — ห้ามนับ warnings ทั้งก้อน ไม่งั้นคำเตือนคนละเรื่อง
  // (เช่น เตือนวันที่เก่า) ที่เพิ่มทีหลังจะทำเทสนี้ตกทั้งที่เจตนา "ห้ามข้ามเงียบ" ยังถูกอยู่
  assert.equal(r.warnings.filter(w => w.startsWith('ข้าม')).length, 3);
