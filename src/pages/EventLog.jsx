...
export default function EventLog() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (role === 'leader' && userLineId) {
      ...
      const fam = myLine ? getLineFamilyNames(lineData || [], myLine.name) : [];
      const famSet = new Set(fam.map(norm));
      scopedNames = fam;
      visibleLines = visibleLines.filter(l => famSet.has(norm(l.name)));
    } else if (scopeSecs.length) {
      visibleLines = visibleLines.filter(l => inSectionScope(scopeSecs, l.section));
      scopedNames = visibleLines.map(l => l.name);
    }
    ...
    // safety net: กรองซ้ำฝั่ง client เผื่อ line_name มีช่องว่าง/ตัวพิมพ์ต่าง (query .in เทียบ exact)
    let visibleLogs = logData || [];
    if (scopedNames) {
      const okSet = new Set(scopedNames.map(norm));
      ...
      <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="mgrid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <TimeInput24 value={form.event_time} onChange={e => f('event_time', e.target.value)} />
          </div>
        </div>
...
function exportExcel(logs) {
  ...
  // เลขฟอร์มจากทะเบียนเอกสาร (ตั้งที่ /doc-forms) — ไม่ตั้ง = ไฟล์หน้าตาเดิม
  const dfCqi = docFormSync('cqi15_event_log', {});
  const cqiCode = fullCode(dfCqi);
  if (cqiCode) { rows.push([]); rows.push([`เลขฟอร์ม: ${cqiCode}${dfCqi.effective_date ? ` · Effective: ${dfCqi.effective_date}` : ''}`]); }
...
function exportPDF(logs) {
  ...
  win.document.write(withDocFoot(html, 'cqi15_event_log'));
  win.document.close();
}
