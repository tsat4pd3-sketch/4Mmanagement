...
/* ─── Rundown Stock Tab — จำลอง Balance FG รายวัน (แบบไฟล์ rundown stock ของหน้างาน) ──
   Balance วันนั้น = stock พร้อมส่งตอนนี้ − ยอดที่ต้องส่งสะสมถึงวันนั้น (order ที่ยังไม่ส่ง)
   stock เดินอัตโนมัติ realtime: ปิดออเดอร์ → รับเข้า / กดส่งแล้ว → หักออก
   ติดลบ = ถ้าไม่ผลิตเพิ่ม ของจะขาดวันนั้น → เอาไปตัดสินใจเปิด OT/เพิ่มแผนผลิต */
function RundownTab({ refreshKey, custLabel }) {
  const HORIZON = 14;
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: stk }, { data: ods }] = await Promise.all([
      ...
      {view.length === 0 ? (
        ...
        <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border2)', display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
            <span style={{ fontWeight: 800, fontSize: 14, color: 'var(--text)', fontFamily: 'var(--font-display)' }}>📉 Rundown Stock — Balance FG รายวัน</span>
                  ...
                  <tr key={r.mat_no} style={{ borderTop: '1px solid var(--border)' }}>
                    <td style={{ padding: '7px 12px', position: 'sticky', left: 0, background: 'var(--card)', zIndex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--muted)', maxWidth: 190, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {r.part_name || ''}{r.customers.size ? ` · ${[...r.customers].map(c => custLabel ? custLabel(c) : c).join(', ')}` : ''}
                      </div>
    ...
    <div style={{ padding: 'clamp(12px, 2vw, 24px)', maxWidth: 'min(96vw, 1600px)', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {[
          ...
          { id: 'rundown',  label: '📉 Rundown Stock' },
      ...
      {tab === 'rundown' && <RundownTab refreshKey={refreshKey} custLabel={custLabel} />}
