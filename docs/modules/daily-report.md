# Daily Report — ออเดอร์ manual สำหรับไลน์ไม่มี kanban card (2026-07-12)

> ย้ายมาจาก `CLAUDE.md` (2026-09-03 — แยกไฟล์เพื่อลด context) · โหลด**เฉพาะเมื่อแตะโมดูลนี้** · แก้ไฟล์นี้แทน CLAUDE.md เมื่อกฎของโมดูลเปลี่ยน


ไลน์บางไลน์ (เช่น HDF1 ที่ส่งงานต่อ LASER CUT 123) **ไม่มีเลข SAP order ให้สแกน** เปิด-ปิดใบแบบปกติไม่ได้:

- ปุ่ม "✍️ เปิดเป้า (ไม่มีบาร์โค้ด)" ข้างปุ่ม Scan เปิด Order — leader ตั้งเป้า (เลือกสินค้า + จำนวน) → สร้าง `prod_orders` ที่ `is_manual=true`, **`prod_no='MANUAL-YYMMDD-HHmmss-XX'`** (YYMMDD=วันงานของกะ · HHmmss=เวลากดเปิด · XX=สุ่ม 2 ตัว [อักษร/เลขไม่กำกวม] — กันเลขซ้ำข้ามวัน+กันชน 2 ไลน์วินาทีเดียวกัน · เดิม `MANUAL-HHmmss` มีแค่เวลาในวันเลยซ้ำข้ามวันจริง เจอ MANUAL-084627 ซ้ำ 15/07+17/07 กระทบ traceability `/order-trace`+backflush `source_prod_no` · เปลี่ยน 2026-08-03), `qty_target`=เป้า, `qty`=เป้า (ใช้กับ capacity check/บอร์ดเหมือนใบปกติ) · **⚠️ `prod_no` ไม่ unique "ทั้งตาราง" โดยตั้งใจ (ยกยอดข้ามกะสร้างแถวใหม่ prod_no เดิม) — ห้ามใส่ unique index ระดับตาราง** · **แต่ unique ระดับ "ต่อกะ" มีแล้วและห้ามลบ: `prod_orders_session_prodno_uniq` = partial unique `(session_id, prod_no) where status <> 'imported'`** (migration `20260825_prod_orders_session_prodno_unique.sql` · **apply แล้ว 2026-08-26**) — กัน 2 เครื่องสแกนการ์ดใบเดียวกันพร้อมกันแล้วได้ใบซ้ำ (client กันด้วย state อย่างเดียว มีหน้าต่าง race ~1-2 วิ จาก realtime debounce 600ms + latency · ปิดใบซ้ำ = **เข้าคลัง 2 เท่า + ระเบิด BOM 2 รอบ**) · ไม่ขัดกับกฎยกยอดเพราะ chain ยกยอดอยู่คนละ session เสมอ และใบเดิมกลายเป็น `imported` ซึ่งถูกกันไว้ใน `where` แล้ว · ฝั่ง client แปลง error 23505 เป็น "ถูกเปิดโดยเครื่องอื่นแล้ว" ให้แล้ว · มี ⏪ **เปิดย้อนหลัง**เหมือนใบสแกน (บังคับกรอกเวลา + กันหลุดกรอบกะ + `opened_at` anchor กับ work_date — helper ร่วม `backfillIsoFromTime` · 2026-07-13) · **งานคู่ RH/LH**: สินค้ามี `pair_mat_no` → ถาม confirm แล้วเปิดเป้าคู่ให้อัตโนมัติ (`prod_no`+`P`, ผูก `paired_order_id` สองทาง, sync `opened_at`) รองรับคู่**คนละไลน์** — เปิดเข้า session ที่เปิดอยู่ของไลน์คู่ (วัน/กะเดียวกัน) ถ้าไลน์คู่ยังไม่เปิดกะจะเตือนให้ไปเปิดเอง (2026-07-13)
- **งานคู่ RH/LH บนบอร์ด Heijunka = แถบวางขนาน ไม่เรียงต่อกัน (2026-07-21):** แม่พิมพ์คู่ปั๊มครั้งเดียวได้ทั้ง LH+RH = ทำพร้อมกัน (parallel) · บอร์ด (Dashboard + Management) จัดคิว **ระดับ sub-line** ปกติ "1 ไลน์ทีละใบ เรียงต่อกัน" แต่**พาร์ทที่มี `pair_mat_no` และคู่ของมันอยู่ในไลน์เดียวกันจริง → แยกเป็นเลนคิวของตัวเอง (parallel) เริ่มพร้อมกัน** แถบ LH/RH จึงตรงกัน (helper `laneKeyOf` ตอนจัดกลุ่ม `byLane` ก่อนเรียก `computeQueuedPositionsFull`) · เดิมไม่เช็ค pair_mat_no เลยจับ LH เรียงหน้า RH เรียงหลังผิด (นับเวลาซ้ำ 2 เท่า) · พาร์ทไม่มีคู่ยังเรียงต่อกันเหมือนเดิม (1 ไลน์ทีละใบ ห้ามซ้อน)
- **โหมดการไหลงานของไลน์ (`production_lines.flow_mode`) — one-piece flow vs parallel machine (2026-07-23):** ไลน์บางไลน์ (เช่น SUB APRON) มีเครื่อง stand-alone หลายตัว (SP-10/72/74…) วิ่งพร้อมกันคนละรายการ ไม่ใช่สายเดียวไหลทีละชิ้น — เดิมบอร์ดจับ 1 sub-line = 1 เลนเรียงคิว เลยดูเหมือนงานต่อคิวกัน (พนักงาน workaround แตกเป็นไลน์ย่อยแยก) · เพิ่ม `flow_mode` (`one_piece_flow` ดีฟอลต์ / `parallel_machine`) + `parallel_stations` (จำนวนเครื่อง · null = นับจาก `machine_points`) — ตั้งที่ **LineSetup แผง ⚙️ ตั้งค่าไลน์ → 🏭 คุณสมบัติของไลน์นี้** (source of truth `src/utils/lineTypes.js` `FLOW_MODES`/`isParallelLine`) · migration `20260723_line_flow_mode.sql` (Main)
  - **บอร์ด (`byLane` ใน Dashboard+Management) แตกเลนตาม flow_mode:** `parallel_machine` → ใบที่ผูกเครื่อง (`prod_orders.machine_no`) = เลนของเครื่องนั้น (`line||M:no`) · ใบที่ยังไม่ผูก = กระจาย round-robin N เลน (`line||P:i%N`) เริ่มพร้อมกัน · `one_piece_flow` = 1 เลน/ไลน์ เรียงคิวเหมือนเดิม (+ pair RH/LH แยกเลน) · **เลือกเครื่องตอนเปิด Order** → `prod_orders.machine_no` (migration `20260723_prod_orders_machine.sql` DR)
  - **backward-compatible:** select `flow_mode`/`machine_no` เขียนแบบ best-effort (แยก query / retry ตัดคอลัมน์) — ยังไม่ apply migration บอร์ดไม่พัง (ทุกไลน์เป็น one_piece_flow) · **เพิ่มไลน์ parallel ใหม่ ตั้งที่ LineSetup ไม่ต้องแตกไลน์ย่อยปลอมอีก**
- พนักงาน**อัพเดทยอดสะสม (`qty_actual`) ทุกช่วงเบรคตาม break policy** จากช่องบนการ์ดใบ — เห็นยอดจริงทุก ~2 ชม. · `qty_updated_at` เก็บเวลาล่าสุด ใบที่ไม่อัพเดท > 2.5 ชม. ขึ้นเตือนเหลือง (นิ่ง — ตาม Andon เหลืองไม่กระพริบ)
- ปิดใบด้วยปุ่ม "✓ ปิดใบนี้ (ยอดจริง)" (ไม่ต้องสแกน) → `status=confirmed`, **`qty` และ `qty_ok` ถูกแทนด้วยยอดจริง** เพื่อให้ OEE/รายงาน/stock trigger (`coalesce(qty_ok, qty)`) นับจากของที่ผลิตได้จริง — เป้าเดิมยังอยู่ที่ `qty_target`
- ใบ manual ที่ค้างเปิดตอนปิดกะ เข้า flow ยกยอด/กรอก actual ของ modal ปิดกะเหมือนใบปกติ
- **ยอดสะสมของใบ manual ที่ยังเปิด ถูกนับเข้า "ผลิตได้"/ความคืบหน้า/แยกตามชิ้นงาน ทันที** (ไม่ต้องรอปิดใบ — ไม่งั้นเห็น 0 ทั้งกะ) และ**ทุกครั้งที่กรอกถูก log ลง `prod_order_qty_updates`** (qty_accum, qty_delta, is_final, logged_at/by) → การ์ดใบโชว์ชิปประวัติต่อช่วง เช่น "10:00 · สะสม 200 (+200)" "12:00 · สะสม 480 (+280)" (migration `20260713_prod_order_qty_update_log.sql`)
- **บนบอร์ด Heijunka (Dashboard/Management):** การ์ด manual ใช้ไอคอน ✍️ + ยอด `ทำได้/เป้า` และแถบ fill ในการ์ดวิ่งตาม qty_actual (ใบสแกนปกติแสดงเหมือนเดิม)
- migration: `20260712_prod_orders_manual_mode.sql` (DR, additive — ใบสแกนปกติไม่กระทบ)

### 🕐 ดูย้อนหลังใน `/management` — วัน+กะ ต้องคุมทั้งหน้า และย้อนหลัง = อ่านอย่างเดียว (2026-08-27 · feedback หน้างาน)

*"หัวหน้างานจะดูจุดเข้างานของพนักงานย้อนหลังหน้านี้ แต่เลือกกะไม่ได้ กลายเป็นแสดงแค่กะเช้าอย่างเดียว"*
ตรวจแล้วเป็น **2 อาการซ้อนกัน อย่าแก้แค่ข้อแรก**

| | อาการเดิม |
|---|---|
| ① เลือกกะไม่ได้ | `fetchData` ล็อก `getCurrentShift()` ตายตัว · **ไม่มี selector อยู่เลย** |
| ② 🔴 ย้อนหลังไม่ได้จริง | `fetchData` ใช้ `getWorkDate()` (วันนี้) ขณะที่ตัวเลือกวันที่ (`boardDate`) คุมแค่**แถบ Heijunka** ⇒ เลือก 14/08 แล้ว **คนบนผังยังเป็นคนของกะปัจจุบัน** — จอเขียนว่า "📅 ย้อนหลัง 14/08" แต่โชว์ของวันนี้ = **จอโกหก** |

> #### 🔴 กฎเหล็ก — มีตัวเลือกวันย้อนหลังบนหน้าไหน **ทุกแผงในหน้านั้นต้องตามวันเดียวกัน**
> ปล่อยให้บางแผงตามวันที่เลือก บางแผงยังเป็น "ตอนนี้" = คนอ่านแยกไม่ออกว่าอันไหนเป็นอันไหน
> และตัดสินใจจากข้อมูลผิดวันโดยไม่รู้ตัว · `fetchData` ยึด `boardDate` + `viewShift` แล้ว
> (ทั้ง `daily_production_logs` · `line_helpers` · `four_m_logs` · `operator_special_tasks`)

> #### 🔴 กฎเหล็ก — โหมดย้อนหลัง/กะอื่น = **อ่านอย่างเดียว** ห้ามให้ลากคน
> `isLiveView = boardDate === วันงานวันนี้ && viewShift === กะปัจจุบัน` — **นอกเงื่อนไขนี้เขียนอะไรไม่ได้**
> เพราะการลากคนจะไปเขียน `daily_production_logs.assigned_line` ของ**วันนั้น** พร้อม stamp
> `station_assignment_logs` ด้วยเวลา **"ตอนนี้"** ⇒ ประวัติเพี้ยนแบบย้อนแก้แทบไม่ได้
> - guard **ชั้นสุดท้ายอยู่ใน `assignWorker` / `assignSpecialTask`** ไม่ใช่แค่ซ่อนปุ่ม
>   (หมุดคนบนผังกับ `PoolCard` เดิมไม่เช็ค `can()` เลย — พึ่งการซ่อนปุ่มอย่างเดียวไม่พอ)
> - affordance ปิดด้วย (`draggable` / `cursor`) ไม่งั้นลากได้แล้วเด้ง toast ทีหลัง
> - **"ดูสด" คือ วันนี้ + กะปัจจุบันเท่านั้น** → พฤติกรรมของคนที่ใช้งานปกติไม่เปลี่ยนเลยสักอย่าง
>
> #### ⚠️ ตัวเลือกกะต้อง render เสมอ แม้กะนั้นไม่มีคนสักคน
> ผูกไว้กับบล็อกที่ render เมื่อมีคน = สลับไปกะดึกที่ว่างแล้ว **ไม่มีปุ่มให้สลับกลับ** (ทางตันที่มองไม่เห็น)
> · มีปุ่ม **⟳ กลับมาดูสด** ที่คืนทั้งวันและกะในคลิกเดียว
>
> #### ⚠️ 4M ยังบันทึกได้ในโหมดย้อนหลัง แต่ต้องบอกว่าจะลงวันไหน
> 4M = บันทึก "สิ่งที่เกิดตอนนี้" (ต่างจากการจัดคนที่เป็นการแก้ของวันนั้น) จึงไม่บล็อก
> แต่ลิสต์ที่แสดงอยู่เป็นของวันย้อนหลัง → บันทึกแล้วไม่โผล่ = เข้าใจว่าบันทึกไม่ติด
> → แผง 4M ขึ้นเตือนว่า "บันทึกใหม่จะลงวันที่วันนี้ ไม่โผล่ในรายการของ <วันที่ดู>" **ห้ามเงียบ**
>
> **⚠️ ข้อจำกัดที่ยังอยู่:** โหมดย้อนหลังแสดง **"จุดงานสุดท้ายของกะนั้น"** (`assigned_line` ถูกเขียนทับเรื่อยๆ)
> ไม่ใช่ไทม์ไลน์รายช่วง · ของจริงรายช่วงอยู่ที่ `station_assignment_logs` (มี `period`/`started_at`/`ended_at`)
> ถ้าจะทำต้องเพิ่มตัวเลือกช่วงเวลา — ยังไม่ทำ · จอเขียนกำกับไว้แล้ว

### บอร์ด Heijunka — ตู้รวม + ตัวกรอง (Dashboard) · กันผังถูกบีบ (Management) (2026-08-19)

- **ตู้ Heijunka รวมทุกไลน์ = section บอร์ดใน `/dashboard` อยู่แล้ว** (การ์ดบอร์ดต่อกลุ่มไลน์ เรียงลงมาทั้งหน้า + เลือกวันย้อนหลัง) — **ห้ามสร้างหน้าตู้รวมใหม่** ต่อยอดที่นี่ · เพิ่ม filter bar แล้ว: ชิปกลุ่มไลน์ + ช่องค้นพาร์ท/MAT/เลขใบ (`boardLineSel`/`boardQuery`)
  - **⚠️ กรองที่ "ชั้นแสดงผลแถว" (`visRows`) เท่านั้น — ห้ามกรอง cards ก่อนคำนวณคิว** ตำแหน่งใบ/เวลาคาดเสร็จผูกกับคิวทั้งไลน์ ตัดใบออกก่อนคำนวณ = เวลาเพี้ยนทั้งแถว · สรุปหัวการ์ด (ผลิต/เป้า/ล่าช้า) ยังนับทุกแถวตามจริงโดยตั้งใจ
  - **✍️ แถวที่มีแต่ "ใบเปิดเอง" ยุบไว้เป็นค่าเริ่มต้น (2026-08-27 · คำสั่ง user "อยากให้เน้นชิ้นงานที่เป็น order จาก SAP เป็นหลัก · manual ยุบไว้ได้"):** บอร์ดนี้เป็นจอภาพรวม ของหลักคือใบที่สแกนบัตรคัมบังจาก SAP · ใบเปิดเองเรียงตามตัวอักษรขึ้นมาบังของจริง (เคสจริง `291+088` + `GST FRT FNDR` กิน 2 แถวบนสุดของ LINE APRON ASSY)
    - **`isManualRow` = ทุกใบในแถวเป็น `is_manual`** — **แถวที่มีใบสแกนปนแม้ใบเดียว ห้ามยุบ** (จะพาใบ SAP หายไปด้วย)
    - แถว manual **เรียงท้ายเสมอแม้ตอนกางดู** · ยอดผลิต/pace บนหัวการ์ด **ยังนับใบพวกนี้ครบ** (ยุบแค่การแสดงผล)
    - **ห้ามซ่อนเงียบ** — แถบ `✍️ ใบเปิดเอง (ไม่มีบัตรคัมบังให้สแกน) · N พาร์ท` + ปุ่มกางดู อยู่เสมอเมื่อมี · จำต่อเครื่องที่ `esm_board_manual` (สวิตช์เดียวคุมทั้งบอร์ด → ป้ายเขียน "(ทั้งบอร์ด)")
    - **🔴 ห้ามเอาไปทำที่ `/management` (user เคาะแล้ว 2026-08-27: "บริหารไล่หัวหน้างานเองไม่ต้องยุบ ยุบหน้าที่เป็นภาพรวมแหละถูกละ")** — บอร์ดนั้นเป็น**จอทำงานของไลน์** ใบเปิดเองคืองานจริงของหัวหน้ากลุ่ม (ไลน์ที่ไม่มีบาร์โค้ดให้สแกน) ยุบแล้วเขาจะมองไม่เห็นงานตัวเอง · **หลักที่ใช้แยก: จอภาพรวม = ยุบได้ · จอที่คนใช้ทำงาน = ห้ามยุบ** — ใช้กับฟีเจอร์ "ยุบ/ซ่อน" อื่นบนบอร์ดคู่นี้ด้วย
  - แถวที่ถูกกรองซ่อน = แถบนับ "ซ่อน N พาร์ท" เสมอ (ห้ามหายเงียบ) · ชิปไลน์ที่เลือกไว้แต่ไม่มีในวันนั้น = ตกกลับ "ทุกไลน์" (§5.3)
- **Management `boardWouldSquish` ตัดสินจาก "ความสูงจริงที่วัดได้" ห้ามกลับไปใช้สูตรเดา** — เดิมเดา 64px/แถว (แถวจริง ~88px: รูป 46 + 2 เลน + chips/PLANNER เหนือบอร์ด) → ระบบคิดว่า "โชว์ผัง+บอร์ดคู่กันพอ" ทั้งที่ไม่พอ → **ผังไลน์เหลือที่ ~100px marker คน/เครื่อง/จุดงานถูก clamp กองทับกันเป็นก้อนล่างจอ** (เจอจริง 2026-08-19 LINE APRON ASSY 8 แถว)
  - วัดด้วย **`scrollHeight` ของ section wrapper** (`boardSectionRef` — ได้ความสูงเนื้อหาธรรมชาติแม้ตอนถูก clip ในโหมด scroll จึงไม่ oscillate) ใน effect หลัง render ไม่ใช่ ResizeObserver (เนื้อหาโตโดย wrapper ไม่ resize ได้เมื่อทั้งคู่โดน clip) · บอร์ด unmount (สลับไปมุมมองผัง) = คงค่าล่าสุด ห้าม reset เป็น 0 ไม่งั้น squish เด้งกลับ · สูตรเดา (88px/แถว) เหลือเป็น fallback เฟรมแรกเท่านั้น
  - **บทเรียน: เงื่อนไข layout ที่ตัดสินจาก "ค่าประมาณ" จะ drift เมื่อ UI จริงเปลี่ยน** (เพิ่มแถบ PLANNER/chips แล้วไม่มีใครไปแก้สูตร) — ตัดสินจากการวัดจริงเสมอเมื่อวัดได้

### ใบผลิต "ปิดก่อนเปิด" (confirmed_at < opened_at) — กัน 3 ชั้นแล้ว (2026-07-30)

เคยพบ 33 ใบที่ `confirmed_at < opened_at` จาก 2 สาเหตุ: (ก) **clock skew** — เปิดใบใช้ now() ฝั่ง DB (default) แต่ปิดใบใช้ `new Date()` ฝั่งเครื่อง client ที่นาฬิกาช้ากว่า ~25 วิ → ใบสแกนเปิด-ปิดไวติดลบไม่กี่วินาที (ข) **ยิงย้อนหลังกรอกเวลาอนาคต** — guard เดิมกันแค่หลุดกรอบกะ ไม่ได้กันเวลาที่ยังมาไม่ถึง (เจอจริง: ปิด 04:35 กรอกเริ่ม 05:17 → ติดลบได้เป็นชั่วโมง)
- **ชั้น UI:** handleScanOpen + handleManualOpen บล็อกเวลา backfill ที่ > ตอนนี้ ("ยังมาไม่ถึง")
- **ชั้น DB (safety net):** trigger `trg_prod_orders_close_time_guard` (BEFORE INSERT/UPDATE) — ใบ backfill ซ่อม `opened_at := confirmed_at` (เวลาปิดคือของจริง) · ใบปกติซ่อม `confirmed_at := opened_at` (เวลาเปิดฝั่ง server คือของจริง) · migration `20260730_prod_orders_close_time_guard.sql` (DR — apply แล้ว + ซ่อมข้อมูลเก่า 33 ใบเป็น 0)
- โค้ดใหม่ที่เขียน timestamp คู่เปิด-ปิดในตารางอื่น ให้ระวังเรื่องนาฬิกาสองแหล่ง (server default vs client `new Date()`) แบบเดียวกัน
- **ตรวจแล้ว: การซ่อม 33 ใบไม่กระทบ OEE ที่ stamp ไว้** (audit 2026-08-03 — replicate computeOEE ใน harness แล้วเทียบ timestamp เก่า vs ใหม่ ครบ 16 กะที่เกี่ยวข้อง: ค่า A/P เท่ากันทุกกะ ต่างสุด 0.03 จุด) เพราะ window ติดลบถูก `matEnd <= matStart` ข้ามอยู่แล้วในสูตร และ window ศูนย์หลังซ่อมก็ถูกข้ามแบบเดียวกัน — **ไม่ต้อง recompute ย้อนหลัง** · หมายเหตุ: 8 กะ stamped P ต่างจาก harness 4-19 จุดจากสาเหตุอื่น (CT master/break policy เปลี่ยนหลังปิดกะ / override ตอนปิด) — เป็นเหตุผลที่ห้าม blanket-recompute กะเก่าด้วย master ปัจจุบัน

### ถอยใบที่สแกนปิดไปแล้ว (revert confirmed → open) — 2026-07-15

ปุ่ม **↩️ ถอยใบ** บนใบ `confirmed` ใน DailyReport (เคสจริง: หัวหน้ากลุ่มสแกนปิดเกินยอดที่ผลิตได้/ปิดผิดใบ):
- เงื่อนไข: **เฉพาะกะที่ยังเปิดอยู่** (`selSession.status === 'open'`) — หลังปิดกะ/ส่งขออนุมัติ ยอด+OEE ถูก stamp ลง session แล้ว ถอยไม่ได้ · สิทธิ์ = `canEditRecords` (leader ตอนกะเปิด / manager+)
- สิ่งที่ย้อนให้: status→open, ล้าง confirmed_by/at + qty_ok, ใบ manual คืน `qty = qty_target` (ยอดสะสม qty_actual คงไว้) + **ถอนแถว stock ที่ trigger `trg_post_confirmed_output` โพสต์อัตโนมัติ** (ลบ `line_stock_transactions` ที่ `ref_order_id`+`created_by='auto'`+`type='issue'` — ตัวกันโพสต์ซ้ำของ trigger เช็คจากแถวนี้ ลบแล้วสแกนปิดใหม่จะโพสต์ให้ใหม่ถูกต้อง)
- Audit: `prod_orders.reopened_by/reopened_at/reopen_count` (migration `20260715_prod_orders_reopen_log.sql` DR) — การ์ดใบโชว์ชิป "↩️ เคยถอยใบ N ครั้ง · ชื่อ" เสมอ ให้หัวหน้าแผนกตรวจย้อนหลังได้ · update guard `.eq('status','confirmed')` กันถอยซ้ำสองเครื่องพร้อมกัน

### %A ไลน์เครื่องขนาน — DT ผูกเครื่องหักแค่ 1/N (2026-08-04 · แยก N ออกจาก flow_mode 2026-08-05)

ไลน์ที่มีเครื่องหลักหลายตัววิ่งขนาน (เช่น **LASER-345 = เลเซอร์ 3 ตัว LS-03/04/05**) พนักงานลง DT **แยกรายเครื่อง** ถูกต้องแล้ว — แต่ `computeOEE` เดิมบวกนาทีทุกเครื่องรวมแล้วหักจากเวลาไลน์เดียว → DT 3 เครื่องพร้อมกันถูกหัก 3 เท่า **%A โดนกดเป็น 0 ทั้งที่ของออกปกติ** (เจอจริง 2026-08-03: LASER-345 A=0.00, P=100, Q=97)
- แก้ที่ `computeOEE` + `dtOverlapMin` (รับ `weightFn`): DT ที่**ระบุเครื่อง** หักน้ำหนัก **1/N** · DT **ไม่ระบุเครื่อง** (ไฟดับ/รอวัตถุดิบทั้งไลน์) = หยุดทั้งไลน์ หักเต็มเหมือนเดิม · N=1 (ไม่ตั้ง) = พฤติกรรมเดิมเป๊ะ
- **⚠️ N (จำนวนเครื่องขนาน) แยกจาก `flow_mode` แล้ว (2026-08-05 · helper กลาง `parallelUnitsOf(line, fallbackCount)` ใน `src/utils/lineTypes.js`):** 2 แกนคนละเรื่อง —
  - **`parallel_stations` (N)** = หัก DT 1/N ใน %A · **ตั้งได้ทุกโหมดไหลงาน** ที่ LineSetup แผง ⚙️ ตั้งค่าไลน์ → 🏭 คุณสมบัติของไลน์นี้ — เคสหลัก: **ไลน์งานคู่ LH/RH เช่น LASER-345/789** (เลเซอร์ 3 ตัวขึ้น product เดียว 2 พาร์ทซ้าย-ขวา) เป็น `one_piece_flow` บนบอร์ด (เลนคู่มาจาก `pair_mat_no` ไม่ dispatch ผูกเครื่อง) แต่ตั้ง N=3 เพื่อหัก DT 1/3
  - **`flow_mode='parallel_machine'`** = การจัดเลนคิวบนบอร์ด + แผงเลือกเครื่องตอนเปิด Order (เช่น SUB APRON — เครื่อง stand-alone คนละ product) · ถ้าเป็นโหมดนี้แต่ไม่ตั้ง N → fallback นับเครื่อง active ของไลน์
  - จุดที่ใช้ 1/N ครบแล้ว: `DailyReport computeOEE` (ค่า stamp) + **OEE สด `Dashboard` (per-session board) + `FactoryMap` (loadStatus)** — ทั้งคู่ผ่าน `parallelUnitsOf` และ query `downtime_logs` ต้อง select `machine_no` · จุดใหม่ที่คำนวณ %A ให้ใช้ helper นี้เท่านั้น
- **แผงเลือกเครื่องตอนเปิด Order (ไลน์ parallel_machine) ลิสต์เฉพาะเครื่องของไลน์ที่เปิดกะจริง** (เดิมดึงทั้ง family → เครื่อง HYDROFORM 30+ ตัวโผล่ปน · fallback ไป family เฉพาะเมื่อไลน์ไม่มีเครื่องของตัวเอง + dedupe ตาม machine_no · 2026-08-05)
- **migration `20260723_line_flow_mode.sql` เพิ่ง apply บน Main จริง 2026-08-04** (ค้างมาตั้งแต่ 2026-07-23 — ก่อนหน้านี้คอลัมน์ไม่มีจริง โค้ด tolerant เลยเงียบ) · ไลน์ขนานอื่น (SUB APRON ฯลฯ) ตั้งเองที่ LineSetup แผง ⚙️ ตั้งค่าไลน์ → 🏭 คุณสมบัติของไลน์นี้
- **กะที่ stamp A=0 ผิดไปแล้ว (pending_close):** SV กด ✕ ปฏิเสธ → หัวหน้ากลุ่มขอปิดกะใหม่ = recompute ด้วยสูตรใหม่เอง (ห้าม blanket-recompute กะเก่าตามกฎเดิม)
- **✅ backfill กะปิดแล้วของ LASER-345/789 เสร็จ 2026-08-05** (migration `20260805_backfill_oee_a_parallel_one_third.sql` DR — apply แล้ว): replicate สาย A ของ computeOEE เป็น SQL (เงื่อนไขย่อสูตรตรวจกับข้อมูลจริง: DT ทุกแถวระบุเครื่อง+timestamp ครบ · ≤1 order confirmed/กะ · break policies ทั้งหมด common) คำนวณ a_old (weight 1)/a_new (1/3) → อัพเดทเฉพาะกะที่พิสูจน์ได้ว่า stamp ด้วยสูตรเก่า (|a_old−stamp|≤4) · ตรวจหลัง apply: 71/71 กะตรง a_new หมด · ตัวหนักสุด: 789 03/08 กะเช้า 31.69→77.23, 01/08 46.45→85.53, กะ A=0 สี่กะขึ้นเป็น 91-100 · กะที่ปิดหลัง deploy (stamp สูตรใหม่แล้ว) ไม่ถูกแตะ · **ไลน์ทั้งคู่ตั้ง `one_piece_flow` + `parallel_stations=3` แล้ว** (งานคู่ LH/RH — บอร์ดเลนคู่จาก pair_mat_no, ไม่มีแผงเลือกเครื่อง)

### %A ต้องนับ Downtime ที่กรอกแค่จำนวนนาที (ไม่มีเวลาเริ่ม) — แก้ 2026-07-24

`computeOEE` สาย A แบบแยกตาม MAT ใช้ `dtOverlapMin` (ทับซ้อนช่วงเวลา) ซึ่ง**ข้าม DT ที่ `started_at` = null** (โหมดกรอกแค่นาที) → เคสจริง: หยุดนอกแผน 20 นาทีแต่ %A = 100 (Sup Assy2) · แก้: หลัง loop ต่อ MAT หัก untimedPlanned/untimedUnplanned ที่**ยอดรวม** (`totalNetAvailByMat`/`totalRunMinByMat`) ก่อนหาร — ไม่ต้องรู้ว่าตกช่วง MAT ไหน · fallback ทั้งกะ (ไม่มี order) นับอยู่แล้วผ่าน duration sums

### 🔴🔴 เวลาที่กรอกต้องอยู่ใน "กรอบกะ" เสมอ — 2 ต้นเหตุที่ทำให้เวลาเพี้ยน (2026-09-23 · user จับได้จากไทม์ไลน์)

**เคสที่จุดชนวน:** `Laser GOR 23/09 กะเช้า` (เปิด 08:00) มี downtime **04:19–04:39** โผล่บนไทม์ไลน์
= ก่อนเปิดกะ 3 ชม. 41 นาที · บันทึกตอน **16:39** ⇒ ตั้งใจ 16:19–16:39 ชัดเจน
*(user: "น่าจะตั้งใจ 16:19 … ต้องทำ errorproof ดักไม่ให้เกิดอีกให้ได้")*

ไล่ฐานทั้งระบบ (9,664 แถว) พบ **86 แถวหลุดกรอบกะ** ตั้งแต่ 26/06 ถึง 23/09 — แยกได้ **2 ต้นเหตุคนละตัว**:

| # | ต้นเหตุ | ลายเซ็น | จำนวน |
|---|---|---|---|
| **A** | **บั๊กโค้ด** — กฎเลื่อนวัน hardcode `shift === 'night' && ชั่วโมง < 8` | กะดึกล้วน · ก่อนเปิดกะ ~12 ชม. เป๊ะ | 15 แถว / 890 นาที |
| **B** | **ไม่มีด่านตรวจเลย** — AM/PM สลับ / พิมพ์ผิด ไหลเข้าฐานเงียบๆ | กระจายทุกกะ | ที่เหลือ 71 แถว |

#### A) บั๊กโค้ด — `ชั่วโมง < 8` ใช้กับกะดึกที่จบ 08:00+ ไม่ได้
กะดึกเริ่ม 20:00 **จบ 08:00 หรือเลยไป** (5ส. 07:40–08:00 · ส่งกะ) ⇒ คนกรอก `08:00`/`08:30`
ไม่เข้าเงื่อนไข `< 8` → ถูก anchor ไว้ "วันเดียวกับ work_date" = **ก่อนเปิดกะ 11.5–12 ชม.**
· **ไม่ใช่ความผิดคนกรอก** — กรอกถูกแล้ว โค้ดวางผิดวันเอง
· สูตรเดียวกันถูกก๊อปไว้ **3 จุด**: `buildDT` (downtime) · `backfillIsoFromTime` (ใบผลิตย้อนหลัง) ·
  `carryImportOpenedAt` (ใบยกยอด) — แก้ครบทั้ง 3 แล้ว

#### 🧰 ของกลางใหม่ — `src/utils/shiftWindow.js` (pure · มีเทส 13 เคส)
| export | ทำอะไร |
|---|---|
| `shiftWindow(session, now)` | กรอบกะ `{ startMs, endMs, hardEnd }` — กะยังไม่ปิด ขอบบน = "ตอนนี้ + ผ่อนผัน 60 นาที" เพดาน `MAX_SHIFT_MIN` (16 ชม.) |
| `resolveShiftTime(hhmm, session, now)` | **'HH:mm' → ms ที่ตกในกรอบกะ** — ลอง offset วัน −1/0/+1 แล้วเลือกตัวที่เข้ากรอบ |
| `checkShiftTime(ms, session, now)` | `{ ok, kind: before/future/after/unknown, minutesOff, suggestHHmm }` |
| `fmtOffset` / `windowLabel` | "3 ชม. 41 นาที" · "08:00 → ตอนนี้" |

🔴 **ห้าม hardcode "ก่อน 08:00 = วันถัดไป" อีก — มีด่าน `regressionGuards` (`shift-midnight-hardcoded-hour`) แล้ว**
เลือก offset วันจาก **กรอบกะจริงของ session นั้น** ⇒ กะดึกที่เริ่ม 22:30 หรือกะเช้าที่ลาก OT ข้ามเที่ยงคืน
ก็ถูกต้องโดยไม่ต้องแก้โค้ดเพิ่ม · แบ่งงานกับ `oee.js`: `shiftFrameOf()` = กรอบไว้ **รัด** ตอนคิด OEE ·
`shiftWindow.js` = **ด่านตอนกรอก** (และเรียก `shiftFrameOf` ต่อ ไม่คิดสูตรซ้ำ)

#### B) ด่านกันเวลาเป็นไปไม่ได้ — 3 ชั้น (ห้ามเตือนแล้วปล่อยผ่าน)
1. **ตาเห็นตั้งแต่กรอก** — `TimeInput24` โชว์ `= 16:19 น.` ต่อท้ายช่องเสมอ
   · `<input type="time">` เก็บค่า 24 ชม. ก็จริง แต่ **แสดงผลตามเครื่อง**: มือถือ/จอที่ตั้งเป็น 12 ชม.
     มีช่อง AM/PM แยก **ไม่แตะ = ค้างที่ AM** ⇒ ตั้งใจ 16:19 ได้ 04:19 ← ต้นเหตุเคสนี้
2. **แถบกรอบกะใต้ช่องเวลา** ในโมดัล Downtime — `⏱ กรอบกะนี้: 08:00 → ตอนนี้` + เตือนแดงสดทันทีที่หลุดกรอบ
3. **ด่านตอนกดบันทึก** (`handleAddDT`) — หลุดกรอบ = **ไม่ให้บันทึก**
   · **±12 ชม. แล้วเข้ากรอบพอดี** = ลายเซ็น AM/PM สลับ → เสนอแก้ให้คลิกเดียว
     (เติมลงฟอร์มแล้วให้คนกดบันทึกเอง — **ห้ามเขียนลงฐานแทนคน**)
   · ก่อนเปิดกะ ≤ 60 นาที → ถามยืนยัน (ไลน์เริ่มก่อนเวลาที่ลงไว้ เป็นของจริงได้)
   · นอกนั้นบล็อก + บอกกรอบกะ และชี้ว่า "ถ้าเวลาเปิด-ปิดกะตั้งผิด ให้ไปแก้ที่ ✏️ แก้เวลากะ"
     (ห้ามดัดเวลา downtime เพื่อเลี่ยงด่าน)
   · ช่วงยาวเกิน 1 กะ (> `MAX_SHIFT_MIN`) ก็บล็อก

⚠️ **กฎที่ตกผลึก:** ค่าที่คนกรอกหลุดกรอบ **ห้ามดัดให้เข้ากรอบเอง** — `resolveShiftTime` คงค่าเดิมไว้
แล้วตั้งธง `inWindow:false` ให้จอไปเตือน (ดัดให้ = เดาแทนคน ซึ่งผิดได้เงียบกว่าเดิม)

#### 📦 Backfill 86 แถวที่หลุดกรอบ — `20260924_dt_outside_shift_backfill_dr.sql` (apply แล้ว 24/09)

สำรองก่อนแก้ครบทุกแถวที่ **`archive.dt_outside_shift_backfill_20260924`** (เก็บ old/new ทั้ง `started_at`+`ended_at`
⇒ ย้อนได้ด้วย `update downtime_logs set started_at=old_started_at, ended_at=old_ended_at from ...`)
🔴 **ตารางสำรองอยู่ schema `archive` ไม่ใช่ `public`** (กฎ + ด่าน `backup-tables-go-to-archive`)

| รอบ | fix_kind | เงื่อนไขที่พิสูจน์ได้ | แถว |
|---|---|---|---|
| 1 | `day_wrap` | `+1 วัน` แล้วตกในกรอบกะพอดี = บั๊กโค้ด A ล้วน | 17 |
| 1 | `ampm_12h` | `±12 ชม.` แล้วตกในกรอบกะพอดี = ลายเซ็น AM/PM สลับ (B) | 53 |
| 2 | `day_wrap_grace` | `+1 วัน` แล้วเลยปิดกะ ≤ 60 นาที (ลงตอนส่งกะ) | 1 |
| | | **รวมแก้** | **71** |

**ลำดับสำคัญ: เช็ค `+1 วัน` ก่อน `±12 ชม.` เสมอ** — บั๊กโค้ดชดเชย 12 ชม. ได้เหมือนกันในบางเคส
ถ้าเช็ค 12 ชม. ก่อนจะถูกตีเป็น "คนกรอกผิด" ทั้งที่โค้ดผิดเอง · **แก้เฉพาะแถวที่มีคำตอบเดียว** —
เลื่อนแล้วเข้ากรอบพอดีเท่านั้น ที่เหลือไม่เดา

##### 🚫 เหลือ 16 แถวที่ **ไม่แก้** — คนละสาเหตุ (เวลา downtime ถูก · `start_time` ของกะผิด)
ก่อนเปิดกะ 14 แถว (เฉลี่ย 1.6 ชม. · สูงสุด 4.2) + หลังปิดกะ 2 แถว — เลื่อนวัน/12 ชม. แล้ว**ไม่เข้ากรอบสักทาง**

- ลายเซ็น: **กะดึกเปิด `22:30` แต่ downtime ลง 20:00–22:00** (`Set up Machine` 4 กะ · `แก้ไขปัญหาคุณภาพ` ฯลฯ)
  ⇒ คนมา OT ตั้งแต่ 20:00 แต่ **เปิดกะด้วยเวลา default 22:30** · เวลา downtime คือของจริง
- แก้ให้ถูกต้องต้องไป**แก้ `start_time` ของ 12 session (ปิดแล้ว)** ซึ่งเปลี่ยน `shift_min` ⇒
  **`oee_a`/`oee_p`/`oee` ที่ stamp ไว้เป็นโมฆะทั้งกะ** (ค่าปัจจุบัน 63.45–96.67)
- 🔴 **ห้ามเลื่อนเวลา downtime ให้เข้ากรอบแทน** = สร้างข้อมูลเท็จ (กฎเดียวกับ "ห้ามดัดค่าที่คนกรอก")
- **รอ user ตัดสิน** ว่าจะแก้ `start_time` + คำนวณ OEE 12 กะใหม่ หรือปล่อยไว้ตามที่บันทึกจริง
- ด่านใหม่ในข้อ B ปิดทางไม่ให้เกิดเพิ่มแล้ว (ลงเวลานอกกรอบกะ = บันทึกไม่ได้)

### Downtime — เครื่องจักร + ชิ้นงาน เป็น optional ทุกประเภท (2026-08-03 · คำสั่ง user)

ฟอร์มเพิ่ม Downtime ใน DailyReport เดิมบังคับเลือก**เครื่องจักร + ชิ้นงาน (mat_no)** ทุกกรณี → **ไม่บังคับทั้งคู่ทุกประเภทแล้ว** (`dtMachineOptional = true` เสมอ) เพราะ downtime หลายอย่างเป็นการหยุด**ระดับไลน์** ไม่ผูกเครื่อง/ชิ้นงานเฉพาะ — รอวัตถุดิบ/รอคน/ไฟดับ/ประชุม/**5ส/QA recheck** (หลายอย่างถูกจัด category `planned` เช่น 5ส แต่ก็ไม่ผูกเครื่อง) หรือเครื่องยังไม่ลงทะเบียน (เคสจริง Sub Assy2 SP-xx, Laser LWR) · **⚠️ เดิมลองแยกปลดเฉพาะ unplanned แล้วไม่พอ** — 5ส เป็น planned เลยยังบังคับ (หัวหน้ากลุ่มมองว่า "นอกแผน" แต่ระบบจัด planned) → เลิกแยก planned/unplanned ปลดหมด · คุมด้วย const `dtMachineOptional` (แก้ทั้ง 2 ฟิลด์ × 3 จุด: validation `handleAddDT`, label `(ถ้ามี)`, ปุ่ม disable) · `machine_no`/`mat_no` nullable + top-DT aggregation/notification/OEE (แยกด้วย category ไม่ใช่ mat) จัดการ null อยู่แล้ว · จะผูกเครื่อง/ชิ้นงานก็ยังเลือกได้ · **แต่ไม่ปล่อยข้ามเงียบ: unplanned (เครื่องเสีย 90%) ถ้าไม่เลือกเครื่อง เด้ง `window.confirm` ถามยืนยันก่อนบันทึก** (เผื่อ 10% พาร์ทหมด/ไฟดับทั้งโรงงานที่ลงเครื่องไม่ได้) · planned ไม่ถาม (ไม่ผูกเครื่องอยู่แล้ว) · คำสั่ง user "ห้ามปล่อยเงียบ" 2026-08-03

> #### 🔴 กฎเหล็ก — picker เครื่องจักรต้องกาง "ครอบครัวไลน์" ห้ามกรอง `line_name` ตรงเป๊ะ (2026-08-26 · feedback หน้างาน)
> *"user ก็ลงว่า robot103 แต่ทำไมบอกไม่ระบุเครื่อง"* — จอห้องช่างขึ้น **"ไม่ระบุเครื่อง"** ทั้งที่หมายเหตุเขียน
> `Robot 103 …` ไว้ชัดเจน · **ไม่ใช่ความผิดคนกรอก — เครื่องไม่เคยโผล่ในลิสต์ให้เลือก**
> 2 จุดในโมดัลเดียวกันกรองไลน์ตรงเป๊ะทั้งคู่ (กะเปิดที่ **ไลน์ลูก** แต่เครื่องลงทะเบียนใต้ไลน์แม่/พี่น้อง):
> | จุด | อาการเดิม |
> |---|---|
> | dropdown เครื่องจักร | `machines.filter(m => m.line_name === selSession.line_name)` → **เครื่องหายจากลิสต์** เหลือทางเดียวคือพิมพ์ลงช่อง "รายละเอียด" |
> | สแกน QR บนเครื่อง | `mc.line_name !== selSession.line_name` → **ปฏิเสธการสแกน** ทั้งที่ยืนอยู่หน้าเครื่องนั้น |
>
> - **นี่คือบั๊ก class เดียวกับที่แก้ไปแล้ว 2 รอบ** — `dtMatOptions` (ช่องชิ้นงาน **ในโมดัลเดียวกัน** · 2026-08-14)
>   และ `machineOpts` ของ `/improvements` (2026-08-19) · **จุดใหม่ที่มี picker อุปกรณ์/สินค้า ต้องถามก่อนเสมอว่า
>   "ถ้าของลงทะเบียนไว้ที่ไลน์แม่ จะยังเลือกได้ไหม"** · **เจอรอบที่ 4 แล้ว 2026-08-31 ที่ picker วัสดุของจุด WIP —
>   ดูกฎถัดไป (`/layout-setup` → จุด WIP)** ⇒ pattern นี้ยังงอกใหม่เรื่อยๆ ต้องตรวจทุกครั้งที่เขียน picker
> - **⚠️ "ครอบครัวไลน์" = แม่ + พี่น้อง + ลูก → ใช้ `getLineFamilyNames(lines, name)` เสมอ**
>   เขียนเองเป็น `[line, ...ลูกของ line]` = **ไลน์ลูกจะไม่เห็นของที่ลงทะเบียนไว้ที่ไลน์แม่** ซึ่งเป็นเคสที่พบบ่อยที่สุด
>   (เทสของ `wipMatOptions` จับได้ตอนเขียน fix รอบที่ 4 — ผมเองก็พลาดท่านี้)
> - ตอนนี้ `dtMachineOptions` ใช้ **`SearchSelect`** (ค้นทั้งทะเบียน 500+ ตัว) จัด optgroup:
>   🏭 ไลน์นี้ → 🔗 ครอบครัวไลน์ → 🏢 เครื่องอื่นทั้งโรงงาน → 🔨 แม่พิมพ์ท้ายสุด — **ไม่ตัดอะไรทิ้ง แค่เรียงลำดับ**
>   (`_i` = ลำดับจาก DB เป็น tiebreaker เพื่อคง `sort_order` เดิมในแต่ละกลุ่ม)
> - สแกน QR: **ในครอบครัว = รับเลย · นอกครอบครัว = `confirm` ไม่บล็อก** (เครื่องกลาง/utility จ่ายหลายไลน์มีจริง)
> - **ผลของการไม่ระบุเครื่องต้องเขียนบนจอ ห้ามให้เดา** — `confirm` ตอนบันทึกบอกว่า "ต่อใบซ่อม/OEE รายเครื่อง/
>   จัดคิวทีมช่างไม่ได้" และ**ถ้าหมายเหตุมีเลขที่ตรงกับทะเบียนจริง ให้บอกไปเลยว่าน่าจะหมายถึงเครื่องไหน**
>   (`normCode` + ต้องยาว ≥3 ตัวกัน false positive) — **เสนอเท่านั้น ห้ามเติมให้เอง** · จอห้องช่างขึ้น
>   "⚠ ไม่ระบุเครื่อง" สีส้ม + บรรทัดอธิบายว่าจัดคิวให้ทีมไม่ได้จึงแสดงให้ทุกทีม
>
> **⚠️ กฎนี้ใช้กับ "ลิสต์ให้คนเลือก" เท่านั้น — ห้ามเหมาไปใช้กับตัวคำนวณ** (sweep แล้ว 2026-08-26 · 2 จุดนี้
> ตั้งใจกรองตรงเป๊ะ **อย่าไปแก้**): `sessionProcessType` (majority process ของกะ → break policy) — กางครอบครัว
> แล้ว LWR BAR จะเอา laser (metal_forming) มาชนะ assy (welding_assembly) = **หักเวลาพักผิด policy** ·
> `parallelUnitsOf` fallback (นับเครื่อง active ตอนไม่ได้ตั้ง `parallel_stations`) — กางครอบครัวแล้ว N โตเกินจริง
> = หาร DT มากไป = **%A สูงเกินจริง** ซึ่งเป็นทิศที่อันตราย (นับน้อยไปแค่ทำให้ %A ต่ำกว่าจริง = fail-safe)

### หมายเหตุผู้อนุมัติปิดกะ (optional · 2026-07-24)

SV กรอก remark ได้ตอนกด ✅ อนุมัติใน modal ตรวจสอบคำขอปิดกะ (ไม่บังคับ — "เผื่อหัวหน้ามีอะไรเพิ่มเติม") → เก็บ `production_sessions.close_approve_note` (migration `20260724_session_close_approve_note.sql` DR — additive · โค้ด update แยก best-effort) · แสดงในแท็บประวัติตอน expand กะ (กล่องฟ้า 📝 + ชื่อผู้อนุมัติ)
- **remark เข้าแจ้งเตือน Telegram ด้วย (คำสั่ง user 2026-07-24):** client แนบ `approve_note` ใน payload `closed_approved` (reject มี `reject_reason` อยู่แล้ว) · edge `send-notification` ต่อบรรทัด "📝 หมายเหตุผู้อนุมัติ: …" / "📝 เหตุผล: …" ท้ายข้อความ approve/reject — **ต้อง deploy edge `send-notification` ใหม่** (ก่อน deploy: แจ้งเตือน approve/reject ยังส่งปกติ แค่ไม่มีบรรทัดหมายเหตุ — payload field ใหม่ backward-compatible)

### ปฏิเสธคำขอปิดกะ + remark ให้หัวหน้ากลุ่ม (2026-07-23)

leader กด "📋 ขอปิดกะ" → `pending_close` → SV ตรวจแล้วเลือก **อนุมัติ** หรือ **✕ ปฏิเสธ** · ตอนปฏิเสธ **บังคับพิมพ์ remark** (modal `showRejectModal` — บอกว่าต้องกลับไปแก้อะไร เช่น NG ไม่ตรง/ลืมปิด Downtime/เวลาผิด) กะกลับเป็น `open` แล้ว **หัวหน้ากลุ่มเห็น banner แดง (static ไม่กระพริบ) พร้อมข้อความ + ชื่อผู้ปฏิเสธ** ในหัว session จนกว่าจะแก้แล้วส่งขอปิดกะใหม่
- เก็บที่ `production_sessions.close_reject_reason / close_reject_by_name / close_reject_at` (migration `20260723_session_close_reject_reason.sql` DR — additive) · **เขียนเป็น update แยก best-effort (try/catch)** ต่อจากการ revert สถานะ → ยังไม่ apply migration = ปฏิเสธได้ปกติ แค่ยังไม่เก็บข้อความ (ไม่ทำ flow หลักพัง)
- 2 ทางเข้า reject (ปุ่มหัว session + ปุ่มใน modal ตรวจสอบ) เปิด `showRejectModal` ตัวเดียวกัน — `handleRejectClose` อ่าน `rejectReason` (บังคับไม่ว่าง) · เงื่อนไขเดิมยังอยู่: ปฏิเสธไม่ได้ถ้ายอดยกถูกกะถัดไปรับไปแล้ว (`prod_orders.status='imported'`)
- notify `closed_rejected` แนบ `reject_reason` (edge function ยังไม่ใช้ field นี้ — backward-compat, banner ในแอปเป็นช่องทางหลัก)

### การ์ด Order ยกยอด — โชว์ผลิตจริง/ยกไป ไม่ใช่เป้าเฉยๆ (2026-07-23)

หัวหน้างานงง: ใบที่ผลิตไม่จบ ยกยอดข้ามกะ การ์ดโชว์ยอด **เป้า** (เช่น 35) เหมือนผลิตครบ ทั้งที่ผลิตจริง 18 ยกไป 17
- **การ์ด order (`renderOrderRow`) แยกการแสดงยอดตามสถานะ** (เดิมโชว์ `o.qty` ก้อนเดียว label "ชิ้น" เหมือนกันหมด → open(เป้า) กับ confirmed(ผลิตจริง) ดูไม่ออกต่างกัน):
  - `carry_over` (ยกออก) → **`qty_actual/qty_target` label "ผลิตจริง/เป้า"** + บรรทัด "➡ ยกไป N ชิ้น" (N = เป้า−ผลิตจริง)
  - `confirmed` → label "ผลิตจริง (ชิ้น)" · `open` ปกติ → "เป้า (ชิ้น)" · carried-in (`carry_over_from_session_id`) → "ยังต้องทำ (ชิ้น)" + "เป้าเดิม X · กะก่อนทำ Y" (parse จาก `carry_over_note` `\d+/\d+`)
  - `cancelled` → `qty_actual/qty_target` "ทำได้/เป้า"
- **ภาพรวมทั้งกะ "ผลิตได้" รวม `qty_actual` ของใบ carry_over ด้วย** (เดิมนับแค่ confirmed + manual-open → ผลิตจริงส่วนที่ยกยอดหายจากยอดกะนี้ · เคสจริง live โชว์ 468 แต่ SV review โชว์ 486 = +18 ที่ยกยอด) — productRows `confirmed` + null-mat `totalConfirmed` เพิ่ม carry_over qty_actual · ที่เหลือ (remainQty) ไปนับที่กะถัดไป ไม่ double-count

### ยอดที่ "จะส่งต่อกะหน้า" — เห็นตั้งแต่ระหว่างกะ ไม่ต้องไปเปิดดูกะถัดไป (2026-08-10 · คำขอหัวหน้ากลุ่ม)

ฝั่ง **รับ** ทำไว้ดีแล้ว (ชิป "➡ ยกยอดมา (24/35)" + "ยังต้องทำ 11 ชิ้น") แต่ฝั่ง **ส่ง** ก่อนปิดกะเห็นแค่ "35 · เป้า" — หัวหน้ากลุ่มต้องไปเปิดดูกะถัดไปถึงรู้ว่าส่งต่อไปเท่าไหร่ ("มันขึ้นว่า 35 ตัวรอปิด")
**ต้นเหตุ:** สถานะ `carry_over` (ซึ่งมี "➡ ยกไป N ชิ้น" อยู่แล้ว) เกิดตอน **ปิดกะเท่านั้น** และ**ใบสแกนไม่มีช่องกรอกยอดระหว่างทาง** (มีเฉพาะใบ manual) → ระบบไม่รู้ยอดจริงจนกว่า leader จะกรอกใน modal ปิดกะ
- **ใบสแกน (kanban) ที่ยัง `open` กรอกยอดสะสมได้แล้ว — ไม่บังคับ** (`scanOpen` ใน `renderOrderRow` · ใช้ `handleManualQtyUpdate` ตัวเดียวกับใบ manual) · กรอกแล้วการ์ดเปลี่ยนเป็น **"ทำได้/เป้า" + "➡ ปิดกะตอนนี้ ยกไป N ชิ้น"** เหมือนฝั่งรับ · ยังไม่กรอก = หน้าตาเดิมเป๊ะ
- **⚠️ ใบสแกน "ไม่มี" ปุ่มปิดด้วยมือ** (`✓ ปิดใบนี้` เห็นเฉพาะ `manualOpen`) — ปิดใบยังต้องสแกนเสมอ ไม่งั้นเสีย traceability + ยอดเข้า stock · และกรอกเกินจำนวนใบไม่ได้ (kanban card จำนวนตายตัว — ใบ manual เกินได้เพราะเป็นเป้าที่ตั้งเอง)
- **แบนเนอร์สรุประดับกะ "⏭ จะส่งต่อกะหน้า ~X ชิ้น"** (คู่กับแบนเนอร์ "➡ มียอดค้างจากกะก่อน" ที่มีอยู่เดิม) แยก **ยืนยันยกยอดแล้ว** (`carry_over`) กับ **ใบที่ยังไม่ปิด** (`open`) · ใบที่ยังไม่กรอกยอด **นับเต็มใบไว้ก่อน** และขึ้นเตือนสีเหลืองว่ากี่ใบยังไม่กรอก + มี `~` นำหน้าตัวเลข — **ห้ามโชว์เลขคาดการณ์เหมือนเลขยืนยัน**
- **modal ปิดกะ prefill ช่อง "ยอดที่ทำได้จริง" จาก `qty_actual` ที่กรอกไว้ระหว่างกะ** (แก้ทับได้) — **ยังห้ามให้ระบบเดาจากเวลาที่เหลือเหมือนบั๊กเก่า** ข้อยกเว้นนี้ใช้ได้เพราะเป็นเลขที่ "คนกรอกเอง" · seed ตอนกดเปิด modal (ไม่ใช่ fallback ตอน render) เพราะ validation/สรุป/ตอนเซฟ อ่าน `carryQtyActual` state ตรงๆ — fallback เฉพาะตอนแสดงผลจะทำให้ **เซฟเป็น 0**
- **⚠️ ผลข้างเคียงที่ตั้งใจ — ยอด "ผลิตได้" ระหว่างกะขยับ:** `FactoryMap`/`OEEAnalytics`/`MorningMeeting`/`OrderTrace`/`oee.js computeLiveOee` ใช้สูตร `confirmed ? (qty_ok ?? qty) : (qty_actual ?? 0)` อยู่แล้ว → พอกรอกยอดใบสแกน จอพวกนี้เริ่มนับทันที (เดิมได้ 0) · **จึงแก้ภาพรวมทั้งกะใน DailyReport ให้นับใบ `open` ทุกชนิด** (เดิมนับเฉพาะ `is_manual && open`) ไม่งั้น 2 จอเลขไม่ตรงกัน · ไม่กรอก = `qty_actual` null = บวก 0 = พฤติกรรมเดิม · ค่าที่ stamp ตอนปิดกะไม่กระทบ
- **จุดใหม่ที่นับ "ผลิตได้" ระหว่างกะ ให้ใช้สูตร `confirmed ? (qty_ok ?? qty) : (qty_actual ?? 0)` เท่านั้น** ห้ามกรอง `is_manual` อีก
- **⚠️ ช่องกรอกยอดของใบสแกน "พับไว้" เป็นค่าเริ่มต้น (2026-08-20 · หัวหน้าแผนกทัก "ขึ้นมาเต็มเลย รกมาก")** — ระบบดึงเปิดใบค้างพร้อมกันได้ 20-30 ใบ (เจอจริง 20 ใบ · การ์ดละ 10 ชิ้น) แต่ **"ใบที่ทำค้างจริง" มีไม่กี่ใบ** ที่เหลือคือยังไม่เริ่ม (ยกเต็มใบ = ถูกอยู่แล้ว) หรือเดี๋ยวสแกนปิดเต็มใบ → กางช่องกรอกทุกใบ = จอรก **และชวนเข้าใจผิดว่าต้องไล่กรอกให้ครบทุกใบ**
  - ใบสแกนที่ยังไม่กรอก = ปุ่มเล็ก **"✏️ ใบนี้ทำค้าง — กรอกยอด"** บรรทัดเดียว · กดแล้วช่องจึงกาง (`qtyEditIds`)
  - **ใบที่ `qty_actual > 0` กางค้างไว้เสมอ** — นั่นคือใบที่กำลังทำอยู่จริง ต้องเห็น/แก้ได้ตลอด
  - **ใบ manual ไม่กระทบ** — ยังกางเต็มเหมือนเดิม (บังคับกรอกทุกช่วงเบรคตาม break policy)
  - คำอธิบายยาวย้ายไปแบนเนอร์ที่เดียว · ข้อความเตือนเปลี่ยนจาก "N ใบยังไม่ได้กรอกยอด" (อ่านแล้วเหมือนต้องกรอกครบ) เป็น **"นับเต็มใบไว้ก่อน N ใบ — ถ้ามีใบที่ทำค้างอยู่ กรอกเฉพาะใบนั้น"**

### 🔴 `imported` = ใบยกยอด "หลังกะถัดไปกดรับแล้ว" — ต้องนับ/โชว์เท่ากับ `carry_over` (2026-09-09 · feedback หน้างาน)

feedback: *"กะที่ส่งยอด อยากเห็นว่าผลิตไปเท่าไหร่ ส่งไปเท่าไหร่ ตอนนี้ต้องไปดูกะถัดไป"*

**วงจรของใบยกยอด 1 ใบ:** `open` → ปิดกะเลือก "ยกยอด" → `carry_over` → กะถัดไปกด "รับยอดค้าง" → **`imported`**
(ใบใหม่ของกะถัดไปถือ **ยอดที่เหลือ** `qty − qty_actual` เท่านั้น — `handleImportCarryOrders`)

**2 บั๊กที่เกิดจากการมองข้ามสถานะ `imported` (แก้พร้อมกัน):**
1. **จอ:** `renderOrderRow` มี 4 กิ่ง (`confirmed`/`carry_over`/`cancelled`/`scanPartial`) — `imported` ตกกิ่งสุดท้าย
   โชว์แค่ `qty` = **"35 เป้า (ชิ้น)" + ป้าย "● ผลิต"** ทั้งที่ `qty_actual = 5` อยู่ในแถวนั้นแล้ว
   ⇒ ข้อมูล "ผลิตไป 5 ส่งไป 30" **หายทันทีที่กะถัดไปกดรับ** → ตอนนี้ใช้ `carriedOut = carry_over || imported`
   ป้ายแยกคำ: `➡ ยกยอด (รอกะถัดไปรับ)` vs `➡ ส่งกะถัดไปแล้ว` (แท็บประวัติ + export ใช้คำเดียวกัน)
2. **ตัวเลข:** ยอด "ผลิตได้" นับ `carry_over` แต่ไม่นับ `imported`
   ⇒ **วินาทีที่กะหน้ากดรับ ยอดผลิตของกะที่ทำจริงลดลงเงียบๆ** (เคสจริง Assy LWR 08/09: 415 ทั้งที่ทำได้ 420)
   วัดทั้งฐาน 2026-09-09: **794 ใบ · 139 ใบมียอดจริง · 3,213 ชิ้น ใน 170 กะ** หายจากยอดผลิตทั้งระบบ

**สูตรกลางยุบเหลือที่เดียว: `orderProducedQty(o)` ใน `src/utils/oee.js` §6** (เดิมสูตรเดียวกันถูกก๊อป **7 ที่**
— DailyReport · OEEAnalytics · Dashboard · MorningMeeting · QualityControl · wipChain · computeLiveOee) · มีเทส
`src/utils/__tests__/orderProducedQty.test.mjs` · **ห้ามเขียนสูตรนี้ซ้ำในหน้าอีก**
- ⛔ **ห้ามใช้คิด "เป้า"** — เป้าของใบ `imported` ย้ายไปอยู่ใบของกะถัดไปแล้ว จุดที่รวมเป้าด้วย `o.qty` ดิบ
  ต้องกรอง `imported` ออกเหมือนเดิม ไม่งั้นนับซ้ำ (35+30=65) · Dashboard แยก 2 ฝั่งชัดเจนแล้ว (`handed` vs `active`)

### ⏰ ยอดค้างเด้งซ้ำข้ามกะ — ต้องดู "ใบล่าสุดของ prod_no" ไม่ใช่แค่ใบต้นทาง (2026-09-09 · feedback หน้างาน)

feedback: *"งานยกยอดที่กะเมื่อคืนเคลียร์จากกะเช้าเมื่อวานไปแล้ว ยังมาเด้งโชว์ต่อกะเช้าวันนี้ เพราะยังไม่ปิดกะรึป่าว"*
→ **ใช่ เกี่ยวกับกะที่ยังไม่ปิด** แต่ต้นเหตุจริงคือด่านกรองไม่ครบ:

ตัวดึงยอดค้าง (`loadProdOrders`) รับใบสถานะ `open`/`carry_over` จาก **5 กะล่าสุดของไลน์ที่สถานะ `closed` หรือ `pending_close`**
— กะที่ SV ยังไม่อนุมัติจึงยังอยู่ในสายตาเสมอ · เดิมกรองแค่ (ก) `prod_no` มีในกะนี้แล้ว (ข) `qty_actual >= qty`
**ไม่มีด่านเช็คว่า prod_no นั้นถูกกะที่ใหม่กว่ารับไป/ทำจนปิดแล้วหรือยัง** ⇒ ใบที่ค้างในกะเก่าถูกเสนอซ้ำทุกกะไปเรื่อยๆ

**แก้ (2 รอบ — รอบแรกยังปิดไม่หมด):**
1. ดึง **ทุกสถานะ** (ไม่ใช่แค่ `carry_over`/`open`) → หา "ใบล่าสุดของแต่ละ prod_no" (กะใหม่สุดก่อน แล้วค่อย `opened_at`)
   → **เสนอเฉพาะเมื่อใบล่าสุดยังเป็น `carry_over`/`open`** (เป็น `confirmed`/`imported`/`cancelled` = งานจบแล้ว ห้ามปลุกซ้ำ)
2. **⭐ ต้องดึงกะที่สถานะ `open` มาด้วย** (feedback รอบ 2: *"ถ้ากะก่อนหน้ารับไปแล้ว และคอนเฟิมยอดแล้ว เหลือแค่ปิดกะ
   ก็ไม่ควรเด้งไปหากะถัดไปแล้วสิ"*) — กะที่รับยอดไปทำจนคอนเฟิมแล้วแต่ยัง**ไม่กดปิดกะ** สถานะยังเป็น `open`
   ⇒ รอบแรกที่ดึงเฉพาะ `closed`/`pending_close` **มองไม่เห็นใบ confirmed ของกะนั้น** ใบเก่าที่ค้างในกะก่อนหน้า
   จึงกลายเป็น "ใบล่าสุด" แล้วถูกเสนอซ้ำอยู่ดี

**แยกบทบาท 2 อย่างให้ชัด (กับดักหลักของเรื่องนี้):**
| | กะที่นับ |
|---|---|
| **หยิบยอดค้างมาได้** | `closed` / `pending_close` เท่านั้น (`takeableSess`) — กะที่ยังเปิดอยู่ห้ามแย่งงานมา |
| **ใช้ตัดสินว่างานจบแล้ว** | **ทุกกะ รวม `open`** — ไม่งั้นมองไม่เห็นว่ามีคนรับไปทำแล้ว |

· ดึง 8 กะล่าสุด (จาก 5) เพราะตอนนี้มีกะ `open` ปนอยู่ในโควตาด้วย
· วัดกับข้อมูลจริง 2026-09-09: ยอดค้างที่ควรเสนอยังได้ **เท่าเดิม** (ไม่บล็อกเกิน) และมี **70 prod_no**
  ที่ใบล่าสุดอยู่ในกะที่ยังเปิด — ถูกกันไม่ให้เด้งไปกะถัดไปแล้ว

### 🔴 เปิดกะซ้ำ 2 กะ → ยอดยกหายทั้งก้อน (2026-09-10 · feedback หน้างานทาง LINE)

feedback (หัวหน้ากลุ่ม Assy LWR · 09:05): *"กะกลางคืนส่งยอดต่อกะ เข้านี้ 25 ตัว · ตอนเข้าเปิดกะมาไม่โชว์ขึ้น ·
แต่กะกลางคืนมันมีเปิดกะ มา 2 อัน"* — **ทั้ง 3 ประโยคคือเรื่องเดียวกัน กะซ้ำเป็นต้นเหตุ ยอดหายเป็นผล**

**สิ่งที่เกิดจริง (ใบ `0133259280` · Assy LWR):**
1. Assy LWR กะดึก 09/09 ถูกเปิด **2 กะ** ห่างกัน 0.12 วินาที (`a98428c0` 11 ใบ · `80919b74` 2 ใบ)
2. กะดึกกด "ส่งกะถัดไป" ใบ 25/35 → ต้นทางในกะ `a98428c0` เป็น `carry_over` เหลือยก 10 ชิ้น
3. มีคน**กดรับยอดค้างขณะอยู่บนกะซ้ำ** (`80919b74`) ⇒ ใบลูก 10 ชิ้นไปเกิดใน**กะดึกอีกกะหนึ่ง** ไม่ใช่กะเช้า 10/09
   และต้นทางถูก mark เป็น `imported`
4. กะเช้า 10/09 เปิดมา → ตัวเสนอยอดค้างเลือก "ใบล่าสุดของ prod_no" ด้วย**ความใหม่ของกะ (`created_at`)**
   → ได้ใบ `imported` (อยู่ในกะที่ created ใหม่กว่า 0.12 วิ) → เข้าเงื่อนไข "งานจบแล้ว" → **ทิ้งทั้ง prod_no**
   ⇒ ใบ carry_over 10 ชิ้นตัวจริงไม่เคยถูกเสนอ · จอกะเช้าขึ้น `Prod Orders (0 ใบ)`

**บทเรียนที่ตกผลึก — `imported` ไม่ใช่ "จุดจบของงาน"**
`imported` แปลว่า *"ใบนี้ถูกกะอื่นรับไปแล้ว"* ⇒ **มีใบต่อจากมันเสมอ** (ต่างจาก `confirmed`/`cancelled` ที่จบจริง)
เอา `imported` ไปตัดสินว่างานจบ = ตัดสินจากป้ายบอกทาง ไม่ใช่ปลายทาง
→ `latest` ใน `loadProdOrders` เรียงด้วย **(ไม่ใช่ imported ก่อน, แล้วค่อยความใหม่ของกะ)**
ปลอดภัยเพราะใบต่อของ `imported` อยู่ในกะที่ใหม่กว่าเสมอ หรือไม่ก็กะปัจจุบัน (ซึ่งกรองด้วย `currentProdNos` อยู่แล้ว)
⇒ เคสที่ §ก่อนหน้าแก้ไว้ (`confirmed` ของกะใหม่กว่าชนะใบ `carry_over` เก่า) ยังทำงานเหมือนเดิมทุกประการ

**ทำไมกะถึงซ้ำ — 2 ทาง แก้คนละแบบ (ไม่ใช่ผู้ใช้กดรัว):**
| ทาง | กลไก | แก้ |
|---|---|---|
| `Checkin.jsx` `handleConfirmOpenShift` | เช็คชื่อทั้ง**ไลน์แม่ + ไลน์ลูก**พร้อมกัน ⇒ ไลน์แม่ถูก expand เป็นลูก **แล้วไลน์ลูกที่ถูกเช็คเองเข้าคิวอีกรอบ** ⇒ ลูป insert 2 แถว (**deterministic** ไม่ใช่ race) | dedupe ด้วย `askedLineNames` + `seen` ตอน insert |
| `DailyReport.jsx` `handleOpenSession` | ด่านกันซ้ำเป็น read-then-insert (TOCTOU) — 2 คำขอพร้อมกันผ่านทั้งคู่ | `openingSession` guard + ปุ่ม disabled |
| **ทั้งสองทาง** | ไม่มี uniqueness ฝั่ง DB เลย | **unique index** `production_sessions_one_open_per_line_shift` |

· `.maybeSingle()` ในด่านเช็ค "มีกะแล้วหรือยัง" ของ Checkin เป็นกับดักซ้อน — คืน error `PGRST116` เมื่อเจอ >1 แถว
  และโค้ดเดิมกลืน error ทิ้ง ⇒ ไลน์ที่มีกะซ้ำอยู่แล้วถูกอ่านว่า "ยังไม่มีกะ" **แล้วชวนเปิดซ้ำทุกวัน**
  (LASER E50 ซ้ำเกือบทุกวันตั้งแต่ 13/08 — 15 วัน) · เปลี่ยนเป็น `.limit(1)` + อ่าน error (กฎเหล็กข้อ 1)
· ขนาดปัญหาที่วัดได้ 01/08–10/09: **25 คู่/สามกะซ้ำ** ใน 5 ไลน์ (LASER E50 · LASER EXPORT · Assy LWR · Line 61 · SUB APRON)
  — ทุกคู่ที่ห่างกันต่ำกว่า 1 วินาที = 2 ทางข้างบน · ที่ห่างเป็นนาที/ชั่วโมง = คนเปิดใหม่หลังปิดกะจริง (ปกติ ไม่ต้องแก้)

**migration `20260910_production_sessions_no_dup_open.sql` (DR · apply แล้ว 10/09):**
partial unique index บน `(line_name, work_date, shift)` **where status in ('open','pending_close')**
· partial เพราะ "ปิดกะแล้วเปิดใหม่ของวัน+กะเดิม" (เปิดต่อทำ OT) เป็นพฤติกรรมที่ใช้จริง — ล็อกทั้งตารางจะพังของเดิม
· เงื่อนไขนี้ = กฎเดียวกับที่ `handleOpenSession` เช็คอยู่แล้วเป๊ะ ⇒ **index ห้ามเฉพาะสิ่งที่แอปถือว่าผิดอยู่แล้ว ไม่ปิดทางไหนใหม่**
· ทั้ง 2 หน้าแปลง error `23505` เป็นข้อความ "ไลน์นี้มีกะเปิดอยู่แล้ว" ไม่ใช่โยน error ดิบใส่หน้าคนหน้างาน
· ล้างกะซ้ำที่ค้างอยู่ 1 แถว (LASER E50 10/09 · prod_orders/downtime/defect/kanban = 0 ทั้งหมด จึงไม่มีข้อมูลผลิตหาย)

**ยอด 10 ชิ้นของใบ `0133259280` ไม่ต้องซ่อมข้อมูล** — แถว `carry_over` ยังอยู่ครบในกะ `80919b74` (ปิดแล้ว)
พอโค้ดใหม่ deploy กะเช้าจะเห็นในแถบ "ค้างจากวันก่อน" เอง

### 🔲 ผังหน้ากะสด — ภาพใหญ่หลายคอลัมน์ (2026-09-09 · feedback user)

*"แต่ละ box เป็น column เดียว แต่ภาพใหญ่ 3 เรื่องนี้คนละ column กัน"* (เรียกชิ้นส่วนสโตร์ · Prod Orders · Downtime)

- **เต็มความกว้างเสมอ:** หัวกะ (LIVE/ปุ่มเปิด-ปิดกะ) + แถบ KPI 7 ช่อง + progress bar — user เคาะไว้
- **`PANEL_COLS` = `cardGrid(460, 12)` + `rowGap: 0`** — grid **แถวเดียว ลูก 3 ตัว = 3 คอลัมน์**
  · จอ 1920 → คอลัมน์เนื้อหา ~1525px = **3 คอลัมน์ (~500px/คอลัมน์)** · ~1000-1420px = 2 · แคบกว่า 940px = 1
  | คอลัมน์ | เรื่อง | แผงข้างใน (stack แนวตั้งด้วย `PANEL_STACK`) |
  |---|---|---|
  | ① ซ้าย | 📦 ใบสั่งผลิต | Prod Orders |
  | ② กลาง | ⚠️ ปัญหาในกะ | บันทึกงานเสีย + Downtime |
  | ③ ขวา | 🔩 ชิ้นส่วนเข้าไลน์ | `StoreLotQueue` + `LinePartCallPanel` + `LineWipPanel` — **กล่องเดียว** (`.dr-partbox`) |
  - **ลำดับนี้ user เคาะเอง (2026-09-09):** *"เอา order ซ้าย downtime กลาง wip ขวา"*
    — ซ้าย = งานที่ต้องทำ · กลาง = สิ่งที่ขวางอยู่ · ขวา = ของที่ป้อนเข้าไลน์ **ห้ามสลับเอง**
- **คอลัมน์ ③ เป็น "กล่องเดียว" ตามคำขอ user** *"เรียกชิ้นส่วนกับ WIP ในไลน์ อยู่ box เดียวกันดีมั้ย
  มันคือเรื่องเดียวกัน"* → ทำด้วย class `.dr-partbox` ใน `index.css` (ถอดกรอบ/มุม/margin ของแผงลูก
  เหลือเส้นคั่นบางๆ) **ไม่ได้แก้ตัวคอมโพเนนต์ทั้ง 3 ตัวเลย** · `.dr-partbox:empty { display:none }`
  ทำให้ไลน์ที่ไม่เกี่ยวกับชิ้นส่วน (ทั้ง 3 แผง return null) ไม่เหลือกรอบเปล่าและ track หายไปเอง
  ⇒ **`.dr-partbox` ต้องเป็นลูกตรงๆ ของ grid ห้ามมี div ครอบอีกชั้น** (รายละเอียด UI-CONVENTIONS §6.14.2)
- **ชั้นในไม่ต้องทำอะไรเพิ่ม** — `ORDER_GRID`/`DT_GRID`/`DEFECT_GRID` ใช้ `min(100%, N)` อยู่แล้ว
  พอคอลัมน์เหลือ ~500px มันยุบเหลือคอลัมน์เดียวเอง = ได้ "กล่องละคอลัมน์เดียว" ตามที่ขอโดยไม่ต้องเขียนเงื่อนไข
- **ข้อแลกที่รู้ตัว:** ที่ 500px/คอลัมน์ แถวยาว (สโตร์ · Downtime ที่มีปุ่ม 5 ตัว) จะ wrap เป็น 2 บรรทัด
  — ยอมแลกกับการเห็น 3 เรื่องพร้อมกันโดยไม่ต้องเลื่อน · จอ 2560 ได้ ~840px/คอลัมน์ = ไม่ wrap เลย

> #### 🔴 รอบแรกทำผิด — ส่งขึ้น main แล้ว user ตีกลับ (2026-09-09)
> *"เกือบดีละ แต่แบบนี้ ตรวจแล้วหรอ แบบนี้ใช้ไม่ได้นะ"*
> เวอร์ชันแรกโยนแผงทั้ง 6 เป็น**ลูกแบนๆ ของ `cardGrid` ตัวเดียว** → grid row สูงเท่าลูกที่สูงที่สุด
> ⇒ แผงเตี้ย (สโตร์/WIP) มีช่องดำยาวใต้ตัวเอง + Downtime ตกไปแถว 2 คนเดียว = โหว่กลางจอหนักกว่าเดิม
> **กติกาที่ตกผลึก (UI-CONVENTIONS §6.14.1):** ลูกสูงไม่เท่ากัน → ต้องเป็น "คอลัมน์ที่ stack เอง" เสมอ
> **และ layout ทุกงานต้องเปิดดูจริงก่อน push** — build/lint/test ผ่านไม่ได้แปลว่าหน้าตาใช้ได้ (§6.14.3)
> วิธีถ่ายภาพจอจริง: เปิด `npx vite --config audit/vite.audit.mjs` → playwright เปิด
> `/audit/index.html?p=DailyReport` → **คลิกการ์ดกะ** (ไม่คลิก = ไม่มีเนื้อหา) → **กางแผงที่ default ยุบ** → screenshot

### 📚 ลิสต์ยาวในหน้านี้ — default ยุบ + จำกัดแถวแรก (2026-09-09 · feedback user)

*"พอข้อมูลเยอะๆ ต้อง default ยุบและมีปุ่มให้ขยายดูรึป่าว"* — ใช่ ตาม `docs/UI-CONVENTIONS.md` §drill-down
- ค่าคงที่ module scope: **`AUTO_COLLAPSE_ROWS = 8`** (เกินนี้ = แผงเริ่มต้นแบบยุบ) · **`LIST_FIRST = 8`** (แถวแรกเมื่อกาง)
  ที่มาของ 8: ใบผลิตแถวละ ~67px → 8 แถว ≈ 540px = ยังกวาดตาจบในจอเดียวโดยไม่ต้องเลื่อน
- แผงที่ผูก auto-collapse: `prod_orders` · `defects` · `downtime` (เดิม auto เฉพาะตอนว่าง) — ผ่าน `liveCollapsed(key, auto)`
  **⚠️ `toggleLiveCollapse` ต้องส่ง `auto` ตัวเดียวกับที่ `liveCollapsed` ใช้** ไม่งั้นกดครั้งแรกไม่สลับ (ค่า default คนละตัว)
  · ค่าที่ผู้ใช้กดเองชนะ auto เสมอ (จำใน `localStorage['dr_live_collapse_*']`)
- ลิสต์ที่จำกัดแถว + ปุ่ม "▼ แสดงอีก N (ทั้งหมด M)" / "▲ ย่อกลับ" (helper `limitRows` + `moreRowsBtn`):
  ใบที่ยังเปิด · ใบที่ปิดแล้ว/ยกเลิก · งานเสีย · downtime · **กะค้างใน sidebar**
- **หัวข้อโชว์จำนวนเต็มเสมอ ห้ามซ่อนเงียบ** · sidebar กะค้าง: **กะที่เลือกอยู่ต้องติดมาด้วยเสมอ** แม้เกินเพดาน
  (ไม่งั้นกดจาก banner แล้วหาการ์ดไม่เจอ — กับดักเดิมของถัง `__stale_bucket__`)

### หมายเหตุหัวหน้ากลุ่มตอนขอปิดกะ + ไอคอน "ถูกปฏิเสธ" ในลิสต์กะ (2026-08-10 · คำขอหัวหน้ากลุ่ม)

- **`production_sessions.close_request_note`** (migration `20260810_session_close_request_note.sql` DR · additive) — หมายเหตุที่**ผู้ขอปิดกะ**เขียนเอง (เช่น "ยอดไม่ถึงเพราะรอ material ตั้งแต่ 14:00") · ครบชุด 3 ช่อง: **ขอ** (`close_request_note`) → **อนุมัติ** (`close_approve_note`) / **ปฏิเสธ** (`close_reject_reason`) · แสดงใน modal ตรวจสอบคำขอของ SV (อ่านก่อนตัดสินใจ) + ประวัติกะ · เขียนแบบ **update แยก best-effort** — ยังไม่ apply migration = ปิดกะได้ปกติ **ห้ามยัดลง payload หลัก** (คอลัมน์ไม่มี = ปิดกะพังทั้งระบบ)
- **ชิป "✏️ ต้องแก้" แดง (นิ่ง ไม่กระพริบ) ในลิสต์กะ** เมื่อ `status === 'open' && close_reject_at`
  - **⚠️ ห้ามใช้ `✕`** — ในหน้าเดียวกัน `✕ ปฏิเสธ` คือ**ปุ่มกด** เห็นในลิสต์แล้วนึกว่ากดเพื่อลบ/ปิด (เคยใช้แล้ว user ทัก 2026-08-13) · **ห้ามใช้ `↩️`** — หน้านี้ใช้เป็น "ถอยใบ" ของ order ไปแล้ว
  - **หลักที่ใช้แยก 2 สถานะ: ⏳ = ลูกบอลอยู่ฝั่ง SV (รออนุมัติ · เจอเยอะสุด เอาแค่ไอคอน) vs ✏️ ต้องแก้ = ลูกบอลอยู่ฝั่งเรา (ใช้ชิปข้อความ อ่านออกทันทีไม่ต้องเดาไอคอน)** — เดิมรู้ว่าถูกตีกลับต่อเมื่อกดเข้าไปในกะ หัวหน้าที่ดูแลหลายไลน์เลยไม่เห็น · คู่กับ ⏳ (pending_close) ที่มีอยู่เดิม · tooltip บอกคนปฏิเสธ + เหตุผล
- **ส่งขอปิดกะใหม่ = เคลียร์ `close_reject_*`** (เดิมเอกสารบอกว่าเคลียร์ แต่โค้ดไม่ได้ทำ → ไอคอน ✕ จะค้างทั้งที่แก้แล้ว) · เคลียร์ใน best-effort update ก้อนเดียวกับ note
- **⏰ กะค้าง = แท็บของตัวเอง `?tab=stale` ห้ามเอากลับไปกอง Live (2026-08-26 · user "live กะนี้ ไม่ควรโชว์กะค้าง — แยกแท็บไล่ปิดให้หัวหน้าแผนกไปเลย")**
  - **กติกา/สูตรอยู่ `src/utils/staleSessions.js` ที่เดียว** (`STALE_SESSION_DAYS` · `sessionAgeDays` · `ballSideText` · `fetchStaleSessions` · hook `useStaleSessions`) — **ห้าม copy เกณฑ์ไปเขียนซ้ำในหน้า** (เดิมนิยามอยู่ใน DailyReport.jsx ใช้ได้ที่เดียว)
  - **โหลดที่หน้าแม่ (`DailyReport`) ไม่ใช่ใน LiveTab** — badge บนแท็บต้องเห็นจากทุกแท็บ ไม่งั้นสลับไปแท็บอื่นแล้วงานค้างหายจากสายตา · LiveTab เรียก `stale.reload()` ท้าย `load()` (ผ่าน **ref ห้ามใส่ใน deps** ไม่งั้น `load` วนซ้ำ) → ปิด/อนุมัติกะแล้วตัวเลขขยับเอง
  - **แท็บ ⏰ อ่านอย่างเดียว + พาไปที่กะนั้นในแท็บ Live** (`focusSessionId` ส่งลง LiveTab แล้ว `setSelSession` ตอน load) — **ห้ามใส่ปุ่มปิด/อนุมัติรวบทีเดียวที่นั่น** ปิดกะต้องยืนยันยอด/OEE รายกะผ่าน modal ปิดกะ (กดรวบ = stamp ตัวเลขที่ไม่มีคนดู หลักเดียวกับข้อห้าม auto-close)
  - **แท็บโผล่เมื่อมีกะค้างเท่านั้น** (ไม่มี = ไม่มีแท็บให้รก) · **โหลดไม่สำเร็จก็ต้องโผล่** พร้อมข้อความ error + ปุ่มลองใหม่ (0 กับ "อ่านไม่ได้" คนละเรื่อง ห้ามขึ้น "ไม่มีกะค้าง")
  - **บนแท็บ Live เหลือชิปบรรทัดเดียว** (กดไปแท็บ ⏰) — **ห้ามตัดทิ้งทั้งหมด** สัญญาณต้องยังเห็นจากหน้าทำงานจริง
  - **⚠️ scope ของ leader = ทั้งครอบครัวไลน์** — ตัวเดิมใน LiveTab เทียบ `o.line_name === myLine.name` ตรงตัว ทำให้หัวหน้าไลน์แม่ไม่เห็นกะค้างของไลน์ลูก (บั๊ก class เดียวกับกฎ scope ของ leader) · `fetchStaleSessions` ใช้ family แล้ว
  - **ลิสต์ซ้ายไม่ต้องซ่อนกะค้างซ้ำอีก** — มี "ถังพับ ⏰ ค้างจากวันก่อน" อยู่แล้ว (กฎถัดไป) · 2 กลไกนี้ทำคนละหน้าที่: **ถัง = ยังกดถึงได้ในที่เดิม · แท็บ = คิวไล่ปิดของหัวหน้าแผนก** ห้ามยุบเหลืออันเดียว
    (เคยเขียน `hideStale` ซ่อนกะค้างออกจากลิสต์ในคอมมิทนี้ แล้วถอดออกตอน rebase เพราะถังของ session ขนานครอบแล้วและดีกว่า — ของที่ยัง "กดถึงได้" ชนะของที่ "ถูกซ่อน")
- **⚠️ กลุ่มไลน์ในลิสต์กะต้อง "ย่อได้เสมอ" แม้กำลังเลือกกะในกลุ่มนั้น (2026-08-26 · user แจ้ง 2 รอบ)** — เดิมเขียน `collapsed = has(group) && !hasSel` ("กะที่เลือกต้องไม่ถูกซ่อน") ผลคือ**กลุ่มที่กำลังใช้งานกางค้างถาวร กดย่อไม่ติด** (HYDROFORM 18 กะ · LWR BAR 10) เบียดกลุ่มอื่นตกจอจนหาไลน์ถัดไปไม่เจอ · อาการหลอกตา: ย่อแล้ว "เหมือนหายไปแป๊บ" เพราะพอสลับไปเลือกกลุ่มอื่น กลุ่มเดิมถึงยุบตามค่าที่เคยกดไว้
  - **กติกาใหม่: ย่อแล้วเหลือ "แถวเดียวคือกะที่เลือก" + บอกจำนวนที่ซ่อน (`+N กะในกลุ่มนี้ถูกย่อไว้ — กางดู`)** → ได้ทั้งย่อจริงและไม่ทำให้กะที่ทำงานอยู่หายจากสายตา · หัวกลุ่มโชว์ `1/18` ให้รู้ว่าเลขคือทั้งกลุ่ม ไม่ใช่จำนวนที่เห็น
  - **หลักที่ใช้ซ้ำได้: "ห้ามซ่อนของที่กำลังใช้งาน" ≠ "ห้ามให้ย่อ"** — เจอ pattern `&& !isActive` ที่ล็อก UI ไว้ ให้เปลี่ยนเป็น "ย่อแต่คงตัวที่ active ไว้" เสมอ
- **⏰ กะค้างไม่ยอมปิด/อนุมัติ — escalation 7 วัน "เตือนดัง ห้าม auto-ปิด" (2026-08-25 · user ขอ lead time 7 วันเพื่อบีบหัวหน้าแผนก · เจอจริง 57 กะค้าง):** เพดานอยู่ที่ `STALE_SESSION_DAYS = 7` (`src/utils/staleSessions.js`) · แท็บ ⏰ เรียงเก่าสุดขึ้นบน + ชิปอายุ (เหลือง ≥3 วัน · แดงเกิน 7 — **นิ่ง ไม่กระพริบ** เป็นงานค้างไม่ใช่ alarm) + **บอกว่าลูกบอลอยู่ฝั่งใคร** (pending_close = "รอ SV อนุมัติ (ขอโดยใคร)" · open = "หัวหน้ากลุ่มยังไม่ขอปิดกะ") + ประโยคผลกระทบ "OEE ของกะนี้หายจากรายงานเดือน" (แรงกดดันที่แท้จริง — ระบบนับเฉพาะกะปิดแล้ว) · ลิสต์กะซ้ายมีชิป ⏰ Nว รายกะ · `/dept-dashboard` ฝั่งผลิตมีคิว "กะค้างยังไม่ปิด N กะ" **ยุบเป็นแถวเดียว** (57 แถวจะกลบคิวงานจริง — บทเรียน 4M [Auto] 323 ใบ) · **ห้ามทำ auto-close/auto-approve เมื่อครบ 7 วัน** — ปิดกะ = stamp OEE จากยอดที่คนยืนยัน auto = สร้างข้อมูลเท็จ + auto-approve = โกหกว่ามีคนพิจารณา (precedent เดียวกับกฎเคลียร์คิว 4M: ห้าม approved) · เรื่อง "หนักระบบ" ตรวจแล้วไม่จริง (กะค้างหลักสิบแถวจิ๋วมาก + จอ live กรองตามวันอยู่แล้ว) — ปัญหาจริงคือรูโหว่รายงานเดือน+ความรก
> #### ⚠️ กฎเหล็ก — sidebar รายชื่อกะ ต้องแยก "กะที่กำลังทำ" ออกจาก "กะค้าง" (2026-08-26 · feedback "ปวดหัวกับกะที่รก ค้างจังเลย")
> ข้อมูลจริงที่หน้างานเจอ: **sidebar ขึ้น 49 กะ ในนั้น 37 กะเป็นของวันก่อนที่ยังไม่ปิด** (เก่าสุด 23 วัน)
> → กะของวันนี้ ซึ่งเป็นสิ่งที่ต้องกรอกตอนนี้ **จมอยู่ในกำแพงการ์ด**
> - `freshSessions` / `staleSessions` แยกด้วย `work_date >= workDate()` (ตัด 08:00 ตามกฎวันงาน **ห้ามใช้วันปฏิทิน**)
> - กะค้าง = **ถังพับ "⏰ ค้างจากวันก่อน (N)"** — ค่าเริ่มต้นพับ · กางเองเมื่อกะที่เลือกอยู่ในถัง
>   (ไม่งั้นคลิกจาก banner แล้วหาการ์ดไม่เจอ)
> - **⚠️ flag ใน `sessGroupCollapsed` ของถังนี้กลับด้านกับกลุ่มไลน์** — มีคีย์ `STALE_BUCKET_KEY` = "ผู้ใช้กางเอง"
>   (เพราะค่าเริ่มต้นคือพับ) · ปุ่ม "ย่อ/กางทั้งหมด" คุมเฉพาะกลุ่มไลน์ **ต้องคง sentinel ไว้เสมอ**
> - **ห้ามซ่อนหาย** — ตัวนับบนหัว (`กะที่เปิดอยู่ (N + ค้าง M)`) + banner ⚠ เดิมยังนับครบเหมือนเดิม
> - **ยังห้าม auto-close/auto-approve เมื่อครบ 7 วัน** (กฎเดิม) — นี่คือการจัดจอ ไม่ใช่การเคลียร์งานแทนคน
>
> #### 🧹 เคลียร์ "กะเปล่า" ที่ค้างได้ทีเดียว — แต่เฉพาะกะเปล่าเท่านั้น (2026-08-26)
> ส่วนหนึ่งของกะค้างคือกะที่เปิดผิด/เปิดทิ้งไว้ **ไม่มี Order/Downtime/ของเสียเลย** แล้วไม่มีใครกล้าแตะ
> เพราะต้องเข้าไปกดลบทีละกะ → ปุ่ม **🔎 ตรวจหากะเปล่า → 🗑 ลบกะเปล่า N กะ** ในแท็บ ⏰ กะค้าง
> - **ลบกะเปล่าไม่ได้สร้างข้อมูลเท็จอะไร** — ต่างจาก auto-close ที่ต้อง stamp OEE จากยอดที่ไม่มีคนยืนยัน (ยังห้ามทำ)
>   **กะที่มีข้อมูลจริงต้องให้คนปิดเองเสมอ** และจอต้องบอกว่าเหลืออีกกี่กะที่ระบบไม่ปิดให้
> - **ยิงคิวรีเฉพาะตอนกดปุ่ม ไม่ใช่ตอนเปิดแท็บ** (กฎ egress) · ผ่าน **`fetchByIds`** เสมอ
> - **⚠️ `error`/`truncated` ห้ามกลืนเด็ดขาด** — โหลดไม่ครบแล้วเดาว่า "เปล่า" = **ลบกะที่มีข้อมูลทิ้ง**
>   → ขึ้น "ตรวจไม่สำเร็จ · ยังลบไม่ได้" **ห้ามแสดงเป็น "ไม่มีกะเปล่า"**
> - สิทธิ์ = `daily_report:delete_session` เดิม (seed `admin` เท่านั้น — ไม่เพิ่ม key ใหม่)

> #### 🔄 สลับ "Daily Report ↔ จัดการไลน์" แบบพาบริบทไปด้วย — **ไม่รวมเป็นหน้าเดียว** (2026-08-26 · user ถามว่าควร combine เป็นแท็บไหม)
> **ตอบ: ไม่ควรรวม** — เหตุผลที่วัดได้:
> 1. **ขนาด** DailyReport 6,959 + Management 3,359 = **10,300+ บรรทัดในหน้าเดียว** (chunk ใหญ่ โหลดช้าทั้งคู่แม้ใช้แท็บเดียว)
> 2. **layout ขัดกันโดยดีไซน์** — Management เป็น **บอร์ดจอ TV** (ไม่มี PageHeader · เต็มความกว้าง ไม่ cap 1800px)
>    ส่วน DailyReport เป็น**หน้ากรอกข้อมูล** (PageHeader + ฟอร์ม + modal) → รวมแล้วต้องทิ้งอย่างใดอย่างหนึ่ง
> 3. **แท็บที่ mount ทีละอัน = เท่ากับ route เดิมเป๊ะ ไม่ได้อะไรเพิ่ม** · แท็บที่ mount ค้างทั้งคู่ =
>    Management poll DB (`RATE.BOARD`) ตลอดเวลาที่กรอก downtime = เปลือง egress ฟรี (งบใช้ไป ~24% แล้ว)
>
> **ปัญหาจริงคือ "บริบทไลน์ไม่ส่งต่อกัน"** — 2 หน้าเลือกไลน์คนละที่ สลับทีต้องเลือกใหม่ทุกครั้ง
> → แก้ด้วย **ปุ่มสลับที่พา `?line=` ไปด้วยทั้ง 2 ทาง** (pattern เดียวกับ FactoryMap→Dashboard · HeijunkaKanban→Management)
> - DailyReport หัวกะ → ปุ่ม **🔄 จัดกำลังคน** → `/management?line=<ไลน์ของกะที่เลือก>&from=daily-report`
> - Management sidebar → ปุ่ม **📊 Daily Report (ชื่อไลน์)** → `/daily-report?line=<selectedLine>`
> - **DailyReport รับ `?line=` แล้ว** (เดิมไม่รับเลย) — เลือกกะของไลน์นั้นให้ · ไม่มีกะเปิดของไลน์นั้น = ตกไป default **ห้ามจอว่าง**
> - **⚠️ ล้าง param ต้องลบเฉพาะ `line` ห้าม replaceState ทับทั้ง search** — จะพา `?tab=` ของ `useTabParam` หายไปด้วย
> - **ทั้ง 2 ปุ่มเช็ค `canAccessPage()` ก่อนเสมอ** ไม่มีสิทธิ์ = ไม่โชว์ปุ่ม (ห้ามพาไปแล้วโดนเด้ง)
> - **เฟสถัดไปถ้ายังไม่พอ:** ฝัง "ผังไลน์ + คน" เป็นแผงพับใน DailyReport — แต่ตัวลากคน + 4M Man auto-log
>   ฝังอยู่ใน Management ~2,000 บรรทัด **ต้องแยก component ก่อน** และมีความเสี่ยงกับ flow อนุมัติ 4M ที่ทำงานอยู่
- **⚠️ ไอคอน ✕ ขึ้นได้ต่อเมื่อ apply `20260723_session_close_reject_reason.sql` แล้ว** (เพราะ `close_reject_at` เขียน best-effort) — ถ้าแบนเนอร์แดงในกะยังไม่เคยโชว์ข้อความ แปลว่ายังไม่ apply

> ### ⚠️ กฎเหล็ก — `try { await supabaseDR...update() } catch {}` **ไม่ใช่ best-effort มันคือกลืน error ทิ้ง** (2026-08-10)
> **supabase-js "คืน" `{ error }` ไม่ได้ throw** → `try/catch` ที่ไม่ destructure `error` จะไม่มีวันจับอะไรได้เลย
> คอลัมน์ยังไม่มี (42703) = update ล้มเหลว **แต่หน้าจอบอกว่าสำเร็จ** — เจอจริงกับ `close_approve_note`/`close_reject_reason`/`close_request_note` ทั้ง 3 จุด (SV พิมพ์เหตุผลปฏิเสธ → หัวหน้ากลุ่มเห็นแบนเนอร์เปล่า ไม่รู้ว่าต้องแก้อะไร)
> **pattern ที่ถูกสำหรับคอลัมน์ additive ที่อาจยังไม่ apply:**
> ```js
> const { error } = await supabaseDR.from(t).update({...}).eq('id', id)
> if (error) { console.warn(...); toast.error('งานหลักสำเร็จ แต่ <ฟิลด์> ยังบันทึกไม่ได้ — ยังไม่ได้ apply migration <ชื่อ> (แจ้ง admin)') }
> ```
> **flow หลักต้องไม่พัง + ผู้ใช้ต้องรู้ว่าอะไรไม่ถูกบันทึก** — เงียบแปลว่าไม่มีใครไป apply migration สักที

> ### ⚠️ กฎเหล็ก — `.update()` ที่ถูก **RLS บล็อก ไม่ error** มันคืน "สำเร็จ 0 แถว" (2026-08-17)
> คนละเคสกับคอลัมน์ไม่มี (42703 ที่ error จริง) — **RLS ไม่ให้แก้ = PostgREST คืน 200 ไม่มี error เลย**
> โค้ดที่เช็คแค่ `if (error)` จึงขึ้น toast เขียวทั้งที่ไม่มีอะไรถูกเขียน
> **เคสจริง:** หัวหน้ากลุ่มวาดลายเซ็นในโมดัล "จัดการลายเซ็น" → ขึ้น "บันทึกลายเซ็นเรียบร้อย" → **logout/login แล้วหาย ต้องเซ็นใหม่ทุกครั้ง** (ฝั่งอ่านถูกอยู่แล้ว — `fetchProfile` select `signature_url` เข้า state ตอน login · ที่พังคือฝั่งเขียน) · รูปโปรไฟล์ (`avatar_url`) เป็นบั๊กชนิดเดียวกัน
> **ต้องนับแถวเสมอ:** `.update({...}).eq('id', id).select('id')` แล้วเช็ค `data.length` — 0 แถว = ล้มเหลว **ห้ามขึ้นว่าสำเร็จ**
> **การให้ user แก้โปรไฟล์ตัวเอง ห้ามใช้ policy `for update using (id = auth.uid())` แบบตรงๆ** — ทดสอบแล้ว **user แก้ `role` ตัวเองเป็น `admin` ได้จริง** (RLS จำกัดรายคอลัมน์ไม่ได้ · column GRANT ก็ใช้ไม่ได้เพราะ `/add-user` ต้องให้ admin แก้ role/section ของคนอื่นผ่าน client ตรงๆ)
> **ทางที่ใช้: SECURITY DEFINER แตะเฉพาะคอลัมน์ที่อนุญาตของแถวตัวเอง** — `set_my_signature(text)` / `set_my_avatar(text)` (migration `20260817_profile_self_update_rpc.sql` · Main) เรียกผ่าน **`saveMyProfileMedia(field, url)` ใน `src/utils/profileSelf.js` จุดเดียว** (RPC ก่อน → ยังไม่ apply migration ค่อยถอยไป update ตรงแบบนับแถว)
> **จุดใหม่ที่ให้ user แก้ข้อมูลโปรไฟล์ตัวเอง ให้เพิ่มคอลัมน์เข้า RPC + helper นี้ ห้าม update ตรง**

### 🔴 เลิก "ชวนเปิดกะอัตโนมัติหลังเช็คชื่อ" — ไลน์ไปเปิดเอง (2026-09-16 · คำสั่ง user)

user: *"มีบางไลน์ที่ใช้ระบบเช็คชื่อแต่ยังไม่ลงข้อมูลในรีพอร์ท ละมันบวม"* → *"ให้เค้าเปิดกันเองดีกว่า"*

**ของเดิม:** กดบันทึกเช็คชื่อใน `/checkin` เสร็จ → เด้ง modal "📊 เปิดกะ Daily Report?" ลิสต์ไลน์ที่เช็คชื่อ
**โดยติ๊กถูกไว้ให้ครบทุกไลน์** (`subLineSelections` default `true`) กดปุ่ม "เปิดกะ" ครั้งเดียว = insert
`production_sessions` ทุกไลน์รวด — ไลน์แม่ถูก expand เป็นไลน์ลูกด้วย

**ทำไมถึงบวม:** ไลน์ที่ใช้เช็คชื่อ **แต่ยังไม่ได้ใช้ Daily Report** ได้กะเปล่าแถมมาวันละ 1-2 ใบ
ค้าง `status='open'` ไม่มีใครปิด เพราะเจ้าของไลน์ไม่รู้ด้วยซ้ำว่ามีกะเปิดอยู่
· วัดจริง 16/09: **PD1 4 ไลน์ (LINE GWM · LINE MAIN TSRA-1/2 · LINE SUB-STATIONARY) = 32 กะ เปล่าทั้ง 100%**
ตั้งแต่วันแรกที่เริ่มเช็คชื่อ (09/09) — ไม่มีสักใบที่มี order/downtime/defect
· **ลายเซ็นที่ฟ้องว่าเป็น modal ไม่ใช่คนกด: 4 ไลน์ถูกสร้างห่างกัน 0.35 วินาที ทุกวัน**
· รวมทั้งระบบ: กะเปล่า **214 จาก 1,382 ใบ (16%)** · 123 ใบมี `oee_a`/`oee_q` stamp ค้าง (A=100/Q=100
ของกะที่ไม่มีงาน) รั่วเข้าค่าเฉลี่ย %A ในกราฟเทรนด์ — ลบแล้ว avg %A **91.12 → 90.16**

**ที่แก้:** ถอด modal + `handleConfirmOpenShift` + state (`openShiftModal`/`subLineSelections`/`openingShift`/
`parentChildrenMap`) ออกจาก `Checkin.jsx` ทั้งชุด — **หน้าเช็คชื่อไม่แตะ DR project อีกต่อไปแล้ว**
(ถอด import `supabaseDR` ออกด้วย) · เหลือไว้แค่การคำนวณ `anyOtNight` สำหรับสรุป Telegram (พฤติกรรมเดิมเป๊ะ)
· **การเปิดกะเป็นการตัดสินใจของไลน์ → ไปกดเองที่ `/daily-report`** (ENGINEERING-PRINCIPLES §2 —
ห้ามให้ระบบเดาแล้วเขียนข้อมูลแทนคน)
· ผลพลอยได้: บั๊ก "เปิดกะซ้ำ 2 กะ" (§ด้านบน 2026-09-10) หมดต้นตอฝั่งเช็คชื่อไปด้วย

> #### ✍️ ตามด้วย: ต้องเขียนบนจอให้ชัดว่า "เช็คชื่อ ≠ เปิดกะ" (2026-09-17 · user)
> user รายงานถัดมา: *"เปิดกะจากหน้าเช็คชื่ออะไม่ติด แต่ไม่ต้องออโต้ นี่ย้ายไปมันก็เลยเปลี่ยนพฤติกรรมสิ
> แต่จะต้องเขียนให้ชัดมั้ยว่าเปิดกะสำหรับลงข้อมูลการผลิตเท่านั้น จะดีกว่ามั้ย"*
> **บทเรียน: ถอดพฤติกรรมเดิมออกเงียบๆ = หน้างานอ่านว่า "ระบบพัง" ไม่ใช่ "ตั้งใจเอาออก"**
> (ENGINEERING-PRINCIPLES "ห้ามล้มเหลวเงียบ" ใช้กับ*การถอดฟีเจอร์*ด้วย ไม่ใช่แค่ error)
> **ที่ทำ (ไม่เอาออโต้กลับมา):**
> - กล่องข้อความใน popup ยืนยันบันทึกของ `/checkin` — จุดเดียวกับที่ modal เปิดกะเคยเด้ง:
>   *"ℹ️ เช็คชื่อไม่ใช่การเปิดกะ · ระบบไม่เปิดกะให้อัตโนมัติ — **เปิดกะ = สำหรับไลน์ที่จะลงข้อมูล
>   การผลิตเท่านั้น** (ยอดผลิต · Downtime · ของเสีย · OEE) · ไลน์ที่ต้องลงข้อมูลผลิตให้ไปกด "เปิดกะ"
>   ที่หน้า Daily Report ของไลน์ตัวเอง · **ไลน์ที่ใช้แค่เช็คชื่อ ไม่ต้องเปิดกะ**"*
> - toast หลังบันทึกบอกทางไปต่อ: *"บันทึกเช็คชื่อสำเร็จ! · ไลน์ที่ต้องลงข้อมูลผลิต ไปกด "เปิดกะ" ที่หน้า Daily Report"*

**ล้างของเก่า:** `20260916_delete_empty_sessions_dr.sql` (**apply แล้ว 2026-09-16** — DR/"Product DB")
ลบ 214 ใบ · สำรองครบทุกคอลัมน์ใน `production_sessions_empty_backup_20260916` (ทางถอย)
· 🔴 **เงื่อนไข `work_date < work_date_bangkok()` ห้ามถอดออกถ้าเอาไปรันซ้ำ** — **กะที่เพิ่งเปิดต้นกะ
ยังไม่สแกนใบแรก หน้าตาเหมือนกะเปล่าเป๊ะ** · ตอนรันจริงมี 7 ไลน์ที่ทำงานอยู่ (Line 60/61 · HDF1 ·
LASER-345/E50 · SUB APRON · BENDING E50) เพิ่งเปิดกะ 08:11 และยังว่าง — ถ้าไม่มีเงื่อนไขนี้คือ
**ลบกะที่หน้างานกำลังใช้ทิ้ง 7 ไลน์** (เช็คหลังลบ: ทั้ง 7 รอดครบและมี order 1-27 ใบแล้ว)
· `work_date_bangkok()` ตัดที่ 08:00 = จังหวะที่กะดึกของวันก่อนเพิ่งจบพอดี → ขอบเขตปลอดภัยพอดีเป๊ะ

---

### เปิดกะผิด (กะเปล่า) — ลบ + ไม่ทิ้ง phantom OEE (2026-07-23)

- **ลบกะเปล่าจากจอ Live ได้เลย** (ปุ่ม 🗑 ลบกะเปล่า ในหัว session) — เห็นเมื่อ `can('daily_report','delete_session')` **และ**กะ `open`/`pending_close` **และไม่มี Order/Downtime/Defect เลย** (guard ซ้ำใน `handleDeleteEmptySession`) · เดิมลบได้เฉพาะกะ **closed** ในแท็บประวัติ (`HistoryTab`) → หัวหน้าหาไม่เจอตอนกะยังเปิด
- **⚠️ สิทธิ์ `delete_session` seed ให้ `admin` เท่านั้น** (`20260708_phase0_permission_catalog.sql`) — supervisor/หัวหน้าแผนกไม่เห็นปุ่มลบ · ถ้าอยากให้ลบเองได้ admin เปิดที่ `/permissions`
- **ปิดกะที่ไม่มีผลผลิต (`totalProduced===0 && P==null`) → stamp `oee_a`/`oee_q` = null** (ไม่ใช่ 100/0) ทั้ง handleCloseSession + edit-times recompute · เดิมกะเปล่ามีเวลาเดินกะแต่ไม่มี DT → A=100/Q=100 ค้าง **รั่วเข้าค่าเฉลี่ย %A/%Q ในกราฟเทรนด์** (กรองแค่ != null) · OEE รวมไม่เคยกระทบ (oee=null ถูกกันอยู่แล้ว) · สอดคล้อง cleanup `20260715_oee_null_noproduction_cleanup.sql` (กันตั้งแต่ปิดกะ ไม่ต้องมาไล่ลบทีหลัง)

---

### 🔴 บั๊ก "รอสโตร์จ่ายวัตถุดิบ" ในคิวสั่งผลิตจากสโตร์ (2026-09-10 · user จับได้)

*"อีกอย่างเราว่ามันบัคนะ นี่มันงานของไลน์มันเอง จะไปรอสโตร์จ่ายทำไม"* →
*"2xxx 3xxx 5xxx ต้นทางอาจจะต่างกัน แต่ขั้นตอนที่จะเข้าไลน์ผลิตคือสโตร์จ่ายทุกตัวอยู่ดี
**แต่มันต้องอิง routing ไง** เบอร์ 2 คือผลิตภายใน บางทีมาจากไลน์อื่น
แต่หลักๆ มันไม่มีทางเรียกงานที่ตัวเองผลิตจากสโตร์"*

**บั๊ก A — แถบ "🪨 รอสโตร์จ่ายวัตถุดิบ N/N" เป็น false alarm ถาวร (แก้ฝั่งจอแล้ว)**
`raw_withdrawal_requests.status` ถูกเซ็ตเป็น `issued` **ที่เดียว** = ตอนคนกดปิดล็อต/กดจ่ายรายตัว
ที่ `/heijunka` (`HeijunkaKanban.jsx` ~1512 และ `issueRaw`) — ไม่มีใครกด
วัดจริง 10/09 ทั้งตาราง: **pending 727 · issued 13 (ครั้งสุดท้าย 27/08) · cancelled 700**
⇒ ทุกล็อตขึ้น "รอสโตร์จ่าย" ตลอดกาล ทั้งที่ Assy GOR ผลิตวันนั้น 475/475 = 100%
**มันไม่ได้วัดว่าของถึงไลน์หรือยัง — มันวัดว่ามีคนกดปุ่มบนบอร์ดสโตร์หรือยัง**
→ เลิกใช้เป็นสัญญาณเตือน · เปลี่ยนเป็น **"ของที่ต้องใช้มาจากไหน"** (ตอบว่ารอ*ใคร*):
`🏭 ต่อจาก <ไลน์> (n)` · `🛒 ของซื้อ n` · `❓ routing ยังไม่ตั้ง n` · `⚠️ ไลน์นี้ทำเอง n`
(เลขใบเบิก/ปิดแล้วกี่ใบ ย้ายไป title — ไม่ได้ลบข้อมูล แค่ไม่โชว์เป็นไฟแดงปลอม)
· routing อ่านจาก `dr_products.mat_no → line_name` ของวัตถุดิบแต่ละตัว (คิวรีเพิ่ม 1 ครั้ง)
· **`⚠️ ไลน์นี้ทำเอง` = กฎจาก user "ไม่มีทางเรียกงานที่ตัวเองผลิตจากสโตร์"** — วัดแล้วยังไม่มีแถวแบบนี้
  ในฐาน (0 แถว) แต่ต้องเฝ้าไว้ ถ้าโผล่แปลว่า BOM/routing ผิด

**บั๊ก B — routing ขาด ⇒ ใบสั่งผลิตพาร์ทลูกกลายเป็น "ใบกำพร้า" (ยังไม่แก้ — ต้องเติม master data)**
`fn_explode_child_demand` หา `v_srcline` จาก `b.source_line` → ถอยไป `dr_products.line_name`
ถ้าไม่เจอ **เขียน `child_lot_requests.source_line = null` แล้วออกใบต่อไปเงียบๆ**
แต่ทุกจอที่แสดงคิว (StoreLotQueue · Heijunka) กรองด้วย `source_line`
⇒ **ใบออกไปแล้วแต่ไม่โผล่บนจอไลน์ไหนเลย**
วัดจริง 10/09: ใบเปิดค้าง **100 ใบ · เป็นใบกำพร้า 27 ใบ / 8 พาร์ท (เบอร์ 2 ทั้งหมด)**
เก่าสุดออกใบ 03/08 — พาร์ทที่ค้างมากสุด `20066332` 11 ใบ / 33,000 ชิ้น
**ทางแก้ = ตั้ง `line_name` ให้ 8 พาร์ทนี้ใน Product Master แล้ว backfill `source_line` ของใบที่ค้าง**
(ยังไม่ทำ — รอ user ยืนยันว่าแต่ละพาร์ทเป็นของไลน์ไหน · ห้ามเดา เพราะ backfill ผิด = ใบไปโผล่ผิดไลน์)
**และควรกันไม่ให้เกิดซ้ำ:** ให้ `fn_explode` ที่หา routing ไม่เจอ ไปลง `v_demand_flow_blocks`
(block_reason ใหม่ `no_routing`) แทนการออกใบกำพร้า — ยังไม่ทำ

### 🔩 ตัวเลือก MAT.NO ของโมดัล "Scan เปิด Prod Order" — ชั้นแข็ง own → parent → family (2026-09-15)

*"ไลน์ย่อยยังจะเห็นพาร์ทของไลน์ย่อยอื่นหรอ เราว่าไม่น่านะ มันจะทำให้เกิด human error"* (user · หน้า LASER-345)

**ตรวจแล้ว: ไลน์พี่น้องไม่เคยโผล่อยู่แล้ว** — `getLineFamily` เดินสายล่าง BFS **จากตัวเองเท่านั้น**
(ไม่ไล่ลงจาก ancestors) ⇒ family ของ LASER-345 = ตัวเอง + HYDROFORM เท่านั้น
5 รายการที่เห็น = **ของ HYDROFORM ไลน์แม่ล้วน** · ของ LASER-789 (20065635/20065715) ไม่ได้โผล่
(ถ้าโผล่จะเป็น 7) · ⚠️ คอมเมนต์เดิมใน `lineHierarchy.js` เขียนว่า family รวม "พี่น้อง" ซึ่ง**ผิด** — แก้แล้ว

**รากจริง:** LASER-345 ไม่มีพาร์ทผูกกับตัวเองเลย · วัด 15/09 ทั้งโรงงานมีแค่ **12 ไลน์** ที่มีพาร์ทผูกไว้
(HDF1 · HDF2 · LASER-345 · BENDING E50 · BENDING EXPORT · Laser GOR = 0 ทั้งหมด)

**ที่แก้:** `src/utils/matScope.js` (`scopeMatRows` · pure · มีเทส) แทน union แบนทั้งครอบครัว
| ชั้น | เงื่อนไข | ผล |
|---|---|---|
| `own` | ไลน์มีพาร์ทของตัวเอง | ใช้แค่ของตัวเอง — **ตัดของไลน์แม่ทิ้งด้วย** |
| `parent` | ไม่มีของตัวเองเลย | ถอยไปของสายบน (ไลน์แม่ = ระดับแผนก) |
| `family` | ยังไม่มีอีก | ลูก/หลานของตัวเอง |
| `none` | ไม่มีเลย | แบนเนอร์แดงเดิม (ห้ามเป็น dropdown ว่าง) |

- **ทุกแถวที่ไม่ใช่ของไลน์ตัวเองติดป้าย `🏭 ผูกกับไลน์ <ชื่อ>`** ในบรรทัดย่อยของตัวเลือก + ค้นด้วยชื่อไลน์ได้
- `scope !== 'own'` → แถบเหลืองเหนือ dropdown บอกว่าไลน์นี้ยังไม่มีพาร์ทของตัวเอง + ให้ไปตั้งที่ Product Master
- หัวช่องเปลี่ยนจาก "รายการของครอบครัวไลน์นี้" (กำกวม) → `(N รายการ · ของไลน์นี้ / ของไลน์แม่ <ชื่อ>)`
- **ไม่ย้อนกลับไปเป็นบั๊ก 17/08** ("ลิสต์ว่าง → เปิดใบไม่ได้ทั้งกะ") เพราะชั้น parent/family ยังรับอยู่
- เทสที่ล็อกไว้: **`LASER-345 ห้ามเห็นของ LASER-789`** — family logic เปลี่ยนแล้วพี่น้องหลุดเข้ามา = build พัง

**ช่องโหว่ harness ที่เจอระหว่างทาง (แก้แล้ว):**
1. `kanban_standards.dr_products` ใน mock **ไม่มี `line_name`** ⇒ ตัวเลือก MAT.NO ว่างตลอด
   ⇒ **โมดัล Scan เปิด Order ไม่เคยถูก render ใน crashsweep เลยสักครั้ง**
2. mock ไม่มี `qty_per_kanban` ⇒ บรรทัดย่อยขึ้น `undefined ชิ้น/ใบ`
3. `audit/main.jsx` ล็อก `role: 'manager'` ตายตัว ⇒ ปุ่มที่ยิงผ่าน `can()` ของ role อื่นไม่เคยโผล่
   → รับ `?role=<role>` แล้ว (default ยังเป็น manager เหมือนเดิม)

---

## 🏷️ เลข MAT บนจอต้องบอกได้ว่าเป็นชิ้นงานอะไร — `<MatLabel>` (2026-09-24 · feedback หน้างาน)

**คำร้อง 2 ใบ วันเดียวกัน (23/09) คนละจอ แต่เป็นอาการเดียวกัน:**
- สุทธวีร์ สุขะกูล — *"Production order ที่เปิดค้างไว้ มองเห็นแค่เลข Material number ทำให้ยากต่อการดูว่าคือชิ้นงานอะไร อยากให้แสดง Part name / Part number"*
- ณัฐวุฒิ ทรงดอน — *"ฟังก์ชัน เรียกชิ้นส่วนจากสโตร์ อยากให้โชว์ Part No. เพิ่มจาก Mat.SAP"*

**ไม่ปะทีละจุด** — เลข MAT ถูกโชว์เปล่าๆ อยู่หลายสิบจุดทั้งระบบ แก้ทีละที่จะได้ 3 รูปแบบใน 3 หน้า
⇒ ของกลาง `<MatLabel>` (`src/components/MatLabel.jsx`) + สูตร `src/utils/matLabel.js` · กติกาเต็ม → `docs/UI-CONVENTIONS.md` §6.21

### ตัวเลขที่ทำให้ตัดสินใจแบบนี้ (วัด 24/09 · `prod_orders` 30 วันล่าสุด)

| | ใบ | % |
|---|---:|---:|
| ใบทั้งหมด | 6,484 | 100% |
| มี `part_name` เก็บบนแถวแล้ว | 927 | **14%** |
| ว่าง แต่เติมจาก `dr_products.name` ได้ | 5,557 | 86% |
| มี `p_no` ในทะเบียน | 6,447 | 99.4% |
| ไม่มีแถวทะเบียนเลย | 1 | 0.02% |

⇒ อาการ "เห็นแต่เลข MAT" ไม่ใช่บั๊กที่จอ แต่เป็นเพราะ **`prod_orders.part_name` เป็น snapshot ที่ 86% ว่าง**
🔴 **ไม่ backfill คอลัมน์** — backfill วันนี้ พรุ่งนี้ทะเบียนเปลี่ยนชื่อก็ค้างชื่อเก่าเงียบๆ · อ่านสดจาก
`useProducts` (cache ก้อนเดียว 168 แถว ~1.5 KB **ไม่มีคิวรีเพิ่ม**) แล้ว **ค่าบนแถวชนะทะเบียนเสมอ**
(= "ตอนเปิดใบเรียกว่าอะไร" — กฎเดียวกับชื่อไลน์ตัวเองชนะไลน์แม่)

### จุดที่เปลี่ยนแล้ว
| ไฟล์ | จุด |
|---|---|
| `DailyReport.jsx` | การ์ดใบผลิตของกะสด · ลิสต์ Prod Orders ในประวัติกะ · ลิสต์ "ตัดสินใจยกยอด/ยกเลิก" ตอนปิดกะ |
| `LinePartCallPanel.jsx` | แถวเสนอให้เรียกของ · แถวใบที่ค้างอยู่ · แถว ⏸ พักไว้ก่อน |

`line_part_levels` ที่ตั้งจุดเรียกเติมไว้ **60 พาร์ท — อยู่ในทะเบียนครบ 60 และมี `p_no` ครบ 60** ⇒ แถวเรียกของเห็น Part No. ทุกแถว
⚠️ **ห้ามเอา `line_part_levels.note` มาใส่ `name`** — เป็นโน้ตอิสระ ไม่ใช่ชื่อพาร์ท (ใส่ผิด = จอโกหกแบบดูไม่ออก)
