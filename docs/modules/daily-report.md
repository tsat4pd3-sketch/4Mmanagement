...
- **`PANEL_GRID` = `cardGrid(460, 12)` + `rowGap: 0`** ครอบแผงเนื้อหาทั้งชุด (StoreLotQueue ·
  LinePartCallPanel · LineWipPanel · Prod Orders · งานเสีย · Downtime)
  · จอ 1920 → คอลัมน์เนื้อหา ~1525px = **3 คอลัมน์ (~500px/แผง)** · ~1000-1420px = 2 · แคบกว่า 940px = 1
  · `rowGap: 0` เพราะแผงเดิมมี `marginBottom: 16` ของตัวเองอยู่แล้ว
- **ชั้นในไม่ต้องทำอะไรเพิ่ม** — `ORDER_GRID`/`DT_GRID`/`DEFECT_GRID` ใช้ `min(100%, N)` อยู่แล้ว
  พอแผงเหลือ ~500px มันยุบเหลือคอลัมน์เดียวเอง = ได้ "กล่องละคอลัมน์เดียว" ตามที่ขอโดยไม่ต้องเขียนเงื่อนไข
- **ข้อแลกที่รู้ตัว:** ที่ 500px/แผง แถวยาว (สโตร์ · Downtime ที่มีปุ่ม 5 ตัว) จะ wrap เป็น 2 บรรทัด
  — ยอมแลกกับการเห็น 3 เรื่องพร้อมกันโดยไม่ต้องเลื่อน · จอ 2560 ได้ ~840px/แผง = ไม่ wrap เลย
- แผงที่ไม่มีข้อมูลจะ `return null` อยู่แล้ว (StoreLotQueue/WIP) → คอลัมน์ไหลมาเติมเอง ไม่เหลือช่องว่าง
