...
- **⚠️ dropdown เลือกสินค้าของ "เปิดเป้าผลิต" โชว์ทุกแถวของ family ไลน์ — ไม่กรอง `is_operation` และไม่รู้ว่าแถวไหนคือวัตถุดิบ** (DailyReport.jsx ~4769)
  ⇒ **แถว `dr_products` ที่เลิกใช้แล้วต้องปิด `is_active=false` ทันที ห้ามปล่อยไว้เฉยๆ** — ปล่อยไว้ = หน้างานเลือกได้ = เปิดใบผิดเลขซ้ำแน่นอน
  · เคสจริง LASER-345: เลขคอยล์ `50031601/02` ถูกย้ายใบผลิตออกไปเลข OP แล้วเมื่อ 21/08 แต่แถวยัง active ⇒ กลับไปเปิดใบบนเลขคอยล์อีก 58 ใบ
  จนต้องย้ายรอบสองวันที่ 15/09 (`20260915_laser345_orders_back_to_op.sql` · เรื่องเต็ม + บทเรียนอยู่ใน `docs/modules/oee.md` §ชั้น Operation)
