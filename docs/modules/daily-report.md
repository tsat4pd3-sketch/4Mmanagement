...
**แก้:** ดึง **ทุกสถานะ** จาก 5 กะล่าสุด → หา "ใบล่าสุดของแต่ละ prod_no" (กะใหม่สุดก่อน แล้วค่อย `opened_at`)
→ **เสนอเฉพาะเมื่อใบล่าสุดยังเป็น `carry_over`/`open`** (เป็น `confirmed`/`imported`/`cancelled` = งานจบไปแล้ว ห้ามปลุกซ้ำ)
· วัดกับข้อมูลจริง: ยอดค้างที่ควรเสนอยังได้ **2 ใบเท่าเดิม** (กติกาใหม่ไม่บล็อกเกิน) แต่เคส "กะหลังทำจบแล้ว" ถูกกันออก
