...
## 📋 ใบ A3 Report — export โปรเจคปรับปรุงเป็นใบเดียวจบ (PDCA / DMAIC · 2026-09-17 · คำสั่ง user)

ปุ่ม **📋 A3** บนการ์ดทุกใบ → modal เลือกกรอบ + กรอกส่วนที่ระบบไม่รู้ → **🖨 พิมพ์ A3 แนวนอน จบใน 1 หน้า**
(`src/lib/improvementA3Print.js` · กฎการเล่าเรื่อง = `src/utils/improvementA3.js` pure + เทส `improvementA3.test.mjs`)

- **ช่องบนใบ 8 ช่อง เหมือนกันทั้ง 2 กรอบ เปลี่ยนแค่ป้ายขั้น — ห้ามทำเป็นใบคนละชุด**
  `① ความเป็นมา ② สภาพปัจจุบัน ③ เป้าหมาย ④ สาเหตุราก | ⑤ มาตรการ ⑥ แผนงาน ⑦ ผลลัพธ์ ⑧ มาตรฐาน/ขยายผล`
  · PDCA = P,P,P,P | D,D,C,A · DMAIC = Define, Measure, Define, Analyze | Improve, Improve, Control, Control
- **ช่อง ②③⑥⑦ ระบบเติมจากข้อมูลจริงล้วน** (baseline/เป้า/milestone/ผลก่อน-หลัง/cost saving/คำขอ PE) —
  modal กรอกเฉพาะ `background · root_cause · countermeasures · standardize · team` (คนเท่านั้นที่รู้)
- **🔴 ตัวเลขบนใบต้องมาจากที่หน้าจอคำนวณแล้วเท่านั้น** (`computeResult`/`costSavingOf`/`targetSavingOf` ส่งเข้าไป)
  ห้ามคำนวณซ้ำในตัวพิมพ์ — มี 2 ชุดเมื่อไหร่ ใบพิมพ์กับจอจะเถียงกันเอง (หลักเดียวกับกฎ SCADA ห้ามคิด OEE เอง)
- **🔴 กฎ "ห้ามพูดเกินจริงบนกระดาษ" = `resultMode()` ตัวเดียวตัดสินทั้งใบ** (เทสล็อกไว้แล้ว)
  | mode | เงื่อนไข | ใบพูดว่าอะไร |
  |---|---|---|
  | `nodata` | ไม่มีกะที่ปิดแล้วในช่วงเทียบ | "วัดไม่ได้" — **ห้ามพิมพ์ 0** |
  | `baseline` | ยังไม่ยืนยัน `do_started_at` | ระดับปัจจุบัน + เป้าหมาย · เงิน = "มูลค่าปัญหาก่อนแก้ (เพดาน)" |
  | `waiting` | หลังแก้ < `MIN_AFTER_DAYS` วันผลิต | "รอผล n/5 วัน" · เงินยังเป็นเพดาน |
  | `confirmed` | ผ่านทั้ง 2 ด่าน | % ที่ลดได้จริง + เงินที่ประหยัดจริง |
  · **ไม่มีกะก่อนวันเริ่มแก้เลย (`beforeDays = 0`) = ขึ้นกล่องเตือนแทนแถบ 0** — 0 บนใบอ่านเหมือน "ไม่มีปัญหาแล้ว"
  · **`MIN_AFTER_DAYS` ย้ายไปนิยามที่ `src/utils/improvementA3.js` แล้ว** หน้า Improvements import ไปใช้ —
    ห้ามมีเลขนี้ 2 ที่ ไม่งั้นวันหนึ่งใบบอก "ประหยัดแล้ว" ขณะที่จอบอก "รอผล"
- **แผนงานเก็บ phase เป็น PDCA เสมอ** (`improvement_milestones.phase`) — โหมด DMAIC คือ **"การแสดงเทียบ"**
  ขั้น `plan` โชว์เป็น **DMA (Define–Measure–Analyze)** เพราะยุบเป็นขั้นเดียวไม่ได้และ**ห้ามเดา**
  · **ห้ามเขียนค่า define/measure/... ลง DB** (แผนงานจะมี 2 มาตรฐานปนกันในตารางเดียว → filter/สรุปพัง)
- เนื้อหาที่คนเขียนเก็บใน **`improvements.a3` jsonb** (migration `20260917_improvement_a3_dr.sql` · **apply แล้ว 2026-09-17**
  ผ่าน MCP) — jsonb ก้อนเดียวเพราะเป็นเนื้อความของใบพิมพ์ ไม่มีใคร query/รวมยอด (pattern เดียวกับ `vsm_maps.data.a3`)
  · ยังไม่ apply = **พิมพ์ใบได้ปกติ** แค่บันทึกข้อความไม่ได้ + toast บอกให้แจ้ง admin (fallback 42703 เหมือน target_mode)
- ทะเบียนเอกสาร: `doc_key = 'improvement_a3'` (migration `20260917_doc_form_improvement_a3.sql` · **apply แล้ว** ·
  `layout_locked=false` → เปลี่ยนแนว/ขนาดกระดาษจาก `/doc-forms` ได้) — เลขฟอร์ม/Rev/ช่องเซ็น/โลโก้อ่านจากทะเบียนล้วน
- สิทธิ์: **พิมพ์ได้ทุก role** (อ่านอย่างเดียวก็ต้องเอาใบเข้าห้องประชุมได้) · แก้เนื้อหา = `improvements:manage`
  (ช่องเป็น `readOnly` + `ReadOnlyNote` บอกเหตุผล ไม่ใช่ซ่อนเงียบ)
- รูปก่อน/หลังแปลงเป็น **data URL ก่อนพิมพ์** — ปล่อยเป็น URL แล้วบางครั้ง `print()` ทำงานก่อนรูปโหลดเสร็จ
  = ใบออกมาช่องรูปว่างโดยไม่มีใครรู้ · โหลดไม่ได้จริง = เขียน "โหลดรูปไม่สำเร็จ" ตามจริง
- รายการคำขอแก้เอกสาร PE บนใบตัดที่ 8 แถว + เขียนว่าเหลืออีกกี่รายการ (**ห้ามตัดเงียบ** · ยาวเกินจะดันทั้งใบให้ถูก zoom ย่อ)
- "จบใน 1 หน้า" ใช้ **`zoom`** ไม่ใช่ `transform: scale` (transform ไม่ลดกล่อง layout → เบราว์เซอร์ยังนับหลายหน้า)
- ตรวจแล้วด้วย harness จริง (`audit/crashsweep.mjs` 71 หน้า พัง 0 · `mobilesweep.mjs` 390px ปัญหา 0 หน้า)
  และเปิดใบจริงจากหน้าจอ: 8 กล่องครบ · ไม่มี `color-mix` (เพดาน Chromium 94 ของจอ TV)
