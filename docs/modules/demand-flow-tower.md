...
>
> - **สูตรอยู่ `src/utils/moveTargets.js` (pure · เทส 16 เคส) ห้ามเขียนตัวเสนอปลายทางเองในหน้า**
>   `moveTargets(mat, ctx)` คืน **กลุ่มพร้อมเหตุผล** — ⭐ ไลน์ขึ้นรูปในแผนกนี้ → ⭐ ไลน์ที่ผลิตพาร์ทนี้ → 🏭 ไลน์ขึ้นรูปแผนกอื่น → 📋 ตาม BOM (ขั้นถัดไป) → 📦 ไลน์ลูกอื่นในแผนกนี้ → **🏢 ไลน์อื่นทั้งโรงงาน**
> - **🔴 กลุ่มสุดท้าย 🏢 = ตะกร้ารับที่ห้ามถอด (2026-09-09 · feedback หน้างาน "มันขึ้นไลน์ไม่ครบนะ LINE C (200 Ton), (110&300 Ton) ก็ไม่ขึ้น")**
>   กลุ่มข้ามแผนกกลุ่มเดียวที่มีคือ `forming_out` ซึ่ง**กรองด้วย `isFormingLine(line_type)`**
>   ⇒ ไลน์ที่ **`line_type` ยังเป็น null** (`LINE C ( 200&250 Ton )` · `LINE D ( 110&300 Ton )` ของ PD1)
>   ตกทุกกลุ่ม = **หายจาก dropdown เลือกไม่ได้เลย** ทั้งที่เป็นปลายทางที่ถูก
>   **กฎที่ตกผลึก: master data ไม่ครบ ต้องทำให้ "จัดลำดับได้แย่ลง" ไม่ใช่ "ทำให้เลือกไม่ได้"**
>   — ทุกตัวกรองที่อ่าน master ต้องมีตะกร้ารับท้ายลิสต์เสมอ · ตั้ง `line_type` ที่ `/line-setup` แล้วไลน์นั้นเลื่อนขึ้นกลุ่มบนเอง
> - **ด่าน confirm ตอนกดย้ายเช็คแบบ "กลุ่มที่ยกเว้น" (`forming/forming_out/made/made_out`) ไม่ใช่ "กลุ่มที่ต้องเตือน"**
>   — กลุ่มใหม่ที่เพิ่มทีหลังจะตกมาเตือนเองโดยไม่ต้องไล่แก้ `StockMoveToChild`
...
### 🃏 audit การ์ดมอนิเตอร์สต๊อก → ของกลาง `<PartCard>` (2026-09-25 · คำสั่ง user)

*"หลายจอที่เป็นการ์ดมอนิเตอร์สตอคในระบบ มีหลายแบบมาก audit และปรับให้เป็นมาตรฐานที"*

**ผล audit:** การ์ดพาร์ทวาดเองทุกจอ · `borderRadius` ที่ใช้จริง **10 ค่า** (5/6/7/8/10/11/12/14/20/50%) ·
มีรูปชิ้นงานแค่ 1 จาก 7 จอ · เลข MAT บางจอวาดเปล่า บางจอเอาเหตุผลยาวมาต่อเป็นย่อหน้า

**แก้:** `src/components/PartCard.jsx` + `src/utils/usePartImages.js` (cache TTL 4 ชม.)
· กติกา/API เต็ม → `docs/UI-CONVENTIONS.md` §6.23

| จอ | สถานะ |
|---|---|
| `/heijunka` คิวสโตร์ทุกตู้ (2xx/3xx/5xx) + แท็บความต้องการ | ✅ แปลงแล้ว |
| `/store-monitor` เฝ้าระวังสต๊อก | ✅ แปลงแล้ว |
| `/rack-center` packaging | ✅ แปลงแล้ว (`showImg={false}` — ภาชนะไม่ใช่ชิ้นงาน) |
| `StoreWaitCards` (บอร์ด TV) | 🔵 ไม่แปลง — สรุป**รายไลน์** ไม่ใช่รายพาร์ท คนละหน่วยข้อมูล |
| `/customer-demand` (1xx) | 🔵 ไม่แปลง — การ์ดรอบส่งเป็น**ขั้นตอนเวลา** ไม่ใช่พาร์ท (user ระบุเองว่า workflow ต่างกันได้) |

🔴 **ช่องว่างที่ยังเหลือ:** `/store-monitor` การ์ดยัง**ไม่มีตัวเลขหลัก** เพราะวิว `v_store_abnormal`
คืนเฉพาะประโยค `detail` ไม่มีคอลัมน์ตัวเลข (on_hand / min / max) — ถ้าจะให้การ์ดโชว์
"คงเหลือ X / Min Y" แบบจออื่น ต้องเพิ่มคอลัมน์ในวิว (เติมท้าย = backward compatible)
**ยังไม่ทำ รอ user สั่ง** · ตอนนี้ใช้ `note` แสดงประโยคเดิมไปก่อน ไม่แกะสตริงเอง (เปราะ)

### 🖐 UX review จอสโตร์ — เป้ากด/รูป/ทางเดิน (2026-09-25 · คำสั่ง user "ด่วน")

*"ตอนนี้ไม่เหมาะกับการใช้งานหน้างานเลย รูปเล็ก การ์ดเล็ก จุดที่ user ต้อง interactive ด้วยก็ดูบาง หายาก"*

**3 อาการ · วัดแล้วทุกตัว (ตารางตัวเลขเต็ม → `docs/UI-CONVENTIONS.md` §6.24):**

1. **ทางเดินตัน** — ปุ่ม "☑ เลือกพาร์ทไปส่ง → Store Time Chart" ใน Store Board สลับแท็บให้เฉยๆ
   แต่แผงติ๊กพาร์ทอยู่ต่ำกว่าขอบจอ **2.5 จอ** (หน้าสูง 3,285px) ⇒ กดแล้วเหมือนไม่มีอะไรเกิดขึ้น
   → แก้ด้วย `focusPick` (ตัวนับ) → `scrollIntoView` + เรืองกรอบ 2.6 วิ
2. **ปุ่มลงมือเล็กและจาง** — เล็กกว่า 40px แทบทุกตัว (`จ่าย` = 39×26 ทั้งที่ตัดของออกจากคลังจริง)
   และพื้นเป็น alpha .10 บนพื้นเข้ม = คอนทราสต์ต่ำจนดูเป็นข้อความ
   → `storeBtn()` ของกลางใน `src/utils/storeUi.js` · **ทุกปุ่มลงมือในโมดูลนี้ต้องผ่านตัวนี้**
3. **รูปเล็ก** — 56px มองจากระยะยืนไม่ออกว่าชิ้นไหน → 84px + การ์ดกว้าง 330px

🔴 **ที่ยังไม่ได้แก้ (ตั้งใจ · รอ user ตัดสิน):** chrome ของหน้า (แท็บ 38px · ช่องวันที่ 34px ·
ปุ่มกะ 32px) มาจาก token `--ctl-h: 34px` **มาตรฐานทั้งแอป** — ขยายเฉพาะโมดูลสโตร์ = หน้าตาแตกจากหน้าอื่น
ต้องตัดสินใจระดับระบบว่าจะยกทั้งแอปไหม

### 🧭 แท็บของ `/heijunka` + รูปชิ้นงาน (2026-09-25 · คำสั่ง user)

*"หน้านี้คือแดชบอร์ดที่สโตร์จะเปิดมอนิเตอร์ ความต้องการของไลน์ผลิตใช่มั้ย ควรจะมีรูปภาพชิ้นงานด้วยนะ"*
*"สับสนการใช้งาน แต่ละ tab มากๆ"*

**อาการเดิม 4 อย่าง (วัดจากจอจริงที่ user ส่งมา):**

| # | อาการ | ทำไมสับสน |
|---|---|---|
| 1 | แถบปุ่ม 7 ปุ่มวาดเอง **กลางหน้า ใต้แถบสรุป 3 ชั้น** | ผิดลำดับ UI-STANDARD (ชื่อหน้า → แท็บ → แถบกรอง → เนื้อหา) · คนเลื่อนลงมาเจอเนื้อหาก่อนเจอปุ่ม เลยนึกว่าหน้านี้มีมุมมองเดียว (อาการเดียวกับที่ user เคยทักเรื่อง `StockCountSheet`) |
| 2 | ไม่ผูก URL (`useState`) | refresh / แชร์ลิงก์ / กด Back → เด้งกลับแท็บแรกเสมอ |
| 3 | "🎴 การ์ด" กับ "📋 ตาราง" เป็น 2 แท็บ | **ข้อมูลชุดเดียวกัน คนละหน้าตา** — ป้ายแท็บไม่ได้บอกเลยว่าต่างกันตรงไหน |
| 4 | ป้ายแท็บสั้นๆ ไม่มีคำอธิบาย | ไม่รู้ว่าแท็บไหน "กดทำงานได้" แท็บไหน "ดูอย่างเดียว" |

**แก้แล้ว:**
- 🔴 **แท็บย้ายขึ้น `PageHeader`** (`tabs=`/`tab=`/`onTab=`) — ได้ breadcrumb + ตัวชี้ไถล + overflow เป็น dropdown ฟรี **ห้ามวาดแถบแท็บเองกลับมาอีก** (UI §6.8)
- 🔴 **ผูก `?view=`** ผ่าน `useTabParam(keys, 'unified', 'view')` — **ไม่ใช่ `?tab=`** เผื่อหน้านี้ถูกฝังใต้หน้าแม่ที่ถือ `?tab=` (UI §6.8 ข้อ 2.4)
- **ยุบ `cards` + `table` → แท็บเดียว `demand`** ("📋 ความต้องการวันนี้") สลับหน้าตาด้วย `<Segmented>` ข้างใน ⇒ เหลือ 6 แท็บ (5 เมื่อไม่มีรอบ) ตามเพดาน §6.8 ข้อ 3
- **`VIEW_META` = ทะเบียนมุมมองที่เดียว** — `label` · `act` (กดทำงานได้/ดูอย่างเดียว) · `purpose` ("แท็บนี้ตอบคำถามอะไร") วาดเป็นบรรทัดเดียวใต้แท็บเสมอ **ห้ามปล่อยให้ป้ายแท็บอธิบายตัวเอง**
- **badge งานค้างบนแท็บ** `unified` / `pull` — สโตร์รู้ว่าต้องเข้าแท็บไหนก่อนโดยไม่ต้องกดไล่ (นับเฉพาะใบที่ยังไม่จบ ให้ตรงกฎเหล็ก 1 ข้างล่าง)

**🖼️ รูปชิ้นงาน** — สโตร์หยิบของจากหน้าตาชิ้นงาน ไม่ได้หยิบจากเลข mat
- ของกลาง `src/utils/partImages.js` (มีเทส 8 เคส) + `src/components/PartThumb.jsx`
- รูปอยู่ **2 ทะเบียนฝั่ง DR**: `parts_master.image_url` (พาร์ทย่อย/ของซื้อ) · `dr_products.image_url` (พาร์ทแม่) — **`parts_master` ชนะ** เมื่อมีทั้งคู่ · ลำดับนี้อยู่ที่เดียว ห้ามให้แต่ละหน้าเลือกเอง
- โชว์แล้วที่: การ์ดคิวทุกตู้ (`QueueCard`) · แท็บ 📋 ความต้องการวันนี้ (ทั้งการ์ดและตาราง)
- 🔴 **ไม่มีรูป = กล่อง 🖼️ + tooltip บอกว่าไปเพิ่มที่ไหน ห้ามเว้นว่างเงียบ** — วัดจริง 25/09: พาร์ทย่อยที่ขึ้นบอร์ด **155 ตัว มีรูป 52 ตัว (34%)** ถ้าไม่บอก คนจะคิดว่าจอพัง · รูปโหลดไม่ขึ้น (ไฟล์ถูกลบ) ตกเป็นกล่อง ⚠️ ไม่ใช่ไอคอนรูปแตกของเบราว์เซอร์
- โหลดทะเบียนรูป **ครั้งเดียวตอนเปิดหน้า** (2 ตาราง × 2 คอลัมน์ · หลักร้อยแถว) ไม่ผูกกับวัน/ไลน์ — ถูกกว่า join ในคิวรีบอร์ดทุกครั้งที่เปลี่ยนวัน (กฎเหล็ก DB ข้อ 11)
...
## 📋 ตรวจนับ/เฟิร์มยอด + กระทบยอดขาออกที่หลุด — `/line-stock?tab=count` (2026-09-23)

**ที่มา:** feedback หน้างานจากแพลนนิ่ง (AJCHARA) หลังอัปไฟล์ Monitoring สำเร็จ —
*"Stock บางตัวไม่ตรง"* + *"ให้ PD คีย์ผลิต แล้วเฟิร์มยอดให้เลยได้มั้ยคะ"*

### สิ่งที่ไล่ ledger แล้วเจอจริง (23/09 · ตัวเลขวัดจากฐาน ไม่ใช่ประมาณ)

| ขา | สถานะ | หลักฐาน |
|---|---|---|
| **เข้า** (PD คีย์ผลิต → ยอดเข้าคลัง) | ✅ ทำงานอยู่แล้ว | trigger `fn_post_confirmed_output` เขียน `issue` ให้เอง **11,971 แถว / 376,432 ชิ้น** · กฎรับเข้าครอบคลุมพาร์ทในไฟล์ Monitoring **121/121 ตัว** |
| **ออก** (กด "ส่งแล้ว" → หักคลัง) | ⚠️ **หลุด 17%** | ส่งไป **914 ใบ / 72,053 ชิ้น** · หักจริง **759 ใบ** ⇒ **155 ใบ / 22,781 ชิ้น ไม่ถูกหัก** |
| **ตรวจนับ** | ❌ ไม่มีเครื่องมือ | 🔧 ปรับยอด เดิมทำได้ทีละแถว และต้อง**คิดผลต่างเอง** (กรอก "เพิ่ม/ลดกี่ชิ้น" ไม่ใช่ "นับได้กี่ชิ้น") |

> 🔍 **ขาออกไม่ได้พังเพราะไม่มีโค้ด** — `CustomerDemand.advance()` หักให้อยู่แล้ว ผูก `ref_shipment_id`
> และ**เตือนถูกต้อง**เมื่อหักไม่ได้ (`⚠️ ส่งแล้ว — แต่ไม่ได้หักสต็อก`) · ปัญหาคือ **toast ดับไปพร้อมจอ**
> ยอดค้างสูงกว่าจริงตลอดไปโดยไม่มีที่ไหนคอยตามต่อ
> ⇒ บทเรียนข้ามโมดูล: **การเตือน ณ วินาทีนั้นไม่ใช่การปิดงาน — ของที่ทำไม่สำเร็จต้องมี "รายการค้าง" ถาวร**

### แท็บใหม่ = 2 แผง (คนละสาเหตุ ห้ามยุบรวม)

- **🚚 ส่งแล้วแต่ยังไม่ถูกหัก** — รายการค้างถาวรแทน toast ที่หายไป · ปุ่มหักย้อนหลังเขียน `consume`
  ผูก `ref_shipment_id` (idempotent) · แยกกอง **จับคู่เลข SAP ไม่ได้** ออกมาต่างหาก — ระบบ**ไม่เดา**
  ต้องไปแก้ `p_no` ที่ Product Master ก่อน (กฎเดิมของ `matResolve.js`)
- **📋 ตรวจนับ/เฟิร์มยอด** — กรอก **ยอดที่นับได้** ทั้งคลังในตารางเดียว ระบบคิดผลต่างเอง แล้วบันทึก
  เป็น `adjust` ทีเดียว **ผ่านคิวอนุมัติเดิม** (`REVIEW_TYPES`) · คนที่มี `line_stock:approve` ติ๊ก "อนุมัติเลย" ได้

### 🔴 กฎเหล็ก — ห้ามหักย้อนหลังข้ามรอบตรวจนับ

ยอดที่ได้จากการตรวจนับ = ของจริงบนชั้น ณ วันนั้น ⇒ ใบที่ส่งไป **ก่อน** รอบนับถูกสะท้อนใน
ยอดที่นับได้ไปแล้ว **การหักซ้ำ = ยอดหายสองเท่าเงียบๆ**

> **เกือบพลาดจริง:** 23/09 อัปไฟล์ Monitoring ตั้งยอดตามที่แพลนนิ่งนับมา 88 พาร์ท (12:38 น.)
> แล้วเกือบ backfill ใบ shipped ย้อนไปถึง 17/08 ทับลงไปอีก — จะทำให้ FG หายไปอีก 72,053 ชิ้นโดยไม่มีใครรู้
> **จึงตัดสินใจไม่ backfill ใน migration เลย** ให้ผ่านแผงนี้ซึ่งรู้เส้นรอบนับเป็นราย (คลัง, mat)

`pendingShipCuts()` จึงแบ่งใบเป็น 3 กอง: `open` (หักได้) · `closedByCount` (ปิดด้วยการนับแล้ว **ห้ามแตะ**) ·
`unresolved` (จับคู่เลขไม่ได้)

### 🔴 ลำดับแผงบนหน้า — worklist ต้องพับ ห้ามดันงานหลักตกจอ (24/09 · feedback user)

รอบแรกวาง 🚚 worklist ไว้**บนสุดและกางทิ้งไว้** — มันยาว 162 แถว ⇒ ผู้ใช้เลื่อนลงไม่ถึง
📋 ตรวจนับ *"ข้อมูลโชว์เยอะมาก จนดันฟีเจอร์ตรวจนับลงไปล่าง แบบนึกว่ามีแต่ตาราง"*

**กฎที่ใช้กับทุกหน้าที่มี "งานหลัก + รายการค้าง" อยู่ด้วยกัน:**
1. **งานหลักขึ้นก่อนเสมอ** · รายการค้าง/ประวัติอยู่ล่าง
2. **รายการที่ยาวได้ไม่จำกัด = พับเป็นค่าเริ่มต้น** เห็นแค่หัวข้อ + ตัวนับ + ปุ่มลงมือ
3. กางแล้วต้องอยู่ใน**กล่องเลื่อนที่จำกัดความสูง** (`maxHeight` + `overflowY:auto` + หัวตาราง `sticky`)
   — ห้ามปล่อยให้ตารางยืดหน้าเพจ
4. **ปุ่มลงมือต้องอยู่หัวแผง ไม่ใช่ท้ายตาราง** (ท้ายตาราง = ต้องเลื่อนผ่าน 162 แถวก่อนถึงปุ่ม)

### ไฟล์

| ไฟล์ | หน้าที่ |
|---|---|
| `src/utils/stockCount.js` | สูตรล้วน — `countDelta` · `toAdjustTxns` · `countSummary` · `lastCountAt` · `pendingShipCuts` (**เทส 20 เคสใน `__tests__/stockCount.test.mjs`** — อยู่ในด่าน `npm run build`) |
| `src/components/StockCountSheet.jsx` | 2 แผงข้างบน (แท็บ `count` ใน `/line-stock`) |

**กติกาที่ล็อกไว้ด้วยเทส:** ช่องว่าง = ยังไม่นับ (≠ 0) · กรอก 0 = นับแล้วไม่มีของ · `adjust` ลดเก็บ `qty` ติดลบ ·
`lastCountAt` นับเฉพาะ `adjust` ที่ `approved` (ใบที่ยังรออนุมัติยังไม่มีผลกับยอด จึงปิดใบส่งไม่ได้) ·
คลังคนละตัวไม่ปิดใบให้กัน · ไม่มี `shipped_at` ถอยไปใช้ `due_date` สิ้นวัน

### สิทธิ์ (ณ 23/09 — ไม่ได้เพิ่ม key ใหม่ ใช้ของเดิมทั้งหมด)

`page:/line-stock` = admin · dept_admin · display · document_control · leader · manager · **planner_store** · qa · supervisor
· `line_stock:issue` (กรอก/หักย้อนหลัง) = admin · dept_admin · leader · manager · **planner_store** · supervisor
· `line_stock:approve` = admin · dept_admin · manager
⇒ **แพลนนิ่ง (`planner_store`) เข้าและกรอกได้เอง แต่ต้องให้สโตร์/หัวหน้าอนุมัติ** — เป็นการคุมที่ตั้งใจ ไม่ใช่บั๊ก

### ยังไม่ได้ทำ (รู้ตัว)

- ไม่มีปุ่ม import ผลตรวจนับจาก Excel (ตอนนี้กรอกในตาราง) — รอดูว่าใช้จริงแล้วช้าไหมก่อน
- ไม่มีการเตือนเชิงรุกเมื่อ "หักไม่ได้" ซ้ำหลายรอบต่อพาร์ทเดิม (แผงบอกได้อยู่แล้วว่าค้างกี่ใบ)
