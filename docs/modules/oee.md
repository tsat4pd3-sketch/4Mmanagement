...
2. **MAT ไม่มี CT → ยอดหายจาก %P เงียบ** — `computeOEE` โยนเข้า `unknownQty` แล้วไม่นับใน `Σ(qty×CT)`
   ...
   **⚠️ ควรมีด่านเตือนตอนปิดกะแบบเดียวกับ `pOver` — ตอนนี้เงียบสนิท**
3. **จับกลุ่ม product ด้วย "ชื่อ" ไม่ทนกับการสะกดต่าง** — พาร์ทเดียวกันคนละลูกค้าสะกดคนละแบบ
   ...
   **เจตนาของกฎถูกแล้ว แต่คีย์ผิด** — ทางเลือกที่ทนกว่า: `p_no` ที่ normalize แล้ว หรือ `family_id`
   · **ยังไม่แก้ — เปลี่ยนแล้วกระทบ %P ทุกไลน์ที่มีหลาย MAT = ต้องให้ user ตัดสินใจก่อน ห้าม auto-merge**
