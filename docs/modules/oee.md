...
> ### 🟡 known gap — %A **สด** กับ %A **ตอนปิดกะ** เป็นคนละสูตร (audit 2026-09-14 · ยังไม่แก้ รอ user สั่ง)
> | | ตัวตั้ง | ตัวหาร | หักเวลาพัก | หัก DT วางแผนออกจากตัวหาร |
> |---|---|---|---|---|
> | **`computeOEE` (ปิดกะ · DailyReport)** | netAvail − DT ไม่วางแผน | `netAvail = shift_min − DT วางแผน − เวลาพัก` | ✅ | ✅ |
> | **`computeLiveOee` (สด)** | elapsed − **DT ทุกชนิด** | elapsed | ❌ | ❌ |
>
> ⇒ กะเดียวกัน %A **กระโดดขึ้นตอนปิดกะ** ทั้งที่ข้อมูลเท่าเดิม (จอ TV/FactoryMap/Dashboard เห็นเลขต่ำกว่ารายงาน)
> เป็นของเดิมมาตั้งแต่แยกไฟล์ ไม่ใช่ของใหม่ · **แก้เมื่อไหร่ = แตะทุกจอที่โชว์ OEE สด** ต้องเทียบเลขก่อน-หลังรายกะ + `node audit/crashsweep.mjs` ก่อน merge
> (แท็บ ⚙️ รายอุปกรณ์ ใน `/mtn-repair` แก้ให้ตรงนิยาม "ปิดกะ" ไปแล้ว 2026-09-14 — หักเวลาพัก + มีโหมดถ่วง 1/N · ดู `docs/modules/mtn-work-order.md`)
