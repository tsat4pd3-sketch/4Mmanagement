...
> - **🔴🔴 เคสเดียวกัน "กลับมาเป็นซ้ำ" · ปิดวงจรแล้ว 2026-09-15 (user ทัก "ข้อมูลที่เคยลงผิดบนเบอร์ 500x ย้ายมาใส่ OP ได้มั้ย")**
>   migration 21/08 ย้ายใบผลิตไปเลข OP ใหม่ครบก็จริง แต่ **ปล่อยแถว `dr_products` ของ `50031601`/`50031602` ไว้ `is_active=true`**
>   (ตอนนั้นตั้งใจ "ไม่ลบ" เพราะกลัว p_no/pair หาย — แต่ลืมว่า *ไม่ลบ* กับ *ยังเลือกได้* คนละเรื่อง)
>   ⇒ dropdown "✍️ เปิดเป้าผลิต (ไม่มีบาร์โค้ด)" ใน `/daily-report` (`DailyReport.jsx` ~4769) โชว์**ทุกสินค้าของ family ไลน์
>   ไม่กรอง `is_operation`** ⇒ เลขคอยล์ยังยืนคู่เลข OP ในลิสต์ → 21/08 กะดึกเป็นต้นมาหน้างานกลับไปเลือกเลขคอยล์อีก
>   **29+29 ใบ · 26,383 ชิ้น ถึง 15/09** (เลข OP จริงหยุดรับใบตั้งแต่ 24/08) พร้อม downtime 38 แถว + ใบ scrap 2 แถวที่เกาะเลขผิดไปด้วย
>   **แก้รอบนี้** (`20260915_laser345_orders_back_to_op.sql` · apply แล้ว · snapshot 4 ตาราง `*_bak_laser345_20260915` ย้อนได้):
>   ย้าย prod_orders/downtime(เฉพาะกะ LASER-345)/scrap ไป `90031603`(RH)/`90031604`(LH) · ยก `p_no`/`customer`/`target_per_shift`/รูป
>   ที่คนกรอกไว้บนแถวคอยล์มาเติมให้แถว OP (ซึ่งถูกสร้างแบบมีแต่โครง) · แล้ว **ปิด `is_active=false` + ล้าง `pair_mat_no` ของ 50031601/02**
>   · ตรวจแล้วไม่มียอดซ้อน (ไม่มีกะไหนเปิด 2 เลขพร้อมกัน) · **ไม่แตะตัวตนคอยล์เลย** — parts_master · BOM 11 สูตร ·
>   purchase_requests 1,904 · kanban_standards (min 3000/max 4500) · line_stock P409/P411 อยู่ครบเหมือนเดิม
>   **บทเรียนที่ต้องจำ: ล้างข้อมูลผิดอย่างเดียวไม่พอ ต้องปิด "ประตูที่ทำให้ผิดซ้ำ" ในคอมมิทเดียวกัน**
>   — แถว master ที่เลิกใช้แล้วแต่ยัง `is_active=true` = คนหน้างานเลือกได้ = กลับมาใหม่ภายในไม่กี่วันเสมอ
>   · ⚠️ เหลือ 1 แถว downtime ของ `50031602` (HYDROFORM 01/07 · 390 นาที) จงใจไม่ย้าย — เกิดก่อนแถวคอยล์ถูกสร้าง 17/08 ที่มาไม่ชัด ห้ามเดา
...
>   1. ✅ **แก้แล้ว 2026-09-15** (`20260915_laser345_orders_back_to_op.sql` + `20260915_op_parent_by_side_clear_fake_mat.sql`) — ย้ายใบ 58 ใบไป 90031603/04 · ปิด `is_active` ของ `50031601` (คอยล์จริง ห้ามลบ) · **ลบแถว `50031602` ทิ้ง** (เลขปลอม ไม่มีในทะเบียนไหนเลย — user สั่ง "เคลียเลย") · ดูบล็อก 🔴🔴 "กลับมาเป็นซ้ำ" ด้านบน · ปัญหาเดิมที่บันทึกไว้ 04/09: 🔴 **`50031601`/`50031602` ยังถูกเปิดใบทุกวันที่ LASER-345** — หลัง migration 21/08: 13+13 ใบ confirmed 12,097 ชิ้น + เปิดค้าง 2 ใบ (04/09) · ทุกใบ `prod_no = MANUAL-…` เปิดเป็นคู่จาก dropdown "เปิดใบ manual" · **ต้นเหตุ: migration ปลดธง OP แต่คงแถว `dr_products` ไว้ `is_active=true` ที่ `line_name='LASER-345'` + pair กันเอง** → ยังโผล่ในรายการสินค้าของไลน์ · ตอนนี้ `is_op=false` = นับเป็นพาร์ทจริง ซ้ำกับ 90031603/04 ที่ขั้นเดียวกัน (LASER-345 มี 6 เลขปนกัน) · ไม่มีสต็อกผี (prefix 5 ไม่มี inflow rule) · **ทางแก้ = ปิด `is_active` 2 แถวนี้ + ย้ายใบไป 90031603/04 เหมือน 21/08** — BOM picker อ่าน `parts_master` ไม่ใช่ dr_products → 9 สูตร BOM ไม่กระทบ
>   2. ✅ **ฝั่งข้างแก้แล้ว 2026-09-15 · ส่วน "1 OP หลายปลายทาง" ยังเป็นข้อจำกัดที่รับไว้** — user เคาะจากจอ /products ว่า
>      *"parent มันเป็น OP ที่ผลิตงานคู่ซ้ายขวา แล้วไปแยกออกเป็นตัวงานที่มีเลข mat สองอันนี้"* (Line 60 = RH · Line 61 = LH)
>      ⇒ **RH (`90031601`/`90031603`) → `10100385` · LH (`90031602`/`90031604`) → `10100401`** (AAT ทั้งคู่ = ใบเยอะสุดทั้งสองข้าง 2,175/2,180 + สมมาตร RH/LH)
>      · ตรวจ BOM 15/09: ปลายทางของสายนี้มี **8 ตัว** กิน coil `50031601` ตรงๆ — Line 60 RH ×3 ลูกค้า · Line 61 LH ×3 · LASER-789 ก่อนแพ็ค `20065715`/`20065635`
>      · **`op_parent_mat` เก็บได้ค่าเดียว ⇒ ตั้งได้แค่ "ตัวแทน 1 ตัวต่อข้าง"** — ของจริงที่แตกเป็นหลายปลายทางต้องรอ routing master (ดูข้อ 4)
>      · งานฝั่ง export (HDF2/LS789) เปิดใบที่ `20065715`/`20065635` ตรงอยู่แล้ว = ทางเลือก (ข) ของที่เคยเสนอไว้ ไม่ต้องแตกเลข OP เพิ่ม
>      · ปัญหาเดิมที่บันทึกไว้ 04/09: 🔴 **OP ตัวเดียววิ่ง 2 สาย ปลายทางคนละตัว แต่ `op_parent_mat` มีค่าเดียว** — 90031601 @HDF2 ควรชี้ 20065715 แต่ชี้ 10100385 (สาย HDF1) · 90031602 (LH) ชี้ 20065715 = **RH ผิดข้าง** (ควร 10100401 @HDF1 / 20065635 @HDF2) · 90031604 (LH @LS345) ก็ชี้ 20065715 ผิดข้าง · ทางเลือก: **(ก)** ต้องมอนิเตอร์ HDF2 แยก → แตก `90031605` RH@HDF2 / `90031606` LH@HDF2 ชี้ 20065715/20065635 · **(ข)** ไม่ต้อง → HDF2 เปิดใบที่ 20065715/20065635 ตรง (เคยทำจริง 05–10/08 = 25 ใบ) แล้วเลิกใช้ OP ฝั่ง HDF2 · **ทางไหนก็ตาม 90031602/90031604 ต้องย้ายไปฝั่ง LH**
...
>   4. 🟡 1 OP ป้อนหลาย FG (90031601 → Line 60 มี FG 3 ตัว AAT/FTM/FVL) แต่ parent ผูกได้ตัวเดียว (ธรรมเนียม = ตัวที่ใบเยอะสุด 10100385) — **ยังไม่ต้องแก้ · ยืนยันซ้ำ 2026-09-15**
>      **ถ้าวันหน้าจะทำ "1 OP → หลายพาร์ทจริง" ให้ถูกต้องจริงๆ ห้ามยัดเพิ่มในคอลัมน์ `op_parent_mat`** (ค่าเดียว = ได้แค่ตัวแทน)
>      ทางที่ถูกคือตารางเส้นทาง — `part_routings` (DR) มีโครงครบแล้ว (mat_no · seq · step_name · line_name · machine_no · ct_sec · wip_label)
>      แต่ **ยังไม่ถูกใช้จริง (1 แถวทั้งตาราง) และ `collapseOps` ยังไม่อ่านมัน** ⇒ เป็นงานเฟสถัดไป ไม่ใช่แก้รายเคส
