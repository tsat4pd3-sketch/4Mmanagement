...
3. `QUARTERLY & <เดือน> : <ไลน์>` — OEE **รายไตรมาส Q1–Q4 + ทั้งปี + เป้า** · โดนัท A/P/Q เดือนนี้ · ยอดผลิตรายวัน (stacked กะ)
...
**2 สไลด์ต่อส่วนงาน:**
- `TAG YELLOW` — `quality_bin_records` (DR) ทั้งปี แยก 4 สถานะ: **รอพิจารณา (qty) · ซ่อมเสร็จภายในวัน (`repair_date` = `work_date`) · ซ่อมย้อนหลัง (`repair_date` > `work_date`) · ค้างซ่อม (ไม่มี `repair_date`)** + ยอดถังแดง · แท่งรายเดือน + รายไตรมาส
...
- **OEE รายไตรมาสถ่วงน้ำหนักด้วยเวลารับภาระ** ห้าม mean-of-percentages — `weightedOeeOf()` ใน `src/utils/oee.js`
- **สูตรใหม่ทั้งหมดอยู่ใน `src/utils/oee.js`** (`DEFAULT_OEE_TARGET` · `targetOeeOf` · `normOeeTarget` · `weightedOeeOf` · `quarterOfMonthKey`) — กฎโปรเจค: **util OEE มีไฟล์เดียว ห้ามแตกเพิ่ม** · เทส `src/utils/__tests__/oeeTarget.test.mjs` 6 เคส
- **เดือนที่ไม่มีกะปิด = ช่องว่างในกราฟ ไม่ใช่ 0** · ไตรมาสที่ยังไม่ถึง = ว่าง ไม่ใช่ 0
...
- 🟡 **แท่ง "ทั้งปี YEAR" จริงๆ คือ YTD** — เด็คเดือน มี.ค. โชว์ "ทั้งปี 2026" ที่แท้จริงคือ Q1 → เขียน `YTD ม.ค.-<เดือน>` เว้นแต่เดือนรายงาน = ธ.ค.
