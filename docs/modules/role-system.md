...
- **⚠️ กฎเหล็ก — เพิ่มหน้า/สิทธิ์ใหม่ ต้องลง "ทะเบียน" ด้วย ไม่งั้น admin ปรับจาก UI ไม่ได้เลย (audit 2026-08-06):** `role_permissions` เก็บ**ค่า** แต่ตัวที่ทำให้ "โผล่ให้ติ๊ก" ในหน้า `/permissions` เป็นคนละที่ — seed แถวอย่างเดียวไม่พอ:
  ...
  - **action ใหม่** → insert แถวใน **`permission_catalog`** ด้วย · ไม่ insert = ไม่ขึ้นในตาราง (เคยตกหล่น 8 key: `lpa:*` 3 · `ojt:*` 2 · `doc_forms:manage` · `pm_coord:manage` · `transport:manage` — แก้แล้ว migration `20260806_permission_catalog_gaps.sql`)
    ...
    - ⚠️ **`group_name` ต้องเป็นชื่อหมวดตาม `NAV_GROUP_ORDER` เท่านั้น + `sort` ตามช่วงของหมวด** (**ช่วงต่อหมวดอยู่ในตาราง `nav_groups` — `select name, sort_lo, sort_hi from nav_groups order by seq`** · ณ 2026-09-07: ภาพรวม 100-149 · จอแสดงผล 150-199 · ฝ่ายผลิต 2xx · วิเคราะห์ & รายงาน 3xx · พนักงาน & ทักษะ 4xx · Logistic ขาเข้า 500-519 / ขาออก 520-539 / แผนงาน 540-599 · ซ่อมบำรุง 6xx · คุณภาพ & วิศวกรรม 7xx · ตั้งค่า 900-949 · ผู้บริหาร & เดโม 950-999 · 8xx ว่าง — เลือกเลขที่ยังว่าง ห้ามซ้ำ · เช็คหลัง seed: `select * from v_permission_catalog_sort_drift` ต้องว่าง) · **ตั้งแต่ 2026-09-07 `permission_catalog.group_name` มี FK → `nav_groups(name)`** (migration `20260907_nav_groups_registry.sql` **apply แล้ว**) — seed ด้วยชื่อหมวดที่ไม่มีในทะเบียน = migration **error ทันที** ไม่ใช่ warning · **เปลี่ยน/เพิ่มหมวดเมนู = แก้ `NAV_GROUP_ORDER` + เขียน migration ใหม่ชื่อ `*_nav_groups_*.sql` (upsert ทั้งชุด) ในคอมมิทเดียวกัน** — เทส `src/utils/__tests__/navGroupsRegistry.test.mjs` อ่านไฟล์ตัวล่าสุดเทียบ App.jsx ไม่ตรง = build ไม่ผ่าน · เปลี่ยนชื่อหมวด: update `nav_groups.name` แถวเดียว catalog ตามเอง (on update cascade) — พิมพ์ชื่อหมวดเอง = หมวดกำพร้าโผล่กลางตาราง (audit 2026-08-19 เจอ 'ซ่อมบำรุง'/'ประชุมแถวเช้า' เป็นหมวดเดี่ยว + sort ชนกัน 8 คู่ → จัดใหม่ทั้งตารางด้วย migration `20260819_permission_catalog_regroup.sql` **apply แล้ว 2026-08-19** — cosmetic ล้วน ไม่แตะ role_permissions · ผลตรวจ: 9 หมวด 89 แถว ตรงเป๊ะ) · **⚠️ แล้วยังหลุดอีกรอบ:** `20260818_org_divisions.sql` seed `org:manage_divisions` ด้วย `group_name='ตั้งค่า/ฐานข้อมูล'` (ชื่อผิด) แต่ถูก apply **หลัง** audit รอบนั้น จึงตกสำรวจ → เก็บด้วย `20260824_permission_catalog_org_divisions_group.sql` (**apply แล้ว 2026-08-24** · ผลตรวจ: 9 หมวด 94 แถว · หมวดตั้งค่าฯ sort ถึง 940) — **บทเรียน: migration ที่ seed catalog แล้ว apply ทีหลัง ต้องตรวจ group_name ซ้ำเสมอ audit รอบเก่าไม่ครอบให้** · **`PAGE_GROUPS` ใน PermissionsManagement.jsx ต้อง mirror `NAV_ITEMS` เสมอ** (หมวด/ลำดับ/ชื่อ — เพิ่มหน้าใหม่แก้ 2 ที่ให้ตรงกัน) · QC checklist ข้อ C6
...
|---|---|---|
| `shopfloor` (default) | พนักงานหน้างาน + **ช่างทุกทีม** (ช่างเช็คชื่อครบ 25/25 คน) | ✅ |
| `support` | QA · PE/วิศวกรรม · ธุรการ · สโตร์ · เซลล์ | ❌ |

> **📛 เปลี่ยนชื่อค่า 23/09** (`direct`→`shopfloor` · `indirect`→`support`) เพื่อเลิกชนกับ
> `org_nodes.labor_type` ที่ใช้คำเดียวกันคนละความหมาย — ดูตารางเทียบ 2 แกนท้ายหัวข้อนี้
> · ความหมายเดิมทุกประการ · migration `20260923_staff_kind_shopfloor_support.sql`
> (**apply แล้ว 23/09** — 223 shopfloor + 2 support · **ไม่มีตัวเลขไหนขยับ**)
> · **check constraint ยังรับค่าเก่าไว้โดยตั้งใจ** (ช่วง deploy มีแท็บเปิดค้างด้วย JS ตัวเก่า
> ถ้าบังคับค่าใหม่ทันที การเพิ่มพนักงานจากแท็บนั้นจะล้ม 23514) — ตัวอ่านใน `staffKind.js`
> เข้าใจทั้ง 2 ชุด (`SUPPORT_VALUES`) และมีเทสตรึงไว้
> · ชื่อฟังก์ชันใหม่: `onlyShopfloorStaff` / `isShopfloorStaff` / `shopfloorOnly`
> (ชื่อเดิม `onlyDirectStaff` ฯลฯ ยัง export ไว้เป็น alias — **ห้ามใช้ในโค้ดใหม่**)

- **🔴 จอที่ "นับคน" ต้องกรองผ่าน `onlyShopfloorStaff()` (`src/utils/staffKind.js`) เท่านั้น
...
#### 🔴 ทะเบียน ≠ จอที่นับคน — `/operator` ต้องเห็น**ทุกคน** (23/09 · บั๊กของผมเอง)

ผมเผลอใส่ `onlyDirectStaff()` ที่คิวรีหลักของ `/operator` ⇒ พนักงานสายสนับสนุน 2 คนที่เพิ่งสร้าง
จาก `/add-user` (เจนนิภา · สุทธวีร์) **หายจากทะเบียนทั้งคู่ ⇒ เปิดแก้ข้อมูลเขาไม่ได้เลย**
— *"มีอยู่ในฐานแต่มองไม่เห็น" แย่กว่า "โผล่มาแล้วกรองทิ้ง"*

- **ทะเบียน/จอแก้ข้อมูล = แสดงทุกคน** แล้วให้ชิป 🧑‍🏭 หน้างาน / 🗂️ สนับสนุน กรองดูเอง
- **การกันไม่ให้ปนใน "กำลังคน" ทำที่จอที่นับคนเท่านั้น** (Checkin · Report · ShiftOrganize ·
  WorkforceInsight) — ลิสต์นี้อยู่ใน `staffKindGuard.test.mjs` พร้อมเหตุผลที่ `/operator` **ไม่อยู่ในลิสต์**
- **🔎 ช่องค้นหาในทะเบียน (feedback: "พนักงานหลักร้อย เลื่อนหาแย่เลย")** — ค้น ชื่อ/รหัส/ตำแหน่ง
  ผ่าน `normSearch` ของกลาง (`SearchSelect.jsx`) ⇒ ทนสะกดไทยเหมือน picker ทุกตัว
  · ค้นไม่เจอต้อง**บอกว่าไม่เจอ + บอกทางออก** ห้ามโชว์ลิสต์ว่างเฉยๆ (บทเรียนเดียวกับ `/add-user`)
- **กฎทั่วไป:** จอไหนที่ "แก้ข้อมูลของคน/ของสิ่งนั้น" ได้ ห้ามซ่อนแถวด้วยตัวกรองที่ปิดไม่ได้

#### ⚠️ มี direct/indirect **2 แกน** ชื่อชนกัน — อย่าสับสน (23/09)

| แกน | เก็บที่ | ใช้ตอบ | ช่าง (MTN) |
|---|---|---|---|
| `org_nodes.labor_type` (2026-07-22) | **แผนก** (คนรับค่าจากแผนกที่สังกัด · `laborTypeOf()`) | ต้นทุน/หัวคนตาม cost center | `indirect` (บัญชีไม่คิดเป็นแรงงานตรง) |
| `employees.staff_kind` (2026-09-21) | **รายคน** | เช็คชื่อ/สกิล/นับกำลังคนไหม | `direct` (เช็คชื่อครบ 25/25) |

**ค่าซ้ำกันแต่ตอบคนละคำถาม และขัดกันจริงที่ช่าง** — ห้ามเอามาแทนกัน/ห้าม sync หากัน
· ชิปบนจอ `/operator` จึงเขียนคนละคำ: `labor_type` = **Direct/Indirect** · `staff_kind` = **🧑‍🏭 หน้างาน / 🗂️ สนับสนุน**
· ถ้าจะเปลี่ยนชื่อค่าให้เลิกชนกัน (เช่น `shopfloor`/`support`) = งานเปลี่ยน check constraint + backfill — **ยังไม่ทำ รอ user เคาะ**

### 🔴 ทีม C ของ **พนักงาน** ต้องโผล่ในใบเช็คชื่อของหัวหน้าทีม A/B ด้วย (2026-09-21 · feedback หน้างาน)

> *"พนักงานบางคนมีปัญหาด้านสุขภาพ เค้าลงทีม C เพราะไม่ได้สลับกะ แต่หัวหน้าจะไม่เห็นเค้าในการเช็คชื่อ"*

**21/08 แก้ไปแค่ครึ่งเดียว** — แก้ฝั่ง "หัวหน้าเป็นทีม C" (`seesAllTeams`) แต่ฝั่งตรงข้ามยังพัง:
หัวหน้าทีม A/B กรอง `.eq('team', ตัวเอง)` ⇒ **พนักงานทีม C หายจากใบเช็คชื่อของหัวหน้าทั้งทีม A และ B**
ทั้งที่ทีม C = กะเช้าตลอด จึงยืนหน้างานกับทีมที่เข้าเช้าเสมอ

**วัดจริง 21/09:** พนักงาน direct ทีม C **23 คน** (operator 7) — **2 คนไม่เคยถูกเช็คชื่อเลยสักครั้ง**
(ปิยพงษ์ · มาเนตร — LINE D 110&300 Ton) · อีก 2 คนหยุดถูกเช็คพร้อมกันตั้งแต่ 27/08

- **แก้:** `teamsVisibleToLeader(team)` (`src/utils/shiftAssign.js`) → หัวหน้าที่หมุนกะได้ `[ทีมตัวเอง, 'C']`
  · หัวหน้าที่ไม่หมุนกะคืน `null` (ไม่จำกัด — กฎเดิม 21/08) · ใช้ใน `Checkin.jsx` 2 จุด (ลิสต์ + export)
- **ไม่รกกะดึก:** `resolveAssignedShift()` ให้ทีม C = `day` เสมอ + จอเช็คชื่อกรอง "เฉพาะกะนี้" อยู่แล้ว
  ⇒ กะดึกไม่เห็นเขา · กะเช้าเห็น = ตรงกับของจริง
- **ขอบเขตไลน์ยังคุมเหมือนเดิม** — ปลดเฉพาะแกนทีม · เทสล็อกไว้ใน `shiftRotation.test.mjs`

### 🔴 ช่องติ๊ก "ขอบเขตส่วนงาน" ต้องมีแผนกขึ้นตรงฝ่ายด้วย (2026-09-21 · feedback "ทำไม section ไม่ขึ้น")

ผังองค์กรมี `section` แค่ **PD1-4 · Planning&Store · TEST** — ส่วน **QA · MTN · JIG MTN · DIE MTN**
เป็น `department` ที่ `parent_id is null` (ขึ้นตรงฝ่าย ไม่มี section) แต่ `/add-user` เรนเดอร์ช่องติ๊ก
จาก `org_nodes kind='section'` อย่างเดียว ⇒ **บัญชีที่ scope = `JIG MTN` (6 ใบ) หรือ `QA` (6 ใบ)
เปิดโมดัลมาแล้วไม่มีอะไรติ๊กเลย** ทั้งที่ตั้งไว้แล้ว (ค่าไม่ได้หาย แค่มองไม่เห็น = อ่านหน้าจอแล้วเข้าใจผิด)

- **แก้:** ช่องติ๊ก = `section` + **แผนกขึ้นตรงฝ่าย** (`orphanDepts()` จาก `sectionScope.js` · ป้าย 🏛️)
  + **ค่าที่บัญชีนี้มีอยู่แล้วแต่ไม่อยู่ในทะเบียน** (กรอบประ + ⚠) — pattern เดียวกับ `positionOptionsWith`
- **กฎที่ตกผลึก: ฟอร์มต้องโชว์ค่าที่เก็บไว้เสมอ แม้ค่านั้นจะไม่อยู่ในรายการตัวเลือกแล้ว**
  ซ่อน = คนอ่านหน้าจอสรุปผิดว่า "ยังไม่ได้ตั้ง" แล้วไปตั้งทับ

> **🔴 ตามด้วยบั๊กจากการแก้รอบแรก (22/09 → แก้ 23/09):** คิวรี `org_nodes` ใน `/add-user`
> ดึงแค่ `code, name, kind` **ไม่มี `parent_id`** ⇒ `orphanDepts()` ที่ตัดสินจาก `!d.parent_id`
> มองว่า **ทุกแผนกขึ้นตรงฝ่าย** ⇒ GOR · LWRBAR · BIG PRESS · HYDROFORM · LINE APRON ASSY
> ติดป้าย 🏛️ ทั้งที่อยู่ใต้ส่วนงานผลิต · **เงียบสนิท ไม่มี error** — คอลัมน์ที่ไม่ได้ดึง = `undefined`
> **กฎทั่วไป: helper ตัดสินจากคอลัมน์ไหน ทุกคิวรีที่ป้อนมันต้องดึงคอลัมน์นั้น**
> (ด่าน `src/utils/__tests__/orgNodeSelect.test.mjs` สแกนให้แล้ว — ไฟล์ที่ใช้ `orphanDepts`/
> `deptOptionsFor`/`deptNodeFor`/`sectionValueForEdit` ต้องมี `parent_id` ใน `.select()`)

### 🔎 ค้นชื่อคนไทยต้อง "ทนการสะกด" — `normSearch` (2026-09-23 · เกิดจริง)
...
- **แก้ที่ของกลาง:** `normSearch` (`src/components/SearchSelect.jsx`) ตัด**เครื่องหมายประกอบของไทย**
  (สระบน/ล่าง · วรรณยุกต์ · ทัณฑฆาต ์ · ไม้ไต่คู้ = `U+0E31` · `U+0E34–0E3A` · `U+0E47–0E4E`)
  ก่อนเทียบ ⇒ **ทุก picker ในระบบทนการสะกดพร้อมกันหมด** (เดิมมีปัญหานี้ทุกตัว) · เทส `normSearch.test.mjs`
- **ห้ามเขียนตัวเทียบคำค้นเอง** — ใช้ `normSearch` เสมอ (AddUser เคยเขียนเองแล้วพลาดเพราะ ์ ตัวเดียว)
- **ผลข้างเคียงที่ยอมรับ:** คำที่ต่างกันแค่วรรณยุกต์จะ match กัน (มา/ม้า) — เป็นแค่ตัวกรอง
  ไม่ได้เปลี่ยนค่าที่บันทึก · **กว้างไว้ดีกว่าแคบจนหาไม่เจอ**
- **2 อย่างที่ทำคู่กัน (ห้ามถอด):** ① ค้นไม่เจอสักคน = **แสดงรายชื่อทั้งหมดแทน** + ป้ายบอกว่าไม่พบคำค้น
  (ลิสต์ว่าง = คนสรุปผิดว่าไม่มีคนนี้) ② กรอกรหัสที่ซ้ำ = **ผูกบัญชีกับคนนั้นให้เลย** ไม่ใช่แค่บอกว่าซ้ำ
  (ระบบรู้อยู่แล้วว่ารหัสนั้นคือใคร — บอกว่า "ซ้ำ" แล้วปล่อยคนตัน คือโยนงานกลับให้คนโดยไม่จำเป็น)

### 🔔 สายสนับสนุนต้องเลือกส่วนงานได้ — ไม่ใช่บังคับ "ขึ้นตรงฝ่าย" (2026-09-23)

รอบแรก (21/09) ผมบังคับ `staff_kind='indirect'` ⇒ `section = null` ซึ่ง**ผิดกับของจริง**:
ธุรการ/QC ที่ประจำส่วนงานมีอยู่จริง (ชญาดา = `clerk` สังกัด **PD3**)

**และมันมีผลกับการแจ้งเตือนโดยตรง** — `notify_recipients()` มี fail-open:
`ไม่มี sections[] + ไม่มี p.section + ไม่มี e.section` ⇒ **ผ่านตัวกรองส่วนงานทุกข้อ**
⇒ วัดจริง 23/09: **15 บัญชี** อยู่ในสภาพนี้ (ADMIN · Display ×2 · `processengineering` ·
ผจก.ชัยยุทธ · บัญชีกลางช่าง 3 · ช่าง MTN/DIE 6 คน · admin ณัฐพงศ์)
⇒ ทั้ง 15 ได้รับแจ้งเตือน**ทุกส่วนงาน** ของกฎ 22 ข้อที่ตั้ง `inapp_match_section`

- **แก้:** ฟอร์มเลือกส่วนงานได้ทั้ง 2 ประเภท · "🏛️ ขึ้นตรงฝ่าย" เป็น*ตัวเลือกหนึ่ง* (sentinel `ORPHAN_SECTION`
  → บันทึกเป็น null ตามผังจริง) · **แผนก cascade ตามส่วนงาน** ผ่าน `deptOptionsFor()` ของกลาง
  (เดียวกับหน้าลงทะเบียนพนักงาน) — เดิมโชว์ทุกแผนกปนกันโดยไม่สนส่วนงานที่เลือก
- **เตือนก่อนสร้าง:** ไม่ระบุส่วนงาน + บัญชีไม่ได้ติ๊กขอบเขต ⇒ ขึ้นป้ายสีส้มบอกตรงๆ ว่า
  "จะได้รับแจ้งเตือนของทุกส่วนงาน"
...
เป็น null ทั้งคู่** ⇒ ลิสต์ผู้ใช้ขึ้น **"ไม่ระบุชื่อ"** · ทุกจอที่โชว์ชื่อผู้ทำ/ผู้อนุมัติได้ค่าว่างตามไปด้วย

**ต้นเหตุ:** ช่อง "พนักงาน (ตัวตนของบัญชีนี้)" มีดอกจัน `*` แต่**ไม่มีตัวตรวจจริง** และชื่อที่พิมพ์ลง
**ช่องค้นหา** (`empSearch`) ไม่ได้ถูกบันทึกที่ไหนเลย — คนกรอกเข้าใจว่าพิมพ์ชื่อแล้ว = มีชื่อแล้ว

- **แก้:** `validateIdentity()` บล็อกทั้งตอนสร้างและตอนแก้ — `person` ต้องมี `employeeId`
  · ข้อความชี้ทางออก 2 ทาง: กดปุ่ม **"＋ เพิ่มคนนี้เข้าฐาน"** หรือเปลี่ยนเป็น **บัญชีกลาง (ไม่ใช่คน)**
- **กฎ: ดอกจัน `*` บนฟอร์มต้องมีตัวตรวจจริงเสมอ** — ไม่งั้นมันเป็นแค่ของตกแต่ง

### ❓ คำถามที่ถามซ้ำ: "ซิงค์ทีม/ไลน์จากฐานพนักงาน แล้วเขาจะเห็นแคบลงไหม" (ตอบ 2026-09-21)

**ไม่แคบ** — เพราะ `profiles.line_id` **ถูกใช้ตัดสินการมองเห็นเฉพาะ `role = 'leader'` เท่านั้น**
(ตรวจครบทุกจุด 21/09: `scopedLineNames` · `Checkin` · `Report` ×3 · `ShiftOrganize` · `Management`
— ทุกที่เขียน `role === 'leader' && userLineId ? … : scopeSecs.length ? … : ทั้งหมด`)

| role | อะไรกำหนดขอบเขต | ซิงค์ไลน์จากฐานพนักงานแล้ว |
|---|---|---|
| `leader` | **ครอบครัวไลน์ตัวเอง** + ทีม | เปลี่ยนจริง — นี่คือจุดที่ซิงค์ *แก้บั๊ก* (เคสทีมสลับกัน 21/08) |
| `supervisor` · `document_control` · `qa` · ฯลฯ | `sections[]` | **เท่าเดิม** — `line_id` ไม่ถูกอ่าน |
| `admin` · `mtn` · `engineer` | ไม่จำกัด | เท่าเดิม |

**⚠️ `syncFromEmployee` เขียนแค่ `section` (เดี่ยว) ไม่แตะ `sections[]`** และ `effectiveSections()`
ให้ `sections[]` ชนะเสมอ ⇒ คนที่ตั้ง `sections[]` ไว้แล้ว (ทั้ง 13 บัญชีในคิว 21/09) scope ไม่ขยับเลย

**🔑 ทีม C ไม่ได้ทำให้แคบ — กว้างกว่า A/B ด้วยซ้ำ:** `seesAllTeams()` (`shiftAssign.js`) คืน true
สำหรับทีมที่ไม่หมุนกะ ⇒ หัวหน้า/ช่าง/ออฟฟิศที่เป็นทีม C **เห็นคนทั้งไลน์ทุกทีม** (ตามดีไซน์:
"ไม่ได้ยืนหน้างานกะใดกะหนึ่ง") · ตั้งทีม C ให้คนกลุ่มนี้จึงถูกแล้ว

**ทิศทางที่ต้องระวังกลับกัน:** บัญชีที่มีไลน์อยู่แล้วแต่**ฐานพนักงานว่าง** → ซิงค์ = ไลน์หาย
(เคสจริง: ณัฐวุฒิ แสนทวีสุข) ⇒ ไปเติมที่ฐานพนักงานก่อน แล้วค่อยซิงค์ — คิวใน `/add-user`
แยกกลุ่มนี้ออกมาเตือนไว้แล้ว ("ฐานพนักงานยังไม่ได้กรอก")
...
> ### 🔁 อัปเดต 25/09 — `profiles.scope_depth` เป็นเจ้าของขอบเขตแล้ว (ลำดับ 1-5 ข้างล่างคือ**สูตรเก่า**)
> `effectiveSections(role, sections, section, **scopeDepth**)` ตัดสินตามนี้:
> `admin` → ไม่จำกัด (ตาข่ายกันตาย ห้ามแตะ) · `scopeDepth==='all'` → ไม่จำกัด ·
> แคบกว่านั้น → `sections` ถ้ามี ไม่มีก็ `[section]` ไม่มีทั้งคู่ก็ไม่จำกัด (= **fail-open ที่ยังเหลือ** — จอ
> `/add-user` ขึ้นแถบแดงให้แล้วผ่าน `scopeIneffective()`)
> · 🔴 **`scopeDepth === undefined` = ยังโหลด profile ไม่เสร็จ ⇒ ตกไปใช้สูตรเก่า (1-5) ไม่ใช่ "ไม่จำกัด"**
> — จึงยังต้องเก็บสูตรเก่าไว้อ่าน · `FACTORY_WIDE_ROLES` ยังใช้ในเคสนั้นเคสเดียว
> · **พิสูจน์แล้วว่าสลับแล้วไม่มีใบไหนเปลี่ยน 97/97** (SQL + `__tests__/scopeSwitch.test.mjs` ที่แช่สูตรเก่าไว้เทียบทุก build)
> 📄 `docs/ORG-AXES-DECISION.md` §7.7
...
- > ### ⚠️ กฎเหล็ก — scope ของ leader = **ทั้งครอบครัวไลน์** ห้าม `.eq('line_id', lineId)` ตรงตัว (2026-08-10)
  ...
  > - **ไม่ต้องกดคืน (feedback หน้างาน 2026-09-07 "พอจะยืมใหม่มันติดว่ายืมอยู่แล้ว"):** การยืมผูกกับ `work_date+shift` → หมดกะ/ข้ามวันหลุดเอง แถวเก่าไม่ขวางการยืมวันถัดไป · ติด 23505 = **คนเดียวกัน วัน+กะเดียวกัน** ถูกยืมไปอีกไลน์อยู่ (เคสจริง 2026-09-06: 4 คนจาก TSRA ถูกยืมไป APRON ASSY แล้วมีคนยืมซ้ำในกะเดียวกัน) · `resolveBorrowConflict()` ใน Checkin: บอกไลน์+คนที่ยืม · ปลายทางเดิมอยู่ใน scope ผู้กด → confirm **ย้าย** (update `to_line_id` แถวเดิม นับแถวจาก `.select('id')`) · นอก scope → ให้ไลน์นั้นกด ✕ คืน หรือรอกะถัดไป
  > - **✅ ผูก audit แล้ว (2026-09-08 · migration `20260908_line_helpers_audit.sql` apply แล้วบน Main):** `trg_audit` + `trg_set_updated_at` + `updated_at` → ยืม (INSERT) / ย้าย (UPDATE) / คืน (DELETE) ขึ้นหน้า `/audit-log` พร้อมชื่อคนกด · เดิมแถวถูก **DELETE จริง**ตอนกดคืน = หลังคืนแล้วสืบไม่ได้เลย
  >   - มาพร้อม **snapshot ชื่อ `emp_name` / `to_line_name`** เติมโดย trigger ฝั่ง DB (`fn_line_helpers_snapshot_names`, best-effort — หาชื่อไม่ได้ = ปล่อยว่าง ห้ามล้มการยืม) **ไม่ใช่ของแถม**: `audit_log` เก็บแถวดิบและ `AuditLogViewer` โชว์ค่าดิบ — ถ้ามีแต่ uuid จอประวัติจะได้ `employee_id: <uuid> → <uuid>` ตอบไม่ได้ว่าใครถูกยืมไปไหน · ทำฝั่ง DB ไม่ใช่ให้ client ส่งมา = ทุกจุดที่เขียนได้ฟรี + ดริฟท์ไม่ได้ · เป็น snapshot ตามเจตนา (เปลี่ยนชื่อคน/ไลน์ทีหลัง แถวเก่าไม่ตาม)
  >   - ป้ายไทยอยู่ใน `src/utils/auditLabels.js` (ตาราง + `work_date`/`shift`/`emp_name`/`to_line_name`/`created_by_name`) และ `rowLabel()` อ่าน `emp_name` ด้วย ไม่งั้นขึ้น "(ไม่ระบุชื่อ)"
  >   - เทสแล้วด้วย `auth.uid()` ไม่ null ตามกฎเหล็กใน `traceability-audit-log.md` (INSERT/UPDATE/DELETE ได้ actor ครบทั้ง 3 แล้วลบแถวเทสทิ้ง)
  > - **OJT ให้คนที่ยืมมา (2026-09-07 feedback "ยืมข้ามส่วนงานแล้ว OJT ให้ไม่ได้"):** picker ผู้เข้าอบรมใน `/ojt-training` เดิมกรองตาม section/ครอบครัวไลน์สังกัดเดิม → คนที่ยืมมาข้ามส่วนงานไม่โผล่ · แก้แล้ว: รวมคนใน `line_helpers` ที่ `to_line` อยู่ใน scope ผู้เปิดใบ (ป้าย "🤝 ยืมจาก X มาช่วย Y")
  > - **อิง `train_date` ของใบ ไม่ใช่ "กะปัจจุบัน" (แก้รอบ 2 · 2026-09-08):** รอบแรกล็อกไว้ที่กะปัจจุบัน ⇒ ใบที่บันทึก**ย้อนหลัง** (พรุ่งนี้มากรอกของเมื่อวาน = เรื่องปกติของงานเอกสาร) ยังเลือกคนยืมไม่ได้อยู่ดี · ตอนนี้ค้นจากวันที่ในใบ และเอา **ทั้ง 2 กะ** ของวันนั้น (ใบ OJT ไม่มีช่อง "กะ" — เดากะผิดแล้วตัดคนทิ้ง แย่กว่าโชว์เกินแล้วให้คนเลือกเอง) · ป้ายบอกกะ+วันที่จริงกำกับให้
  >   - **กับดักกะดึกก่อน 08:00:** ใบลงวันที่ "วันนี้" ตามปฏิทิน แต่การยืมของกะที่กำลังทำงานอยู่ถูกบันทึกใต้ `work_date` = เมื่อวาน (กฎตัด 08:00) → เมื่อ `train_date` = วันนี้ตามปฏิทิน ระบบมองวันงานปัจจุบันเพิ่มอีกวันเสมอ (เรียก `mergeBorrowedEmployees` ซ้ำได้ ฟังก์ชันตัดคนซ้ำให้เอง) · **ไม่แตะ default `train_date`** โดยตั้งใจ — หัวหน้ากะเช้าที่มาเตรียมใบตอน 07:30 ต้องได้วันที่วันนี้เหมือนเดิม
  >   - โหลดตอน "เปิดใบ" เท่านั้น และผูก deps กับ **วันที่** ไม่ใช่ object `editing` (ไม่งั้นพิมพ์ทีละตัวอักษรยิงคิวรีใหม่ทุกครั้ง) · ผังจัดกำลังคน (4M Man cross-line + แนบรูป OJT) รองรับคนยืมอยู่แล้วผ่าน `helperMap` · **กฎ: หน้าไหนกรองพนักงานตาม scope แล้วมีงานที่หัวหน้าไลน์ปลายทางต้องทำให้คนยืม ต้องรวม line_helpers กะนี้ด้วยเสมอ**
  > - **✅ ทำครบแล้ว 2026-09-08 — helper กลาง `src/utils/lineHelpers.js`** (`mergeBorrowedEmployees` + `isBorrowIntoScope` + `currentWorkShift` · มีเทส `src/utils/__tests__/lineHelpers.test.mjs`) · **ห้ามเขียนคิวรี `line_helpers` เองในหน้าใหม่** — กฎนี้ถูกใช้ ≥3 หน้าแล้ว
  >   - จุดที่ต่อแล้ว: `/ojt-training` (picker ผู้เข้าอบรม) · `/operator` (ตารางพนักงาน + คะแนนทักษะ) · `/report` แท็บ Skill Matrix (ใบประเมินรายบุคคล F-PRS-P1-119 + PDF/CSV) · ป้าย 🤝 ตาม UI-CONVENTIONS §6.13
  >   - **สิทธิ์: ขยายเฉพาะ "แก้สกิล" ไม่ขยาย "แก้ทะเบียนพนักงาน"** — `canEditSkillsFor()` ใน `operator.jsx` ยอมให้หัวหน้าไลน์ปลายทางให้คะแนนคนที่ยืมมากะนี้ได้ (เขาคือคนเดียวที่เห็นคนนั้นทำงานจริงวันนี้ · สิทธิ์หมดอายุเองเมื่อจบกะ) แต่ `canEditEmp()` **คงเดิม** — ชื่อ/ส่วนงาน/ไลน์/ตำแหน่ง ยังเป็นของต้นสังกัด ไม่งั้นไลน์ที่ยืมไปแก้ `line_id` แล้วคนหายจากไลน์เดิมถาวร
  >   - Skill Matrix กรองคนยืมด้วย **filter ที่เลือกอยู่** (เลือกไลน์ตัวเอง = เห็นคนที่มาช่วยไลน์นั้น) และกรอง ทีม/แผนก ซ้ำฝั่ง client เพราะคนยืมไม่ได้ผ่าน filter ฝั่ง server
  >   - ทั้ง 2 หน้าเติม guard กันคำตอบเก่าทับจอใหม่ (request id) เพราะ merge เพิ่ม await อีกจังหวะ
...
- **สาเหตุ (จาก `audit_log`):** 2026-09-02 15:49:33 บัญชี **role `leader`** (mongkol@pd4) update `full_name` ของ **49 บัญชีอื่น**
  ...
  30 นาทีถัดมา (16:19) migration `protect_profile_privilege_columns` ถูก apply (commit `ceb8153` "audit รอบ 5" — หัวไฟล์เขียนเองว่า "แก้ชื่อ/ตำแหน่ง/ลายเซ็นคนอื่น 🔴 เขียนสำเร็จ 71 แถว")
  → **= การทดสอบช่องโหว่ของ QC audit รอบ 5 บนฐานจริง แล้วขั้น "คืนค่าเดิม" ตั้งเป็น null แทนชื่อเดิม** (ไม่ใช่ระบบรีเฟรช/ไม่ใช่โค้ดหน้าเพจ — `AddUser.handleUpdate` แก้ทีละ id เสมอ)
- **ช่องโหว่จริง:** policy `auth_update_profiles` = `using(true) with check(true)` สำหรับ authenticated → role อะไรก็ update โปรไฟล์คนอื่นได้ทั้งตาราง ·
  ...
  (ไฟล์ `20260902_protect_profile_privilege_columns.sql` มาจาก audit รอบ 5 · policy `using(true)` ยังคงไว้เพราะ admin update ผ่าน client)
...
- **กฎที่ตกผลึก:** (1) **ห้ามทดสอบช่องโหว่ด้วยการเขียนทับข้อมูลจริงบน production** — ถ้าจำเป็นให้ใช้ `begin … rollback` หรือแถวทดสอบของตัวเอง และต้องคืนค่าเดิมจาก snapshot ไม่ใช่ null
  (2) การทดสอบที่เขียนข้อมูลจริงต้องบันทึก "ผลกระทบข้างเคียง" ไว้ในไฟล์ migration/เอกสารด้วย — รอบนี้หัวไฟล์บอกแค่ "เขียนสำเร็จ 71 แถว" ไม่บอกว่าชื่อไม่ได้คืน ทำให้ user ต้องพิมพ์เองอยู่ 2 วัน
  (3) `audit_log` บน `profiles` คือสิ่งที่ทำให้กู้ได้ครบ — ตาราง master ใหม่ต้องผูก `trg_audit` เสมอ

---

## 🏬 role `planner_store` เคยมี 0 บัญชี — สิทธิ์งานสโตร์ทั้งชุด "แขวนอยู่กับ role ที่ไม่มีใครใช้" (2026-09-22)

**ที่มา:** ใบสรุปประชุม 21/09/2569 ข้อ 3 (Planning *"กดผลิตไม่ได้ — ขึ้น list ให้ผูกแล้วแต่กดไม่ได้"*)
และข้อ 4 (Store *"เพิ่ม organize ส่วนงานไม่ได้"* / *"ปรับยอด Stock over all ไม่ได้"*)

**วัดกับฐานจริง 22/09 — 2 ชั้นซ้อนกัน ต้องแก้ทั้งคู่ถึงจะใช้ได้:**

**ชั้นที่ 1 — บัญชีอยู่ผิด role**
`planner_store` ถูกออกแบบ + seed สิทธิ์งานสโตร์ไว้ครบ แต่**ไม่มีบัญชีไหนใช้ role นี้เลย (0 คน)**
บัญชีจริง 6 ใบ (`planningstore` · `warehouse1/2` · `delivery1/2` · `billing`) ถูกตั้งเป็น `sale`
⇒ อาการคลาสสิก **"เปิดหน้าได้ แต่ไม่มีปุ่ม"** เพราะ `page:*` ให้มา แต่คีย์ action ไม่ได้ให้:

| ที่หน้างานแจ้ง | คีย์ที่ต้องใช้ | `sale` |
|---|---|---|
| กดผลิตบนบอร์ดคัมบังไม่ได้ | `page:/heijunka` ✅ · `heijunka:operate` | ❌ เห็นคิว แต่ปุ่ม ▶ เริ่มผลิต ไม่โผล่ |
| ปรับยอดสต๊อกไม่ได้ | `page:/line-stock` (หน้าที่มีเมนู 🔧 ปรับยอด) | ❌ เปิดหน้าไม่ได้เลย (เห็นยอดผิดจาก /store-monitor แต่แก้ไม่ได้) |
| จ่ายของ/ชั้นวาง/ขนส่ง/สินค้า | `line_stock:issue` · `rack_center:operate` · `transport:manage` · `products:edit` | ❌ ทั้งหมด |

**ชั้นที่ 2 — ชื่อส่วนงานไม่ตรงกัน (ต้นเหตุจริงของ "เพิ่ม organize ไม่ได้")**
`OrgSetup` หา "ส่วนงานของฉัน" โดยจับคู่ `profiles.section` กับ `org_nodes.code` **หรือ** `.name`
· บัญชีสโตร์: `profiles.section = 'Planning&Store'` · node ในผัง: `name = 'PLN & STO'`, `code = null`
⇒ จับคู่ไม่ติด → `mySecId = null` → ปุ่มเพิ่มแผนก/กลุ่มปิดหมด **ต่อให้ถือคีย์ครบก็ยังกดไม่ได้**
และคีย์ `org:manage_own_unit` เองก็ไม่มีทั้งใน `sale` และ `planner_store` (เดิมมีแค่ admin/dept_admin/manager/supervisor)

**ที่แก้ (user เลือกทางเลือก ก. — ย้าย role ไม่ใช่ติ๊กสิทธิ์ให้ `sale` ทั้งก้อน):**
1. ย้าย 3 บัญชีงานสโตร์/วางแผน (`planningstore` · `warehouse1` · `warehouse2`) → role `planner_store`
   · `delivery1/2` · `billing` **คงเป็น `sale`** — งานจัดส่ง/วางบิลไม่ควรได้สิทธิ์กดผลิต/ปรับยอดสต๊อกติดไปด้วย
   (= เหตุผลที่ไม่เลือกทาง "ติ๊กสิทธิ์สโตร์ให้ `sale` ทั้ง role")
...
migration `20260922_store_accounts_planner_store_role.sql` (**apply แล้ว 22/09 · MAIN `ewhdfqwfwofivojtsizn`**)

> **กฎที่ตกผลึก (คลาสบั๊กที่ต้องไม่เกิดซ้ำ):**
> 1. **สิทธิ์ที่ seed ไว้กับ role ที่ไม่มีบัญชีใช้ = สิทธิ์ที่ไม่มีอยู่จริง** — เพิ่ม role ใหม่/แจกสิทธิ์ตาม role
>    ต้องเช็คเสมอว่า **มีกี่บัญชีถือ role นั้นจริง** (`select role, count(*) from profiles group by role`)
>    ไม่ใช่ดูแค่เมทริกซ์ `/permissions` ว่าติ๊กแล้ว
> 2. **เปิด `page:X` ให้ role ไหน ต้องไล่ดูด้วยว่าปุ่มในหน้านั้นใช้คีย์ action อะไร** — ให้แค่หน้าคือการส่งคนไปเจอจอที่กดอะไรไม่ได้
>    (คลาสเดียวกับ audit 2026-09-04 ที่ RLS แคบกว่าปุ่มบนจอ)
> 3. **ค่าที่ใช้ "จับคู่ข้ามตาราง" (`profiles.section` ↔ `org_nodes.code/name`) ต้องมีที่ให้ตั้งให้ตรงกันได้**
>    — ไม่ตรง = ฟีเจอร์ตายเงียบโดยไม่มี error ให้เห็น · ส่วนงานที่ยังไม่มี node ในผังเลย: **JIG MTN · QA**
>    (คนใน 2 ส่วนงานนี้ที่ถือ `org:manage_own_unit` จะเจออาการเดียวกัน — ยังไม่ได้แก้ รอ user สั่ง)

> ⚠️ **หลังย้าย role ผู้ใช้ต้องรีเฟรชหน้า (F5) หรือเข้าระบบใหม่** — `fetchProfile` อ่าน role ตอนโหลดเซสชันเท่านั้น
...
## 🔀 ยุบ `sale` เข้า `planner_store` — เส้นแบ่ง role ของ Logistic = **2 ไม่ใช่ 3** (2026-09-23 · user)

*"store แผนงาน ข้อมูลอะ รวมกัน · planner_store sale ก็รวมกัน"*

ผลสรุปสุดท้ายของการจัด role ฝั่ง Logistic (หลังแก้ 3 รอบในวันเดียว):

| role | ครอบงานอะไร | บัญชี |
|---|---|---|
| 📦 `planner_store` | **Store + แผนงาน & ข้อมูล** — บริหารสต๊อก · จ่ายของเข้าไลน์ · จ่ายงานในโรงงาน · รับ Forecast/Order · วางแผนผลิต | `planningstore` |
| 🚚 `warehouse_delivery` | **Warehouse + Delivery + Billing** — เก็บ FG · รอบส่งลูกค้า · ภาชนะ/Packaging · ออกใบขาย | `warehouse1/2` · `delivery1/2` · `billing` |
| 🚫 `sale` | **เลิกใช้** — ยุบเข้า `planner_store` | — |

**ยุบได้โดยไม่ต้องย้ายสิทธิ์เลย** เพราะวัดแล้ว `planner_store` ⊇ `sale` อยู่แล้วทุกคีย์
ยกเว้น `skills:edit_high` ตัวเดียวที่**ตั้งใจไม่ให้** (งานสโตร์/วางแผนไม่ใช่ผู้ประเมินฝีมือ)
· ตอนยุบมี **0 บัญชี** ถือ `sale` (billing ย้ายออกไปก่อนหน้าในวันเดียวกัน) = ไม่มีใครกระทบ

> **🔴 กติกาของ role ที่เลิกใช้ (ค่า enum ลบทิ้งไม่ได้):**
> - **คงรายการไว้ใน `ROLE_META` เสมอ** — ถอดออกแล้วประวัติเก่า/`audit_log` ที่บันทึก role นี้
>   จะแสดงเป็นคีย์ดิบ อ่านไม่รู้เรื่อง และแถวสิทธิ์เดิมจะจัดการที่ `/permissions` ไม่ได้อีก
> - ติดธง **`retired: true`** + ใส่คำว่า **“(เลิกใช้ — ใช้ … แทน)” ในตัว label** ให้เห็นตั้งแต่ใน dropdown
>   ของ `/add-user` · ตั้งใจ**ไม่กรองออกจากลิสต์** เพื่อไม่ให้บัญชีที่เผลอถือ role นี้กลายเป็นแก้ไม่ได้
>   (เลือกไม่ได้เพราะอ่านแล้วรู้ ไม่ใช่เพราะซ่อน)
> - **ห้ามลบแถวใน `role_permissions` ของ role ที่เลิกใช้** — เก็บไว้เป็นทางย้อนถ้าตัดสินใจกลับ

⚠️ **หมวดเมนู/ทะเบียนสิทธิ์ยังเป็น 3 ฝั่งเหมือนเดิม** (`LOGISTIC_GROUPS`) — **คนละเรื่องกับ role ห้ามยุบตามกัน**
หมวด = "งานชิ้นนี้เป็นของฝั่งไหน" (ใช้จัดเมนู/จัดกลุ่มคีย์สิทธิ์) · role = "คนกลุ่มนี้ทำอะไรได้บ้าง"
ฝั่งหนึ่งมีได้หลาย role และ role หนึ่งครอบได้หลายฝั่ง

migration: **ไม่มี** — ไม่ต้องแตะ DB เลย (0 บัญชี · สิทธิ์ครอบอยู่แล้ว) เปลี่ยนเฉพาะป้าย/เอกสาร
