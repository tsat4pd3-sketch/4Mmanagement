...
>   · **⚠️ 2026-09-07 — "วางอยู่บนผังไลน์" เป็นแค่คะแนนเสริม ไม่ใช่เหตุผลให้เสนอ:** เดิมเกณฑ์นี้ผ่านคนเดียวได้ (10 คะแนน)
>     → แท็บ ⚙️ เสนอ **107 เครื่องจริง** (Denyo/ABB/SEYI ทุกตัวที่อยู่บนผัง) ผู้ใช้เข้าใจว่า "ลิสต์นี้ = เครื่องที่ยังไม่ได้ตั้งค่า"
>     ตั้งแล้วก็ไม่หาย · แก้: ต้องมีเบาะแสจากเลข/ชื่อ (JHYD/GPHYD/JIG/FIXTURE/GRIPPER/…) ก่อน ผังค่อยบวกคะแนนเรียงลำดับ
>     · **แท็บนี้คือ "ตัวเสนอ" ให้ติ๊กเป็นชุด — ลิสต์ว่าง = ดี** (ไม่เหลือเครื่องที่น่าสงสัยว่าเป็นจิ๊ก) ไม่ใช่ to-do ที่ต้องเคลียร์ทุกแถว
>     · ตั้งเป็น "จิ๊ก/ฟิกเจอร์" แล้วหายจากลิสต์ทันที (กรอง `equipment_kind === 'machine'`) · ตั้งเป็น "เครื่องจักร" = ค่าเดิม ไม่เปลี่ยนอะไร
...
> ### ⚠️ กฎเหล็ก 12 — จุดชิม "ปักบนรูปเครื่องชุดเดียวกับใบตรวจ PM" และกรอกค่าได้จากใบตรวจ PM (2026-09-08 · คำสั่ง user)
> user: *"shim record กับการตรวจสอบ ควรใช้กลไกเดียวกัน — ต้องกำหนดจุดตามตำแหน่งในรูป"* → **แยกตาราง แต่รวมกลไก**
> - **รูป = ของ PM Setup** (`jig_images` ผูก `jigs.id` แถวเงา) — `/fixture` ไม่อัปโหลด/ลบรูปเอง (SpinAnnotator `readOnlyFrames`) ไม่มีรูป = บอกให้ไปอัปที่ PM Setup ห้ามทำทางอัปโหลดซ้ำ
> - **หมุดของจุด = `pointPin(point, cpById)`** (`fixturePoints.js` · pure · เทส): x_pos/y_pos ของตัวเอง (image_id null = เฟรมแรก — convention เดียวกับ `jig_checkpoints`) → ไม่มีค่อยยืมจากจุดตรวจ PM ที่ `checkpoint_id` → ไม่มีทั้งคู่ = null **จอต้องบอกว่า "ยังไม่ปัก" ห้ามเดา** · migration `20260908_fixture_points_pin.sql` (DR · apply แล้ว: `fixture_points.x_pos/y_pos/image_id` + `fixture_shim_events.inspection_id`)
> - **ใบตรวจ PM (`PMCheckData`)** โหลด `fixture_points` ของ `jigs.machine_id` → หมุด**สีม่วง** `#a78bfa` label = เลขจุด บนเฟรมเดียวกับหมุดตรวจ · ท้ายรายการตรวจมีบล็อก "🔩 จุดชิม" กรอก "ค่าชิมรวมที่วัดได้ (mm)" + หมายเหตุ · **ไม่บังคับ ไม่นับใน isFormReady** (ความถี่จุดชิมเป็นของจุด — กฎเหล็ก 1) · สิทธิ์ `fixture_shim:record` (ไม่มี = เห็นแต่กรอกไม่ได้ + บอกเหตุผล) · บันทึกหลัง `inspection_results` สำเร็จ ผูก `inspection_id` — ล้มต้อง toast ห้ามให้ใบตรวจหลักล้มตาม
> - **การเขียน event ทุกทางต้องผ่าน `src/utils/fixtureShimApi.js` (`recordShimEvent`)** — FixtureShimPanel ย้ายมาใช้แล้ว · `deriveShimAction(before, after)` แปลงค่าที่พิมพ์เป็น check/add/remove · **บั๊กที่เจอระหว่างทำ: `last_check_at`/`last_check_shot` ไม่เคยถูกเขียนเลย** → `pointDueStatus` ไม่เคยรีเซ็ตหลังตรวจ (จุดถึงกำหนดค้างตลอด) — helper stamp ให้ทุก event แล้ว · ยังไม่ apply migration → 42703 ถอยไปบันทึกโดยไม่ผูกใบตรวจ + toast
> - ปักหมุดที่แท็บ 📋 ทะเบียนจุด (สิทธิ์ `fixture_point:manage`): ปุ่ม "วาง/ย้าย" ในคอลัมน์ "บนรูป" → คลิกบนรูป · หมุดที่ยืมจาก PM ถอนที่นี่ไม่ได้ (แก้ที่ PM Setup)
> - **ตรวจฐาน 2026-09-08:** จุดชิม 0 · event 0 · จิ๊ก 146 ตัว มีรูป 10 / มีหมุดตรวจ 9 · **Line 60/61 ยังไม่มีรูปเลย** → นำร่องต้องอัปรูปที่ PM Setup ก่อน

- **นำร่อง Line 60/61** (คำสั่ง user) — จัดชนิดให้แล้ว (Line 60 = 11 จิ๊ก · Line 61 = 13) **เริ่มตั้งจุดชิมได้เลย**
- **ยังไม่ทำ:** ยังไม่ผูกกับใบ MO อัตโนมัติ (`mtn_order_id` มีคอลัมน์รอไว้แล้ว · ใบตรวจ PM ผูกแล้ว 2026-09-08) · ยังไม่แจ้งเตือนตอนจุดถึงกำหนด
...
### ปิดจุดล้มเงียบ + ทะเบียนแม่พิมพ์โหลดพังแล้วอ่านว่า "ไม่มีข้อมูล" (แก้ 2026-09-08)

- `FixtureRegistry.removePoint()` ทำ `update({is_active:false})` แล้วเช็คแค่ `error` → **RLS ปฏิเสธ = 0 แถว
  ไม่มี error** ⇒ ขึ้น toast เขียว "ปิดใช้งานจุดแล้ว" ทั้งที่จุดยังอยู่ (กฎเหล็กข้อ 2) · แก้เป็น `.select('id')`
  แล้วนับแถว แบบเดียวกับ `savePoint` ในไฟล์เดียวกันที่ทำถูกอยู่แล้ว
- `DieRegistry.load()` รับ 5 คิวรีแรกด้วย `const { data: x }` = **กลืน error 100%** (รวมตัวแม่พิมพ์เองกับชุด)
  ⇒ คิวรีล้มแล้วจอขึ้น "ไม่มีแม่พิมพ์" ผู้ใช้อ่านว่าข้อมูลหาย · แก้เป็นเก็บ `warn[]` แล้วโชว์แถบแดง `dataWarn`
  (ยกรูปแบบจาก `FixtureRegistry.load()` ที่ทำถูกอยู่แล้ว)

### 📌 รูปเครื่องตรึงไว้ตอนเลื่อนดูตารางจุด (2026-09-23)

แผง "📍 ตำแหน่งจุดชิมบนรูปเครื่อง" เป็น `position:sticky; top:0` — เลื่อนดูตารางจุดแล้วรูปไม่หาย
(user: *"จอที่ต้องใช้รูปอ้างอิงตอนตรวจ ควรจะตรึงรูปไว้"*) · กติกาเต็ม + อีก 2 จอที่ทำเหมือนกัน
→ `docs/UI-CONVENTIONS.md` §จอ "รูปอ้างอิง + รายการที่ต้องกรอก"
