...
- **🔒 สเปคจุดตรวจมีเจ้าของจุดเดียว = `src/utils/qaSpec.js` (`specLabel` + `judgeVariable` · เทส `qaSpec.test.mjs` · 2026-09-07)**
  ...
  · **จุด variable ที่ตั้ง limit → ตัวเลขคือความจริง** label = `5 (4 – 6) mm` เสมอ · `spec_text` เป็นแค่ข้อความตามแบบ (โชว์เป็น tooltip ในตาราง setup)
  · ไม่มี limit → ใช้ `spec_text` · ไม่มีอีก → attribute `GO / NOGO` / variable `—` · ฟอร์ม setup มีพรีวิว "จอตรวจจะแสดงสเปคว่า …" จากค่าชุดเดียวกัน
  + เตือนเมื่อยังไม่ตั้ง LSL/USL (= ระบบไม่ตัดสินอัตโนมัติ)
  · ใบตรวจ · ตาราง setup · snapshot ลง `qa_inspection_results.spec_text` เรียก `specLabel` ทั้งหมด — **ห้ามประกอบข้อความสเปคเองในหน้าไหนอีก**
...
> (ตั้งแต่ 2026-09-03 สวิตช์ใน `/qa` ⚙️ **กันเอง** — เปิดไม่ได้จนกว่าผูกพาร์ทได้ > 0 รุ่น และเลือกห้อง `qa_fme_call` แล้ว · ดู 🚦 preflight ด้านล่าง)
...
> - สูตรทั้งหมดเป็น pure อยู่ **`src/utils/qaFmeForecast.js`** (เทส 16 เคส `src/utils/__tests__/qaFmeForecast.test.mjs` — เขียนจริง 2026-09-07 · ก่อนหน้านี้เอกสารอ้าง "31 เคส" ที่ไม่เคยมีไฟล์ · เทส bundle ด้วย rolldown เพราะไฟล์ import `./pairTotals` ไม่มีนามสกุล): `addWorkMinutes` (เดินเวลาข้ามเบรค ·
...
- **🚦 preflight ที่ตัวสวิตช์ (2026-09-03):** เปิดแผง ⚙️ แล้วระบบตรวจความพร้อมเองทุกครั้ง (`src/utils/qaFmeReadiness.js` pure · เทส 8 เคส
  `src/utils/__tests__/qaFmeReadiness.test.mjs`) — **checkbox เปิดถูก disable จนกว่าจะผ่านข้อ hard ทุกข้อ** · ปิดสวิตช์ได้เสมอ
  | เช็ค | hard? | อ่านจาก | ผ่านเมื่อ |
  |---|---|---|---|
  | พาร์ท QA ผูกกับรุ่นที่ผลิต | ❌ hard | `qa_parts` (Main) × `dr_products` (DR · `cachedMaster('dr_products_link')`) | ผูกได้ > 0 รุ่น — ใช้ตัวจับคู่ **สำเนาเดียวกับ edge `qa-fme-scan` §5 `resolvePart`** (mat_no ตรง → `p_no` ลูกค้า → part_no=เลข SAP) |
  | ห้อง `qa_fme_call` | ❌ hard | `notification_rules.channel_ids` | เลือกแล้ว ≥ 1 ห้อง + rule ไม่ถูกปิด |
  | ห้อง `qa_fme_overdue` | ⚠️ soft | เดียวกัน | เตือนเฉยๆ ไม่กันเปิด |
  · ชุดไหนโหลดไม่ได้ = `ok:null` "ประเมินไม่ได้" + เหตุผล และ**นับเป็นยังไม่พร้อม** (fail-closed — ห้ามแปลงเป็นผ่าน)
  · แถบเตือน "ปิดอยู่" บอกจำนวนข้อที่ติดให้คนที่ไม่มีสิทธิ์ `qa:manage` เห็นด้วย (UI-CONVENTIONS §6.9 ห้ามซ่อนเหตุผล)
  · **⚠️ แก้ `resolvePart` ใน edge เมื่อไหร่ ต้องแก้ `makeQaPartResolver` ให้ตรงในคอมมิทเดียวกัน** (Deno import ไฟล์ src ไม่ได้ จึงต้องมี 2 สำเนา — drift = จอบอกพร้อมแต่คิวจริงเปิดใบไม่ได้)

---

### 🔴 หมุดบนแบบ: "ยังไม่ตรวจ" ต้องดูออกจาก "ตรวจแล้ว" (2026-09-23)

บั๊กเดิม: `RANK.SC.color = #ef4444` **เท่ากับ** `JUDGE.ng.color = #ef4444` เป๊ะ ⇒ จุด SC ที่ยังไม่ตรวจ
หน้าตาเหมือนจุดที่ตรวจแล้ว NG บนแบบ — อ่านผิดได้ 2 ทาง (ข้ามจุดที่ยังไม่ตรวจ / ตกใจว่ามี NG)

แก้: แยกที่ **รูปทรง** — ยังไม่ตรวจ = `<CalloutPin hollow>` วงโปร่งเส้นประ · ตรวจแล้ว = วงทึบ
**สีขอบยังเป็นสี rank เหมือนเดิม** (M/SC ต้องเห็นบนแบบตลอด ห้ามกลืนเป็นสีกลาง) + legend บอกทรง
กติกากลาง → `docs/UI-CONVENTIONS.md` §จอ "รูปอ้างอิง + รายการที่ต้องกรอก"

---

## 📊 แท็บ Dashboard คุณภาพ — พาเรโต + การ์ด KPI (2026-09-23 · ก้อน C)

### พาเรโตของเสียตามประเภท — ย้ายมาใช้ `<ParetoChart>` ของกลาง
เดิมหน้านี้ประกอบพาเรโตเอง (`<ComposedChart>` + `<Bar>` + `<Line dataKey="cum">`) จึง**หลุดด่าน**
`pareto-via-ParetoChart` ไป (ด่านเดิมจับได้แค่แท่งนอนที่คำนวณ `width` เอง) — และมันผิด 2 ข้อ:

1. 🔴 **`.slice(0, 10)` ก่อนคิด % สะสม** ⇒ เส้นสะสมจบ 100% ที่ประเภทที่ 10 ทั้งที่ของจริงยังมีที่ 11+
   = จอประกาศว่า "10 ประเภทนี้คือของเสียทั้งหมด" ซึ่งไม่จริง
   · ตอนนี้ส่ง**ทุกประเภท**เข้า `classifyAbc` แล้วให้ `collapseTail` ของ `<ParetoChart>` ยุบหางเป็นแท่ง
     "อื่นๆ" ตามมาตรฐาน ⇒ % สะสมจบ 100% ของจริง และกดกางหางยาวดูได้
2. 🔴 **ทาแท่งด้วยสีประจำประเภทของเสีย** (`v.color` จากทะเบียน taxonomy) ⇒ แท่งเขียว/เหลือง/แดง
   เรียงกันโดยสีไม่ได้แปลว่าหนักเบา ชนกับไฟ Andon บนจอเดียวกัน · ตอนนี้ใช้สี **ABC**
   (A แดง = 80% แรกต้องแก้ก่อน · B ส้ม · C เทา) = สีสื่อลำดับความสำคัญจริง

**ตำแหน่งบนจอ:** อยู่**นอกกริด** เป็นบล็อกเต็มความกว้างของตัวเอง — แท่งเยอะ + ป้ายไทยยาว
บีบลง 1 ใน 3 ของจอแล้วอ่านป้ายไม่ออก · ⚠️ ห้ามย้ายกลับเข้ากริดแล้วใส่ `gridColumn: '1 / -1'`
(กริดเป็น `auto-fit` → ถูกดันลงบรรทัดใหม่ แล้วเหลือช่องว่างครึ่งแถวข้างบน — ลองแล้ว 23/09)

### แถวการ์ด KPI
- สีทุกใบมาจาก `statusTone` ชุดเดียวกับทั้งระบบ (เดิมตั้ง hex เอง 6 จุด)
- 🔴 **เดิม "ยอดผลิตรวม" และ "NG" ถูกทาเขียวเมื่อเป็น 0** — แต่ 0 ของ 2 ใบนี้คนละความหมายสิ้นเชิง:
  NG = 0 คือดีจริง · **ยอดผลิต = 0 คือยังไม่มีข้อมูล ไม่ใช่เรื่องดี** ⇒ ห้ามใช้สีเดียวกัน
- **PPM เป็นใบพระเอก** (38px) — ใบเดียวในแถวที่เทียบข้ามไลน์/ข้ามเดือนได้
  (ยอดผลิต/NG เป็นเลขดิบที่ขึ้นกับว่าเดือนนั้นผลิตเยอะแค่ไหน)
- ทุกใบมีหน่วย (ชิ้น/ใบ/%) + เกณฑ์ที่ใช้ตัดสินสีเขียนอยู่บนการ์ด (UI §6.18)
