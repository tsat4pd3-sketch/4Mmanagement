...
- **`line_helpers` (ยืมพนักงานข้ามไลน์รายกะ) ผูก audit แล้ว 2026-09-08** — migration `20260908_line_helpers_audit.sql` (Main · **apply แล้ว**) · ใช้ `fn_audit()` ตัวปกติ (เขียนโดยคนล็อกอินเท่านั้น ไม่มี job แตะ) · **เคสตัวอย่างของตารางที่ "ลบแถวจริง" แล้ว audit คือหลักฐานเดียวที่เหลือ** — กดคืนคน = `delete` แถวหาย ถ้าไม่มี audit ก็จบ
  - ⭐ **บทเรียนที่ใช้ซ้ำได้: ตารางที่อ้างของอื่นด้วย id ล้วน ต้อง snapshot "ชื่อ" ลงแถวก่อนผูก audit** — `audit_log` เก็บ `old_data`/`new_data` เป็นแถวดิบ และ `AuditLogViewer` โชว์ค่าดิบตรงๆ ⇒ แถวที่มีแต่ uuid/fk ทำให้จอประวัติได้ `employee_id: <uuid> → <uuid>` = audit ที่ **ตอบคำถามที่ตัวเองถูกสร้างมาตอบไม่ได้**
  - วิธีที่ใช้: trigger `fn_line_helpers_snapshot_names()` (BEFORE INSERT OR UPDATE OF fk) เติม `emp_name`/`to_line_name` **ฝั่ง DB** ไม่ใช่ให้ client ส่งมา → ทุกจุดที่เขียนได้ฟรีทันที ดริฟท์ไม่ได้ ไม่ต้องแก้โค้ดหน้าไหน · best-effort (`exception when others then null`) — หาชื่อไม่เจอ = ปล่อยว่าง **ห้ามล้ม write หลัก** (หลักการเดียวกับ `fn_audit`)
  - เพิ่มป้ายไทยใน `src/utils/auditLabels.js` แล้ว และ **`rowLabel()` ต้องรู้จักฟิลด์ชื่อของตารางใหม่ด้วย** (เพิ่ม `emp_name` เข้าเชน) ไม่งั้นทุกแถวขึ้น "(ไม่ระบุชื่อ)"
...
## 🆔 ผูก uid คู่ชื่อคน — "คนเดียวกันต้องนับเป็นคนเดียว" (2026-09-16 · คำสั่ง user)

> **ปัญหาที่แก้:** ชื่อคนถูกเก็บเป็น text ล้วนไม่ผูกรหัสอะไร และมี **ทะเบียนคนหลายชุดที่พิมพ์มือแยกกัน**
> ⇒ คนเดียวกันกลายเป็นหลายคนในรายงาน ตอบไม่ได้ว่า "งานนี้มีคนทำจริงกี่คน · คนนี้แตะงานอะไรบ้าง"
>
> | ทะเบียนจริง (profiles/employees — **ตรงกันเป๊ะ ไม่ใช่ตัวปัญหา**) | `mtn_technicians` (พิมพ์มือแยก) |
> |---|---|
> | อภิสิทธิ์ จำปา**ต้า** | นายอภิสิทธิ์ จำปา**ต้น** |
> | ณัฐภัทร **ฐ**านันต๊ะ | นายณัฐภัทร **ถ**านันต๊ะ |

### หลักการ — เพิ่ม uid **คู่กับ** ชื่อ ไม่แทนที่ชื่อ
- **ชื่อในแถว = snapshot ณ วันที่ทำงาน** ฟอร์มพิมพ์/ใบเซ็นต้องได้ชื่อเดิมเสมอ แม้คนนั้นเปลี่ยนชื่อ/ลาออก
- **uid = คีย์ที่ join/นับได้** — ชื่อสะกดต่างไม่ทำให้กลายเป็นคนละคน
- 🔴 **uid ฝั่ง DR ไม่ใช่หลักฐานที่ verify ฝั่ง server ได้** (supabaseDR เป็น anon ไม่มี JWT)
  มันคือคีย์สำหรับนับ/join **ไม่ใช่ลายเซ็น** — ข้อจำกัดเดิมของ DR ไม่ได้แย่ลง (known gap เดิม)

### เจ้าของเดียว = `src/utils/actorStamp.js`
`setActor(uid, name)` / `getActor()` / `actorFields(prefix)` / `applyStepActors()` /
`normPersonName()` / `personKey()` / `samePerson()` / `countPeople()` — pure + มีเทส 23 เคส
**ห้ามเก็บตัวตนผู้ใช้ซ้ำที่อื่น** (เคยมี 2 เจ้าของใน supabaseClient.js แล้ว drift)

- 🔴 **`normPersonName()` ตั้งใจ "ไม่" แก้สะกดผิดให้** — จัดการแค่ความต่างเชิงกลไก (ช่องว่างซ้ำ ·
  คำนำหน้า นาย/นาง · zero-width จาก Excel) · การตัดสินว่า "จำปาต้น = จำปาต้า" เป็น
  **การตัดสินใจของคน ไม่ใช่ของระบบ** (ENGINEERING-PRINCIPLES §2)
- `countPeople()` คืน `{ people, byUid, byName }` — **byName > 0 = ตัวเลขเป็นค่าประมาณ
  หน้าจอต้องเขียนกำกับ ห้ามโชว์เฉยๆ เหมือนเป็นค่าจริง**

### 2 ชนิดของ "ใครทำ" — อย่าสับสน
| ชนิด | คอลัมน์ | ใครเติมให้ |
|---|---|---|
| **ใครกดเซฟแถวนี้ล่าสุด** | `updated_by_name` + `updated_by_uid` | wrapper `supabaseDR.from` อัตโนมัติ (`DR_AUDIT_TABLES` 45 ตาราง) |
| **ใครทำขั้นตอนนี้ของงาน** | `opened_by_*` · `tech_main*` · `approver_*` ฯลฯ | wrapper (`DR_STEP_ACTORS`) เติมให้**เฉพาะเมื่อชื่อ = คนที่กำลังกด** · ชื่อคนอื่นให้หน้าส่ง uid มาเอง |

> 🔴 **กฎความถูกต้องของ `applyStepActors`: "เขียนชื่อเมื่อไหร่ ต้องเขียน uid ทับด้วยเสมอ"**
> ถ้าเขียนชื่อใหม่แล้วปล่อย uid เดิมไว้ = แถวที่ชื่อเป็นคนหนึ่ง uid เป็นอีกคน ⇒ รายงานนับงานให้ผิดคน
> **แย่กว่าไม่มี uid เลย** เพราะดูน่าเชื่อถือแต่ผิด · resolve ไม่ได้ → เขียน `null` (แปลว่า "ไม่รู้" ซึ่งซื่อสัตย์)

### กับดักที่เจอจริงตอนทำ (คนถัดไปน่าจะเจอซ้ำ)
1. **`fn_audit` เทียบ `changed_fields` โดยยกเว้นแค่ `updated_at`/`updated_by_name`**
   ถ้าเพิ่ม `updated_by_uid` โดยไม่ยกเว้นด้วย ⇒ ทุก UPDATE ถูกมองว่า "มีอะไรเปลี่ยน"
   ⇒ กฎ "UPDATE ที่ไม่เปลี่ยนจริงไม่ log" พัง ⇒ **audit_log บวมทุกครั้งที่ใครกดเซฟ**
   (audit_log เป็นตารางที่โตเร็วที่สุดของ project นี้อยู่แล้ว)
2. **`fn_audit` ฝั่ง DR อ่าน `to_jsonb(NEW)` ตอน DELETE ซึ่งเป็น null เสมอ** ⇒ `actor` หายทุกแถว
   วัดจริง: **แถว DELETE 3,557 แถว actor = null ครบ 100%** ทั้งที่เอกสารเขียนว่าใช้ได้
   → แก้ให้อ่าน `OLD` เมื่อเป็น DELETE แล้ว (แถวเก่ากู้ไม่ได้ ของใหม่มีชื่อครบ)
   **บทเรียน: เอกสารที่เขียนว่า "ทำแล้ว" ไม่เท่ากับวัดแล้ว — ไปนับใน DB ก่อนเชื่อเสมอ**
...
### เฟส 4 — เติม uid ให้ "ชื่อคนอื่น" ด้วยทะเบียนกลาง (2026-09-17)

**ปัญหาที่เหลือหลังเฟส 1-3:** wrapper เติม uid ได้เฉพาะตอนที่ "ชื่อที่เขียน = คนที่กำลังกด"
แต่คอลัมน์ส่วนใหญ่เก็บ **ชื่อคนอื่น** (ช่างที่มอบหมาย · ผู้อนุมัติ · ผู้ตรวจรับ)
⇒ ถ้าหยุดแค่เฟส 3 uid เกือบทั้งระบบจะเป็น null = **งานที่ทำมาแทบไม่มีผลกับรายงานจริง**

**ทำไมไม่ไล่แก้ 91 จุดที่เรียก `<PersonSelect>` ให้ส่ง `res.uid`:**
- แต่ละจุดต้องแก้ **2 ที่** — `onChange` กับจุดที่ประกอบ payload ส่ง DB (หน้า workflow ประกอบ `upd` รายขั้น) = ~180 จุดแก้มือ
- **ตกหล่นที่เดียว = แถวที่ชื่อกับ uid เป็นคนละคน** — แย่กว่าไม่มี uid
- จุดที่คนพิมพ์ชื่อเอง (`allowFree`) ก็ไม่ได้ uid อยู่ดี

**ที่ทำแทน — ทะเบียน "ชื่อ → uid" ใน `src/utils/actorStamp.js`** (เกณฑ์เดียวกับ backfill เฟส 3):
| | |
|---|---|
| ป้อนทะเบียน | `loadProfilesPeople()` ใน `src/utils/usePeople.js` — **จุดเดียว** (มีด่าน regressionGuards) |
| อุ่นตั้งแต่ login | `App.jsx` เรียก `loadProfilesPeople()` (cache ร่วมกับ picker — ไม่เพิ่มคิวรี) |
| เกณฑ์ | ชื่อ norm แล้ว**ตรงคนเดียวเท่านั้น**ถึงเติม uid · ซ้ำ/ไม่รู้จัก/ยังไม่โหลด → `null` |
| วัดจริง 17/09 | profiles 91 คน → ชื่อ norm ไม่ซ้ำ 90 · ซ้ำ 1 ชื่อ ("ธวัช พิมพ์วงศ์" 2 บัญชี) |

**ลำดับการ resolve ใน `applyStepActors()`:** หน้าส่ง uid มาเอง → ตัวคนกด → ทะเบียนชื่อ → `null`

**wrapper ครอบฝั่ง Main ด้วยแล้ว** — `withActorStamp()` ใน `supabaseClient.js` ใช้ร่วมกันทั้ง 2 client:
- DR → `DR_AUDIT_TABLES` (updated_by_*) + `DR_STEP_ACTORS` (88 คู่ · ตรวจคอลัมน์ครบแล้ว 17/09)
- Main → `MAIN_AUDIT_TABLES` (bbs_*) + `MAIN_STEP_ACTORS` (48 คู่ — LPA/OJT/QA/NPI/PE/WIP · ตรวจคอลัมน์ครบแล้ว)
- **ตารางที่ไม่อยู่ในลิสต์ = คืน query builder เดิมตรงๆ** (ไม่มีผลข้างเคียงกับ auth/การอ่าน)
- ⚠️ **เพิ่มคู่ในแมพไหนก็ตาม คอลัมน์ uid ต้องมีจริงในฐานก่อน** — ไม่งั้น write พัง 42703 ทั้งหน้า

ที่ได้: ไม่ต้องแก้ call site เลยสักจุด · ครอบทั้งชื่อที่เลือกจาก picker และชื่อที่พิมพ์เองที่บังเอิญตรงกับทะเบียน
(เทส 33 เคสใน `src/utils/__tests__/actorStamp.test.mjs` · crashsweep 71 หน้า พัง 0)

### ⏳ ยังไม่ทำ (เฟสถัดไป — ตั้งใจไม่ทำตอนนี้ ไม่ใช่ลืม)
- **`mtn_technicians` ยังเป็นทะเบียนคนชุดที่ 3 ที่พิมพ์มือ** — ควรผูกกับ `employees`/`profiles`
  (ต้นเหตุจริงของชื่อเพี้ยน — ชื่อที่สะกดต่างจาก profiles ทะเบียนกลาง resolve ไม่ได้ โดยตั้งใจ)
  · ช่องช่างซ่อมใน MtnRepair ยังเป็น `<select>` ไม่ใช่ `PersonSelect`
- **ยังไม่มีจอที่โชว์ "ชื่อที่ผูก uid ไม่ได้"** ให้หัวหน้ากดรวมคนเอง — ตอนนี้เห็นได้จาก SQL เท่านั้น
- **ชื่อที่สะกดผิดต่างจากทะเบียน** (พิมพ์มือ/คนนอกองค์กร) ยังได้ uid = null ตามการออกแบบ
  — การรวมคนเป็นการตัดสินใจของคน ห้ามให้ระบบเดา (ENGINEERING-PRINCIPLES §2)
