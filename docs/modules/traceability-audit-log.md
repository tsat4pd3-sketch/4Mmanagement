...
- **PersonSelect ~47 จุดใน 26 ไฟล์ยังทิ้ง `res.uid`** เก็บแค่ `res.name`
  (picker คืน `uid`/`employee_id`/`employee_code` มาให้อยู่แล้ว — แค่ call site ไม่ได้เก็บ)
  ⇒ ชื่อ "คนที่ถูกเลือก" (ช่างซ่อม/ผู้อนุมัติ/ผู้ตรวจรับ) ยังได้ uid เฉพาะจาก backfill
  **จุดที่ควรทำก่อน: MtnRepair (ใบ MO 7 ขั้น) · ScrapReport · OjtTraining · LayerProcessAudit**
- **`mtn_technicians` ยังเป็นทะเบียนคนชุดที่ 3 ที่พิมพ์มือ** — ควรผูกกับ `employees`/`profiles`
  (ต้นเหตุจริงของชื่อเพี้ยน) · ช่องช่างซ่อมใน MtnRepair ยังเป็น `<select>` ไม่ใช่ `PersonSelect`
