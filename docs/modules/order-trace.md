...
- **🗓 ค้นด้วยเลข Julian ที่ปั๊มบนชิ้นงาน (2026-09-15 · คำขอ user "เพิ่มให้หาจาก julian date ด้วยได้มั้ย จะได้ง่ายขึ้น"):** พิมพ์เลข Julian ลงช่องค้นหาเดิมได้เลย ไม่มีช่องใหม่/โหมดใหม่ · เกณฑ์ตีความอยู่ที่ **`src/utils/julianDate.js` ที่เดียว** (pure · เทส 12 เคส) **ห้ามแปลงวันเองในหน้า**
  · **รองรับ 3 รูปแบบพร้อมกัน แยกด้วยจำนวนหลัก** — `258` (3 หลัก = วันที่ N ของปี **ไม่บอกปี**) · `6258` (4 หลัก = เลขท้ายปี+วัน · แบบที่ค่ายรถใช้บ่อย) · `26258` (5 หลัก = ปี 2 ตัวท้าย+วัน) — **ยังไม่ได้ยืนยันกับหน้างานว่าที่นี่ใช้แบบไหน จึงรับทั้งหมด ห้าม hardcode แบบเดียว**
  · **รับเฉพาะตัวเลขล้วน 3-5 หลัก** เพื่อไม่ชนกับการค้น MAT (เลข SAP 8 หลัก) และ `prod_no` (มีตัวอักษร/ขีด) — เทสล็อกไว้ทั้ง `10100385` และ `MANUAL-…`
  ...
  · **ปีที่ระบบ "เดา" ต้องโชว์ + กดเปลี่ยนได้เสมอ** (หลัก "ไม่รู้ ≠ เดาแล้วเงียบ") — 3 หลักเดา "ครั้งล่าสุดที่ผ่านมาแล้ว" (วันที่ยังไม่ถึงในปีนี้ = ปีก่อน · สอบกลับมองย้อนหลังเสมอ) · 4 หลักเลขท้ายปีซ้ำทุก 10 ปี → เสนอรอบก่อนหน้าให้ด้วย · 5 หลักไม่ต้องเดา
  · **แถวผลค้นหาโชว์ Julian ของวันผลิตทุกแถว** (ไฮไลต์ส้มแถวที่ตรงวัน · tooltip บอกครบทั้ง 3 หลัก/4/5) ให้เทียบกับตัวปั๊มบนชิ้นงานได้ทันที
  · **⚠️ ระบบไม่มีคอลัมน์ Julian ที่ไหนเลย** (ตรวจ DR ทั้งฐาน 15/09) — ที่แสดง/ค้นทั้งหมด **derive สดจาก `production_sessions.work_date`** ไม่เก็บซ้ำ · ถ้าวันหลังมีการยิงเลข lot จริงลงชิ้นงาน (คนละตัวกับวันผลิต) ต้องเพิ่มคอลัมน์แล้วให้ตัวค้นเทียบคอลัมน์นั้นก่อน ค่อยถอยมา work_date
