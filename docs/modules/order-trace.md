...
- **🗓 ค้นด้วยเลข Julian ที่ปั๊มบนชิ้นงาน (2026-09-15 · คำขอ user "เพิ่มให้หาจาก julian date ด้วยได้มั้ย จะได้ง่ายขึ้น"):** พิมพ์เลข Julian ลงช่องค้นหาเดิมได้เลย ไม่มีช่องใหม่/โหมดใหม่ · เกณฑ์ตีความอยู่ที่ **`src/utils/julianDate.js` ที่เดียว** (pure · เทส 13 เคส) **ห้ามแปลงวันเองในหน้า**
  · **🔴 รูปแบบจริงของโรงงานนี้ = `DDDYY` + ตัวอักษรกะ (user ยืนยัน 15/09)** — `24726A` = วันที่ **247** ปี **26** (2026-09-04) กะ **A** · **`DDD` มาก่อน `YY` ห้ามสลับ** · **A = กะกลางวัน · B = กะกลางคืน** (มีหรือไม่มีก็ได้) · ย่อเหลือ `247` (วันอย่างเดียว) ก็รับ — ระบบเดาปีให้แล้วโชว์ว่าเดาปีไหน + กดเปลี่ยนปีได้
  · **⚠️ รอบแรกเขียนผิดเป็น `YYDDD` (26258) แก้แล้ววันเดียวกัน** — ถ้าอ่านสลับ เลขอย่าง `10026` (วัน 100 ปี 26) จะกลายเป็น "ปี 2010 วันที่ 26" = **ได้วันผิดแบบเงียบๆ อันตรายกว่าอ่านไม่ออก** · เทสล็อกเคสนี้ไว้แล้ว · **ห้ามเติมรูปแบบอื่นกลับเข้ามาโดยไม่มีคนยืนยันว่าโรงงานนั้นใช้จริง** (โรงงานอื่นในกลุ่มใช้คนละแบบได้ → ถึงตอนนั้นทำเป็น master ตั้งค่าต่อโรงงาน ไม่ใช่เดาเพิ่ม) · **4 หลักถือว่าไม่ใช่ Julian**
  · **รับเฉพาะเลข 3 หรือ 5 หลัก (+A/B)** เพื่อไม่ชนกับการค้น MAT (เลข SAP 8 หลัก) และ `prod_no` (มีตัวอักษร/ขีด) — เทสล็อกทั้ง `10100385` และ `MANUAL-…`
  · **ตัวอักษรกะแคบผลค้นให้เลย** — `…A` → `.eq('production_sessions.shift','day')` · `…B` → night
  · **🔴 ขา Julian ค้นทั้งฐาน ไม่ผูกกับช่วงวันงานด้านบน (ตั้งใจ)** — เลขที่ปั๊มบนชิ้นงานมักเก่ากว่าช่วง default 30 วัน ถ้ากรองตามช่วงจะได้ 0 ใบเงียบๆ แล้วคนอ่านว่า "ค้นไม่เจอ" · แถบสีส้มใต้ช่องค้นหาเขียนบอกไว้แล้ว
  · **ผลค้น = ใบของวัน Julian ขึ้นก่อน แล้วต่อด้วยใบที่ข้อความตรงคำค้น** (merge unique ด้วย id) — ไม่ทิ้งขาใดขาหนึ่ง
  · **แถวผลค้นหาโชว์เลข Julian ที่ควรปั๊มของใบนั้นทุกแถว** (`toJulian(work_date, shift)` → `24726A` · ไฮไลต์ส้มแถวที่ตรงวัน) ให้เทียบกับตัวปั๊มบนชิ้นงานได้ทันที
  · **⚠️ ระบบไม่มีคอลัมน์ Julian ที่ไหนเลย** (ตรวจ DR ทั้งฐาน 15/09) — ที่แสดง/ค้นทั้งหมด **derive สดจาก `production_sessions.work_date` + `.shift`** ไม่เก็บซ้ำ · ถ้าวันหลังมีการยิงเลข lot จริงลงชิ้นงาน (คนละตัวกับวันผลิต) ต้องเพิ่มคอลัมน์แล้วให้ตัวค้นเทียบคอลัมน์นั้นก่อน ค่อยถอยมา work_date
