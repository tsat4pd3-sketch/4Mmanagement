...
### 🗂️ ฟอร์มผังองค์กรต้องถามเฉพาะสิ่งที่หน่วยนั้นตอบได้ (2026-09-24 · feedback user)

> *"ลูกของ indirect ไม่น่าต้องเลือกไลน์ผลิตนะ"* — เคสจริง: กลุ่ม `Store Semi` ใต้ `PLN & STO › STORE`
> (indirect ทั้งสายขึ้นไป) แต่ฟอร์มแก้ไขกลุ่มยังถาม "ผูกกับไลน์ผลิตจริง (production_lines)"

- **`labor_type` ตั้งได้แค่ระดับ section/department** — กลุ่ม (`kind='line'`) ไม่มีคอลัมน์ของตัวเอง
  ⇒ ต้องไต่ขึ้นหาแม่ด้วย **`laborTypeOfNode(nodeId, nodes)`** (`src/utils/laborType.js`)
  **ห้ามอ่าน `node.labor_type` ตรงๆ แล้วสรุปว่า "ไม่ได้ตั้ง"** (กลุ่มทุกตัวจะเป็น null หมด)
- **ไม่มีบรรพบุรุษตัวไหนตั้งไว้ = คืน `null` ห้ามเดาแทน** — เดาให้ = ซ่อน/โชว์ช่องผิดโดยที่คนตั้งค่าไม่รู้
- **🔴 ซ่อนช่องได้เฉพาะตอนที่ยังไม่มีค่า** — ถ้ากลุ่มนั้นผูกไลน์ไว้แล้วต้องโชว์เสมอ + เตือนเหลืองว่าผิดปกติ
  · *ซ่อนช่องที่มีข้อมูลอยู่ = ข้อมูลถูกเก็บไว้แต่มองไม่เห็นและแก้ไม่ได้* (บทเรียนเดียวกับทะเบียนพนักงาน 23/09)
  · ของจริงมีทั้ง 2 เส้นทาง: Store Semi / Store Raw Material (ไม่ผูก) · `test line` (ผูก id 14)
- ⚠️ `labor_type` = แกน **ต้นทุน/หัวคน ราย*แผนก*** — **ห้ามเอาไปตัดสินสิทธิ์/ผู้รับแจ้งเตือน**
  และคนละแกนกับ `employees.staff_kind` (ราย*คน* · ขัดกันที่ช่าง MTN โดยตั้งใจ) → `docs/ORG-AXES-DECISION.md` §5.4


---

## 📐 ผังองค์กรทางการของ TSAT4 — เทียบกับที่ระบบเก็บ (2026-09-24)

> **ที่มา:** user ส่งเอกสารจริง 3 ใบ — `FM-HRM-1-00102 Organization TSAT4 Rev.03` (แม่แบบ HR) ·
> `ORG-001 TSAT4 Overall Rev.09` (ผังรวมโรงงาน) · `ORG-015 TSAT4 HDF Rev.06` (ผังย่อย PD3/HYDROFORM)
>
> ✅ **ได้ไฟล์ต้นฉบับ `ORG-001_TSAT4_Overall_Rev.09.xlsx` แล้ว (24/09)** — ถอดชื่อหน่วย/Cost/คน/เกรด
> ได้ครบทุกกล่อง และไล่สายบังคับบัญชาจากพิกัดคอลัมน์ของ shape ได้ → ดู §8

### 1. ลำดับชั้นทางการ = 4 ชั้น (คั่นด้วยเส้นประแดงในผัง)

```
บริษัท → ฝ่าย → ส่วน → แผนก → กลุ่ม
```

| ชั้นทางการ | ระบบเก็บที่ไหน | ตรงไหม |
|---|---|---|
| **ฝ่าย** | `org_divisions` (5 แถว) + `org_nodes.division` (ป้ายบนโหนด) | 🟡 จำนวนตรง (5) แต่เป็น**ป้าย ไม่ใช่โหนด** |
| **ส่วน** | `org_nodes kind='section'` (6) | ✅ |
| **แผนก** | `org_nodes kind='department'` (14) | ✅ |
| **กลุ่ม** | `org_nodes kind='line'` (19) | ✅ (จอเขียน "Group / กลุ่ม" ถูกแล้ว) |
| — | `org_nodes kind='team'` (12) | ⚠️ **ไม่มีในผังทางการ** — ของเราคือ *ทีมกะ A/B/C* คนละแกน (ดู §5) |

**แต่ละกล่องในผังทางการบรรจุ:** ชื่อหน่วย · **Cost Center** · ชื่อคน · ตำแหน่ง **(เกรด)** · อัตรากำลัง · อัตราทดแทน
⇒ เรามี cost center บนโหนดแล้ว ✅ · **ขาด: เกรด · ผู้รับผิดชอบระดับฝ่าย · อัตรากำลัง/ว่าง**

### 2. 🎓 บันไดเกรดทางการ (จากแม่แบบ sheet "สายโรงงานระดับส่วน") — **ระบบยังไม่มีแกนนี้เลย**

| ตำแหน่ง | เกรด |
|---|---|
| ที่ปรึกษา (Advisor) | `VD` |
| ผู้อำนวยการ | `D1-D3` |
| ผู้จัดการทั่วไป / ผู้ชำนาญการพิเศษ | `G1-G3` |
| ผู้จัดการ / ผู้ชำนาญการ | `M1-M3` |
| **หัวหน้าส่วน** | `S1` |
| **หัวหน้าแผนก** | `S2-S3` |
| วิศวกร / วิศวกรอาวุโส · เจ้าหน้าที่ / เจ้าหน้าที่อาวุโส · เจ้าหน้าที่เทคนิค | `S1-S3` |
| **หัวหน้ากลุ่ม** · ช่างเทคนิค · พนักงาน | `T1-T3` |
| พนักงานทั่วไป | `T6-T8` |
| พนักงานชั่วคราว | `Y1` |

🔴 **เลขน้อย = สูงกว่า ภายในตัวอักษรเดียวกัน** (หัวหน้าส่วน `S1` สูงกว่า หัวหน้าแผนก `S2-S3` ·
พนักงานทั่วไป `T6-T8` ต่ำกว่า `T1-T3`) — **ห้ามเรียงจากมากไปน้อย**
ยืนยันจากผังรวม: `ส่วน Maintenance … รก.ผู้จัดการ(S1)*` = คนเกรด S1 รักษาการตำแหน่ง M (สูงกว่า)

> **`positions.level` ของเรา (operator/technician/engineer/leader/supervisor/manager/staff)
> เป็นของที่เราคิดเอง ไม่ใช่ของบริษัท** — "ระดับพนักงาน" ที่ user พูดถึงคือ **เกรด** ตารางนี้
> ⇒ ถ้าจะให้ `scope_depth` ผูกกับ "ระดับ" ได้จริง ต้องผูกกับเกรด ไม่ใช่ level ที่เราตั้งเอง

### 3. 🔄 รักษาการ (Acting) — **ระบบไม่มีเลย และเกี่ยวกับสิทธิ์โดยตรง**

แม่แบบกำหนดสัญลักษณ์หลังตำแหน่ง:
| | ความหมาย |
|---|---|
| `*` | รักษาการในตำแหน่งที่ **สูงกว่า** ตำแหน่งจริง |
| `**` | รักษาการในตำแหน่งที่ **เท่ากับหรือต่ำกว่า** ตำแหน่งจริง |
| (อีกหมายเหตุ) | รักษาการในตำแหน่งที่สูงกว่า **อยู่ Cost Center เดียวกัน** |

⚠️ **นี่คือ "สิทธิ์ชั่วคราว"** — คนรักษาการหัวหน้าส่วนต้องเห็นกว้างเท่าหัวหน้าส่วน *ชั่วคราว*
วันนี้เราทำได้ทางเดียวคือ**เปลี่ยน role ถาวร แล้วลืมเปลี่ยนกลับ** = สิทธิ์ค้าง
ซึ่งเป็นสิ่งที่ ISO 27001 A.5.18 จับโดยตรง (ดู `docs/ACCESS-CONTROL-STANDARDS.md` §6)

### 4. 📊 อัตรากำลัง vs คนจริง — ผังทางการเก็บ "ตำแหน่ง" ไม่ใช่ "คน"

กล่องกลุ่มในผัง PD3 เขียนเป็นบรรทัดละ **ตำแหน่ง + (เกรด) + (จำนวนคน)**:
```
.....(ว่าง).....หัวหน้ากลุ่ม.....(T3).....
พนักงานทั่วไป.....(T7).....(1).....
พนักงานชั่วคราว...(Y1)...(2).....
```
`(ว่าง)` = ตำแหน่งว่าง (แม่แบบสั่ง "กรณีมีตำแหน่งว่าง ไม่ต้องระบุข้อมูลใดๆ")
⇒ ผังทางการ = **ผังอัตรากำลัง (establishment)** · ระบบเรา = **ทะเบียนคนจริง (actual)** เท่านั้น
⇒ ตอบไม่ได้ว่า "ขาดคนกี่ตำแหน่ง" — คนละเรื่องกับสิทธิ์ แต่ควรรู้ว่าเราไม่มี

### 5. ⚠️ จุดที่ชื่อไม่ตรงกัน — ต้องยืนยันกับ HR ก่อนแก้

| ผังทางการ | ระบบเรา (`org_divisions`) |
|---|---|
| `ฝ่าย Logistic and Sales` | `logistic` = **"ฝ่ายวางแผน-คลัง"** |

และ **`team` 12 โหนดของเราไม่ใช่ชั้นในผังองค์กร** — เป็น *ทีมกะ A/B/C* (แกนหมุนกะ)
**ห้ามเอาไปเทียบกับผังทางการแล้วสรุปว่าผังเราเกิน** · ควรเขียนกำกับในจอ `/org-setup` ว่าชั้นนี้เป็นของเรา

### 6. สรุปช่องว่างที่ต้องตัดสินใจ

| ช่องว่าง | เกี่ยวกับงานสิทธิ์ไหม | ข้อเสนอ |
|---|---|---|
| **เกรดทางการ (D/G/M/S/T/Y)** | 🟡 ใช่/ไม่ — ดู §7 | ✅ **ทำแล้ว 24/09** |
| **รักษาการ (acting)** | 🔴 ใช่ — สิทธิ์ชั่วคราวที่ต้องหมดอายุ | ทำต่อจากเกรด |
| **ฝ่ายเป็นโหนดจริง (มี cost center + หัวหน้า)** | 🟡 บางส่วน — `scope_depth=branch` ที่ฝ่ายใช้ได้อยู่แล้วผ่าน `orgScope` | ทำเมื่อต้องการ cost center ระดับฝ่าย |
| **อัตรากำลัง / ตำแหน่งว่าง** | ❌ ไม่ | ไว้ทีหลัง (งาน HR) |
| ชื่อฝ่ายไม่ตรง · สายสำนักงาน vs สายโรงงาน | ❌ ไม่ | ยืนยันกับ HR ก่อน |


### 7. 🎓 แกนเกรด — ทำแล้ว 24/09 (และสิ่งที่พบระหว่างทำ)

**ของที่ลงไปแล้ว** (`migration 20260924_grades_registry.sql` · apply แล้ว):
- ตาราง `grades` 20 แถว (VD · D1-D3 · G1-G3 · M1-M3 · S1-S3 · T1-T3 · T6-T8 · Y1)
  มีคอลัมน์ **`rank` (มาก = สูง)** เพราะโค้ดเรียงเองไม่ได้
- `employees.grade` (ว่างทั้งหมดวันที่ apply ⇒ ไม่มีตัวเลขไหนขยับ)
- `positions.grade_codes[]` = เกรดที่แม่แบบกำหนดต่อตำแหน่ง (จับคู่ได้ 12 ตำแหน่ง · ที่เหลือ null **ไม่เดา**)
- `src/utils/grades.js` + เทส · ช่องเลือกเกรดใน `/operator` (เตือนเมื่อไม่ตรงตำแหน่ง **ไม่บล็อก**)

> #### 🔴 ข้อค้นพบสำคัญ: **เกรดใช้ตัดสินขอบเขตการมองเห็นไม่ได้**
> `S1` เป็นได้ทั้ง **หัวหน้าส่วน** และ **วิศวกรอาวุโส** — เกรดเท่ากันแต่ขอบเขตคนละเรื่อง
> ⇒ **`scope_depth` ต้องมาจาก "ตำแหน่ง" ไม่ใช่ "เกรด"** (ที่ `src/utils/scopeDepth.js` ทำอยู่ถูกแล้ว)
> ⇒ เกรดตอบเรื่องอื่น: ผลิตผังทางการ · ตรวจเกรดไม่ตรงตำแหน่ง · **ตัดสินเครื่องหมายรักษาการ**

**เครื่องหมายรักษาการที่ถอดจากแม่แบบ** (`actingMark()` ใน `grades.js`):

| เงื่อนไข | เครื่องหมาย |
|---|---|
| รักษาการตำแหน่งที่**สูงกว่า** | `*` |
| รักษาการตำแหน่งที่สูงกว่า **แต่ cost center เดียวกัน** | *(ไม่ใส่)* |
| รักษาการตำแหน่งที่**เท่ากับหรือต่ำกว่า** | `**` |

⚠️ ช่องเครื่องหมายของเคสกลางในแม่แบบเขียนว่า `(ไม่ระบุ*)` ซึ่งผมตีความว่า "ไม่ต้องใส่เครื่องหมาย"
— **ยังไม่ได้ยืนยันกับ HR** ถ้าผิดให้แก้ที่ `ACTING_MARKS` จุดเดียว

**ยังไม่ได้ทำ:** ตัวฟีเจอร์รักษาการจริง (ใครรักษาการตำแหน่งไหน · ถึงวันไหน · ได้ขอบเขตเพิ่มชั่วคราว)
— นี่คือ "สิทธิ์ชั่วคราวที่ต้องหมดอายุ" ตาม ISO 27001 A.5.18 · เป็นงานขั้นถัดไป


### 8. 🏢 ผังองค์กรจริงของ TSAT P4 (ORG-001 Rev.09) — ถอดจากไฟล์ต้นฉบับ 24/09

**ยอดบนสุด:** `HEAD OF SPG TSAT` — นายพัฒนา ดอนชัย · **ผู้อำนวยการ**
**ถัดมา:** `ฝ่าย Steel Auto Office Plant 4` Cost `2140900002` — นายชัยยุทธ พรหมภา · **ผู้จัดการทั่วไป (G2)**

#### ฝ่าย 5 ฝ่าย → ส่วน 14 ส่วน (จัดกลุ่มจากพิกัดคอลัมน์ในไฟล์)

| ฝ่าย | Cost | ส่วนใต้ฝ่าย (Cost · หัวหน้า · ตำแหน่ง) |
|---|---|---|
| **Logistic and Sales** | 2140320000 | `Log&Sales/Planning&Store` 2140429000 · วิทยา คำสุขุม · ผจก.(M1)<br>`W/H&DEL/Rack Center` 214025000 · ปราโมช ดอยลอม · **รก.**ผจก.(S2)`*` |
| **Engineering** | 2140340000 | `Quality Assurance` 2140441000 · เด่นดวง ดวงธนู · ผจก.(M2)<br>`Process Engineering` 2140446000 · เพ็ญศิริ วงศ์ภักดี · **รก.**ผจก.(S1)`*` |
| **Production** | 2140360000 | `Assembly 1` 2140470000 · ศิริพร แซ่ก๊วย · **รก.**ผจก.(S1)`*`<br>`Press Production` 2140461000 · ศักดา กาละศรี · **รก.**ผจก.(S1)`*`<br>`Hydroforming` 2140462000 · ดุลยทรรศน์ ลาภธนศาลสมบัติ · ผจก.(M2)<br>`Assembly 2` 2140471000 · ดุลยทรรศน์ (คนเดิม) · ผจก.(M2)<br>`Tooling MTN.` 2140459100 · ศักดา (คนเดิม) · **รก.**ผจก.(S1)`*` |
| **Steel Auto Office Plant 4** | 2140360000 | `Maintenance` 2140456000 · อำพรรณ์ กมลรักษ์ · **รก.**ผจก.(S1)`*` |
| **สนับสนุนกลาง** | — | `สนับสนุนบัญชี` 2100413500/600 · พัชรียา · ผจก.(M1)<br>`สนับสนุน HRM` 2100415024 · ขวัญใจ · ผจก.(M2)<br>`จัดซื้อ` 2100414300 · ศิรธันย์ · ผจก.(M1)<br>`CIC` 2100416124 · เกษราภรณ์ · Specialist B<br>`QSM` 2100418020 · โกวิทย์ · **รก.**ผจก.(S1)`*` |

#### 🔴 3 ข้อเท็จจริงที่เปลี่ยนลำดับความสำคัญของงานสิทธิ์

**1. 7 จาก 14 ส่วน (50%) หัวหน้าเป็น "รักษาการ"**
ทุกตัวเป็น `รก.ผู้จัดการ (S1/S2)*` = คนเกรด S ทำหน้าที่ตำแหน่งเกรด M
⇒ **"รักษาการ" ไม่ใช่เคสหายาก แต่เป็นสภาพปกติของโรงงานนี้**
⇒ ระบบที่ไม่รองรับสิทธิ์ชั่วคราว = ครึ่งหนึ่งขององค์กรถูกบังคับให้ตั้งสิทธิ์ถาวรแบบผิดๆ

**2. คนเดียวคุมหลายส่วนพร้อมกัน — โมเดล "1 คน 1 หน่วย" ของเราใช้ไม่ได้**
- ศักดา กาละศรี = `Press Production` **+** `Tooling MTN.`
- ดุลยทรรศน์ = `Hydroforming` **+** `Assembly 2`
⇒ `employees.org_node_id` (คอลัมน์เดียว) เก็บได้แค่หน่วยเดียว — ถูกสำหรับ "สังกัด"
  แต่ **"คุมหน่วยไหนบ้าง" เป็นอีกเรื่อง** ต้องเป็นหลายค่า (วันนี้ `profiles.sections[]` ทำหน้าที่นี้อยู่)

**3. ฝ่ายในระบบเรา 2 ใน 5 ไม่มีอยู่จริงในผัง**
| `org_divisions` ของเรา | ในผังจริง |
|---|---|
| `production` ฝ่ายผลิต | ✅ ฝ่าย Production |
| `logistic` **"ฝ่ายวางแผน-คลัง"** | 🟡 ชื่อจริง **"Logistic and Sales"** |
| `office` ฝ่ายสำนักงาน | 🟡 น่าจะ = Steel Auto Office Plant 4 หรือ สนับสนุนกลาง |
| `quality` **ฝ่ายคุณภาพ** | 🔴 **ไม่มีฝ่ายนี้** — QA เป็น *ส่วน* ใต้ฝ่าย Engineering |
| `maintenance` **ฝ่ายช่าง** | 🔴 **ไม่มีฝ่ายนี้** — Maintenance / Tooling MTN เป็น *ส่วน* |
| *(ไม่มี)* | 🔴 ขาด **ฝ่าย Engineering** |

#### เทียบ PD1-PD4 กับส่วนจริง — **แก้ตรงกันครบแล้ว 24/09**

| ของเรา | Cost | ส่วนจริงตามผัง | หัวหน้า |
|---|---|---|---|
| **PD1** | `2140461000` | Press Production | ศักดา ✅ |
| **PD2** | `2140470000` | **Assembly 1** | ศิริพร ✅ |
| **PD3** | `2140462000` | Hydroforming | ดุลยทรรศน์ ✅ |
| **PD4** | `2140471000` | **Assembly 2** (GOR · LWRBAR) | ดุลยทรรศน์ ✅ |
| **PLN & STO** | `2140429000` | Log&Sales/Planning&Store | วิทยา |
| `TEST` | `21404444` | *(ข้อมูลทดสอบ ไม่มีในผัง)* | — |

**สิ่งที่ผิดคือ cost center เท่านั้น — การผูกตัวคนถูกมาตลอด**
เดิม PD2 ถือเลขของ Assembly 2 · PD4 ถือเลข `2140472000` ที่ไม่มีอยู่ในผังเลย
⇒ ทุกจอที่จับคู่หน่วยงานด้วย cost center (KPI การเงิน · Obeya · ต้นทุน) ชี้ผิดส่วนมาตลอด
· แก้ด้วย `20260924_fix_pd2_pd4_cost_center.sql` (user เคาะ 24/09)
· 🔴 **ลำดับสำคัญ** — ต้องปล่อย `2140471000` จาก PD2 ก่อนแล้วค่อยให้ PD4 (`cost_centers.code` เป็น PK)

> ✅ **ปิดแล้ว 24/09 — `2140470000` (10 หลัก) ถูกต้อง** · user พิมพ์ยืนยันเองว่า
> *"PD2 = 2140470000 , PD4 = 2140471000"* ตรงกับผังทางการ (ที่ 9 หลักก่อนหน้าคือพิมพ์ตกไป 1 ตัว)

> 🔴 **บทเรียนข้าม session — ห้ามเดารหัส master จาก "แพตเทิร์นของตัวเลข"**
> มี session ที่แก้ PD4 เป็น `2140472000` เพราะรหัสส่วนงานดู*เหมือน*เดินเป็นคู่ (461/462 · 471/472)
> และช่อง 472 ว่างพอดี — เข้าเค้ามากแต่ **ผิด** เพราะเลขจริงเรียงตาม**ส่วนงานในผัง**
> (Press · Assembly 1 · Hydroforming · Assembly 2) ไม่ใช่ตามชื่อ PD1-PD4 ที่เราตั้งเอง
> ⇒ **รหัส cost center / รหัสหน่วยงาน อ้างได้จากเอกสารผังทางการเท่านั้น** (ORG-001 · §8 นี้)
> · user พิมพ์ว่า "น่าจะ" = ยังไม่ยืนยัน ⇒ ต้องเปิดผังก่อน ไม่ใช่รีบเขียนลงฐาน

#### ✅ จุดที่ผังยืนยันว่าเราตั้งถูกแล้ว
- **ชัยยุทธ พรหมภา = ผู้จัดการทั่วไป (G2) ของทั้งโรงงาน** ⇒ `scope_depth = all` **ถูกต้อง**
  (แต่วันนี้มาแบบ `legacy_open` = บังเอิญถูก ไม่ใช่ตั้งใจ — ควรกดยืนยันให้เป็น `manual`)
- **อำพรรณ์ = รก.ผจก. ส่วน Maintenance** ⇒ `role = mtn` ได้ `all` ตามนโยบาย
  *"หน่วยงานช่างทั้งหมดต้องเห็นของทั้งโรงงาน"* (คำสั่ง user 2026-08-19) ✅


### 9. 👔 หัวหน้าหน่วย / รักษาการ — `org_assignments` (2026-09-24)

ผังจริงพิสูจน์ 2 เรื่องที่โมเดลเดิมรับไม่ได้ ⇒ ต้องมีตารางแยก:
1. **7 จาก 14 ส่วน (50%) หัวหน้าเป็นรักษาการ** — ไม่ใช่เคสขอบ
2. **คนเดียวคุมหลายหน่วย** (ศักดา = Press Production + Tooling MTN)

| คำถาม | เก็บที่ไหน |
|---|---|
| "คนนี้**สังกัด**หน่วยไหน" (1 ค่า) | `employees.org_node_id` |
| "คนนี้**คุม**หน่วยไหนบ้าง" (หลายค่า + มีวันหมดอายุ) | **`org_assignments`** |

**กติกาของตาราง:**
- `kind` = `head` (หัวหน้าตัวจริง) / `acting` (รักษาการ — ชั่วคราวโดยนิยาม)
- 🔴 **หมดอายุคิดตอนอ่าน (`ends_on < วันนี้`) ไม่มี cron** — cron ที่ไม่ได้รัน = สิทธิ์ค้างเงียบ
  ซึ่งคือปัญหาที่ตารางนี้มาแก้พอดี
- 🔴 **ห้ามใส่วันสิ้นสุดให้เองโดยการเดา** — ผังจริงไม่ได้เขียนไว้เลยสักใบ
  `acting` + `ends_on` null = สถานะ `open_ended` = **คิวงานที่ต้องตามเก็บ** (ISO 27001 A.5.18)
- `source='org_chart_rev09'` + `confirmed_at` null = ระบบใส่จากผัง **ยังไม่มีคนยืนยัน**
- ฟังก์ชันทุกตัวใน `src/utils/orgAssignments.js` **รับ `today` เข้ามาได้** (กันเทสระเบิดเวลา)

**seed จากผัง 4 แถว** (เฉพาะที่ cost center ตรงเป๊ะ + ชื่อคนตรง):
ศักดา→PD1 (acting) · ศิริพร→PD2 (acting) · ดุลยทรรศน์→PD3, PD4 (head)
ส่วนที่เหลือในผัง (Process Engineering · W/H&DEL · QSM · CIC · จัดซื้อ · HRM · บัญชี · Tooling MTN)
**ยังไม่มีโหนดในระบบ** จึง seed ไม่ได้ — เพิ่มเองจากแผง 👔 ใน `/org-setup`

> ### ⚖️ ทำไมการ "เพิ่มขอบเขต" ตรงนี้ไม่ขัดกฎ role-centric
> กฎใน `docs/ACCESS-CONTROL-STANDARDS.md` §2 ห้าม **อนุมานสิทธิ์กว้างจากการเว้นว่าง** (fail-open)
> ซึ่งคนละเรื่องกับตารางนี้:
> · เป็น**ข้อมูลที่คนกรอกชัดเจน** ไม่ใช่การเดาจากช่องว่าง
> · ขยาย**ขอบเขต (เห็นที่ไหน)** ไม่ได้ขยาย**สิทธิ์ (ทำอะไรได้)** — เพดานยังเป็น role เหมือนเดิม
> · และ**มีวันหมดอายุ** ซึ่ง fail-open ไม่มี

**🔴 ยังไม่ต่อเข้าตัวคำนวณขอบเขตจริง** — เก็บ/แสดง/เตือนเท่านั้น
ต่อสายพร้อม `scope_depth` ครั้งเดียว (`docs/ORG-AXES-DECISION.md` §7.6)
