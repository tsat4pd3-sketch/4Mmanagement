...
- **⚙️ แท็บ "รายอุปกรณ์ (MTTR/MTBF)" — Downtime · MTTR · MTBF รายตัว (2026-09-11 · คำขอทีม MTN)**
  ...
  - **🔴 ให้ตรงนิยามเดียวกับ %A ของ Daily Report (audit 2026-09-14 · คำถาม user "logic การนับดาวไทม์ต่างกันตรงไหน")**
    ...
    3. **ยังไม่แก้ (ของเดิม ไม่ใช่ของแท็บนี้):** `computeLiveOee` **ไม่หัก planned DT ออกจากตัวหาร และไม่หักเวลาพัก** ⇒ %A สดกับ %A ตอนปิดกะเป็นคนละสูตร ตัวเลขกระโดดตอนปิดกะ — รอ user สั่ง (แตะ `oee.js` = กระทบทุกจอ ต้องเทียบเลขก่อน-หลัง + crashsweep)
  - **"ตั้งแต่ผลิตแจ้งจนกลับมารันได้" วัดได้แค่ยอดรวม** — หน้าต่างเวลาทั้ง 3 ตัวเหมือนกันคือ `started_at → ended_at` (รวมเวลารอช่าง) แต่**แยกช่วงย่อยไม่ได้**: `call_mtn` ใช้จริง 17 จาก 8,429 แถว · `call_mtn_ack_at` **ไม่เคยถูกใช้เลย** · `fix_at` 93 แถว
    · และเวลาต้นทางเป็นการ**นึกย้อนกรอก**: `created_at − started_at` มัธยฐาน 93 นาที (5,197 จาก 8,435 แถว ช้ากว่า 1 ชม.) ⇒ MTTR ละเอียดได้ระดับ "สิบนาที" ไม่ใช่ระดับนาที · อยากได้ MTTR แยก "รอช่าง vs ซ่อมจริง" แบบโปรแกรมส่วนกลาง **ต้องบังคับกรอก `call_mtn` ตอนเรียกช่างก่อน**
  - **ยังไม่ทำ:**
