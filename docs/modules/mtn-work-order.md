...
## ➡️ ส่งต่องานข้ามทีมช่าง — ไม่ต้องเปิดใบใหม่ (2026-09-14 · คำสั่ง user)

feedback: *"ช่างฝ่ายผลิตเข้าไป action ในรอบแรกและไม่สามารถแก้ไขได้ สามารถส่งใบต่อไปให้ช่างส่วนที่เกี่ยวข้อง
ดำเนินงานต่อ โดยเห็นรายละเอียดจากช่างฝ่ายผลิตที่เข้าไปดูอาการเบื้องต้นมาก่อนแล้ว · ตอนนี้กลายเป็นว่า
พนักงานจะเปิดใบใหม่ไปให้ช่างเฉพาะทางอีก"* → ตรงกับกล่อง **"แก้ไขได้? → ไม่ใช่ → แจ้งขอเปิดส่งต่อให้
หน่วยงานรับผิดชอบ"** ในผังทางการ ซึ่งระบบไม่เคยมี

**ขนาดปัญหา:** ใบทีม `production` (ช่างฝ่ายผลิต first-response) = 291 ใบ = **91%** ของใบทั้งหมด
⇒ ทางที่ขาดคือทางที่คนเดินบ่อยที่สุด · ผลของการเปิดใบใหม่ = ประวัติขาดคนละใบ + KPI งานเดิมค้าง
+ ทีมใหม่เริ่มจากศูนย์ ไม่เห็นว่าใครดูอะไรมาแล้ว

**⚠️ ทำไมไม่ขยาย `canBounceBack` (ตีกลับ) ให้ถึงขั้น 3 — คนละความหมาย ห้ามรวมกัน:**
| | ตีกลับ (`returned`) | ส่งต่อ (`mtn_order_handoffs`) |
|---|---|---|
| ความหมาย | แจ้งผิดแผนกตั้งแต่แรก | แจ้งถูกแล้ว แต่ทีมแรกแก้ไม่ได้ |
| ใบไปไหน | ถอยไปหา **ผู้แจ้ง** (ขั้น 1) | ไปหา **ทีมใหม่** (รอรับงาน ขั้น 2) |
| ผลงานที่ทำมา | ยังไม่มี (ล็อกที่ขั้น ≤2 เพราะถอยจากขั้น 3 แล้วลายเซ็นหาย) | **เก็บเป็น snapshot** แล้วโชว์ให้ทีมใหม่ |
| ใครกด | ทีมช่างที่ได้รับใบ | ทีมที่ถือใบอยู่ (ขั้น 2-3) |

**โครงสร้าง:** ตาราง `mtn_order_handoffs` (DR) — 1 แถว = 1 ครั้ง เก็บ from/to dept · เหตุผล (บังคับ) ·
ผู้กด/เวลา · **snapshot ผลตรวจเบื้องต้น** (`found_root_cause`/`found_solution`/`found_after_img`/ช่าง/เวลา)
+ `mtn_orders.handoff_count` · RLS: select+insert เปิด anon (ตาม convention DR) **ไม่เปิด update/delete**
— ประวัติการส่งต่อเป็นหลักฐาน แก้ไม่ได้

**พฤติกรรมตอนกดส่งต่อ:**
1. insert snapshot ก่อน — **ล้มตรงนี้ต้องหยุด ห้ามล้างข้อมูลทิ้ง** (`checkWrite` + early return)
2. ย้ายใบ: `mtn_dept`=ทีมใหม่ · `status='pending'` · `current_step=1` · `handoff_count+1`
   · `report_at`=now (KPI ทีมใหม่เริ่มนับจากตอนรับ — หลักเดียวกับ resubmit) เก็บ `first_report_at`
   · ล้างเฉพาะช่องทำงานของทีมเดิม (accept/assign/root_cause/solution/tech/after_img)
     **ของที่ผู้แจ้งกรอก (อาการ/รูปก่อนซ่อม/ไลน์) ต้องอยู่ครบ**
3. ยิง `mtn_reported` ให้ทีมใหม่ (ใช้ rule เดิม ไม่ต้องเพิ่ม event)

**⚠️ เลข MO เดิมไม่เปลี่ยน** — ใบเดียวกัน งานเดียวกัน สอบกลับเส้นเดียว (`mtn_assign_mo_no` idempotent
ทีมใหม่กดขั้น 2 จึงไม่ได้เลขใหม่) · **ผลข้างเคียงที่ยอมรับ: รหัสทีมใน `mo_no` ยังเป็นของทีมแรก**
(เช่น PRD-… ทั้งที่ MTN ทำ) ⇒ **ดูทีมปัจจุบันจาก `mtn_orders.mtn_dept` เสมอ ห้ามอ่านทีมจาก prefix ของ mo_no**

· เกณฑ์ว่าส่งต่อได้ไหม = `canHandoff()` ใน `mtnStepPerm.js` (ขั้น 2-3 · ไม่ใช่ closed/rejected/returned) + สิทธิ์ขั้น 3
· แผง "🔎 ผลตรวจเบื้องต้นของทีมก่อนหน้า" โผล่ใน DetailDrawer ทุกครั้งที่ `handoff_count > 0`
· เทส `canHandoff` 2 เคสใน `mtnStepPerm.test.mjs`
...
- **🔴 ใบกองที่ขั้น 6 "รอรับมอบ" 140 ใบ — ไม่ใช่เรื่องสิทธิ์ แต่ "ไม่มีใครถูกบอกว่าถึงคิวตัวเอง" (2026-09-09 · คำสั่ง user "เร่งทำเลย")**
  ...
  - ⚠️ **ยังไม่ได้ทำ:** `mtn-daily-summary` `WAIT_ORDER` ไม่มีคีย์ `returned` ⇒ ใบที่ถูกตีกลับถูกดึงมาแล้ว**หายจากทุกบล็อกในสรุปเช้า** (พบระหว่างงานนี้ ไม่ได้แก้เพราะนอกขอบเขต)
