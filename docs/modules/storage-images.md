...
>   (ปุ่ม 🗜️ ที่ `/linesetup` · `employees.image_url` · เฉพาะไฟล์รากของ bucket · ข้าม `layouts/`+`factory/`+`.gif`)
...
>
> #### 🔴 บทเรียนรอบที่ 3 — "cache ต่อแท็บ" ไม่พอ ต้อง cache **ข้ามการเปิดแอป** (14 ก.ย. 2026)
> วัดจาก log วันอาทิตย์ (โรงงานหยุด ไม่มีใครทำงานเลย) — ตัวเลขที่ยังเหลือหลังแก้รอบ 2:
> | คิวรี | ขนาด/ครั้ง | ครั้ง/วัน | รวม |
> |---|---|---|---|
> | `machines?select=*` | **368 KB** | 329 | 121 MB |
> | `kanban_standards?select=*,dr_products(...)` | 164 KB | 329 | 54 MB |
> | `dr_products?select=*` | 107 KB | 329 | 35 MB |
> | `notification_settings` (VAPID config) | เล็ก | **2,490** | — |
>
> **≈ 200 MB/วัน ในวันที่ไม่มีใครทำงาน ≈ 6 GB/เดือน > โควต้าทั้งเดือน (5 GB)**
> ทั้ง 3 ตัวยิงพร้อมกัน 329 ครั้งเท่ากันเป๊ะ = `loadMasters` ของ **Daily Report** ตอนเปิดหน้า
> `cachedMaster` ทำงานถูกแล้ว แต่ cache อยู่ใน memory ของแท็บ ⇒ **เปิดแอปใหม่/กด F5 = โหลดใหม่ทั้งชุด**
> (โรงงานใช้ผ่านมือถือ/แท็บเล็ตหน้าไลน์ = เปิด-ปิดแอปทั้งวัน — สมมติฐาน "เปิดค้างทั้งกะ" ผิด)
> **แก้แล้ว:** `masterCache` ย้ายลง **localStorage** (TTL 4 ชม. เท่าเดิม) — เทส 9 เคสใน
> `__tests__/masterCache.test.mjs` · `send-push` cache VAPID ในตัว function (deploy v12)
> · ⚠️ **สิ่งที่เปลี่ยนไป: กด F5 ไม่ล้าง cache อีกแล้ว** (นั่นคือจุดประสงค์) — ทางล้างที่เหลือคือ
>   `invalidateMaster()` (หน้าที่แก้ master เรียกเอง · ล้าง localStorage ด้วย) · **deploy ใหม่ = ล้างทั้งหมด**
>   (เทียบ build stamp) · หรือรอ TTL · **ห้ามถอด 3 ทางนี้ออก** ไม่งั้นข้อมูลค้างแล้วแก้ไม่ได้เลย
> · **ยังไม่ทำ (หนี้ที่มองเห็น):** `DailyReport` ยังใช้ `machines.select('*')` — เอาเฉพาะคอลัมน์ที่ใช้จริง
>   เหลือ 208 KB (ลด 43%) แต่โค้ดอ้างหลายคอลัมน์กระจาย ตัดผิดแล้วจอพังเงียบ ⇒ ทำตอนไปแตะหน้านั้นครั้งหน้า
>
> #### 📊 ตัวเลขจริงจาก Supabase Usage (14 ก.ย. 2026) — พิสูจน์ว่าการแก้ได้ผล
> รอบบิล 11 ก.ย.–11 ต.ค. (Pro): **Egress 0.88 GB / 250 GB** · Cached Egress 0.33 GB · Storage 0.286 GB
> | วัน | Egress | Cached Egress | หมายเหตุ |
> |---|---|---|---|
> | 11 ก.ย. | **569 MB** | **287 MB** | วันที่ถูกล็อกบริการ (ก่อนแก้) |
> | 12 ก.ย. | 160 MB | 30 MB | หลัง fix cache-control รูป |
> | 13 ก.ย. | **30 MB** | 5 MB | เสาร์ |
> | 14 ก.ย. | 140 MB | 5 MB | อาทิตย์ (ตรงกับที่คำนวณไว้ = master 3 ตัว) |
>
> **cached egress 287 → 5 MB (−98%)** = กฎ `uploadOpts()` ทำงานเต็มที่ · **ห้ามถอด**
>
> #### 🖥️ งบใหม่เมื่อจะมี 10+ จอ TV (เป้าหมาย user: ยกเลิก Pro กลับ Free 5 GB · 2026-09-14)
> วัด payload สดของ `FactoryMap.loadStatus` ใหม่: **26 KB/รอบ** (prod_orders 17 KB = 65% ของก้อน)
> · ที่ความถี่เดิม 5 นาที = **0.26 GB/เดือน/จอ จาก loop เดียว** และ FactoryMap มี **6 loop**
> ⇒ 10 จอ ≈ **4-6 GB/เดือน = เกินโควต้า Free ทั้งก้อน**
> (งบเดิม ส.ค. เขียนว่า 0.24 GB/จอ — **นับแค่ loop เดียว** นั่นคือจุดที่ประเมินพลาด)
>
> **แก้แล้ว: ยืดทุกระดับใน `refreshRates.js` ×2-3** — ANDON/BOARD 5→15 นาที · ANALYTIC 10→20
> · BACKUP 15→30 · SLOW 30→60 ⇒ **~0.15-0.2 GB/เดือน/จอ → 10 จอ ≈ 1.5-2 GB** เหลือให้มือถือ/PC ~3 GB
> · ยืดได้โดยไม่เสียอะไรเพราะ **แจ้งเตือนจริงมาจาก edge cron + realtime push ไม่ได้มาจาก poll**
> · **ถ้ายังไม่พอ ห้ามยืดเกิน 15-20 นาที** — ให้ไปลด payload แทน (view/RPC สรุป `prod_orders` ต่อ session)
>   ⚠️ **ห้ามย้ายสูตร OEE ไป SQL** — `src/utils/oee.js` เป็นเจ้าของสูตรจุดเดียว (กฎ SCADA ใน CLAUDE.md)
>
> **⚠️ ตัวที่ตึงที่สุดคือ Egress — ก่อนเพิ่มจอ/หน้า/realtime channel/ทางอัปโหลดรูป ให้ประเมินตรงนี้ก่อนเสมอ**
> **และก่อนบอกว่า "cache แล้ว" ให้ถามต่อเสมอว่า cache นั้นอยู่รอดตอนผู้ใช้ปิด-เปิดแอปไหม**
> **และเวลาคิดงบต่อจอ ให้นับ *ทุก loop ของหน้านั้น* ไม่ใช่ loop ที่หนักที่สุดตัวเดียว**
