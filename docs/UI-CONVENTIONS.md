...
## 6.12 📦 ชิปกรองฝั่งงาน Logistic (`SideFilterChips`) — เริ่มที่ฝั่งของบัญชี (2026-09-04)

- หน้าที่ลิสต์พาร์ทปนหลายฝั่ง ใช้ `src/components/SideFilterChips.jsx` เท่านั้น (เกณฑ์แยกฝั่งอยู่ที่ `src/utils/logisticSide.js` ห้ามเขียนซ้ำ)
- **ค่าเริ่มต้นของชิป = `defaultSideFilter(sides)` จาก `UserContext.sides`** — บัญชีที่อยู่ฝั่งเดียว (ขาเข้า หรือ ขาออก)
  เปิดหน้ามาเห็นของฝั่งตัวเองก่อน · control (Sales/Planner/Billing ไม่ถือของ) / หลายฝั่ง / ไม่จำกัด = "ทั้งหมด"
  · **ต้องกดสลับดูฝั่งอื่นได้เสมอ** (เป็นแค่ค่าเริ่มต้น ไม่ใช่การซ่อน — ข้อมูลข้ามฝั่งยังต้องเห็นได้ เช่น Store ดู FG ที่รอส่ง)
- ชิป "❔ ไม่ระบุฝั่ง" โชว์เมื่อมีของจริงเสมอ ห้ามซ่อน (กฎเดิม 2026-09-03)
