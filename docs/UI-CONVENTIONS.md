...
- **กัน overlap มุมขวาบน + ลำดับ z-index (audit ทั้งระบบ 2026-07-21):** `NotificationBell` เป็น `position:fixed top:10 right:14 zIndex:1200` **ทุกหน้า** ลอยทับแถบบนขวา ~50px ของทุกหน้า — กติกาที่ทุกหน้าต้องทำตาม:
  ...
  - **ห้ามมี float มุมจอเฉพาะหน้า** (`position:fixed`/`absolute` เกาะ top/right พร้อมปุ่มหลายอัน) ลอยทับ content — ให้ย้ายเข้า toolbar แบบ in-flow (`marginLeft:auto` ชิดขวา) แทน (เคยพลาด: Management filter cluster ลอยทับหัวบอร์ด Heijunka — แก้ 2026-07-21)
