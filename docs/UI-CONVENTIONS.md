...
#### 📋 ผล audit ทั้งโปรเจค (23/09) — จอที่มีสถานะ "ยังทำ/ทำแล้ว"
| จอ | สถานะเดิม | ผล |
|---|---|---|
| `/pm-check` ใบตรวจ PM/AM | ยังไม่ตรวจใช้**สีประเภทจุดตรวจ** (ตั้งเองได้ ⇒ ชนสี OK) | 🔴 แก้แล้ว — วงโปร่ง |
| ใบตรวจ QA (`QaCheckSheet`) | `RANK.SC = #ef4444` **เท่ากับ** `JUDGE.ng = #ef4444` เป๊ะ ⇒ SC ที่ยังไม่ตรวจ = หน้าตาเหมือน NG | 🔴 แก้แล้ว — วงโปร่ง เก็บสี rank ที่ขอบ |
| `/lpa` แผนตรวจ | `○ วางแผน` / `● ตรวจแล้ว` / `⊗ เลยกำหนด` | ✅ ต่างที่สัญลักษณ์ + legend |
| `/pm?tab=plan` ตารางแผน PM | `🔴 เกินกำหนด` / `⚪ ไม่มีรอบ/ยังไม่ตรวจ` | ✅ มี emoji + ข้อความ |
| Daily PM | เขียน "ยังไม่ตรวจ (n)" เป็นข้อความ | ✅ |
| BBS | label ข้อความทุกสถานะ (เหมาะสม/ไม่เหมาะสม/ปรับแก้แล้ว/ไม่ได้ตรวจ) | ✅ |
| ผังชั้นวางอะไหล่ (`RackMap`) | "ว่าง" เทา แยกจาก ปกติ/ต่ำ/หมด + legend | ✅ |
| `/qa-setup` วางหมุด | สี arming (#f59e0b) เท่ากับสี rank M **แต่**ส่ง `selected` ⇒ มีวงไฮไลต์กำกับ | ✅ |
| `/factory-map` | `bad` กับ `down` แดงเท่ากัน — แต่ `down` **กระพริบ** (กติกา Andon) + มีป้ายบนช่อง | 🔵 ตั้งใจ |
| `/factory-map` | `waiting` (เปิดกะยังไม่มี order) กับ `busy` (กำลังทำ PM) ฟ้าเท่ากัน — ต่างที่ป้ายอย่างเดียว | 🔵 ข้อสังเกต ยังไม่แก้ |

**วิธี audit ซ้ำ** (สแกนหาสีซ้ำในแผน status/rank ของไฟล์เดียวกัน — ตัวที่เจอ QA):
สแกนไฟล์ใน `src/` หา `key: { … color: '#xxxxxx' }` แล้วรายงานไฟล์ที่ **hex เดียวกันถูกใช้กับหลาย key**
· ซ้ำ "คนละแกนที่ไม่เคยอยู่บนของชิ้นเดียวกัน" = ปกติ (เช่น `npi.js` จัดกลุ่มความหมายตั้งใจ)
· ซ้ำ **"บนของชิ้นเดียวกัน คนละความหมาย"** = บั๊ก
