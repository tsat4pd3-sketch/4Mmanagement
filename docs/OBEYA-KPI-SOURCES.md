...
### 12.6 ✅ **ลงฐานจริงแล้ว 2026-09-23** — เด็ค H1 FY2026 → `kpi_definitions` + `kpi_manual_entries`

migration `20260923_kpi_seed_h1_fy2026_main.sql` (**apply แล้ว**) · ก่อนหน้านี้ `kpi_definitions` = **0 แถว**
มาตลอด (4 บั๊กที่ทำให้ตั้ง KPI ไม่ได้เลย เพิ่งแก้ 23/09) — ชุดนี้คือข้อมูลจริงชุดแรกที่เข้าระบบ

| ขอบเขต | `scope_kind`/`scope_value` · `section` | นิยาม | ผูก `std_item_id` | ค่ารายเดือน |
|---|---|---|---|---|
| PD3 – HDF | `section`/`PD3` · PD3 | 15 | 13/15 | 6 ตัว × Jan–Jun = 36 |
| PD4 – Assy2 | `section`/`PD4` · PD4 | 15 | 13/15 | 6 ตัว × Jan–Jun = 36 |
| PD4 – JIG Maintenance | `department`/`JIG MTN` · JIG MTN | 13 | 12/13 | 3 ตัว × Jan–May = 15 |

· `section` ตั้งตามค่าที่ `employees`/`profiles` ใช้จริง (`PD3` · `PD4` · `JIG MTN`) ไม่ได้คิดขึ้นใหม่ — จอกรองเจอทันที
· แถวที่ `std_item_id` ว่าง = **ทะเบียนกลุ่มไม่มีหัวข้อนั้นจริง** (`Non NC Major` ทั้ง 3 แผนก · `Defect/Scrap Cost (COPQ)`
  ของ PD3/PD4) — ตรงกับที่ §12.4 บันทึกไว้ว่าเป็น KPI ที่มีในเด็คแต่ไม่มีในไฟล์ Excel · **ห้ามจับยัดแถวที่ใกล้เคียง**
· join ทะเบียนมาตรฐานด้วย **(std_unit, topic)** ไม่ใช่ `seq` — `Production` มี `seq = '6'` ซ้ำ 2 แถว
  (`Cost Reduction` / `Day Sales of Inventory (DSI)`) join ด้วย seq จะได้แถวผิดเงียบๆ
· วิธีรวม 12 เดือนของแต่ละแถว (§12.2) เก็บไว้ที่ `provider_config->>'summary'` แล้ว — **ยังไม่มีจอไหนอ่าน**

**ตรวจย้อน:** ค่ารายเดือนอ่านจากกราฟในสไลด์ (เป็นรูป PNG ไม่ใช่ chart XML — ไม่มีตัวเลขดิบให้ดึง)
แล้ว**รวมกลับเทียบกับ YTD ที่เด็คพิมพ์ไว้เอง ตรงทั้ง 15 เส้น** (PD3 OEE 83.795→83.8 · PD3 Scrap Σ1,628.5→฿1,628K
· JIG MTBF 341.018→341.0) · และ `scoreDef()` ให้ขั้นตรงกับช่อง Assessment ของเด็ค **14/15 แถวที่เทียบได้**

> #### 🔴 2 จุดที่เด็คกับเกณฑ์ทางการไม่ตรงกัน — **ลงฐานตามบาร์ที่เด็คประกาศ ไม่ได้แก้ตาม Assessment**
> 1. **PD4 ข้อ 3.4 Internal Quality Rate** — เด็คเขียน Commitment ≤350 · Target ≤300 · ผล **376 PPM**
>    แล้วตัดสินว่า `◐ Partial (0.5)` · แต่ 376 **เกิน Commitment 350** ⇒ ตามประกาศ QSM-R2 001/2569 ต้องได้ **0**
>    (จอจะขึ้นแดง ไม่ใช่เหลือง) — ต้องถามเจ้าของใบว่าบาร์ผิดหรือ Assessment ผิด
...
⇒ นิยามมีครบ ช่องรายเดือนว่าง = จอขึ้น "ยังไม่มีข้อมูล" (ถูกตามกฎความซื่อสัตย์ของจอ)
**`weight` ว่างทุกแถว** — เด็คไม่มีคอลัมน์น้ำหนัก (อยู่ในใบ Appraisal) ⇒ ผลรวมยังไม่ถึง 50 ตามประกาศ
...
### 15.7 ✅ สูตรหน้า 10 (Maintenance) ลงระบบแล้ว — 2026-09-24 (user ส่ง PDF ซ้ำ: "เคยส่งให้แล้ว ดูให้ละเอียดอีกรอบ")

ทบทวนใหม่ทั้งฉบับ (19 หน้า · ถอดข้อความ + render ทุกหน้า): **สูตรทางการ 4 ตัวของหน่วยงานช่างเขียนไว้ครบ ไม่มีอะไรต้อง "รอเคาะ"**
· ที่เคยเขียนใน §15.3 ว่า "ต้องตรวจก่อนเอาไปใช้" / "ต้องนิยามให้ตรงก่อนคำนวณ" = เรื่องของ **ข้อสมมติตัวหาร** ซึ่งตอนนี้เลือกแล้วและเขียนบนจอ:

| KPI (หน้า 10) | สูตร verbatim | ข้อมูล ESM | ข้อสมมติที่เขียนบนจอ |
|---|---|---|---|
| MO Closed on target | (MO Closed on target / Total MO) × 100 | `mtn_orders.repair_done_at ≤ target_done_at` นับตามเดือนที่ซ่อมเสร็จ · ทีมจาก `mtn_dept` | ใบที่ไม่ตั้ง `target_done_at` = ไม่ตรงเป้า + บอกจำนวน (28 ใบในปี 2026) |
| Machine Break Down | (ชม.ที่เครื่องหยุด / ชม.ที่เครื่องทำงานได้ปกติ) × 100 | `downtime_logs` นอกแผนที่ปิดแล้ว ของอุปกรณ์ชนิดที่ทีมดูแล (`machines.equipment_kind` ↔ `mtn_teams.equip_type`) | **ตัวหาร = 730 × จำนวนเครื่อง** (ฐานเดียวกับ MTBF ในเอกสารเดียวกัน — เอกสารไม่นิยามเพิ่ม) |
| MTBF | [(730 × จำนวนเครื่อง) − Total breakdown times] / จำนวนเครื่อง | เหมือน MBD · จำนวนเครื่อง = `machines` active ต่อชนิด | 730 = ชม./เดือนคงที่ตามเอกสาร **ไม่ใช่ชั่วโมงเดินเครื่องจริง** ⇒ เทียบกับ `mtnMetrics`/`/mtn-analysis` ไม่ได้โดยตั้งใจ |
| MTTR | Total hours of breakdown / Total frequency times | เหมือน MBD | ยังไม่ปิด = ไม่นับ (ไม่รู้ว่านานเท่าไหร่) |

**ของที่ทำ:** RPC `kpi_mtn_rollup(p_from, p_to)` (DR · Σ เท่านั้น · `20260924_kpi_mtn_rollup_dr.sql` **apply แล้ว**) → `src/utils/kpiAuto.js`
(สูตร + เทส 7 เคส) → แท็บ ⚙️: เลือกขอบเขตที่เป็นทีมช่าง (`mtn_teams.dept_name` = ชื่อแผนกในผัง) แถว KPI ที่ชื่อจับคู่ได้จะมีบรรทัด
"⚡ ระบบคำนวณ" ใต้แถวกรอกมือ + ปุ่ม **⬇ ใช้ค่านี้** (เติมเฉพาะเดือนว่าง ไม่ทับที่กรอกมือ · เขียนผ่าน `kpi_manual_entries` เหมือนคน = สอบกลับได้)
· หน่วยตามแถว (JIG ตั้ง "นาที" → ×60 ให้เอง) · เดือนที่โรงงานไม่มี downtime_logs เลย = "·" ไม่ใช่ 0 · ค่าที่กรอกมือต่างจากที่คำนวณ >5% ติด ≠
**ผลจริง 24/09 (JIG MTN):** เครื่อง jig 172 ตัว · ก.ย. เสีย 421 ครั้ง 83.75 ชม. ⇒ MTBF 729.5 ชม. · MTTR 0.20 ชม. · MBD 0.067% · MO ตรงเป้า 0/4
— ต่างจากค่าที่เด็ค H1 กรอกไว้ (MTBF 341 ชม.) มาก ⇒ **เด็คใช้ฐานเวลาอีกแบบ** ต้องให้ JIG MTN ชี้ว่าเลขทางการที่ส่ง KPI Online คำนวณจากอะไร (จอไม่ทับค่าที่กรอก)

**สิ่งที่เอกสารนี้ "ไม่มี" จริงๆ (ตรวจซ้ำแล้ว):** เกณฑ์ Safety รายชั้น (`*Refer to safety announcement` ทุกหน่วย) · เกณฑ์ QCC/Engineering Day/Kaizen
(`*Refer to activity announcement`) · นิยาม "ผ่านอบรม" ของ TS Academy · ตัวหารวัน (Days) ของ DSI — 4 อย่างนี้อยู่ในประกาศแยกที่ยังไม่ได้รับ
