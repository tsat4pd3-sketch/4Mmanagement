...
- ข้อมูลตั้งต้น = บอร์ด 737D MLM ที่ถอดจากรูป อยู่ใน `src/data/nmBoard737D.js`
...
> 🔧 **ปรับ harness ด้วย (2026-09-23):** `audit/main.jsx` ส่ง query ของ URL จริงเข้าเป็น `initialEntries`
> ของ MemoryRouter แล้ว ⇒ crash sweep ทดสอบ**ชั้นที่ลึกกว่าหน้าแรก**ได้ (เดิมเริ่มที่ `/` เปล่าเสมอ
> ทำให้หน้าไหนที่ใช้ `useSearchParams`/`useTabParam` ถูกตรวจแค่แท็บแรกตลอด — ตรวจได้ไม่ครบโดยไม่มีใครรู้)
