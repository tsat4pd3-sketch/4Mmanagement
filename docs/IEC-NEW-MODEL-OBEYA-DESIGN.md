...
## 5.21 📌 บอร์ดที่สอง — **D02D (TMT)** จากอีกโรงในเครือ (2026-09-23)

user ส่งรูปบอร์ดของอีกโรงมาให้ พร้อมข้อมูลว่า *"บอร์ดเดียวกันแต่ข้อมูลครบและแน่นกว่าของเดิม"* — จริง
**และเป็นโปรเจคเดียวกับใบ POP ไฟล์แรกที่แกะไว้ตอนต้น (D02D maruG)** ⇒ ใบ POP บนผนังนั้นคือไฟล์เดียวกัน

**หัวบอร์ดโครงเดียวกันเป๊ะ:** `OVERALL PROJECT EVA` 3 แกน — Result KPI 🔴 · Process KPI 🟡 · Milestone 🟡
(บังเอิญได้สีชุดเดียวกับ 737D) ⇒ ยืนยันว่าโครง EVA 3 แกนเป็นมาตรฐานของบริษัท ไม่ใช่ของโรงใดโรงหนึ่ง

### 🔴 ข้อค้นพบที่เปลี่ยนโมเดลข้อมูล — แกน "ล็อต" เป็นของโปรเจค ไม่ใช่ของลูกค้า

| บอร์ด | ลำดับล็อต/ด่านที่ใช้จริง |
|---|---|
| 737D MLM | CV (CF#1/CF#2/CF#3) → 1A → MPT |
| **D02D (TMT)** | **KS → SKK-A Lot → SKK-B Lot → NS Lot → ACS Lot → 1A → MPT** |

ลูกค้า **Toyota เหมือนกันทั้งคู่ แต่ชุดล็อตไม่เท่ากัน** ⇒ **ห้ามผูกรายการด่านไว้ที่ทะเบียนลูกค้าอย่างเดียว**
ต้องให้โปรเจคทับได้ (แม่แบบลูกค้า = ค่าตั้งต้น · โปรเจคถือชุดจริงของตัวเอง)
· ชื่อล็อตชุดนี้ตรงกับแถบ "ปฏิทินลูกค้า" ในใบ POP ที่แกะไว้ (§5.1: SKK A/B/X-LOT · NS Lot · KS-TMT)

### 🆕 แผงที่บอร์ดนี้มีแต่ 737D ไม่มี (9 แผง)

| แผง | คืออะไร | สถานะบนบอร์ด |
|---|---|---|
| **PROJECT EVA STATUS** | ตารางนับตัวชี้วัด **แยกรายล็อต** — KPI หลัก 14 · ย่อย 110 × (EVA / OK / NG) ต่อล็อต | ติดมุมบนซ้าย |
| **SE Study** | Simultaneous Engineering — ผลศึกษาร่วมกับลูกค้าก่อนล็อกแบบ (ตรงกับ sub-KPI ข้อ 2 ในใบ POP) | 🟢 |
| **LVPT** (+ LVPT Audit + Action Plan LVPT) | ด่าน SPTT#3 ของจริง: แผน · ใบ audit **พร้อมรูปหน้างาน** · แผนแก้สิ่งที่ audit เจอ | เกณฑ์ OTOP Q:100% |
| **HVPT** | ด่าน SPTT#4 | แผงว่าง (ยังไม่ถึง) |
| **Problem Progress** | ภาพรวมว่าปิดปัญหาไปกี่เรื่อง (ตาราง + กราฟแท่ง) — **คนละอย่างกับ Kadai ที่เป็นรายเรื่อง** | มี |
| **ACTIVITY** | ใบเขียนมือ Agenda / Target date / PIC / EVA — ทำหน้าที่เหมือน Meeting Minutes | มี |
| **Quality Status Graph** | D02D (G) TOTAL GRAPH PARTS ACCURACY STATUS (Delivery) — แท่งต่อพาร์ท (ลงวันที่ 20 ส.ค. 69) | 🔴 |
| **Shuken** | D02D(G) TOTAL SHUKEN — ตารางผลตรวจคุณภาพรายพาร์ทแยกล็อต (%) · **⚠️ ศัพท์ลูกค้า ต้องให้ IEC นิยาม** | 🔴 |
| **PESS · C/F Status · QBI · CPPP/SSAS** | สถานะประเมินชิ้นงาน · สถานะ checking fixture · Quality Built-In · CPPP | PESS 🟢 · C/F 🟢 · QBI และ CPPP ว่าง |

**ทั้งหมดลงในรูปทรงเดิม 5 แบบได้** (ส่วนใหญ่เป็น `doc` กับ `matrix`) — ไม่ต้องเพิ่มรูปทรงที่ 6
⇒ โมเดล §6 ผ่านการทดสอบกับบอร์ดที่แน่นกว่าแล้ว

### สิ่งที่บอร์ดนี้ไม่มีแต่ 737D มี
Develop Color · Packaging Preparation · Supply Chain · SPTT (Commodity Risk Mitigation) · Meeting Minutes
⇒ **ยืนยันข้อสงสัยใน §9 ข้อ 2: แผงไม่เหมือนกันทุกบอร์ด** ต้องทำเป็น**แม่แบบบอร์ดที่เลือกแผงได้**
ไม่ใช่ fixed 21 แผง · บางแผงขึ้นกับ*ประเภทงาน* (D02D ไม่มีงานพ่นสี จึงไม่มี Develop Color)
บางแผงขึ้นกับ*ด่านที่ไปถึงแล้ว* (737D ยังไม่ถึง LVPT จึงยังไม่มีแผงนั้น)

### ของที่ยืนยันกฎเดิม
- **แผงว่าง = ยังไม่ถึง ไม่ใช่ NG** — QBI กับ CPPP/SSAS ว่างเปล่าทั้งช่องบนบอร์ดจริง (ตรงกับกฎข้อ 2 ใน `nmBoard.js`)
- **"CPPP Activity (if any)"** ในใบ POP = ทำเฉพาะเมื่อลูกค้าสั่ง ⇒ แผงที่เป็น optional ต้องแยกจากแผงที่ต้องมี
- **Part Overview ติด 2 ใบ** (Maru A / Maru G) — 11 พาร์ท = 2 + 9 ตรงกับใบ POP เป๊ะ
- **Customer Information** = งานที่ลูกค้าขอให้ศึกษา (Task Force Model 02DD(G) #12 · ติดป้าย STUDY ONLY)
  มีข้อตกลงเวลาเขียนไว้บนใบ: *TSA แก้เครื่องมือได้ที่ OTOP 90% ใช้เวลา 15 วันหลังได้รับ ECI*
  ⇒ ช่องแบบนี้ควรเก็บเป็น**ข้อผูกพันที่มีวันที่** ไม่ใช่ข้อความลอย
...
## 6.7 ★ work flow การไล่ดูบอร์ด — **IEC ส่งมาเอง** (`Obeya_E_Board-V2.pptx` · 2026-09-24)

ไฟล์ที่รอมาตั้งแต่ §9 — IEC ทำสไลด์ 4 หน้าบอกว่า "จากไหน เจาะไปไหน" มาให้ **ตอบคำถามค้างได้หลายข้อ**

### ลำดับชั้น (ทำแล้วใน `/nm-board` — ดู §7.6)

```
L1  หน้าแรกของ 1 รุ่น = 4 แผง (IEC เขียนเลข ① ② ③ ④ กำกับบนรูปบอร์ดจริงให้ด้วย)
     ① สรุป EVA 3 ถัง   ② POP STATUS   ③ PART DELIVERY + PART QUALITY   ④ Responsible
L2  POP เต็ม = Main KPI 14 หัวข้อ → sub KPI (แถวขาว · จำนวนต่างกันตามรุ่น)
       แต่ละแถวมี Schedule TSA │ Schedule ลูกค้า │ ⬤ EVA
L3  เจาะหัวข้อ เช่น 7. Document → 7.1 Engineering doc (17 หัวข้อ) · 7.2 QA doc (11 หัวข้อ)
L4  ในหัวข้อย่อย = ⬤ EVA + แนบไฟล์ + เขียนสาเหตุ/วิธีแก้
```

**ทางลัดที่เป็นหัวใจ** — IEC เขียนไว้ตรงๆ ว่า *"ถ้ามี Delay 7 วันสามารถ click ไปยังหัวข้อที่ Delay>7 ได้เลย"*
⇒ ถัง Delay บนหัวบอร์ดนับ sub KPI ที่ EVA แดง **กดทีเดียวข้าม L2/L3 ไปที่รายการปัญหา**
ไม่ต้องไล่ทีละชั้น · 3 ถัง = **Delay** (แดง) · **On plan** (เหลือง กำลังทำ) · **Complete & Finish** (เขียว จบแล้ว)

### POP = Main KPI 14 หัวข้อ (ครบตามที่ IEC ระบุ — แม่แบบอยู่ `src/data/nmBoardPop.js`)

1 Kick off · 2 Technical data review · 3 Tooling (3.1 Stamping die · 3.2 Assembly jig · 3.3 Checking fixture/Inspection jig) ·
4 Packaging · 5 Construction & Area renovation · 6 Production planning ·
7 Document (7.1 Engineering · 7.2 QA · 7.3 Production line · 7.4 Warehouse & Delivery) ·
8 Part Quality · 9 SPTT Milestones (9.1 #1 Preparation · 9.2 #2 Off tools stage · 9.3 #3 LVPT · 9.4 #4 HVPT) ·
10 Part delivery (Each stage) · 11 SOP readiness · 12 SOP · 13 Initial SOP · 14 Project Review

### ล็อตส่งงาน 8 ขั้น + เงื่อนไขที่ต้องผ่าน

`CV/KS (OT Q90%)` → `SKK A (OT Q90%)` → `SKK B (OT Q95%)` → `SKK X (OT Q100%)` →
`ACS (OT/OTOP Q100%)` → `1A (OTOP Q100%)` → `MPT (OTOP Q100%)` → `QCS (OTOP Q100%)`

⚠️ บอร์ด D02D จริงเขียน **"NS Lot"** ตรงตำแหน่ง SKK X — ชื่อล็อตไม่ตายตัว ต้องเก็บเป็นข้อมูลของรุ่น

### 🔴 เกณฑ์สี — **ไม่เหมือนกันทุกแผง ห้ามใช้ชุดเดียวทั้งจอ** (`EVA_RULE` ใน `nmBoard.js`)

| แผง | 🔴 แดง | 🟡 เหลือง | 🟢 เขียว |
|---|---|---|---|
| POP main KPI | delay > **10 วัน** | ทำตามแผน + KPI target ต้องได้ทุกสัปดาห์ | จบแล้ว ไม่ต้อง follow up |
| Part delivery | ส่งไม่ได้/delay — **ต้องระบุตัวไหน สาเหตุอะไร** | ส่งได้แบบมีเงื่อนไข — ต้องระบุเหมือนกัน | ส่งได้ตาม Condition stage |
| Part quality | ไม่ตรง Condition stage · delay > **14 วัน** | delay > **7 วัน + มีแผน improve** | ตรงตาม Condition stage |
| เอกสาร 7.1/7.2 | delay > **10 วัน** | delay > 7 วัน + มีแผน improve | ผ่านแล้ว ไม่ติดปัญหา |

🟠 **ช่องโหว่ที่เจอในเกณฑ์ของเขา (เราตัดสินเองไปก่อน):** เกณฑ์เหลืองบังคับว่าต้อง "มีแผน improve"
⇒ ช้าเกิน 7 วันแต่**ไม่มี**แผน ไม่เข้าทั้งเหลืองและแดง · เราให้เป็น **แดง** (ช้าแล้วไม่มีแผน อันตรายกว่า)
แก้ที่ `suggestEva()` จุดเดียวถ้า IEC ตอบมาเป็นอย่างอื่น

### กฎอีก 3 ข้อที่เขาเขียนกำกับ

1. **กลิ้งขึ้นแบบแย่สุดชนะ** — *"ถ้ามี 1 ตัวเป็นสีแดง ต้องโชว์แดงเลย"* (= `rollupEva` ที่ทำไว้แล้ว ✅)
2. **EVA คนตั้งเอง** — *"Leader EVA สีด้วยตัวเอง"* ⇒ ระบบ**เสนอ**ได้ (`suggestEva`) **ห้ามเขียนทับ**
   และ **แดงต้องเขียนข้อความ**ว่าเกิดอะไร แก้ยังไง ⇒ จอต้องฟ้องเมื่อแดงแล้วไม่มีคำอธิบาย (`redWithoutNote`)
3. **วันที่ Update เป็น auto** หลังกดบันทึก ไม่ใช่พิมพ์เอง

### สิ่งที่เปลี่ยนจากที่เคยเข้าใจ

- เดิมคิดว่าหน้าแรกของรุ่นคือ "แผงทั้งหมดเรียงกัน" — **ผิด** IEC ต้องการ **4 แผง** แล้วค่อยเจาะ
- แผง 21/27 ใบบนบอร์ดกระดาษ = **หลักฐาน**ที่ใบ POP อ้างถึง ไม่ใช่ตัวนำทาง
...
## 7.5 ✅ ทำแล้ว — หน้า `/nm-board` (2026-09-23)

หน้าแรกของงานนี้ลงระบบแล้วใน ESM หมวด **คุณภาพ & วิศวกรรม** (`src/pages/NewModelBoard.jsx`)
- **ไล่ดู 4 ชั้นตามไวท์บอร์ด**: ภาพรวมทั้งฝ่าย → ลูกค้า → รุ่น (บอร์ด 21 แผง) → ในแผง
  · ชั้นอยู่ใน URL (`?cust=&proj=&panel=`) ⇒ แชร์ลิงก์ตรงแผงได้ · ค่ามั่วตกกลับชั้นบน ไม่จอว่าง
- **EVA 3 แกน** (Result / Process / Milestone) ทั้งระดับฝ่ายและระดับรุ่น + ไฟรายแผง + ไฟรายแถว
- **กฎ/สูตรอยู่ `src/utils/nmBoard.js` เท่านั้น** (pure · เทส 10 เคส ตรึงเวลาไว้) — ห้ามคำนวณซ้ำในหน้า
- **ความสดของข้อมูล** แทนแถบ UPDATE STATUS บนกระดาษ: เตือนเมื่อค้าง ≥ 14 วัน · ถือว่าเชื่อไม่ได้ที่ ≥ 28 วัน
- **หน้าแรกมี 2 รายการที่ต้องเห็นทุกวัน**: งานเลยกำหนด (ดึงจาก Kadai + มติที่ประชุม) · แผงที่ต้องรีบดู
- **5 รูปทรงแผง** (activity · matrix · issues · doc · network) ทำเป็นตัววาดกลาง ใช้ซ้ำทั้ง 21 แผง
- ข้อมูลตั้งต้น = **2 บอร์ดจริง** (737D MLM 21 แผง · D02D TMT 27 แผง) ใน `src/data/nmBoard737D.js`
  · D02D มี `lots[]` + `evaStatus` ของตัวเอง ⇒ พิสูจน์แล้วว่าแกนล็อตแยกรายโปรเจคได้
  **โครง object ตั้งใจให้ตรงกับตารางที่จะสร้างจริง ⇒ ย้ายไป DB = แก้แค่ `useBoardData()` จุดเดียว**
- **ลูกค้าที่ไม่มีโปรเจคขึ้นเทา ไม่ใช่เขียว** และ "ยังไม่ถึงด่าน" เป็นเทา ไม่ใช่ NG (กฎความซื่อสัตย์ของจอ)
- ตรวจแล้ว: build ผ่าน · เทสผ่าน · crash sweep 26 เคส (4 ชั้น × 13 เส้นทาง × 1500/390px) ไม่พัง ไม่ล้นแนวนอน
- **📺 โหมดจอ TV 70" — `?tv=1`** (2026-09-23 · คำขอ user *"เปิดทีวีจอ 70 นิ้วแล้วเห็นภาพแบบบอร์ดนี้"*)
  · วางทั้งบอร์ดในจอเดียว **ไม่มีเลื่อน** ทั้ง 4K / 1080p / 1366×768 (ตรวจด้วย `scrollHeight` ทุกขนาด)
  · หัวจอ = ชื่อรุ่น + EVA 3 แกนวงใหญ่ + เหตุผลที่แดง + นาฬิกา · แผงเรียงเป็นตารางเหมือนผนังจริง
  · จำนวนคอลัมน์คำนวณจาก `tvGrid(n)` (21 แผง → 5×5 · 27 แผง → 6×5) **ห้าม hardcode**
  · แผงแดงกระพริบด้วยคลาสเดิม `.mo-card-alert` · เหลือง/เขียวนิ่ง (กฎ Andon)
  · **ตั้ง URL ค้างไว้บนจอได้** เช่น `/nm-board?cust=toyota&proj=d02d-tmt&tv=1` — เปิดมาตรงรุ่นนั้นเลย
  · มีบอร์ดมากกว่า 1 รุ่น → สลับอัตโนมัติทุก 25 วินาที + ปุ่มหยุด + จุดเลือกรุ่น
  · กติกาที่ตกผลึกเขียนไว้ใน `docs/UI-CONVENTIONS.md` §บอร์ดจอ TV (8 ข้อ) แล้ว
- **ยังทำไม่ได้ในเฟสนี้:** แก้ข้อมูลในจอ · ต่อฐานข้อมูล · supplier เข้าใช้ · ลิงก์ไป PFMEA/CP ของจริง
  · **จอ TV ยังไม่รีเฟรชเอง** (ข้อมูลอยู่ในโค้ด) — ต้องเพิ่มตอนต่อฐานข้อมูล

> 🔧 **ตรวจชั้นลึกได้เพราะ harness รองรับแล้ว** — `audit/main.jsx` ส่ง query ของ URL จริงเข้าเป็น
> `initialEntries` ของ MemoryRouter (อีก session ทำไว้ 2026-09-22 ด้วยเหตุผลเดียวกันเป๊ะ: เดิมเริ่มที่ `/`
> เปล่าเสมอ ⇒ หน้าที่ใช้ `useSearchParams`/`useTabParam` ถูกตรวจแค่ชั้นแรกตลอดโดยไม่มีใครรู้)

## 7.6 ✅ ทำแล้ว — จัดหน้า `/nm-board` ตาม work flow ของ IEC (2026-09-25)

- `src/data/nmBoardPop.js` (ใหม่) — แม่แบบ POP 14 main KPI + หัวข้อเอกสาร 7.1/7.2 + ล็อต 8 ขั้น
  **เป็นแม่แบบ ไม่ใช่ค่าตายตัว** (IEC: *"จำนวนข้อเปลี่ยนแปลงได้แล้วแต่ Model"*)
- `nmBoard.js` + เทส 8 เคส — `BUCKETS` · `flattenPop` · `mainEva` · `bucketCounts` · `leavesInBucket`
  · `redWithoutNote` · `EVA_RULE` · `suggestEva` · `evaMismatch`
- หน้าจอ: ชั้นรุ่นเป็น ①②③④ · ชั้นใหม่ `?bucket=` (ทางลัด) · `?pop=` · `?pop=&sub=`
- 🔴 **EVA ของ sub KPI ไม่ได้พิมพ์มือลงไฟล์ข้อมูล** — ดึงจาก "แผงที่ถือหลักฐาน" ของเรื่องนั้น
  (เช่น 8 Part Quality ← แผง Quality Status Graph) · หัวข้อที่ยังไม่มีแผงรองรับ = เทา **ห้ามเดาว่าเขียว**
  ⇒ ผลจริงของ D02D: ประเมินได้ 8 จาก 22 · Delay 4 · On plan 1 · Complete 3 · ยังไม่ประเมิน 14
  และ **แดง 3 ใบไม่มีคำอธิบาย** — จอฟ้องให้เห็นตามกฎข้อ 2 ของ IEC
- ยังทำไม่ได้: แก้ในจอ · แนบไฟล์ · Schedule TSA/ลูกค้ารายแถว · ต่อฐานข้อมูล

---
## 8. ลำดับที่เสนอ (เฟส 1 บางส่วนทำแล้ว — ดู §7.5)
...
> ✅ **ตอบแล้วจาก `Obeya_E_Board-V2.pptx` (2026-09-24)** — ข้อ 2 (แผงไม่ตายตัว ทำเป็นแม่แบบ) ·
> ข้อ 3 (เกณฑ์ G/Y/R ครบ 4 ชุด — ดู §6.7) · ข้อ 4 (Leader เป็นคนตั้งสีเอง) บางส่วน ·
> ข้อ 9 (EVA = Evaluation) · **คำถามใหม่ที่เกิดขึ้นแทน อยู่ท้ายรายการ (ข้อ 10-12)**
...
10. **ช้าเกิน 7 วันแต่ไม่มีแผน improve = สีอะไร** (เกณฑ์ที่ส่งมาไม่ครอบคลุม — เราให้เป็นแดงไปก่อน)
11. **SHUKEN · PFUS · MCS/MICS · Q-point · PCP** คืออะไร (หัวข้อใน 7.2 QA document)
12. **ใบ POP ของรุ่นใหม่เข้าระบบยังไง** — อัปไฟล์ POP.xlsx แล้วระบบอ่าน หรือคีย์ในจอ ·
    และใครคือ "Leader" ที่มีสิทธิ์ตั้งสี EVA (ตำแหน่งไหนใน `role_permissions`)
