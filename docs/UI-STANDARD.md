...
2. **หน้าที่อยู่หลายหมวด (`alsoIn`) ต้องถูกมาร์คทุกหมวด**
   `วางแผนการผลิต` = ฝ่ายผลิต + แผนงาน · `เฝ้าระวังสต๊อก` = สโตร์ + จัดส่ง
   🔴 **ห้ามเช็คด้วยค่าเดียวอย่าง `activeGroup === group`** — มันคืนแค่หมวดแรก แล้วรางจะตอบ
   คนละหมวดแล้วแต่ว่าเปิดแผงไหนค้างไว้ · ให้เช็ค `items.some(i => i.to === pathname)` ต่อหมวด
...
`aria-current` ตรงกับทะเบียนเมนู (`group` + `alsoIn`) ครบพอดีไหม
