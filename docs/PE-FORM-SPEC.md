...
## 0. Export — แยกทางกันชัดๆ ระหว่าง "ตาราง" กับ "ผัง"

> **อัพเดท 2026-08-17** — เอกสารรอบแรกสรุปว่า *"export ต้องใช้ไฟล์ต้นฉบับเป็นแม่แบบเท่านั้น ห้ามวาดใหม่"*
> ข้อสรุปนั้น**ถูกสำหรับ PFC แต่กว้างเกินไปสำหรับอีก 2 ใบ** · ของจริงคือ:

| ใบ | เนื้อหาจริงอยู่ที่ไหน | วิธี export | ความเหมือน |
|---|---|---|---|
| **PFMEA** (FM-PE1-018) | **ตารางในเซลล์** | สร้างใหม่ด้วย ExcelJS จากข้อมูลในระบบ | **~95%** ✅ ทำแล้ว |
| **Control Plan** (FM-PE1-019) | **ตารางในเซลล์** | สร้างใหม่ด้วย ExcelJS จากข้อมูลในระบบ | **~95%** ✅ ทำแล้ว |
| **PFC** (Process Flow) | **รูปทรงใน drawing** (~560 ชิ้น + รูป 59) | export ไฟล์เดิมไม่ได้ — แต่ **ระบบวาดผังเองจากรายการ OP** (§8) | คนละใบ ⚠️ |

**ทำไม PFMEA/CP ถึงเลือก "สร้างใหม่" แทน "แก้ในไฟล์ต้นฉบับ" ทั้งที่แม่แบบให้ 100%:**
เพราะฟอร์มต้นฉบับมีจำนวนแถวตายตัว แต่**ข้อมูลในระบบโตได้** — เจอเคลมแล้วเพิ่ม failure mode ใหม่
คือสิ่งที่โมดูลนี้มีไว้ทำ · การแก้แม่แบบต้อง "แทรกแถว" ซึ่งลาก merge / anchor ของ drawing /
print area ตามไปทั้งชุด = พังง่ายกว่าที่ได้ · แถมต้องเก็บไฟล์ต้นฉบับใน Storage + checksum
และใช้ไม่ได้เลยกับพาร์ทที่พิมพ์เข้าระบบเองโดยไม่มีไฟล์ Excel ตั้งต้น

**5% ที่ยอมให้ต่าง** (บอกผู้ใช้บนจอทุกครั้งที่กดดาวน์โหลด ห้ามเงียบ):
1. กล่องข้อความ/โลโก้ที่วาดทับหัวฟอร์ม (drawing) — ExcelJS สร้าง shape ไม่ได้
2. เส้นขอบในไฟล์จริงถูกแก้มือจนไม่สม่ำเสมอ (hair/thin/medium ปนกันในตารางเดียว) → ตัวสร้างทำให้สม่ำเสมอ
3. ช่องที่ระบบยังไม่มีคอลัมน์เก็บ (Key Date · Core Team · Key Contact) — **ปล่อยว่าง ห้ามเดา**

**ตรวจแล้วว่าไม่มีข้อมูลตกหล่น** (round-trip กับไฟล์จริง MB3B-8C306-BE):
PFMEA 255 record × 9 ช่องข้อความ = **2,295 ช่อง ตรง 100%** · Control Plan 178 record × 7 = **1,246 ช่อง ตรง 100%**
(ประกอบข้อความหลายบรรทัดกลับจากหลายแถวแล้วเทียบกับต้นทางทีละช่อง)

---

### 0.1 เหตุผลที่ PFC ทำไม่ได้ (ข้อมูลวัดจริง — อย่าให้ใครมาลองใหม่)

**ไม่มี library ไหน round-trip ไฟล์ PFC ได้**
...
**→ ถ้าวันหน้าต้องการ PFC 100% จริงๆ วิธีเดียวคือ "แก้เฉพาะค่าในเซลล์ของไฟล์ต้นฉบับ" (surgical edit ใน zip)**
...
> ⚠️ **ห้ามทำ export ของ PFC ด้วยการวาดผังขึ้นใหม่** — ผังคือเนื้อหา ไม่ใช่การจัดหน้า วาดใหม่ = ได้ผังคนละใบ
> (กฎเดิม "ห้ามจำลอง layout ใบจริงมาไว้อีกที่" ใช้กับกรณีที่ **มี layout อยู่ในโค้ดอยู่แล้ว** แล้วไปทำซ้ำอีกที่
> — PFMEA/CP ไม่เข้าข่าย เพราะ layout ต้นทางอยู่ในไฟล์ Excel นอกโค้ด ไม่มีตัวที่ 2 ให้ drift)
...
| 4 | PFMEA หล่นทั้งชีท 18 แถว · จุดควบคุมไปกอง OP ผิด | ไฟล์เขียนเลขผิด: ชีท `140` แต่ในเซลล์เขียน `130` | **ยึดชื่อชีท แก้ให้ + เตือน** (ดู §6.1) |
...
### 6.1 เลข OP ในเซลล์ไม่ตรงกับชื่อชีท — ระบบแก้ให้แล้ว (2026-08-17 · คำสั่ง user)

**ยึด "ชื่อชีท" เสมอ** — ชื่อแท็บคือสิ่งที่คนคลิกเข้าไปกรอกเนื้อหาของ OP นั้น ส่วนเลขในเซลล์คือของที่ติดมา
จากการก๊อปชีท (เจอจริง: ไฟล์ 306 ชีท `140` แต่ในเซลล์เขียน `130` → จุดควบคุม 5 จุดไปกองที่ OP 130
ทำให้ OP 130 มี 10 จุด · OP 140 มี 0 จุด ทั้งที่ OP รอบข้าง 130–170 มีไลน์ละ 5 จุดเท่ากันหมด)

| เงื่อนไข | ระบบทำอะไร |
|---|---|
| ทั้งชีทชี้ไป **OP เดียว** ที่ไม่ตรงกับชื่อชีท | **แก้ให้** (เขียน `sub_op` ใหม่เป็นเลขของชีท) + เตือนว่าแก้ให้แล้ว ชีทไหน จาก→เป็น กี่แถว |
| ชีทมี **หลายเลข OP ปนกัน** และไม่มีตัวไหนตรงชื่อชีท | **ไม่แก้** — กำกวมเกินกว่าจะเดา · เตือนให้ไปดูไฟล์ต้นทางเอง |
| มีเลขที่ตรงชื่อชีทอยู่ด้วย | ไม่ถือว่าผิด (ชีทที่คุมหลาย sub-op เป็นเรื่องปกติ) |

⚠️ **ต้องทำก่อนสร้างรายการ OP** — ไม่งั้นชื่อขั้นงาน/เครื่องจักร/child part ไปลงทะเบียนใต้ OP ที่ผิดไปแล้ว
⚠️ **แก้ข้อมูลให้ ≠ ปิดปาก** — ยังเตือนทุกครั้งเพื่อให้ไปแก้ไฟล์ต้นทาง ไม่งั้นอัพรอบหน้าเจอซ้ำ
...
| แก้เลข OP ที่ในเซลล์ไม่ตรงชื่อชีทให้ + เตือน | ✅ (§6.1) |
| **Export PFMEA / Control Plan เป็น .xlsx ตามฟอร์มเดิม (~95%)** | ✅ `src/lib/peExcelExport.js` · ปุ่มในแท็บ FMEA/CP |
| Export ตัวผัง PFC (รูปทรง) | ⬜ **ไม่ทำ** — ผังเป็น drawing สร้างใหม่ไม่ได้ (§0.1) แก้ใน Excel แล้วอัพใหม่ |

**ลำดับที่ควรทำต่อ:**
1. เพิ่มคอลัมน์เก็บ Key Date / Core Team / Key Contact ใน `pe_doc_sets` — ตอนนี้ export ปล่อยว่างเพราะระบบไม่มีที่เก็บ
   (ตัวแกะหัวเอกสาร `parseFormHeader` อ่านค่าพวกนี้ได้อยู่แล้ว แค่ยังไม่มีที่ลง)
2. ปุ่มพิมพ์ PDF ตรงจากหน้า (ตอนนี้ต้องดาวน์โหลด .xlsx แล้วสั่งพิมพ์จาก Excel)
3. ถ้าวันหน้าต้องการ PFC 100% → เก็บไฟล์ต้นฉบับใน Storage + ตัวเขียน zip แบบ surgical (§0.1)

---

## 8. ผังกระบวนการที่ระบบวาดเอง (Process Flow Chart) — 2026-08-17

**คนละสิ่งกับไฟล์ PFC ต้นฉบับ ห้ามสับสน:**

| | ไฟล์ PFC ของ PE | ผังที่ระบบวาด |
|---|---|---|
| ที่มา | คนวางรูปทรงเองใน Excel ข้าม 5 หน้า | generate จาก `pe_processes` |
| แก้ยังไง | แก้ใน Excel แล้วอัพเข้าระบบใหม่ | แก้ OP ในระบบ → ผังเปลี่ยนตามทันที |
| สถานะ | **เอกสารควบคุมตัวจริง** | ใช้ทบทวน/ประชุม/สอบกลับ |
| export | ทำไม่ได้ (§0.1) | 🖨️ พิมพ์ A3 แนวนอน **1 หน้าเสมอ** |

ไฟล์: `src/components/PeFlowChart.jsx` (ผัง + `layoutFlow` pure) · `src/lib/pePfcPrint.js` (ใบพิมพ์ · doc_key `pe_pfc`)

### โครงผัง (ถอดจากไฟล์จริง MB3B-8C306-BE)
```
[child part] [child part] …        ← ป้ายเลขพาร์ทที่เข้ากระบวนการ
    ◇OP10       ◇OP20   …          ← ช่องทางเข้า (kind='incoming_insp')
     ╎           ╎
  ═══╪═══════════╪══════           ← "บัส" เส้นรวมเส้นเดียว
     ↓
   ▭OP100 → ▭OP110 → … ⓐ           ← สายหลักไหลลงเป็นคอลัมน์ วกด้วยตัวเชื่อม A-D
```

### กติกาที่ห้ามแก้ย้อน
| # | เรื่อง | เหตุผล |
|---|---|---|
| 1 | **สัญลักษณ์นิยามที่ `SYMBOLS` จุดเดียว · legend สร้างจากตัวเดียวกัน** | เพิ่ม `kind` ใหม่แล้วลืมเติม = ตกไปเป็นสี่เหลี่ยมเงียบๆ และ legend โกหก |
| 2 | **เส้นรวมช่องทางเข้าเป็น "บัส" ห้ามลากทุกเส้นมาบรรจบจุดเดียว** | 9 เส้นทับกันเป็นพงรก + พาดทับวงกลมตัวเชื่อม A/B/C (เจอจริงตอนเรนเดอร์ดู) |
| 3 | **ช่องทางเข้าแถวที่ไม่ใช่แถวล่างสุด ต้องเดินสายอ้อมข้าง** | ลากตรงลงมาจะทะลุกล่องแถวถัดไป · เคยพลาด: แถวบนไม่มีเส้นต่อลงบัสเลย |
| 4 | **ทรงผังเลือกด้วย `pickShape` = maximize `min(กว้างกระดาษ/กว้างผัง, สูงกระดาษ/สูงผัง)`** | ตรงกับสิ่งที่อยากได้จริง (ตัวหนังสือใหญ่สุดบนกระดาษ) ไม่ต้องเดาสัดส่วนเป้า · ผอมสูงหรืออ้วนเตี้ยเกินไป โดนบีบทั้งคู่ |
| 5 | **ใบพิมพ์ clone SVG จากจอ (ชุดสีสว่างที่ซ่อนไว้) ไม่วาดใหม่** | กฎเดิม: ห้ามมี layout ชุดที่ 2 ในตัวพิมพ์ |
| 6 | **`.map svg { max-height }` ในใบพิมพ์ = ตัวบังคับ "1 หน้า"** | viewBox + preserveAspectRatio ย่อพอดีกรอบ ไม่ยืดผิดสัดส่วน · คำนวณจากขนาดกระดาษที่ตั้งใน `/doc-forms` |
| 7 | **ลงหน้าเดียวได้ ≠ อ่านออก — ต้องเตือนเมื่อตัวอักษร < 1.6 มม.** | ผัง OP เยอะๆ ย่อจนอ่านไม่ออก ต้องบอกก่อน ไม่ใช่ให้ไปเจอตอนพิมพ์ออกมา |
| 8 | **ข้อความ "ไม่ใช่ไฟล์ PFC ต้นฉบับ" ต้องอยู่ทั้งบนจอและบนใบพิมพ์** | กันเอาไปใช้แทนเอกสารควบคุมตัวจริง |

### ขนาดที่วัดได้จริง (A3 แนวนอน · กรอบผัง 390×215 มม.)
| จำนวน OP | viewBox | ตัวอักษรบนกระดาษ | กล่องขั้นงาน |
|---|---|---|---|
| 12 | 1058×824 | 2.87 มม. (~8pt) | 44 มม. |
| 29 (พาร์ท 306 จริง) | 1310×940 | 2.52 มม. (~7pt) | 38 มม. |
| 45 | 1842×998 | 2.33 มม. | 36 มม. |
| 60 | 2108×1114 | 2.04 มม. | 31 มม. |

**ยังไม่ทำ:** ผังไม่ได้ลากเส้นว่า child part ตัวไหนเข้า OP ไหน (ข้อมูลมีที่ `child_parts` แต่เป็น
ความสัมพันธ์แบบ 1-ต่อ-หลาย เช่น W520781-S เข้า OP 130/140/170/200 → ลากจริงจะเป็นเส้นพันกัน
ตอนนี้รวมเป็นบัสเส้นเดียว) · ยังไม่แสดงขั้นย่อย (10.1 → 10.2) ที่มีใน `pe_cp_items.sub_op`
