...
| 10 | 🔴 | ผลิต FG → เข้าคลังอัตโนมัติ | `src/pages/DailyReport.jsx:1526` | ยอดผลิตบางส่วนของใบที่ยกยอด/ยกเลิก ไม่เคยเข้าคลังเลย — โพสต์เฉพาะ 'ส่วนที่เหลือ' ตอนปิดใบสุดท้าย | ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ) |
| 11 | 🔴 | ผลิต FG → เข้าคลังอัตโนมัติ | `supabase/migrations/20260821_explode_demand_lot_guard.sql:31` | ถอยใบแล้วสแกนปิดใหม่ → fn_explode_child_demand ระเบิด demand ซ้ำ 2 เท่า (ไม่มี dedup ต่อใบแบบฝั่ง inflow) | ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ) |
...
| 17 | 🔴 | ระเบิด BOM + backflush + accumulator | `src/pages/DailyReport.jsx:1281` | ถอยใบ (↩️) แล้วสแกนปิดใหม่ = fn_explode_child_demand ระเบิด BOM ซ้ำ 2 รอบ — หักมินิสโตร์ซ้ำ + demand สะสมซ้ำ | ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ) |
| 18 | 🔴 | ระเบิด BOM + backflush + accumulator | `src/pages/DailyReport.jsx:1526` | ใบยกยอดข้ามกะ: ยอดที่ผลิตจริงในกะแรก (qty_actual) ไม่เคยผ่าน trigger — backflush/demand/FG stock ของส่วนนั้นหายถาวร | ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ) |
...
| 20 | 🟡 | ระเบิด BOM + backflush + accumulator | `supabase/migrations/20260819_demand_flow_routing.sql:128` | ยอดค้างเกินเพดาน MAX_LOTS มองไม่เห็นที่ไหนเลย — v_demand_flow_blocks กรอง lot_size>0 ทิ้ง ขัดกับที่ comment ใน guard migration อ้างไว้เอง | ✅ apply แล้ว 2026-08-25 — วัดผล: no_lot_size 0 · backlog_capped 4 พาร์ท 3,078 ชิ้น (ยอด 1.3 ล้านเดิมไหลออกเป็นใบสั่งซื้อ 2,336 ใบแล้ว) |
...
| 23 | 🔵 | ระเบิด BOM + backflush + accumulator | `supabase/migrations/20260821_explode_demand_lot_guard.sql:38` | trigger ระเบิด demand ไม่กัน is_operation — guard 'OP ห้ามเข้าคลัง' (20260820) แก้เฉพาะ fn_post_confirmed_output | ✅ apply แล้ว 2026-08-25 (OP guard ใน explode — รวมในไฟล์ partial_output_and_explode_dedup) |
...
| 26 | 🔴 | Store ชั้นใน | `supabase/migrations/20260821_store_abnormal_view.sql:40` | เคส C ของ v_store_abnormal ไม่ frame-aware — รอบหลังเที่ยงคืน (00:00-07:59) หลุดผ่านตัวกรอง hour<20 แล้วถูกตีเป็น "เลยเวลา" ปลอม | ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย) |
| 27 | 🟡 | Store ชั้นใน | `supabase/migrations/20260821_store_abnormal_view.sql:29` | เคส A/B เทียบ min/max ระดับ "ต่อ mat" กับ on-hand ระดับ "ต่อ location" + แถว (line,mat) ที่ net เป็น 0 ค้างในวิวตลอดกาลแล้วยิง sev 3 รายวัน | ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย) |
...
| 32 | 🔵 | Store ชั้นใน | `supabase/migrations/20260821_store_abnormal_view.sql:42` | นิยาม dwell ของเคส C ใช้ coalesce(...,0) ต่างจาก default (points··1)×(min··10) ของ getRoundStatus — drift ชนิดที่วิวถูกสร้างมาเพื่อฆ่า | ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย) |
...
- **ไฟล์:** `src/pages/DailyReport.jsx:1526` · **โดเมน:** ผลิต FG → เข้าคลังอัตโนมัติ · **สถานะ:** ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ)
...
- **ไฟล์:** `supabase/migrations/20260821_explode_demand_lot_guard.sql:31` · **โดเมน:** ผลิต FG → เข้าคลังอัตโนมัติ · **สถานะ:** ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ)
...
- **ไฟล์:** `src/pages/DailyReport.jsx:1281` · **โดเมน:** ระเบิด BOM + backflush + accumulator · **สถานะ:** ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ)
...
- **ไฟล์:** `src/pages/DailyReport.jsx:1526` · **โดเมน:** ระเบิด BOM + backflush + accumulator · **สถานะ:** ✅ apply แล้ว 2026-08-25 (`20260825_partial_output_and_explode_dedup.sql` — user รันผ่าน SQL Editor · markers seed 8,985 ใบ)
...
- **ไฟล์:** `supabase/migrations/20260819_demand_flow_routing.sql:128` · **โดเมน:** ระเบิด BOM + backflush + accumulator · **สถานะ:** ✅ apply แล้ว 2026-08-25 — วัดผล: no_lot_size 0 · backlog_capped 4 พาร์ท 3,078 ชิ้น (ยอด 1.3 ล้านเดิมไหลออกเป็นใบสั่งซื้อ 2,336 ใบแล้ว)
...
- **ไฟล์:** `supabase/migrations/20260821_explode_demand_lot_guard.sql:38` · **โดเมน:** ระเบิด BOM + backflush + accumulator · **สถานะ:** ✅ apply แล้ว 2026-08-25 (OP guard ใน explode — รวมในไฟล์ partial_output_and_explode_dedup)
...
- **ไฟล์:** `supabase/migrations/20260821_store_abnormal_view.sql:40` · **โดเมน:** Store ชั้นใน: LineStock / WIP / StoreMonitor · **สถานะ:** ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย)
...
- **ไฟล์:** `supabase/migrations/20260821_store_abnormal_view.sql:29` · **โดเมน:** Store ชั้นใน: LineStock / WIP / StoreMonitor · **สถานะ:** ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย)
...
- **ไฟล์:** `supabase/migrations/20260821_store_abnormal_view.sql:42` · **โดเมน:** Store ชั้นใน: LineStock / WIP / StoreMonitor · **สถานะ:** ✅ apply แล้ว 2026-08-25 (v2 + v3 `20260825_store_abnormal_group_purchases.sql` — A 33→28 · เคส C ปลอมหาย)
