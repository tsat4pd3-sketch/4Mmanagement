...
> **วันที่จัดทำ:** 11 กันยายน 2026 · **แก้ไขครั้งที่ 1:** 14 กันยายน 2026 · **ผู้จัดทำ:** ทีมพัฒนา ESM
>
> **สาระที่แก้ในครั้งที่ 1 (สำคัญต่อ IT):** ตัดระบบแจ้งเตือน **Telegram** ออกจากแผนย้าย
> ⇒ **server ไม่ต้องออกอินเทอร์เน็ตอีกต่อไป** (ฉบับแรกขอ whitelist 5 โดเมน — ยกเลิกทั้งหมด ดู §5)
> ⇒ ความต้องการใหม่ 1 ข้อแทน: ขอใช้ **SMTP ภายในของบริษัท** ส่งอีเมลแจ้งเตือน (§5.2)
...
| **ไม่ต้องมี** | IIS · .NET · SQL Server · Oracle · Active Directory integration (ปัจจุบันยังไม่ใช้) · **อินเทอร์เน็ตขาออกสำหรับใช้งานประจำวัน** (ตัด Telegram แล้ว — §5) |
...
| ข้อที่มักถูกมองข้าม | 1) ต้องเป็น **HTTPS จริง** ไม่ใช่ http (กล้องสแกน QR ทำงานเฉพาะ secure context) · 2) ขอ **บัญชี SMTP ภายในบริษัท 1 บัญชี** สำหรับอีเมลแจ้งเตือน (มาแทน Telegram) · 3) ต้องตั้ง **timezone = Asia/Bangkok + NTP** เพราะระบบตัดวันทำงานที่ 08:00 น. |
...
ผู้ใช้ใน LAN/Wi-Fi        │                                                                  │
  ...
  └─────────────────────►│        │                                                         │
                         ...
                         └──────────────────────────┬───────────────────────────────────────┘
                                                    │ SMTP ภายในบริษัท (ไม่ออกอินเทอร์เน็ต)
                                                    └──► mail server ของบริษัท → อีเมลแจ้งเตือน
```

> **ไม่มีลูกศรออกอินเทอร์เน็ต** — ต่างจากฉบับแรกที่ต้องยิง Telegram / push ออกนอกบริษัท
...
## 5. การเชื่อมต่ออินเทอร์เน็ต — **ไม่ต้องเปิดแล้ว** (เปลี่ยนจากฉบับแรก)

### 5.1 สิ่งที่ยกเลิกไป

ฉบับแรกขอ whitelist 5 กลุ่มโดเมนเพื่อให้ระบบแจ้งเตือนทำงาน **ยกเลิกทั้งหมด** เพราะตัด Telegram ออกจากแผนย้าย:

| ปลายทางที่เคยขอ | เคยใช้ทำอะไร | สถานะ |
|---|---|---|
| `api.telegram.org` | แจ้งเตือนเข้ากลุ่ม Telegram | ❌ **ไม่ใช้แล้ว** — เปลี่ยนเป็นกระดิ่งในแอป + อีเมลภายใน |
| push service ของ Google / Apple / Mozilla | เด้งแจ้งเตือนมือถือแม้ปิดแอป | ❌ **ไม่ใช้แล้ว** (ตัดสินใจพร้อมกัน — ต้องพึ่งบริการนอกเหมือนกัน) |
| `fonts.googleapis.com` · `fonts.gstatic.com` | ฟอนต์ Sarabun | ❌ ไม่ต้องแล้ว — ทีมพัฒนาย้ายฟอนต์มาไว้ในเครื่อง (§13) |
| `api.anthropic.com` | ผู้ช่วย AI รับแจ้งปัญหาผ่าน Telegram | ❌ ถอดพร้อม Telegram |
| Docker Hub · ghcr.io · registry.npmjs.org | ดึง image + ติดตั้ง | ⚠️ **ยังต้องใช้ เฉพาะตอนติดตั้งและตอนอัปเดตเวอร์ชัน** ไม่ต้องเปิดค้างไว้ (หรือใช้ private registry / offline mirror ของบริษัทก็ได้) |

⇒ **ในการใช้งานประจำวัน server ตัวนี้ไม่ต้องคุยกับอินเทอร์เน็ตเลย** ปิด outbound ได้สนิท

### 5.2 สิ่งที่ขอแทน — SMTP ภายในบริษัท 1 บัญชี

การแจ้งเตือนหลังย้ายมี 2 ช่องทาง ทั้งคู่อยู่ในบริษัท:

| ช่องทาง | ทำงานยังไง | ต้องการอะไรจาก IT |
|---|---|---|
| **กระดิ่งในแอป** | เขียนลงฐานข้อมูลแล้วเด้งที่ไอคอนกระดิ่ง + มีเสียง เห็นทันทีที่เปิดแอป/จอหน้าไลน์ | ไม่ต้องการอะไรเพิ่ม (มีอยู่แล้ว) |
| **อีเมลภายใน** | ส่งสรุป/เรื่องด่วนเข้าอีเมลบริษัทของหัวหน้าแต่ละระดับ | **บัญชี SMTP ภายใน 1 บัญชี** |

ข้อมูล SMTP ที่ขอ (กรอกในแบบฟอร์ม §16 ข้อ 13):

| รายการ | ตัวอย่าง |
|---|---|
| SMTP host + port | `mail.thaisummit.local` : 587 (STARTTLS) หรือ 25 |
| วิธียืนยันตัวตน | ไม่ต้อง auth (relay ตาม IP) / หรือ username + password |
| อีเมลผู้ส่ง (From) | `esm-noreply@thaisummit.co.th` |
| อนุญาตให้ IP ของ server นี้ relay | ต้องอนุญาต ไม่งั้นเมลไม่ออก |

> **ข้อแลกที่ต้องยอมรับ (บอกตรงๆ):** ตัดทั้ง Telegram และ push มือถือแล้ว **จะไม่มีอะไรเด้งเข้ามือถือทันทีอีก**
> เรื่องด่วนหน้างาน (เช่น เครื่องหยุดเกิน 15 นาที · เรียกช่างซ่อม) จะรับรู้ผ่าน **จอ Andon หน้าไลน์ + เสียงเตือนในแอป + อีเมล** แทน
> ถ้าภายหลังต้องการให้เด้งมือถือเหมือนเดิม ทำได้โดยเปิด outbound ไปหา push service อย่างเดียว (ไม่ต้องรื้ออะไร)

### 5.3 Inbound จากอินเทอร์เน็ต

**ไม่ต้องเปิดเลย** — ฟีเจอร์เดียวที่เคยต้องการ (Telegram ยิง webhook เข้ามา) ถูกตัดไปพร้อม Telegram
ถ้าต้องการใช้งานจากนอกโรงงาน ให้ผ่าน **VPN ของบริษัท** เท่านั้น (ดูเหตุผลใน §10)
...
## 7. Edge Functions — 21 ตัว (ย้ายจริง **19 ตัว** · ต้องรัน Deno runtime)

เป็นโค้ดฝั่ง server ขนาดเล็ก (TypeScript/Deno) มาพร้อม repo ครบทุกตัว — **ต้อง deploy 19 ตัว** (ตัด `send-push` กับ `telegram-webhook` ออกตามการตัด Telegram/push) ไม่งั้นฟีเจอร์ที่เกี่ยวจะเงียบ

| กลุ่ม | ฟังก์ชัน | ทำอะไร |
|---|---|---|
| แจ้งเตือน | `send-notification`, `send-cqi15-notification`, `send-mtn-notification`, `send-store-notification`, `send-event-notification` | เขียนกระดิ่งในแอป + (หลังย้าย) ส่งอีเมลภายใน — **ถอดส่วน Telegram ออกตอนย้าย** |
| ~~ไม่ต้อง deploy~~ | ~~`send-push`~~ · ~~`telegram-webhook`~~ | Web Push + Telegram bot — ตัดออกทั้งคู่ |
...
| ~~`TELEGRAM_BOT_TOKEN` · `TELEGRAM_CHAT_ID` · `TELEGRAM_WEBHOOK_SECRET`~~ | ~~Telegram~~ | ❌ **ไม่ใช้แล้ว** — ตัดออกจากแผนย้าย |
| ~~`ANTHROPIC_API_KEY`~~ · ~~VAPID keys (Web Push)~~ | ~~ผู้ช่วย AI · push มือถือ~~ | ❌ **ไม่ใช้แล้ว** |
| `SMTP_HOST` · `SMTP_PORT` · `SMTP_USER` · `SMTP_PASS` · `SMTP_FROM` | เชื่อม mail server ภายในบริษัท (§5.2) | **IT ให้ข้อมูล** → ทีมพัฒนาตั้งค่า |
| `CRON_SECRET`, `INGEST_SECRET`, `CLEANUP_TOKEN` | shared secret กันเรียกมั่ว | **สร้างใหม่** ตอนติดตั้ง |
...
| อีเมล | ระบบเดิมไม่เคยส่งอีเมล · **หลังตัด Telegram จะเพิ่มการส่งอีเมลแจ้งเตือนผ่าน SMTP ภายใน** (§5.2) — ใช้เฉพาะ "แจ้งเตือน" **ไม่เกี่ยวกับการ login** · ยังต้อง **ปิด "confirm email" ใน GoTrue** ตอนติดตั้ง ไม่งั้นสร้างผู้ใช้ใหม่แล้ว login ไม่ได้ · รีเซ็ตรหัสผ่านยังทำโดย admin กดปุ่มในแอปเหมือนเดิม |
...
| 6 | ~~ตั้ง Telegram bot webhook ใหม่~~ | ตัดออกแล้ว ไม่ต้องทำ | — |
| 7 | **ปิดช่องว่าง "แจ้งเตือนที่วิ่ง Telegram ทางเดียว"** | สำรวจ 14 ก.ย. 2026 พบ 3 งานตั้งเวลาที่ส่ง Telegram อย่างเดียว ไม่เคยเขียนกระดิ่งในแอป (ถอด Telegram = หายสนิท): สรุป 4M ค้าง · สรุปใบซ่อมค้าง · เฝ้าระวังคุณภาพ FME → **แก้โค้ดครบทั้ง 3 แล้ว** | ✅ ลงใช้จริง 2 · เหลือ `qa-fme-scan` รอ deploy |
| 8 | **เพิ่มช่องทางอีเมลภายใน (SMTP)** | เขียนตัวส่งอีเมลในระบบแจ้งเตือนกลาง + เลือกเปิด/ปิดต่อเหตุการณ์ที่ `/notification-config` เหมือน Telegram เดิม | ⬜ รอข้อมูล SMTP จาก IT (§16 ข้อ 13) |
| 9 | **ถอดโค้ด Telegram / Web Push ออก** | ทำ **ตอน cutover เท่านั้น** ไม่ใช่ตอนนี้ — ระบบบน cloud ยังใช้ Telegram อยู่ ถอดก่อนย้าย = หัวหน้าไม่ได้รับแจ้งเตือนทันที | ⬜ ทำที่ขั้น 8 ของ §14 |
| 10 | **เลือกผู้รับ "ในแอป" ให้ครบทุกเหตุการณ์** | ยังมี **21 เหตุการณ์ที่ไม่ได้ตั้งผู้รับในแอป** (เช็คชื่อ · ปิดกะ · downtime · Daily PM ฯลฯ) — โค้ดรองรับแล้ว แค่ยังไม่ตั้งค่า ต้องตั้งให้ครบก่อนถอด Telegram ไม่งั้นเงียบ · ⚠️ ตั้งแบบเหมาไม่ได้: `downtime` เกิดจริง **145 ครั้ง/วัน** ต้องเลือกผู้รับให้แคบ | ⬜ รอตัดสินใจเลือกผู้รับ |
...
| 6 | Deploy Edge Function 19 ตัว + ตั้ง secret (รวม SMTP) + ตั้ง cron 16 งาน | ทีมพัฒนา | 0.5 วัน |
| 7 | **UAT** — ทดสอบครบทุกฟีเจอร์: login ทุก role · เปิด/ปิดใบผลิต · สแกน QR · อัปโหลดรูป · **กระดิ่งแจ้งเตือน + อีเมลภายใน** · Realtime · export Excel/PDF · จอ TV | ฝ่ายผลิต + ทีมพัฒนา | 3–5 วัน |
...
| ทำงานตอนเน็ตบริษัทล่ม | ใช้ไม่ได้ทั้งระบบ | ✅ **ใช้งานได้ครบ 100%** — ไม่มีส่วนไหนพึ่งอินเทอร์เน็ตอีกแล้ว |
| การแจ้งเตือน | Telegram — ข้อความงานไปอยู่บนเซิร์ฟเวอร์ผู้ให้บริการต่างประเทศ | ✅ กระดิ่งในแอป + อีเมลภายใน **ข้อมูลไม่ออกนอกบริษัท** · ⚠️ แลกกับการไม่มีอะไรเด้งมือถือทันที (§5.2) |
...
| 6 | ~~outbound whitelist 5 โดเมน~~ **ยกเลิกแล้ว** — ไม่ต้องเปิดเน็ตให้ server (ขอแค่ยืนยันว่ารับทราบ) | ............ |
...
| 13 | **ข้อมูล SMTP ภายใน** — host/port · วิธียืนยันตัวตน · อีเมลผู้ส่ง · อนุญาต IP ของ server relay ได้ไหม (§5.2) | ............ |
| 14 | รับทราบว่าหลังตัด Telegram **จะไม่มีแจ้งเตือนเด้งเข้ามือถือทันที** — ใช้จอ Andon + กระดิ่งในแอป + อีเมลแทน (§5.2) | ............ |
