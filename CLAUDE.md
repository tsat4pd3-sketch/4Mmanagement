...
- **แจ้งเตือน Telegram (format mirror ระบบเดิม):** edge function `send-mtn-notification` — `mtn_reported` (แจ้งซ่อม + **แนบรูปก่อน**), `mtn_repaired` (สรุปผลซ่อม ตอนซ่อมเสร็จ step3: เลข MO/ช่าง/สาเหตุ/วิธีแก้ + **รูปหลัง**), `mtn_closed` (อนุมัติปิด) · วันที่รูปแบบ พ.ศ. MM/DD/YYYY · route ผ่าน notification_rules category maintenance (ตั้งค่า/แก้ข้อความที่ `/notification-config`) · **ไม่แจ้งตอนรับงาน step2** (ตามระบบเดิม)
- **พิมพ์ใบ MO / บันทึก PDF:** ปุ่ม 🖨️ ใน DetailDrawer → `printMoReport()` เปิดหน้าพิมพ์ (window.open + print เหมือน Report) layout ตามฟอร์มเดิม **FM-JIG-008**: หัวบริษัท + 6 ส่วน + รูปก่อน/หลัง + ลายเซ็นอนุมัติ 4 ช่อง (JIG/MTN · QA · PD · MGR) — เซฟเป็น PDF จาก dialog พิมพ์ของเบราว์เซอร์
- **Scope:** leader = family ไลน์ตัวเอง (branch มาก่อน) · role อื่นตาม `sections` — pattern มาตรฐาน
...
└── functions/         # 10 ตัว (ซอร์สอยู่ใน repo ครบแล้ว): send-notification, send-cqi15-notification,
