...
- **ไม่มี polling · ไม่มี migration · ไม่มี permission key ใหม่** (อ่านอย่างเดียว ใช้ `page:/daily-report` เดิม)
  · scope = ครอบครัวไลน์ผ่าน `getLineFamilyNames` (`lines` ยังโหลดไม่เสร็จ = ถอยไปชื่อไลน์ตัวเอง **ห้ามได้ set ว่าง**)
  · `.in()` / แบ่งหน้า ผ่าน `fetchAllPages`/`fetchByIds` ตามกฎ · `truncated`/`error` ขึ้นแถบเตือน **ห้ามกลืน**
- **ไลน์ที่ไม่มีทั้ง ledger และการใช้ของ = ไม่ render อะไรเลย** (ไม่รกจอไลน์ที่ยังไม่เข้าระบบ)
