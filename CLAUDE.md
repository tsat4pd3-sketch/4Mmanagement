...
| Logistic - Store | `/line-stock` | LineStock — เมนู “สต๊อกในไลน์” | ทุก role |
...
| คุณภาพ & วิศวกรรม | `/qa` | QualityControl — Dashboard คุณภาพ · **✅ ใบตรวจ (Check Sheet)** · SPC/Cp-Cpk · NCR · CAPA/8D · **🗑️ ถังเหลือง/ถังแดง** · 📮 เคลมลูกค้า · เครื่องมือวัด (ดู section "QA Inspection — setup → ใบตรวจ" + "ใบรายงานปัญหาการผลิต + ถังเหลือง/ถังแดง") | admin/manager/supervisor/leader/qa/doc_control |
...
- **⚠️ ขั้นนี้ยังไม่ย้าย read path** — `profiles.team/line_id/section` ยังเป็นตัวที่ระบบใช้จริงอยู่ (UserContext) การให้ UserContext อ่านจาก `employees` ผ่าน `employee_id` เป็นคนละก้อน **เพราะแตะ scoping ทั้งระบบ** (ใครเห็นข้อมูลอะไร) → ต้องวัดก่อน-หลังแล้วทำแยก commit
