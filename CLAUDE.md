...
- **🔬 แท็บ "เจาะ 1 กะจริง" (2026-08-13) — เดินให้ดูทีละชั้นบนกะที่แย่ที่สุดที่เกิดขึ้นจริง** (3 แท็บแรกตอบ "มองเห็นอะไรได้" แต่ผู้บริหารถามต่อว่า "แล้วช่วยแก้ของจริงยังไง"): ชั้น 1 กะนี้เสียหายเท่าไหร่ (แตก A/P/Q + ชี้ตัวฉุด) → ชั้น 2 เวลาหายไปกับ 4M ตัวไหน → ชั้น 3 ย้อนกลับถามเอกสารออกแบบ (PFC → FMEA → Control Plan) ว่า "เคยคาดไว้มั้ย" → ชั้น 4 สรุป + ทางลัด
  ...
  - **⚠️ ระบบยังไม่มีแกน 4M ที่ master** (`dr_downtime_types` มีแต่ `six_big_loss`/`waste_type`) → `fourMOf()` **เดาจากข้อความที่พนักงานพิมพ์** (`FOUR_M_WORDS` เรียงเจาะจง→กว้าง ตัวแรกชนะ · ตกท้ายด้วย `six_big_loss` ที่หัวหน้าตั้งเองซึ่งเชื่อถือได้กว่าการเดาคำ) → **จอต้อง (ก) ติดป้ายว่าเดาจากข้อความ (ข) โชว์ข้อความต้นฉบับใต้ทุกกลุ่มให้คนตรวจได้ (ค) โชว์รายการที่จัดไม่ได้ ห้ามยัดมั่วเข้ากลุ่มใดกลุ่มหนึ่ง** · ทางแก้ถาวร = เพิ่มช่อง 4M ที่ประเภท Downtime ให้คนเลือกเอง (ยังไม่ทำ)
  ...
  - **ชั้น 3 = จุดที่วันนี้ตอบไม่ได้เลย** — ตาราง `pe_doc_sets`/`pe_processes`/`pe_fmea_items`/`pe_cp_items`/`pe_doc_revisions` (Main) **มีครบตามมาตรฐาน AIAG แล้ว** (FMEA มี S-O-D + recommended/taken action + new S-O-D · CP มี spec/method/sample_size/frequency/control_method/reaction_plan) แต่ **0 แถวทุกตาราง · ไม่มีหน้าจอสักหน้า · ไม่มี migration file ในรีโป** (ถูกสร้างตรงใน DB) → real mode บอกตรงๆ ว่าตอบไม่ได้ · demo mode เติมตัวอย่างคำตอบโดยใช้**ชื่อเครื่อง/อาการ/ประเภทของเสียจริงจากกะนั้น** + ค่า S-O-D/RPN/ความถี่เป็นตัวเลขสมมติ
...
- **⭐ ช่องว่างที่ใหญ่ที่สุดที่ค้นพบจากแท็บเจาะปัญหา (2026-08-13): ไม่มีหน้าจอ PFC / FMEA / Control Plan เลย** — ตารางรออยู่ครบแล้ว (ดูชั้น 3 ข้างบน) แต่ยังไม่มีที่ให้ PE กรอก → ทั้งระบบจึง**วัดความสูญเสียได้ละเอียดมาก แต่เทียบกลับไปที่เจตนาการออกแบบไม่ได้เลย**: ทุกครั้งที่เครื่องเสีย/ของเสีย ตอบไม่ได้ว่า "เคยรู้อยู่แล้วแต่ปล่อย" หรือ "ไม่เคยคิดถึงเลย" · เป็น**เงื่อนไขบังคับของ Prescriptive (ระดับ 4)** — จะบอกว่า "ต้องทำอะไร" ได้ ต้องรู้ก่อนว่าตอนออกแบบตั้งใจคุมอะไรไว้ · ทำเมื่อไหร่ให้ผูก `pe_fmea_items` ↔ `downtime_logs`/`defect_logs` เพื่อปิดลูป "อาการที่เกิดจริง → failure mode ที่เคยคาดไว้ → ทบทวน S-O-D"
