...
> 2. **สวิตช์เปิด EXP farm** — `fn_daily_skill_farm` นับเฉพาะจุดที่ **`min_score >= 70`** (`20260713_skill_farming_server_side.sql:80`) → **สกิลที่ทุกจุดตั้งต่ำกว่า 70 จะไม่สะสมคะแนนเองตลอดกาล** (ผลข้างเคียง ไม่ได้ตั้งใจ · คนตั้งค่าไม่มีทางรู้)
>
> **เกิดจริง:** `Control RB/MC` ตั้ง 25/50 ทั้ง 8 จุด → ไม่เคย farm เลย → คะแนนสูงสุดทั้งโรงงาน = 2 → `skillOk` false ถาวร → วางคนที่ 8 จุดนั้นตก **4M 🔴 (OJT + SV + QA)** ทุกครั้ง ไม่มีวันหลุด
...
> farm ให้ +1/วัน เฉพาะคนที่**ยืนจุดที่ตั้ง ≥70 อยู่แล้ว** → ทั้งโรงงานได้ 26-33 แต้ม/วัน จาก 172 คน **ไม่มีทางสร้างคะแนนตั้งต้นให้สกิลใหม่ทั้งไลน์ได้**
