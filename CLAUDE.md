...
> - backfill จากประวัติจริง: `20260806_backfill_home_positions_from_history.sql` (1 กะ = 1 เสียง · เท่ากันเอาล่าสุด · **ไม่ทับของที่หัวหน้าตั้งเอง** · orphan นับเป็น "ยังไม่ตั้ง" ต้องซ่อม) — **apply แล้ว 2026-08-06: 105 orphan → ซ่อมจากประวัติ 88 + ล้างทิ้ง 17 (ลาออก/พนักงานถูกลบ) · เพิ่มใหม่ 18 · ยืนยัน orphan = 0**
> - **คนที่ยังทำงานแต่ไม่มีประวัติเลย ระบบเดาให้ไม่ได้ — ต้องให้หัวหน้าตั้งเองที่หน้าจัดการไลน์ผลิต** (ตำแหน่งประจำมั่วแย่กว่าไม่มี)
...
> ### ⚠️ กฎเหล็ก — `station_requirements.min_score` ทำ 2 หน้าที่พร้อมกัน (2026-08-06)
> 1. **เกณฑ์ผ่าน** ของ `computeFit` (ตั้งใจ)
> 2. **สวิตช์เร่ง EXP farm รายวัน** — `fn_daily_skill_farm` (+1/วัน) นับเฉพาะจุดที่ **`min_score >= 70`** (`20260713_skill_farming_server_side.sql:80`)
>
> **⚠️ อย่าสับสน 2 ชั้น (เคยสรุปผิดมาแล้ว 2026-08-06):**
> | | เงื่อนไข | เพิ่ม | กรอง ≥70? |
> |---|---|---|---|
> | `fn_daily_skill_farm` | มาทำงานที่จุดนั้น | +1/วัน | ✅ **มี** |
> | `fn_weekly_skill_update` | อยู่ **จุดเดิม ≥3 วัน** ในสัปดาห์นั้น (`group by assigned_line having count(distinct work_date) >= 3`) | +2/สัปดาห์ | ❌ **ไม่มี** |
>
> → สกิลที่ทุกจุดตั้ง <70 **ยังโตได้ผ่านทางรายสัปดาห์ แต่ช้ามาก (+2/สัปดาห์)** — ไม่ใช่ "ไม่โตเลย"
> **เกิดจริง:** `Control RB/MC` ตั้ง 25/50 ทั้ง 8 จุด → ได้เฉพาะ +2/สัปดาห์ → คะแนนสูงสุดทั้งโรงงาน = 2 หลังสร้างมา 3 สัปดาห์ · จาก 2 ไป 50 ต้องใช้ **~24 สัปดาห์** → ระหว่างนั้น `skillOk` false → วางคนที่ 8 จุดนั้นตก **4M 🔴 (OJT + SV + QA)** ทุกครั้ง
> **เพดานขั้น 24/49/74/99** หยุด farm จนกว่าจะ**อนุมัติ level up** — นี่คือเคส "ทำงานทั้งปีสกิลก็ไม่ขึ้น" ที่เกิดได้จริง (คนละเรื่องกับกฎ 70)
> - **LineSetup เตือน inline แล้ว**: ⛔ แดง "ไม่มีใครผ่านเกณฑ์นี้ (สูงสุดที่มีจริง = N)" · ⚠️ ส้ม "ตั้ง <70 ระบบจะไม่สะสมให้เอง"
> - **ห้ามดัน min_score ขึ้น 70 เพื่อให้ farm ทำงาน** — นั่นคือโกหกเกณฑ์ทักษะเพื่อหลอกกลไก จุดที่ต้องการแค่ 25 ก็ควรเขียน 25 · ทางที่ถูกคือประเมินคะแนนให้ หรือแก้กฎ farm (ยังไม่ได้ทำ — เป็นการตัดสินใจเชิงนโยบาย)
>
> ### ⚠️ EXP farm เป็นตัว "เติมทีละนิด" ไม่ใช่ "ประเมินตั้งต้น" — ต้องใช้แท็บ 📊 ประเมินทั้งทีม
> farm รายวันให้ +1/วัน เฉพาะคนที่**ยืนจุดที่ตั้ง ≥70** · รายสัปดาห์ +2 ถ้าอยู่จุดเดิม ≥3 วัน → ทั้งโรงงานได้ 26-33 แต้ม/วัน จาก 172 คน **ไม่มีทางสร้างคะแนนตั้งต้นให้สกิลใหม่ทั้งไลน์ได้**
> ข้อมูลจริง 2026-08-06 (ยิ่งสกิลใหม่ ยิ่งไม่มีใครมี): สร้าง 16/07 = 5-10 คน · 09/06 = 8-27 คน · 29/05 = 19-48 คน · Safety/QC (seed ตอนตั้งระบบ) = **172 ครบทุกคน**
> → **สร้างสกิลใหม่ทุกครั้งต้องไปให้คะแนนตั้งต้นที่ `/operator` แท็บ 📊 ประเมินทั้งทีม** (เลือกสกิล+ไลน์ → กรอกทั้งทีมในจอเดียว · ปุ่มเติมเร็ว 25/50/70/75 ให้เฉพาะคนที่ยังไม่มี · ไม่ส่ง `pending_level` ไปด้วย คนที่ค้างรออนุมัติจึงไม่ถูกล้างสถานะ) ไม่งั้นสกิลนั้นจะไม่มีวันครอบคลุม
...
>
> #### ⚠️ กฎเหล็ก — ปิดใช้งานเครื่อง (`machines.is_active=false`) ต้องหลุดจากหน้า PM/AM ด้วย (2026-08-05)
> **`jigs` (ทะเบียนอุปกรณ์ที่มีแผน PM) ไม่มีคอลัมน์ `is_active`** — แถวเงาของเครื่อง (`jigs.machine_id` ชี้กลับ `machines`) จึงยัง "ดูมีชีวิต" ต่อไปแม้เครื่องจริงถูกปลด → **หน้างานถูกสั่งให้ตรวจเครื่องที่ปลดไปแล้วทุกต้นกะ และไลน์ค้าง 🟠 ตลอดเพราะตรวจไม่ได้** (เจอจริง: BD-05 Bending ค้างในทะเบียน AM ของ HYDROFORM + TEST1 ข้อมูลทดสอบ)
> - **จุดที่ลิสต์อุปกรณ์ PM ต้องกรองด้วย `loadRetiredEquipIds(module)` ใน `src/lib/pmChecklists.js` เสมอ** (คืน Set ของ jig id ที่เครื่องแม่ปิดใช้งาน/หายไป) — ทำแล้วที่ **DailyPM** (ทะเบียน AM) + **PMCheckData** (หน้าตรวจ) · **จิ๊ก/แม่พิมพ์จริง (`machine_id = null`) ไม่เกี่ยว** ไม่มีเครื่องแม่ให้เช็ค แสดงตามเดิม
> - **ห้ามกรองแบบเงียบ** — DailyPM ขึ้นบรรทัด "ซ่อน N เครื่องที่ปิดใช้งานแล้ว" + ลิงก์ไปฐานข้อมูลเครื่องจักร
> - **PM Setup ไม่ซ่อน** (setup ต้องเห็นของทั้งหมด ไม่งั้นย้าย/ล้างจุดตรวจไม่ได้) — การ์ดขึ้นชิปแดง "⛔ เครื่องปิดใช้งานแล้ว — ไม่แสดงในหน้าตรวจ" แทน · และ picker "เลือกจาก Machine Master" กรอง `is_active` แล้ว (ห้ามลงจุดตรวจให้เครื่องที่ปลดไปแล้ว)
> - ข้อมูลค้างล้างด้วย migration `20260805_am_registry_drop_retired_machines.sql` (DR · **apply แล้ว** — ปิด `is_active` 2 แถว ไม่ลบ เก็บเป็นประวัติ · เปิดเครื่องกลับมาใช้ก็ติ๊กลงทะเบียนใหม่ได้)
