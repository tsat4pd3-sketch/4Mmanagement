...
> **โมเดล:** `dr_products.is_operation` (boolean) + `op_parent_mat` (MAT พาร์ทจริง) + `op_seq` (เลข OP ตาม PFC) — migration `20260817_operation_items_dr.sql` (DR · **apply แล้ว 2026-08-17** — user รันเองผ่าน SQL Editor เพราะเซสชันเว็บเข้า DB ไม่ได้ · ติดธงจริง 10/11 — `173 (M6 มีเกลียว)` ชื่อใน DB ไม่ตรง migration เป๊ะ ให้เก็บผ่านกล่อง 🔩 ใน /products หรือ SQL like) · backfill เฉพาะที่ยืนยันจาก PFC/ชื่อตรงกัน (M6/M8/M10→20058626 · E024→20066660) ที่เหลือติดธง OP แต่ `op_parent_mat=null` (**ห้ามเดา parent**)
...
- **⭐ ช่องว่างที่เหลืออยู่หลังมีโมดูล PE แล้ว (2026-08-13): ยังไม่มีเส้นเชื่อม "อาการที่เกิดจริง ↔ failure mode ที่เคยคาดไว้"** — `/pe-docs` เก็บ PFC/PFMEA/CP ได้แล้ว และ `/adoption-outlook?tab=deep` เจาะ OEE→4M ได้แล้ว แต่**สองฝั่งยังไม่คุยกัน**: เปิดใบซ่อม/ลงของเสีย แล้วระบบยังตอบไม่ได้ว่าอาการนั้นตรงกับ `pe_fmea_items.failure_mode` ตัวไหน ทำให้ยัง**วัดความสูญเสียได้ละเอียด แต่เทียบกลับไปที่เจตนาการออกแบบไม่ได้** ("เคยรู้อยู่แล้วแต่ปล่อย" หรือ "ไม่เคยคิดถึงเลย")
  ...
  - **📌 ออกแบบเต็มแล้วที่ `docs/CLOSED-LOOP-8D-PE.md` (2026-08-17 · user สั่ง "ฝากคิด" ยังไม่ลงมือ)** — ตาราง `pe_change_requests` (ต้นเหตุ→บรรทัดเอกสาร · status `proposed→accepted→applied`) + ตัวเสนอ 3 ขาแบบ 8D (occurrence→causes · detection→Control Plan · **systemic→yokoten ข้ามพาร์ท**) + soft gate ตอนปิด D8 + วัด effectiveness จาก `defect_logs` จริง · **`pe_doc_revisions.ref_kind/ref_id` คือช่องผูกต้นเหตุที่ออกแบบรอไว้แล้ว ยังไม่มีใครเขียน** · ⚠️ ก่อนทำต้องมีข้อมูลก่อน — วันนี้ `qa_ncr`/`qa_capa` = 0 แถว
...
docs/                  # ENGINEERING-PRINCIPLES.md (หลักการแก้แบบยั่งยืน — อ่านก่อนทุกงาน) ·
                       ...
                       #   CLOSED-LOOP-8D-PE.md (ลูปปิด 8D → PFMEA/PFC/CP + yokoten ข้ามพาร์ท —
                       #     📌 user สั่ง "ฝากคิด" 2026-08-17 · ออกแบบไว้ ยังไม่ลงมือ ห้ามหยิบไปทำเอง)
```
