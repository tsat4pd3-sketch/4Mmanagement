...
>
> #### 📍 สิ่งที่ SAP มีแต่เรายังไม่มี (ตรวจ schema จริง 2026-09-02 · user ทัก)
> | SAP | เรา | ผล |
> |---|---|---|
> | **Storage Location / Prod.SLoc รายชิ้นส่วนใน BOM** | **ไม่มีเลย** — `bom_items` ไม่มีคอลัมน์นี้ · `purchase_requests` มีแค่ `dest_line` (ชื่อไลน์) · `line_stock_transactions` มีแค่ `line_name` | ระบุไม่ได้ว่าชิ้นส่วนตัวนี้ต้องเบิกจากคลังไหน — ตอนนี้ derive จากชื่อไลน์/`STORE` อย่างเดียว |
> | **Item 0010 / 0020 / 0030** | **ไม่มีคอลัมน์** (`bom_items` ไม่มี item/seq · `dr_products.op_seq` เป็นชั้น OP คนละเรื่อง) | ตารางเรียงตาม `mat_no` = ลำดับสุ่ม · อ้าง "รายการที่ 0030 ของ BOM ตัวนี้" ในใบขอแก้/ECN ไม่ได้ |
> - **Item ของ SAP นับใหม่ต่อ "ตัวแม่" ไม่ใช่นับต่อเนื่องทั้งใบ** (ยืนยันจากภาพจอ: ชั้น `.1` วิ่ง 0010→0120
>   แต่ใต้ `20066660` เริ่ม 0010 ใหม่ · ใต้ `20066662` ก็ 0010 ใหม่) — ตรงกับที่ user อ่าน
> - **⚠️ `storage_zones` (WMS เฟส 1) ไม่ใช่ SLoc** — เป็นทะเบียน**โซนคลัง**ที่ผูก mat แบบ array ไว้ดูว่าของกองอยู่โซนไหน
>   ไม่ใช่ "ชิ้นส่วนบรรทัดนี้เบิกจากคลังไหน" ของ BOM · **ห้ามยัด SLoc ลงตารางนั้น**
