...
- **Hover preview + drill-down popup (2026-07-21):** วางเม้าส์บนกรอบไลน์ = **การ์ดพรีวิวลอยตามเคอร์เซอร์** (เฉพาะ `pointerType==='mouse'` — จอสัมผัสไม่ขึ้น) สรุปทุก metric แบบย่อ (metric ปัจจุบันไฮไลต์) + สี region เข้มขึ้น · คลิกกรอบ/แถว panel = **popup เจาะดูเต็ม** (`detailLine`) การ์ดทุก metric + ตารางแยกไลน์ย่อย (ผลิต/คน/DT/NG จาก `lineStatus`/`manpower` ดิบ) · การ์ด hover ใช้ gradient ทึบ + เงา (ไม่ใช้ backdrop-filter — การ์ดเดียวไม่ใช่ marker ซ้ำ แต่เลี่ยงไว้ให้ปลอดภัยจอ TV)
