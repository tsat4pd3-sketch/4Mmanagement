...
> - **ยังไม่ได้แก้ (หนี้ที่รู้ตัว):** ยังไม่มีแกน **"ระดับงาน" (operator/technician/engineer/…)** → ข้อมูลจริงมี **ธุรการที่ role `mtn` ได้ `pm:approve` เต็ม** เพราะแยกไม่ได้ · เฟส 2 = ทำ `positions` เป็น master ที่มี `level` (ยกเครื่อง `src/utils/positions.js`) แล้วผูก AM/PM eligibility กับ level
>
