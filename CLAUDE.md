...
*"ไม่ระบุกับอื่นๆ มาอันดับ 1 กับ 2 การวิเคราะห์จะไม่มีประโยชน์เลย"* → ขยายผลทุกจอที่วิเคราะห์ input พนักงาน
**กติกา 3 ชั้น + ตัวช่วยอยู่ `src/utils/unclassified.js`** (`isVague` · `splitUnclassified` · `unclassifiedNote`)
1. 🔴 **ต้นทาง — ห้ามเขียน `'อื่นๆ'` ทับค่าที่ระบบรู้อยู่แล้ว** (มีด่าน `regressionGuards`)
   = 2 ใน 3 ของปัญหา ไม่ต้องใช้อัลกอริทึมอะไรเลย
2. **ทะเบียน taxonomy ต้องครบ + มี group** (ทีมที่ไม่มีอาการในทะเบียน = เลือกได้แค่ "อื่นๆ")
3. **จอ: ที่เหลือจริงๆ ต้องบอกตรงๆ ว่าชี้เป้าไม่ได้กี่ %** · แยก "อื่นๆ ที่มีข้อความ" (จับกลุ่มต่อได้)
   ออกจาก "ไม่กรอกเลย" (ต้องแก้ที่การกรอก) · **งานตามแผน (PM) ไม่ใช่ปัญหา กันออกจากพาเรโต แต่ห้ามซ่อน**
