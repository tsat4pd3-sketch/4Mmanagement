...
> **จัดหมวดเมนูใหม่ทั้งระบบ 2026-07-20 (คำสั่ง user):** ภาพรวม (จอแสดงผล/ผู้บริหาร) → ฝ่ายผลิต (งานประจำวัน) → วิเคราะห์ & รายงาน → พนักงาน & ทักษะ (ใหม่) → Logistic 3 ฝั่ง (ขาเข้า/ขาออก/แผนงาน — 2026-09-03) → ซ่อมบำรุง → QA/QC → ตั้งค่า,ฐานข้อมูล — ลำดับอยู่ที่ `NAV_GROUP_ORDER` ใน App.jsx
...
| Logistic - ขาเข้า | `/line-stock` | LineStock · **แท็บ 🏬 โซนคลัง (ผัง) = ทะเบียนโซนจัดเก็บ WMS เฟส 1** (`StorageZonePanel` — ดู section "โซนคลังสินค้า (WMS เฟส 1)") | ทุก role (จัดการโซน: `storage:manage`) |
...
- **🔴 กฎเหล็ก — "กำลังทำ PM อยู่" มาจาก `pm_coordination_tasks` ไม่ใช่ `pm_plans` (2026-08-26)**
  ...
  · งานประสานงานของวันนี้ที่ยังไม่ `done` → สถานะ **`busy` สีฟ้า ชนะทุกสถานะ** (สิ่งที่เกิดอยู่ตอนนี้สำคัญกว่าสิ่งที่ค้าง)
...
- **🚨 แท็บ "จอห้องช่าง" (`?dept=maintenance&view=andon`) — จอ TV ในห้องช่าง (2026-08-24 · feedback หน้างาน):** *"จอช่างควรเจาะให้ชัดเจน เห็นอลามชัดเจน · แจ้งเตือนเครื่องเบรคดาวน์ alarm เสียง มันไม่ได้มีหน้านี้"* — เดิมห้องช่างเปิด `/factory-map` ซึ่งเป็น**ผังภาพรวมใบเดียวกับที่ผู้บริหารดู** ตอบคำถามของช่างไม่ได้ และ**ไม่มี `DowntimeSiren` เลย** · component `src/components/MtnAndonBoard.jsx` (lazy)
  ...
  - **3 ชั้นเรียงตามความเร่งด่วน:** ① เครื่องที่หยุดอยู่**ตอนนี้** (นับนาทีสด · เรียกช่างขึ้นบนสุด) ② ใบซ่อมค้างแยกตามขั้น + ค้างนานสุด ③ PM เกินกำหนด/ครบใน 3 วัน
