...
- **เปลี่ยน/ลบรูปแล้วต้องลบไฟล์เก่าจาก storage เสมอ** (ลบ**หลัง** DB update สำเร็จเท่านั้น + best-effort ห้ามทำ flow หลักพัง) — ทำแล้วใน: operator.jsx (รูปพนักงาน), LineSetup.jsx (ผังไลน์ — เฉพาะผังของตัวเอง **ห้ามลบผังที่ยืมแสดงจากไลน์แม่**), ProductMaster.jsx (dr_products + parts_master — มี guard ไม่ลบรูปที่สินค้า/พาร์ทอื่นแชร์ URL เดียวกัน), QAInspectionSetup.jsx (replace/delete drawing + ลบทั้งโฟลเดอร์ตอนลบ part), PMSetup.jsx (ลบ jig = ลบรูปทั้งชุด frame-*/cp-* **+ โมเดล 3D `models/<id>.glb`** + ลบโมเดลเก่าเมื่อเปลี่ยน/ถอด), SignatureModal.jsx (ลายเซ็นเก่า — เฉพาะโฟลเดอร์ user ตัวเอง) · หน้าใหม่ที่มีการเปลี่ยนรูปต้องทำแบบเดียวกัน ไม่งั้นไฟล์กำพร้าสะสม (เคยค้าง 117 ไฟล์ / 100MB เพราะอัปโหลดชื่อใหม่ `emp_<timestamp>` โดยไม่ลบของเดิม)
- **โมเดล 3D อุปกรณ์ PM (2026-07-10):** อัปโหลด `.glb/.gltf/.stl/.obj/.stp/.step/.igs/.iges` ได้ที่ PM Setup — **normalise เป็น GLB ฝั่ง client เสมอ** (STEP/IGES ผ่าน OpenCascade WASM, STL/OBJ ผ่าน three loaders) helper `src/lib/model3d.js` (dynamic import ของหนักทั้งหมด: three ~600KB + occt wasm ~7.6MB โหลดเฉพาะตอนใช้) · viewer กลาง `src/components/Model3DViewer.jsx` (three.js orbit + auto-rotate) ใช้ทั้ง PMSetup (พรีวิว) และ PMCheckData (สลับ 📷/🧊) · เก็บ GLB ที่ `models/<jigId>.glb` ใน bucket **`jig-images`** (DR project — cap ยกเป็น **40MB** + เพิ่ม mime `model/gltf-binary`, migration `20260710_jig_images_allow_glb.sql`) · schema: `jigs.model_path`/`model_format` (`20260710_jig_3d_model.sql`) · คุมขนาด: CAD deflection 0.002 + เตือนเมื่อ >15MB · **ลบ jig ต้องลบ `models/<id>.glb` ด้วย** (อยู่นอกโฟลเดอร์ `jigs/<id>/`) · `.prt/.sldprt/CATIA` เปิดบนเว็บไม่ได้ → แจ้งให้ export STEP/IGES ก่อน · แหล่งโหลดโมเดลผู้ผลิต: 3Dfindit / TraceParts (SMC/CKD/ABB ฯลฯ เลือก GLB/STEP)
- **Quota Free plan (ต่อ project):** DB 500MB · Storage 1GB · Egress 5GB/เดือน — ตรวจล่าสุด 2026-07-10: Main DB 22MB (~4%), DR DB 18MB (~4%), Storage หลัก ~156MB (~15%), DR jig-images ~3.6MB → พนักงาน ≤300 คน + อัตราข้อมูลโตปัจจุบัน อยู่ได้อีกหลายปี · **โมเดล 3D กินเยอะกว่ารูปมาก** (MB/ตัว vs ~100KB/รูป) ถ้าใส่หลายเครื่องต้องคุมขนาด/เฝ้า egress · ถ้าใกล้เต็มค่อยอัป Pro ($25/เดือน = DB 8GB + Storage 100GB) โดยไม่ต้องย้ายระบบ
