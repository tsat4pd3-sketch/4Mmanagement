...
- **🗺️ ดูได้ 2 แกน (ปุ่มสลับมุมขวาบน):** **ตามพื้นที่ (แผนที่ — default)** `TSG → โซน → บริษัท → ไลน์` · **ตามกลุ่มธุรกิจ** `TSG → กลุ่มธุรกิจ → บริษัท → ไลน์` — โซนพื้นที่ (`ZONES`) = **โซนบางนา / โซนตะวันออก / โซนต่างประเทศ** ตัดขวางกลุ่มธุรกิจ (บริษัทเดียวอยู่ได้ทั้ง 2 แกน) · ชั้นที่ 2-3 ใช้ component/ตัวเลขชุดเดียวกันทั้งสองแกน
- **แผนที่ = `src/components/ThailandZoneMap.jsx` (SVG ล้วน ไม่มี tile/dependency ภายนอก):** ชายฝั่งอ่าวไทยตอนบน + เจ้าพระยา/บางปะกง + แนวบางนา-ตราด/มอเตอร์เวย์ **วาดจากพิกัดจริง (lat/lon) แล้ว project equirectangular** → หมุดโรงงานใช้ `lat`/`lon` จริงของแต่ละบริษัท · **กรอบโซนคำนวณสดจาก bounding box ของหมุด + padding** (ไม่ hardcode ⇒ เพิ่ม/ย้ายบริษัทแล้วกรอบขยับตาม ไม่มีทางหลุดกรอบ) · หมุด = **วงกลม + ป้ายชื่อใต้** ตาม §1 (สี Andon เขียว/เหลือง/แดง **นิ่ง ไม่กระพริบ**) + **de-overlap ป้ายอัตโนมัติ** (คลัสเตอร์บางนาป้ายทับกัน → ดันลงทีละแถว + เส้นประโยงกลับหมุด **ตำแหน่งหมุดไม่ขยับ**) · บริษัทที่ `zone='oversea'` วางบนแผนที่ภาคกลางไม่ได้ → โชว์เป็นชิปใต้แผนที่ **ห้ามปล่อยหายเงียบ** · มือถือเลื่อนแนวนอน (minWidth 720) ตามข้อยกเว้น §6
