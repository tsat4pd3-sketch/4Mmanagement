...
> ### ⚠️ กฎเหล็ก — เพิ่มหน้า/action ใหม่ ต้องลงทะเบียนให้ admin คุมได้ (2026-08-06 · จาก audit ระบบสิทธิ์)
> สิทธิ์มี **2 ชั้น แยกกันคนละที่** — ตั้งค่าได้จริงต่อเมื่อครบทั้งคู่:
> | ชั้น | อยู่ที่ | ถ้าขาด |
> |---|---|---|
> | **ข้อมูลสิทธิ์** | `role_permissions` (migration seed) | role นั้นไม่มีแถว = **เข้าไม่ได้ (fail-closed)** |
> | **ทะเบียนให้ติ๊ก** | หน้า = `PAGE_GROUPS` ใน `PermissionsManagement.jsx` · action = ตาราง `permission_catalog` | **ไม่โผล่บนจอ = admin ปรับไม่ได้เลย ต้องแก้ทางตาราง** |
>
> - **เพิ่มหน้าใหม่** → เพิ่ม entry ใน `PAGE_GROUPS` (หมวดตรงกับ `NAV_ITEMS`) ในคอมมิทเดียวกับที่เพิ่ม route
> - **เพิ่ม action ใหม่** (`can('x','y')`) → `insert into permission_catalog (resource, action, label, group_name, sort)` — **คอลัมน์คือ `resource`+`action` แยกกัน ไม่ใช่ `permission_key`** (key ที่โค้ดเช็คคือ `resource:action` ที่ประกอบขึ้นตอน runtime)
> - **ห้าม register key ที่ไม่มีโค้ดไหนเช็ค** — ติ๊กแล้วไม่เกิดอะไร = หลอกคนตั้งค่า (เคยมี 3 ตัว: `org:manage`/`users:manage`/`permissions:manage` ซึ่งจริงๆ คุมด้วย role admin ตรงๆ — ลบแล้ว)
> - **⚠️ ตรวจก่อนลบ key ที่ดูเหมือนตาย ต้องเช็ค "การเรียกแบบ dynamic" ด้วย** — บาง key ประกอบชื่อตอน runtime หา grep ตรงๆ ไม่เจอ เช่น `can('event_log', \`approve_${cqiRole.toLowerCase()}\`)` และ `STEP_PERM[step]` ใน MtnRepair (เกือบลบผิดมาแล้ว)
> - **วิธีตรวจว่าครบ:** `grep -oE "to: '/[a-z0-9-]*'" src/App.jsx` เทียบกับ `grep -oE "page:/[a-z0-9-]*" src/pages/PermissionsManagement.jsx` — ต่างกัน = มีหน้าที่คุมสิทธิ์ไม่ได้ (audit 2026-08-06 เจอ 8 หน้า: dept-dashboard · group-overview · adoption-outlook · energy · pe-docs · vsm · die-registry · qr-labels)
> - **migration ที่ปิดช่องว่างรอบนี้:** `20260806_permission_catalog_gaps.sql` (เพิ่ม 8 action key ที่โค้ดใช้จริงแต่มองไม่เห็น: `lpa:record/manage/delete` · `transport:manage` · `pm_coord:manage` · `doc_forms:manage` · `ojt:record/delete`) — **ไม่แตะ `role_permissions` สิทธิ์ที่ทุก role ถืออยู่ไม่เปลี่ยน**
> - **กับดัก `enum_range` ยังอยู่** (ดูย่อหน้าถัดไป) — ลงทะเบียนแล้วยังต้องไปติ๊กให้ role ที่ควรได้ที่ `/permissions` เอง
...
- **Quota Free plan (ต่อ project):** DB 500MB · Storage 1GB · Egress 5GB/เดือน — **ตรวจล่าสุด 2026-08-17: Main DB 34MB (~7%) · DR DB 51MB (~10%)** → ยังห่างเพดานมาก · ถ้าใกล้เต็มค่อยอัป Pro ($25/เดือน = DB 8GB + Storage 100GB) โดยไม่ต้องย้ายระบบ
  - **⚠️ ขยะที่โตเงียบฝั่ง DR — ไม่ใช่ข้อมูลธุรกิจ (ตรวจ 2026-08-05):** เคยพบ **43MB จาก 72MB เป็น log ล้วน** → `cron.job_run_details` (ประวัติการรัน pg_cron) + `net._http_response` (buffer ของ pg_net ที่โค้ดยิงแบบ fire-and-forget ไม่มีใครอ่านกลับ) · **ไม่มีโค้ดไหนอ่าน 2 ตารางนี้** (ตารางตั้งเวลาอยู่ที่ `cron.job` คนละตัว ลบ log ไม่กระทบ schedule) · ล้างเหลือ 33MB แล้ว + ตั้ง cron **`purge-cron-logs`** (01:00 ไทย เก็บ 7 วัน) กันโตกลับ ~1MB/วัน · migration `20260805_purge_cron_logs.sql`
  - **⚠️ `DELETE` อย่างเดียวไม่คืนพื้นที่ ต้อง `VACUUM FULL` ตามเสมอ** และ **VACUUM FULL รันใน transaction/migration ไม่ได้** — ต้องรันแยกใน SQL Editor
