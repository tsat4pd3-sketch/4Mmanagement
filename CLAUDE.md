...
> ### 🔴🔴 กฎเหล็กข้าม session — **ฟอร์มที่ยื่นออกไปแล้ว = บันทึก ไม่ใช่ใบที่ generate ใหม่** (2026-09-25)
> ใบที่มีเลขที่/ต้องเก็บตามอายุเอกสาร — พิมพ์แล้วจบ = paperless ไม่จริง หน้างานปริ้นกระดาษเก็บเองทุกวัน (เกิดจริง)
> · **ออกใบ = เขียนทะเบียนก่อนค่อยพิมพ์** เขียนไม่สำเร็จ **ห้ามพิมพ์ต่อเงียบๆ** · **`snapshot` = เนื้อใบ ไม่ใช่ pointer**
> · **พิมพ์ซ้ำ = ใบเดิม ไม่ออกเลขใหม่** · snapshot ไม่ตรงข้อมูลปัจจุบัน = **เขียนบนจอ** ห้ามพิมพ์ของใหม่ทับเลขเดิม
> · เลข running = "เลขสูงสุดของเดือน+1" **ห้าม `count()+1`** · ต้นแบบ `src/lib/prodProblemDoc.js` (มีด่าน)
> · ถังเหลือง/แดง: อายุแท็ก + ผลพิจารณา QA 4 ทาง + เลข WI ซ่อม อยู่ `src/utils/qualityBin.js`/ทะเบียน `repair_wi_registry`
>   **ห้ามประกาศ/hardcode ซ้ำในหน้า** (มีด่าน) · **"ปิดแล้วหรือยัง" คำนวณจากข้อเท็จจริง ห้ามเพิ่ม `closed_at` ที่ต้องมีคนกดปิด**
