...
- **โลโก้ Thai Summit** = ฝังตราจริง (crop จากฟอร์มต้นฉบับ เป็น data URI ใน `TS_EMBLEM`/`tsLogoHtml`) เป็น default · **override ได้ด้วยรูปที่อัปโหลดใน `/doc-forms` (`doc_forms.logo_url`)** → เป๊ะ 100% (ทั้ง 2 ฟอร์มอ่าน logo_url ผ่าน `docFormSync`) — **ห้ามกลับไปวาดโลโก้ด้วยกล่อง T/S แยก** (ไม่ตรงตราจริง)
