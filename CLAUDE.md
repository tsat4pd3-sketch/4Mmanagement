...
- **ยังไม่ทำ (เฟสถัดไป):** พิมพ์/PDF บอร์ด (ต้อง register `doc_forms` ตามกฎเอกสาร) · ผูก action item
  จาก `meeting_action_items` เข้าบอร์ด · แจ้งเตือน Telegram เมื่อมีอุบัติเหตุ (ผ่าน `notifyEvent` — rule ใหม่)
