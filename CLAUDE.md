...
> - **⚠️ seed อย่างเดียวยังไม่พอที่จะให้ลิสต์สั้นลง** — แถวเดิม 28 ตัวยัง `team=null` (common) จึงติดมาในลิสต์ทุกทีมเสมอ · การติ๊กแถวเดิมเป็น `jig_maintenance` **กระทบทีมที่ใช้งานอยู่จริง (ติ๊กผิด = ของหายจากลิสต์เขาเงียบๆ)** → เตรียม SQL ไว้ที่ **`docs/sql/05_mtn_tag_legacy_lists_as_jig.sql` (ยังไม่ apply โดยตั้งใจ)** แยกกลุ่ม A (ชัดเจน) / B (ยังไม่ชัด เช่น เซนเซอร์/ลมรั่ว/CONVEYOR = เจอได้ทุกทีม ปล่อย common) — **ต้องให้หน้างานตัดสินก่อนรัน**
