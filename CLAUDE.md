...
> - migration `20260819_mtn_operator_page_main.sql` เปิด `page:/operator` ให้ role `mtn` (กับดัก enum_range — หน้า seed ก่อน role เกิด) — **ให้เฉพาะ page ไม่แจก `employees:edit`** (ช่างได้แผงสกิลตาม skills:edit เท่านั้น ประวัติพนักงานยังเป็นของฝ่ายบุคคล/หัวหน้าผลิต)
>
...
- > ### ⚠️ กฎเหล็ก — scope ของ leader = **ทั้งครอบครัวไลน์** ห้าม `.eq('line_id', lineId)` ตรงตัว (2026-08-10)
  ...
  > - โค้ดอ่าน/เขียน best-effort ทั้งหมด — ยังไม่ apply migration = ปุ่มยืมขึ้นเตือน + หน้าอื่นทำงานแบบเดิมเป๊ะ · migration **ยังไม่ apply** (2026-08-19 — รอ user รันผ่าน SQL Editor)
  >
