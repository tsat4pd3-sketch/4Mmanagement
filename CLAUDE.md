...
> ### ✅ ชั้น "ฝ่าย (Division)" เข้าระบบแล้ว — single source of truth (2026-08-19 · คำสั่ง user)
> `org_nodes.kind='division'` (migration `20260819_org_division_level.sql` · **apply แล้ว**)
> → ผังในระบบครบ 3 ชั้นตามผังจริงแล้ว: **ฝ่าย → ส่วน → แผนก/กลุ่ม**
> - **จัดการที่ `/org-setup` คอลัมน์แรก** · seed 5 ฝ่ายตามผัง Rev.09 (LOG/ENG/PRD/MTN/SUP)
> - เติมแม่ให้ section ที่จับคู่ได้ชัด: **PD1–PD4 → ฝ่าย Production · Planning&Store → ฝ่าย Logistic and Sales**
>   ที่จับคู่ไม่ได้ปล่อยว่าง → โผล่ในถัง **"⚠️ ยังไม่ระบุฝ่าย"** ให้คนมาจัด (**ห้ามเดา** ผังเป็นเอกสารควบคุม)
> - **⚠️ ห้าม hardcode ชื่อฝ่าย/cost center ในหน้าใดๆ** — อ่านผ่าน **`src/utils/orgDivisions.js`**
>   (`loadOrgDivisions()` + `divisionIndex()`) · อ้างถึงฝ่ายด้วย **`code`** (LOG/ENG/PRD/MTN/SUP) ซึ่งนิ่งกว่าชื่อ
>   · ไม่เจอ code = แสดงว่า "ยังไม่ได้ตั้งฝ่ายในผัง" **ห้าม fallback เป็นชื่อที่เดาเอง**
>   (เคยพลาดมาแล้ว: `/flow-tower` รอบแรก hardcode ชื่อฝ่าย + เลข cost ไว้ในไฟล์หน้า — แก้แล้ว)
> - **ปลอดภัยกับของเดิม:** ตรวจแล้วไม่มีโค้ดไหนสมมติว่า `section.parent_id` ต้องเป็น null
>   (ทุกที่กรองด้วย `kind === 'section'` เท่านั้น) → เพิ่มแม่ให้ section ไม่กระทบ scope/สิทธิ์เดิม
> - ยังไม่ทำ: ย้าย QA/MTN/JIG MTN/DIE MTN จาก "แผนกขึ้นตรงฝ่าย" ขึ้นเป็น "ส่วน" ตามผังจริง
>   (เปลี่ยน `kind` กระทบ scope ของคน ต้องให้ user เคาะก่อน)
