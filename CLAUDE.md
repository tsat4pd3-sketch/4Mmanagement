...
- **⚠️ เพิ่มก้อน rate ใหม่ = เพิ่ม entry ใน `RATE_COMPONENTS` (`src/utils/costSaving.js`) + คอลัมน์ใน `cost_center_rates` เท่านั้น** — ฟอร์ม/ตาราง/ยอดรวม/ตัวเลือกก้อนใน `/improvements` วนจาก array นี้ทั้งหมด **ห้ามพิมพ์ชื่อคอลัมน์ rate ซ้ำในหน้าใดๆ**
  - **⚠️ บัญชีปรับ rate รอบใหม่ = กดปุ่ม “＋ rate ใหม่” (เพิ่มแถวใหม่ที่ `effective_from`) ห้าม update ทับ** (โปรเจคเก่าต้องคำนวณด้วย rate ณ ช่วงเวลานั้น) · ปุ่ม ✏️ ในตารางไว้แก้เฉพาะตอน**กรอกตัวเลขผิด** จึงทำให้จืดกว่าโดยตั้งใจ
  - **🔴🔴 กฎเหล็ก — สิทธิ์ "เข้าหน้า" ไม่ใช่ด่านของ "แผงในหน้า" ห้ามพึ่ง (2026-09-03 · full QC audit)**
    บรรทัดนี้เคยเขียนว่า *"หน้า `/org-setup` เป็น admin-only อยู่แล้ว ไม่ต้องเพิ่ม gate ซ้ำ"* — **จริง ณ ตอนเขียน (2026-08-11)**
    แต่ `20260824_org_manage_own_unit.sql` เปิดหน้าให้ role สนับสนุนดูผังของตัวเอง แล้ว**ไม่มีใครกลับมาปิดแผงที่พึ่งสมมติฐานเดิม**
    ⇒ วัดกับฐานจริง 03/09: `page:/org-setup` = admin · engineer · mtn · planner_store · sale = **บัญชีจริง 16 คน**
    แก้ **Activity Rate 52 cost center** ได้โดยไม่มีอะไรกั้นสักชั้น (ทั้ง UI และ RLS `using(true)`)
    · rate คือ**ตัวคูณของ "เงินที่ประหยัดได้" ทุกจอ** (`/improvements` · มูลค่าดาวไทม์ · มูลค่าของเสีย · เด็ค Monthly Review ผู้บริหาร) แก้เลขเดียว = ตัวเลขเงินทั้งโรงงานขยับโดยไม่มีใครสังเกต
    · **คลาสเดียวกับ `protect_profile_role`** ที่กัน `role` ไว้คอลัมน์เดียวแล้วมีคนเพิ่มแกนสิทธิ์ทีหลัง — **สมมติฐานเรื่องสิทธิ์ที่เขียนไว้ในคอมเมนต์ มีอายุ ห้ามเชื่อโดยไม่วัดซ้ำ**
    - คีย์ **`cost_rate:manage`** (seed **admin + manager ระบุตรงตัว ห้าม `enum_range`**) · **ไม่ให้ bucket `dept_admin`** — dept_admin เป็น tier "ของหน่วยงานตัวเอง" แต่ rate เป็นของทั้งโรงงาน
    - RLS `cost_center_rates_write` ใช้ **`has_perm('cost_rate:manage')`** ตามกฎ "policy ห้าม hardcode role array" · อ่านยังเปิดทุกคน (`/improvements` ต้องอ่านมาคิด saving)
    - **⚠️ UI ต้องนับแถวที่เขียนจริง (`.select('id')`)** — เทสกับ RLS จริงแล้ว: role สนับสนุน **UPDATE ได้ rows=0 · DELETE rows=0 (เงียบสนิท ไม่มี error)** ส่วน INSERT โดน 42501 · เช็คแค่ `if (error)` = ขึ้น toast เขียวทั้งที่ไม่มีอะไรถูกบันทึก
    - **✍️ ผู้เซ็นใบค่าฝีมือ (`section_signers`) ก็ไม่มีด่านเหมือนกัน** — ชื่อที่ตั้งถูกพิมพ์ลงช่องลายเซ็น "ใบสรุปค่าฝีมือ" จริง ⇒ แก้ = เอกสารอ้างผู้อนุมัติผิดคน · **ไม่เพิ่มคีย์ใหม่** ใช้ `org:manage_own_unit` เดิม + UI จำกัดต่อว่าเป็น**ส่วนงานของตัวเอง** (`canEditSigners` — หลักเดียวกับ `isMyUnit` ของตารางกะ · RLS จำกัดรายแถวตาม section ไม่ได้เพราะ section เป็นข้อความอิสระ ไม่ใช่ FK)
    - **ตั้งใจไม่ปิดหน้ากลับเป็น admin-only** — การเปิดให้หน่วยงานแก้ผังตัวเองเป็นสิ่งที่ user สั่งไว้ 24/08 (ไม่งั้นแอดมินหน่วยงานเพิ่มแผนกตัวเองไม่ได้ = ทางตัน) · ที่ต้องแก้คือ**แผงที่ไม่ควรเปิดตามไปด้วย** ไม่ใช่ถอยทั้งหน้า
    - migration `20260903_cost_rate_and_signer_guard.sql` (**apply แล้ว 2026-09-03** · เทสสวมบท `authenticated` ทั้ง 2 ฝั่ง: support role บล็อกครบ · admin ทำได้เหมือนเดิม 52 แถว)
