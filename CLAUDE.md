...
4. build ผ่าน (`npm run build`) ก่อน commit เสมอ · merge เข้า `main` = deploy จริง
   ...
   - **⏰ ห้ามเขียนเทสที่ผลลัพธ์ผูกกับ "วันที่รันเทส" — เป็นระเบิดเวลาใน build gate (2026-09-14)**
     เคสจริง: `pullSignal.test.mjs` assert `warnings.length === 3` แต่ `parsePullFile` เตือนเพิ่มว่า
     "ไฟล์ห่างจากวันนี้ N วัน" เมื่อ fixture (ลงวันที่ 08/09) เก่าเกินเกณฑ์ ⇒ เทสผ่านตอนเขียน
     แล้ว**พังเองวันที่ 14/09 โดยไม่มีใครแก้อะไร** ⇒ `npm run build` ล่ม ⇒ **deploy ออกไม่ได้ทั้งระบบ**
     · วิธีที่ถูก: assert เฉพาะสิ่งที่เทสนั้นล็อกจริงๆ (filter เอาเฉพาะ warning ที่เกี่ยวข้อง)
       ห้าม assert ยอดรวมที่มีตัวแปรตามเวลาปนอยู่ · ถ้าต้องล็อกเวลาจริงให้ inject "วันนี้" เข้าฟังก์ชัน
