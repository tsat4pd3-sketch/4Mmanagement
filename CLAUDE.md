...
> - **ที่กันไว้อยู่แล้ว:** ลบ = soft delete (`is_active=false` ใบเก่ายังอ่านออก กู้ได้) · `mtn_item_types`/`mtn_problem_types`/`mtn_spare_categories` อยู่ใน `DR_AUDIT_TABLES` → `audit_log` รู้ว่าใครแก้ (ยังไม่มี UI ให้ดู — งานถัดไป)
>
...
> - **helper กลาง `src/utils/matResolve.js`** — `normMat` (ตัดขีด/ช่องว่าง + uppercase) · `buildPnIndex(rows)` · `resolveMatNo(mat, idx)` · `matIssueText()` · **ห้ามเขียน logic เทียบ p_no ใหม่ในหน้าอื่น**
> - **resolve เฉพาะที่ "ชัดเจนตัวเดียวและปลายทางเป็นเลขล้วน"** — 5 สถานะ: `sap` (เลข SAP อยู่แล้ว) · `mapped` (จับคู่ได้ชัด) · **`ambiguous`** · **`placeholder`** · **`none`** → 3 ตัวหลัง **ไม่หักสต็อก** และบอกวิธีแก้บนจอ
