...
| ภาพรวม | `/group-overview` | **GroupOverview — 🏢 ภาพรวมกลุ่มโรงงาน (MOCKUP หลายโรงงาน)** ตัวอย่างหน้าจอตอบโจทย์ผู้บริหาร "ระบบดูหลายโรงพร้อมกันได้มั้ย" — โรงงานที่ 1 = **ข้อมูลจริง** ที่เหลือ **จำลอง** จากข้อมูลชุดเดียวกัน (ดู section "Group Overview (Mockup)") | admin/manager (seed) |
...
## Group Overview — 🏢 ภาพรวมกลุ่มโรงงาน (MOCKUP หลายโรงงาน · 2026-08-05)

หน้า `/group-overview` (`GroupOverview.jsx`, กลุ่มภาพรวม) — **เป็นตัวอย่างหน้าจอ (mockup) ไม่ใช่ระบบ multi-plant จริง** สร้างตามคำสั่ง user เพื่อตอบผู้บริหารว่า "ระบบนี้ใช้กับหลายโรงงาน + ดูภาพรวมข้ามโรงได้มั้ย"

- **ข้อมูล:** โรงงานที่ 1 (`PLANT-1` บ้านโพธิ์) = **ข้อมูลจริง** จากฐานปัจจุบัน (กะที่ปิดแล้วของวันที่เลือก · aggregate ต่อ**ไลน์บนสุด** roll up ลูกเข้าแม่) · โรงงานที่ 2-5 = **ปั้นจากข้อมูลจริงชุดเดียวกัน** ด้วยตัวคูณต่อโรงงาน (`qtyF`/`oeeD`/`dtF`/`ngF`/`keep` ใน const `PLANTS`) + **seeded RNG** (`hash32`/`rnd`/`jit` keyed ด้วย โรงงาน×ไลน์×วัน — ตัวเลข**นิ่ง ไม่ดิ้นทุกครั้งที่รีเฟรช** ซึ่งสำคัญกับความน่าเชื่อถือตอนเดโม)
- **ไม่เขียน DB ใดๆ · ไม่มีตาราง `plants`** — ข้อมูลจำลองอยู่ใน memory ของหน้าเท่านั้น · migration มีแค่สิทธิ์เข้าหน้า `20260805_group_overview_permission.sql` (seed **admin/manager** เท่านั้น — จอผู้บริหาร + กันหน้างานเข้าใจผิดว่ามีหลายโรงจริง · role อื่นเปิดเพิ่มที่ `/permissions`)
- **ทุกหน้าจอต้องบอกชัดว่าอันไหนจริง/จำลอง** — แถบ 🧪 MOCKUP บนหัวเรื่อง + banner อธิบาย + ป้าย "จริง"(เขียว)/"จำลอง"(ส้ม ขอบประ) บนทุกการ์ด/ทุกแถวตาราง **ห้ามถอดป้ายพวกนี้ออก**
- **ตัวเลขใช้สูตรกลางเดิมทั้งหมด** (ให้ตรงกับที่แต่ละหน้าเห็น): OEE เฉลี่ย = `wavg` จาก `src/utils/oee.js` ถ่วงด้วยเวลารับภาระ (`shift_min − plannedDT`) · ยอดผลิตนับงานคู่ RH/LH เป็น 1 stroke ผ่าน `pairAwareTotal` · NG ยึด `defect_logs` (qty_ng + qty_suspect) · DT นับเฉพาะนอกแผน
...
- **แผง "🛠️ ถ้าจะรองรับหลายโรงงานจริง ต้องทำอะไรบ้าง"** (พับได้ ท้ายหน้า) = คำตอบเชิงเทคนิคบนจอเดียวกับตัวอย่าง: เพิ่มตาราง `plants` + `plant_id` ที่ **ต้นทางลำดับชั้น** (`org_nodes`/`production_lines`/`profiles`) เท่านั้น — ตารางรายวัน (session/order/downtime/defect) **ไม่ต้องแก้** เพราะสืบโรงงานผ่านไลน์ได้ · ขยาย `effectiveSections()` เป็น โรงงาน→ส่วนงาน · master ที่ต้องแยกต่อโรง: ปฏิทิน/นโยบายพัก/เป้า OEE/ทะเบียนเอกสาร/ห้อง Telegram · 2 project เดิม (Main+DR) ใช้ต่อได้ ไม่ต้องแตก project ต่อโรงงาน
- **ถ้าจะทำ multi-plant จริง** ให้ยึดหน้านี้เป็น reference ของ "ชั้นรายงานรวม" แล้วเปลี่ยนแหล่งข้อมูลจากตัวจำลองเป็น query จริงต่อ `plant_id` — โครง UI/สูตรใช้ต่อได้ทั้งหมด
