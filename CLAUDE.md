...
- **Downtime KPI = ผลรวมนาทีทุกรายการทุกเครื่องทุกไลน์ (เวลาซ้อนกันได้ เกิน 24 ชม./วันได้)** — แสดงเป็น **% เทียบฐานเวลาเครื่องรวม** = Σ ต่อกะที่เปิด (`shift_min` (fallback 570) × จำนวนเครื่องจากทะเบียน `machines` ของไลน์ ไม่มีทะเบียน = 1) · เกณฑ์สี <3% เขียว / 3-8% เหลือง / >8% แดง · Top Downtime/ของเสีย แสดง note ของพนักงาน (description) ใต้แต่ละประเภท — สำคัญกับประเภท "อื่นๆ"
...
- **แจ้งเตือน Telegram:** ผ่าน edge function `send-mtn-notification` (events `mtn_reported`/`mtn_assigned`/`mtn_closed`) — route ผ่าน notification_rules category maintenance (ตั้งค่าที่ `/notification-config`)
- **Scope:** leader = family ไลน์ตัวเอง (branch มาก่อน) · role อื่นตาม `sections` — pattern มาตรฐาน
