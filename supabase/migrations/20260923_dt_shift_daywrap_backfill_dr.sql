/*  ═══════════════════════════════════════════════════════════════════════════════════════
    ...
    **ไม่แตะ** แถวที่เข้าข่าย AM/PM สลับ (±12 ชม.) — นั่นคือการเดาเจตนาคน ต้องให้คนยืนยันเป็นใบๆ
    ...
    ── ROLLBACK ────────────────────────────────────────────────────────────────────────────
      ...
      from archive.dt_daywrap_backfill_20260923 b where b.id = d.id;
    ═══════════════════════════════════════════════════════════════════════════════════════ */

-- 🔴 ตารางสำรองต้องอยู่ schema `archive` ห้ามไว้ใน public (มีด่าน regressionGuards)
--    สำเนาที่ copy มาไม่ได้ RLS ตามมา · ฝั่ง DR วิ่งด้วย anon เสมอ = ใครมี anon key ก็อ่านได้
create schema if not exists archive;
create table if not exists archive.dt_daywrap_backfill_20260923 (
  id uuid primary key,
...
alter table archive.dt_daywrap_backfill_20260923 enable row level security;
...
insert into archive.dt_daywrap_backfill_20260923
...
from archive.dt_daywrap_backfill_20260923 b
where b.id = d.id;
